...
file joins the list in `plans/README.md`. File and line references describe this fork and the
track-changents repo as of 2026-09-05; as with every plans/ document, treat them as dated.
...
review UI is an Obsidian plugin, which is not part of the user's workflow.

This document proposes that romp's file viewer become a host for that sidecar: a Review mode in
the Files pane that shows the agent's pending changes, holds persistent comments on passages,
sends one review message, and accepts or rejects the agent's revisions in place. It reaches the
loop above with no GitHub visit and no Obsidian, in four independently useful slices, and reuses
the sidecar contract byte for byte so the agent-side tooling keeps working unchanged. It asks the
user to approve the slice plan and to answer the open questions at the end. After approval the
implementing session publishes a working note naming the files it owns and lands each slice as
one fork PR with an adversarial review pass, as the file browser did.
...
6. The user comments on passages. The comments persist with the file.
...
- **Steps 1 and 2** need nothing: the chat composer's `sendMessage` op (`kernel/kernel.py:11954`)
  and POST `/send` (`kernel.py:37558`) carry human-to-agent text on every backend, through
...
- **Step 5** has no surface. The viewer renders the current bytes of one file (`renderBody`,
  `ui/webview/file-view.ts:527-536`) over GET `/file`, which stamps `X-Romp-Mtime-Ns`
  (`kernel.py:36688-36752`). Nothing in romp reads a `.trackchanges/` sidecar.
- **Step 6** is ephemeral. Selecting a passage mints a `path:line` quote chip
  (`file-view.ts:538-599`, `ui/webview/docreview.ts:28-60`); Cmd-Enter stages several; Enter
  sends them as one message. The viewer's own per-file comment store was retired on 2026-08-23 in
  favor of chips (`file-view.ts:221-235`).
...
- **Step 8** works once track-changents' `install.sh` has run on the session's machine (it is not
  installed on the user's machine as of 2026-09-05). romp's SDK sessions load user-level settings
  and skills, so the guard and the tracked-changes skill apply; `ROMP_SESSION_NAME` and `ROMP_SID`
  ...
  and dashboard share the owning kernel's disk, the pull from GitHub is already unnecessary.
...
- Two substrates matter for the design. Editing: Edit mounts a lazily loaded CodeMirror 6 chunk
  ...
  (`kernel.py:10981-11052, 37612-37624`) were built on 2026-08-31 for exactly this plugin's review
...
1. **The sidecar is track-changents v3, byte for byte.** romp adds no fields and mints no second
   schema. That is what keeps the agent half (CLIs, guard, skill) and the other two hosts working
   unchanged, and it is what keeps the romp side small.
...
3. **The UI enters through the viewer's action registry** (`registerFileViewAction`,
   ...
   honest-absence pattern, and the gear says why (see Getting into it).
4. **Change awareness by polling, not a watcher.** HEAD `/file` on the report and on the sidecar
   ...
   which the file-browser plan deferred. The poll stands in for that missing event and is the
   Obsidian host's cadence too.
...
`body`, `ts`, `replies[]`, `resolved`, and either an `anchor {quote, prefix, suffix}` for a
passage comment or a `suggestionId` binding the thread to an op (both existing hosts classify a
thread on exactly that: no anchor means a change thread); a top-level `detached[]` of ops the
load-time rebase could not re-place, which a host preserves and shows rather than drops; and a
`fingerprint` over the current text. The file on disk is always the current text with every
suggestion applied. The root is the nearest `.obsidian/`, `.git/`, or `.trackchanges/` ancestor.
The single on/off control is `.trackchanges/config.json` `{v: 2, tracked: [...]}`: a path tracks
one file, a `folder/` entry tracks everything under it, and an optional `untracked: [...]` veto
with the same shapes wins over both the list and the link-closure inheritance
(`store-io.mjs:87-98, 190-192`). `.trackchanges/` is committed with the repo.
...
- **A corrupt or newer-version sidecar must be refused, never replaced.** Two CLIs today replace
  it (`track-edit` and `track-comment`; `track-reply` fails instead), item A2 of the 2026-09-03
  track-changents survey (a session note outside both repos; every item relied on is restated
  here). The host script loads through `loadStoreStatus` (`store-io.mjs:286`), never through
  `loadStore` or `ensureStore`, which collapse corrupt and unsupported to a fresh sidecar.
- **`reviveThreadFromSuperseded` is never called from the host script.** Survey item A1: replying
  into a thread that survives only in a `.superseded` park overwrites the live sidecar with an
...
sending socket, the `listDir`/`saveFile` pattern (`kernel.py:39405-39425`). All mtimes travel as
strings.
...
reply    {type:"trackReviewResult", reqId, verb, root, storePath, tracked,
          fileMtimeNs, storeMtimeNs|null, store|null, hunks, baseline?}
...
only when the request asks for it. The browser builds its cards from `store` and `hunks`; no card
model crosses the wire.

Verbs by slice. Slice 1: `status`; `set-tracked {on, scope: "file"|"folder"}`, where `folder`
writes `<dir>/` (refusing the root) so a folder can be tracked before its report exists;
`comment {quote, note, hintLine?}`; `reply {threadId, note}`; `resolve {threadId, on}`.
Slice 2: `accept {ids}`, `reject {ids}`, `accept-all`, `reject-all`. Slice 3: `save {content,
ops}`. Every mutating verb (all but `status`) carries a fence: `storeMtimeNs` must equal the
sidecar's current mtime, with `""` meaning the sidecar must not exist yet, so two browsers cannot
both create it; `reject`, `reject-all`, and `save` also fence on `fileMtimeNs`; `set-tracked`
fences on `configMtimeNs` the same way, since it writes `config.json`, not the sidecar. A moved
fence refuses, and the client re-issues `status`, re-renders, and retries by stable op or thread
id, surfacing a second refusal verbatim. Nothing merges.
...
`unsupported-version`, `corrupt`, `anchor-not-found`, `anchor-ambiguous`, `no-thread`,
`too-large`.
...
override every file's root for the CLIs, survey item A8). `/version` reports the review verdict
per kernel (`ok`, `no-host`, or `no-node`) so the gear can show why Review is absent.
...
`addComment` and `addReply` functions of `cli/track-comment.mjs` and `cli/track-reply.mjs` by file
URL from that root (the package's exports map covers `./store-io` and `./engine` but not
`./cli/*`). It performs each verb as one load-mutate-write in a single process: root discovery,
sidecar path, the load-time rebase that re-places ops after external edits, anchor creation,
comment and reply construction in the CLIs' own field shapes and id form, accept and reject
through the engine, fingerprint, atomic write, prune-when-empty. For `comment`, it finds the
occurrence of `quote` nearest the hinted line (refusing `anchor-ambiguous` when there is no hint
and several occurrences) and builds the anchor with `engine.makeAnchor`, because `addComment`
alone anchors at the first occurrence. Reject writes the sidecar first, then the file, and
restores the prior sidecar bytes (or removes the sidecar it created, when none existed) if the
file write fails, the order `track-edit` uses (`cli/track-edit.mjs:108-128`); its file write is
atomic (temp file and rename in the same directory, through the realpath, mode preserved) and
applies the same 2 MB and UTF-8 checks as `_save_file`, refusing `too-large` before any write.
The human label rides in as `TRACKCHANGES_SESSION`.
...
`tracked` is the client's last `status` verdict and picks the second bullet of the message.
`desc` is the first 40 characters of the passage for a comment thread, or the change's old and
new text for a change thread. The kernel builds the message below, marker-neutralizes the path
and every body (`_neutralize_romp_markers`, `kernel.py:30786-30798`), and mirrors the
`userTodoAnswer` handler branch for branch (`kernel.py:12198-12250`), factored into a helper both
ops call: with a `todoId`, the user-todos switch off warns; a todo already settled sends the
review anyway with no stamp and a warning naming it; an ended session refuses with the existing
revive-the-session text and sends nothing; otherwise `_send_or_park(be, sid, body,
user_todo=todoId)`, and a "parked" result replies `queued: true` and stamps when the batch drains
...
truthy-or-falsy split decides sent or failed. The review is already on disk before any send, so a
refusal loses nothing.

### The Send review message

The `[obsidian-diff]` shape the skill handles (`skill/SKILL.md`, "Messages from the editor"),
written in the person's voice like every injected body, batched:
...
but is romp's own text. It keeps what the skill keys on: the `[obsidian-diff]` prefix, the
absolute path, a thread id per thread, and the exact `track-reply` and `track-edit --thread`
command lines (`skill/SKILL.md:121-133`). The tracking-on second bullet restates the skill's own
instruction, since no host emits one today (the VS Code bullet says to edit normally, and the
Obsidian host's ping is a stub in its repo). With `tracked` false the second bullet becomes the
VS Code host's wording: edit the file normally and note it with `track-reply`. The closing
sentence is what brings the loop back: the session's next todo is the return signal. The text
names no romp machinery; `tests/test_injected_voice.py` renders it like every injected body. The
skill's ping paragraph should gain one sentence saying a ping may list several threads (a change
in the track-changents repo, by its author).
...
(`file-view.ts:433-436`); Slice 1 amends it for the review case, since comments reach the
...
- **Comment on a selection**: selecting a passage still seeds the quote chip when a chat composer
  ...
  highlight and card appear at once. In Rendered view the selection is rendered prose, so the
  browser first recovers the exact source substring with the quote-chip matcher's normalization
  (`docreview.ts`); if that fails, the composer keeps the typed note and offers to switch to Raw
  and reselect.
- **Send review** lists what goes (N comments, M replies, accepts, rejects), confirms once
  ...
  <time>" and keeps polling while open. The confirm carries up to three checkboxes, all checked
  by default: **answer the todo** when the file was opened from one; **turn Track changes on**
  when it is off, so the session's revisions come back as suggestions (a checked send runs
  `set-tracked` first, fenced, then sends); and **accept the N pending changes as reviewed** when
  suggestions remain, so the session's revisions arrive as fresh suggestions rather than
  coalescing into a pending insertion. One review per file: when a todo names several files, the
...
- **What counts as unsent.** The sidecar carries no send state (choice 1), so the client keeps a
  ...
  rejected}`. A comment or reply counts when its author is `you` and its `ts` is after
  `lastSentTs`; accepts and rejects are tallied by the panel and reset on a successful send; a
  refusal leaves the ledger unchanged. A browser with no ledger offers every `you` thread that
  has no agent reply. The confirm always lists what goes, so a stale count is visible before it
  is sent.
- **Edit while changes are pending** (Slices 1 and 2): the Edit button refuses with a one-line
  reason, resolve the tracked changes in Review first, because a raw save over pending ops
  rewrites their offsets. Slice 3 lifts this.
...
lifted into one exported helper that Save and the review verbs both call), and
`setEditBlocked(reason|null)`. The action row itself stays registry-only. `track-review.ts`
registers its own `message` listener for its four reply types.
...
- From the Waiting-on-you pane: the todo's detail path is a link (Slice 0). The pane is its own
  ...
  (`kernel.py:34355-34362`); the shell forwards `todoId`, `files.ts` passes it into the action
  context, and relative paths resolve on the kernel through `_resolve_open_path`. When the pane
  is not framed by the shell, the detail stays plain text.
- From the viewer: the Review action, for any file under a track-changents root on a machine
  whose kernel reports the tooling. If Review is missing, the gear's Files section names the
  machine and the reason (`no-host` or `no-node`), and the guide says to look there.
...
- An ended session's todo is hidden from Waiting on you until the session is revived, since the
  board lists living sessions only (`kernel.py:25727-25745`), so the morning entry point can
  vanish; the report is still on disk and Review works on it without a todo. The guide says: if a
  review you expected is missing, check the session list for an ended session.
...
with an `open(path, sid)` callback; `render.ts` re-imports it and keeps its previews and
`openPath`; `waiting.ts` uses it at both sites; the shell relay's forwarded object and `files.ts`
gain `todoId`; `docs/guide.md`. Size: ~100 to 200 / ~10 / 0.
...
thread cards; Comment on a selection; highlights in both views; replies from `track-reply`
appearing within the poll interval; Send review as one message with the three confirm
checkboxes; the Edit refusal while suggestions are pending; the gear's Files section reporting
the review verdict.

Acceptance: a sidecar written by the host script, replied to by `track-reply`, deep-equals the
same sequence performed by the CLIs alone (node test), and parses through the VS Code host's
loader (`vscode/src/store.ts`) and the Obsidian host's reader with every field preserved; a
first comment on a clean file creates `.trackchanges/` and the sidecar exactly where
`storePathFor` puts it; the toggle writes `config.json` through `setTracked`, fenced on
`configMtimeNs`; a mutating verb on a moved sidecar refuses and the panel reloads and retries; the
Send review text carries the `[obsidian-diff]` prefix, the absolute path, and each thread id in
the positions the skill reads, and the kernel and webview builders produce identical text for one
and for several threads; a send with `todoId` follows the four `userTodoAnswer` branches; the
first mutating click triggers the consent popup once.

Files: new `ui/webview/track-review.ts` (registration, panel, highlight pass, Comment button,
poll, ledger, send composer), `tools/track-review-host.mjs` and its node tests,
`tests/test_track_review.py`, `ui/webview/track-review.test.ts`; touched: `kernel/kernel.py` (the
two ops, the shared todo-answer helper, the `/version` verdict), `file-view.ts` (the seam above),
`files.ts` (the `/sessions` color map, `todoId`), `gear.js` (the verdict row), `styles.css` and
`files-pane.css` (about 80 lines adapted from the Obsidian host's stylesheet, mapped to
`--accent`, `--bg`, `--fg`), `docs/guide.md`. track-changents code reused unchanged:
`store-io.mjs`, `engine.js` (`makeAnchor`, `locateAnchor`, `toHunks`, `baselineOf`), `addComment`
and `addReply`. Size: ~450 to 600 for `track-review.ts` plus ~60 to 80 for the seam and ~30 for
`files.ts` and the relay / ~150 / ~250, plus about 150 lines of tests on each side.
...
Acceptance: accept changes the sidecar only (the engine's `acceptSuggestions`); reject applies
the engine's reverse edits to the file and writes sidecar then file with rollback; both fence on
the sidecar mtime and reject also on the file mtime; after a reject the owning session receives
the review trace; a `track-edit` landing mid-review makes the next Accept refuse and reload;
accept-all on a file with no comments prunes the sidecar as `pruneIfClean` decides; a sidecar
holding one whole-file insertion renders one card whose Accept clears it.
...
### Slice 3: live CodeMirror editing over pending changes
...
`track-cm.undo.test.mjs`, run through a vitest alias that maps their two imports
(`../src/track-cm.js` and `track-changents/engine`) to romp's review chunk and its CodeMirror,
pass as the behavioral oracle; a save refuses when either mtime moved and keeps the buffer;
`editor-lazy.test.ts` pins that the main bundles stay byte-stable.
...
it), a new lazy `review-chunk` esbuild entry bundling the engine and the 78-line CodeMirror state
field (`obsidian/src/track-cm.js`, unchanged) and the decorations block
(`obsidian/src/track-snapshot.js:433-839`, about 10 lines changed to drop its one Obsidian
import), `file-view.ts` `doSave` sending the `save` verb instead of `saveFile` when Review is
active. Size: ~500 / ~60 / ~40, plus the vendoring decision. Lowest confidence of the four; it is
...
returns `name` only today (`kernel.py:11045, 11048, 11052`) while the track-changents consumer
...
writes disk sits behind the same server-side consent gate, checked before any content check, and
the gate stays the wall while the UI's gating is convenience. The host script runs only on the
owning kernel, on paths resolved by the kernel, and writes only the sidecar, `config.json`, and
(on reject or save) the reviewed file. The mtime fences refuse and never merge. Both ops route by
`sid` to the owning kernel over the existing federation splice; nothing new is exempt from
`_authorize`.
...
- **Event over time** (`CLAUDE.md`, Design): the poll stands in for an event source the Files
  pane does not have; the human's own writes never wait on it.
- **No plugin API** (`ui/webview/editor-chunk.ts:1-15`): Slices 0 to 2 never touch the chunk.
  Slice 3 adds a typed, internal `track` option, not a generic extension hook.
...
- **Two writers on one sidecar** (agent CLIs and the host script). Mitigation: one
  ...
  alternative.
- **Rendered markdown versus offsets.** Mitigation: Raw is exact, Rendered paints by quote match
  reusing the whitespace-normalized matcher in `ui/webview/comments.ts:86-162`, deletions are
  panel-only there, every hunk has a card, and a comment whose selection cannot be recovered in
  the source offers Raw.
- **Raw human saves desync ops before Slice 3.** Mitigation: the Edit refusal from Slice 1 on.
...
- **Prerequisites on the owning kernel** (node on the kernel's PATH, track-changents installed,
  the file under a root). Mitigation: `no-host` and `no-node` are honest no-verdicts surfaced in
  the gear; Review never appears broken. Node on the kernel's PATH under every service unit is
  unverified and is the first thing Slice 1 checks.
- **Tracking inheritance.** The CLIs treat notes reachable through whole-line links and embeds as
  ...
  function as the guard, so verdicts agree by construction.
...
- **Polling cost.** Two HEAD requests every 2.5 s per open panel. Mitigation: only while the
  panel is open and the tab visible; the poll starts after the first `status` supplies the
  sidecar path and keys on the pair (status, mtime) per file, where a 404 is the value "absent",
  so absent-to-present is a transition like any other.
...
- `tests/test_track_review.py` (the `tests/test_savefile.py` hermetic pattern): path resolution,
  ...
  handling, `sid` routing, trace after reject and not after comment, the four todo branches
  (switch off, settled, ended, parked), ping text for one and several threads and for `tracked`
  on and off, the `/version` verdict.
- Host script conformance (node tests beside the script): a sidecar written by `track-comment`,
  replied to by the host script, read by `track-reply` keeps every field; the host's sidecar path
  equals `storePathFor`; `v` stays 3 and the fingerprint matches; accept then `track-edit`
  succeeds; reject yields the engine's baseline for the subset and remaps survivors; rollback
  when the file write fails, including removal of a sidecar the verb created; fence refusals with
  `""` and with a stale value; nothing written on `v: 4` or unparseable JSON; `comment` with an
  ambiguous quote and no hint refuses.
- `ui/webview/track-review.test.ts`: source pins (registry entry, both ops carry `sid`, one
  ...
  state), pure tests for the card model and the unsent-ledger derivation, ping builder equality
  against the kernel's text.
- `ui/webview/user-todo-links.test.ts` rewritten to pin `path-links.ts` and both callers
  (Slice 0); `editor-lazy.test.ts` extended for the typed `track` option (Slice 3).

## Docs

`docs/guide.md`: "Reviewing a document" (`:29-38`) gains Review mode, states that quote chips
remain for untracked files, and tells the user to track the reports folder before an overnight
run; "Waiting on you" notes the linked path and the ended-session case; "Files" (`:119-131`) gains
the panel, the poll, the consent gate the guide omits today, and where to look when Review is
missing. `docs/reference.md`, under install-time switches, notes the User todos switch as a
prerequisite for the morning loop and the node plus track-changents requirement on the owning
kernel. In track-changents (its author): README "Hosts" gains romp; the skill's ping paragraph
gains the batched form.
...
viewer or editor extension API; the Obsidian host's embed trees, explorer badges, status bar,
multi-pane sync, and vault rename re-keying; a scheduler for overnight work; changes to the
Obsidian and VS Code hosts; inline deletions in the Rendered view; undo of accept and reject
before Slice 3; multi-file review batching (one review per file).
...
author: the A1 fix in both CLIs before Slice 1 ships, the skill sentence, and the README row; for
Slice 3, the vendoring decision. The feature is fork-only until track-changents is public, since
it depends on that repo's CLIs.

## Open questions for the user

1. **Slice order.** 0, 1, 2 as separate PRs with 3 designed now and built after 2 has served at
   least one real morning review, or all four scheduled together? Recommended: the former; the
   fork dispatch stays parked until asked for.
...
4. **Vendoring.** None in Slices 0 to 2; if Slice 3 is approved, vendor the engine, display
   planner, protocol constants, and the CodeMirror field (MIT, about 1,300 lines) pinned to a
   commit, or require the package to be public first? Recommended: vendor, unless it is public by
   then.
...
8. **Send review defaults.** The three confirm checkboxes (answer the todo, turn tracking on,
   accept pending changes as reviewed) all checked by default, or off by default? Recommended:
   checked; each is visible in the confirm before the send.
...
11. **The Slice 3 doctrine question**: a curated `track` option in the editor chunk. Yes, typed
    and internal with the header updated, or no live editing over pending changes at all?
    Recommended: yes.

## Upstream

Slice 0 is a pure romp change and a candidate upstream row. Slices 1 to 3 depend on a private
