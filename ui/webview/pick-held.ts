...
// and the pick waits for the turn in flight (the one the CLI starts to deliver the last result, or the
// session's next) to finish; the copy says so instead of "waiting on 0 subagents" (review round 2).

export interface PickHeld { surfaces: string[]; subagents: number; tasks: number }
...
export function pickHeldLine(h: PickHeld): string {
  ...
  return `${text} ${plural ? "apply" : "applies"} when this turn finishes`;
}
