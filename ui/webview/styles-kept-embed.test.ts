// The kept-embed rule in styles.css and the rendering it buys (the 2026-09-06 review of Slice 5). A struck block row
// that holds an `![[embed]]` token the current text still holds is split by track-decorations.ts (departure 4) into
// tc-diff-del-seg spans over the removed runs and a tc-diff-del-kept-embed span over the token, so the sheet can show the
// token un-struck: struck, it reads as "the picture is being deleted" when it only moved. The wrapper
// (.tc-diff-del.tc-diff-del-block) wears the line-through, and CSS propagates a block's text decoration to every in-flow
// inline descendant; a descendant's own `text-decoration: none` does not undo it (the span's computed textDecorationLine
// read `none` while the strike still painted through it). The rule that un-strikes the token must therefore stop the
// propagation, and the sheet does it with `display: inline-block`, the one inline display a propagated decoration does not
// enter. The static leg pins that mechanism in the rule's text. The browser legs mount the real editor chunk under the real
// sheet over the kept-embed fixture and read the pixels: a seg span carries one row of ink across its full width (the
// strike), the kept span carries none — and, forced back to `display: inline` with the rule otherwise intact, it is struck
// again, which is what proves the probe sees a strike where there is one. Skips LOUDLY without playwright or the browser
// (CI installs none), as the other browser legs do. The feed page loads feed.css alone, so the rule belongs in that sheet
// too; the copies are pinned byte-equal where the viewer's shared chrome is (fileview-parity.test.ts).
...
// ── the static leg: the mechanism, in the rule's own text ────────────────────────────────────────

test("styles.css un-strikes the kept embed by stopping the wrapper's propagation (inline-block), not by text-decoration alone", () => {
  assert.equal(CSS.split(".tc-diff-del-kept-embed {").length - 1, 1, "one rule for the kept span");
  const kept = ruleOf(CSS, ".tc-diff-del-kept-embed {");
  ...
  const tag = ruleOf(CSS, ".tc-diff-del-kept-embed::after {");
  ...
  const wrap = ruleOf(CSS, ".tc-diff-del {");
  assert.match(wrap, /text-decoration:\s*line-through/, "the wrapper still strikes the rows; the seg spans are struck by propagation");
  assert.match(CSS, /tc-diff-del-seg over each\s+removed run/, "the sheet's comment names the seg spans and says why they carry no rule");
});

// ── the browser legs: the real chunk under the real sheet, read from the pixels ──────────────────
...
// the page is the chat page's dress for the editor: styles.css alone (its @import and font urls 404 here, harmlessly)
const PAGE_HTML = `<!DOCTYPE html><html><head><meta charset=utf-8><link rel=stylesheet href=/styles.css>
...
async function probe(t: any, engine: "chromium" | "firefox") {
  ...
  try {
    ...
    assert.equal(chain.keptDisplay, "inline-block", "the kept span is the inline-block that stops the propagation");
    ...
    // the propagation is what the inline-block stops, and this probe is what sees it
...
test("in Chromium: under styles.css the kept embed reads un-struck while the removed runs around it are struck; as an inline it would be struck too", async (t) => {
  await probe(t, "chromium");
});

test("in Firefox: the same — the propagation rule is the spec's, not one engine's", async (t) => {
