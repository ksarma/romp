...
test("source: changed on disk (Slice 6 of plans/markdown-viewer.md, item 5): the probe's HEAD through the panel's two readings, its listeners on the window's focus and the document's visibilitychange, registered as probeLive and dropped by both exits and the URL view's replace; the bar's words and its Reload through fetchFile; a landing settles the bar and the failure landing re-arms its own button; no timer anywhere in it", () => {
  ...
  assert.match(openFn, /if \(a && a !== document\.body && !bar\.contains\(a\)\) return;\n\s*if \(typingInPeerFrame\(\)\) return;\n\s*takingKeyboard = true;\n\s*const opts: FocusOptions & \{ focusVisible: boolean \} = \{ preventScroll: true, focusVisible: ring \?\? ringOf\(a\) \};\n\s*try \{ body\.focus\(opts\); \} finally \{ takingKeyboard = false; \}/, "takeKeyboard's gate reads this document's holder, then the focused sibling frame's (review round 2: the chat composer beside a Files iframe), marks its own focus call for the probe's gate, and names the ring through focusVisible from the holder it takes the keyboard from, or the ring a closer read before removing that holder (review round 3: Chromium's script-focus heuristic framed the note on every pointer open)");
  assert.match(openFn, /const ringOf = \(a: Element \| null\): boolean => \{\n\s*if \(a === null \|\| a === document\.body\) return false;\n\s*try \{ return a\.matches\(":focus-visible"\); \} catch \{ return false; \}/, "ringOf: no holder or the document's body wears no ring; a matches() without the selector reads none");
  ...
  assert.match(probe, /raiseHold\.defer\(\(\) => \{ if \(!editing && mtimeNs && wrap\.isConnected && mtimeMoved\(mtimeNs, moved\)\) raiseDiskBar\(\); \}\);/, "the raise waits out a press on the body and re-checks its guards at the release (review round 3: a raise mid-drag moved the body under the pointer and the selection ended on other text)");
  assert.match(openFn, /const raiseHold = pressHold\(body\);/, "a hold of its own: the landing's parks one run at a time, and a raise must never displace a parked landing");
  assert.equal((openFn.match(/pressHold\(body\)/g) || []).length, 2, "the landing's hold and the raise's");
...
test("source: review round 2 of Slice 6 (plans/markdown-viewer.md): the keyboard rule reaches a sibling frame's typing target through the top window and never a flag; the memory's key carries the session for a relative path; the fetch failure's pane closes the Outline popover; a heading target on a non-markdown file is spent at its first text paint; the editor's entry keeps the text view's place for a leave while it is up; a fold that is the remembered block, or the offset's block, is opened only when its content was showing", () => {
  ...
  assert.match(VIEW, /export function placeKey\(path: string, sid: string \| null \| undefined\): string \{\n\s*return \/\^\[\/~\]\/\.test\(path\) \? path : path \+ "\\u0000" \+ \(sid \?\? ""\);\n\}/, "the kernel's rule for a relative path (neither /- nor ~-rooted): the session's cwd resolves it, so the session is part of the file's identity");
  ...
  assert.match(openFn, /if \(\(rendered \|\| !isMd\) && pendingHeading !== null\) \{[^\n]*\n\s*const h = pendingHeading; pendingHeading = null;/, "the spend");
  ...
  assert.match(openFn, /if \(e\.localName === "details" && !e\.hasAttribute\("open"\) && \(otherView \|\| \(depth > 0 && depth >= e\.getBoundingClientRect\(\)\.height - 0\.5\)\)\) e\.setAttribute\("open", ""\);/, "a depth inside the summary's height says nothing (it straddles the edge the same open or shut); a record read in Raw, where every row shows, names the fold's rows whatever the depth (review round 3)");
...
test("source: review round 3 of Slice 6 (plans/markdown-viewer.md): the record carries the Rendered view's open folds by ordinal, read at the leave and put back before the seat at the same mtime; a heading target on a picture or a PDF is judged at its first paint with bytes; the Outline lists no heading under a plain hidden wrapper, names its current row for assistive technology and hands Tab back to its button", () => {
  ...
  assert.match(openFn, /const rec = rememberedPlaceOf\(p, mtimeNs, body\.scrollTop\);\n\s*const folds = openFoldOrdinals\(body\);[^\n]*\n\s*return folds \? \{ \.\.\.rec, folds \} : rec;/, "the live record carries them when the Rendered body is up");
  // put back before the seat, at the record's mtime alone, in the Rendered view alone
  assert.match(openFn, /const restoreFolds = \(rec: RememberedPlace\): void => \{\n\s*if \(!rec\.folds \|\| rec\.mtimeNs !== mtimeNs \|\| ctx\.mode\(\) !== "rendered"\) return;/, "the gate: fold state present, the same bytes, the Rendered view");
  assert.match(openFn, /const rec = pendingPlace; pendingPlace = null;\n\s*restoreFolds\(rec\);\n\s*const kept = placeFromRemembered\(rec, shownText\);/, "before the block is read and seated");
  assert.match(openFn, /if \(kept\) revealRemembered\(kept, depth, ctx\.mode\(\) !== rec\.view\);/, "the fallback rules learn whether the record is the other view's");
  ...
  // a picture or a PDF: the heading judged at the first paint with bytes, the notice the text branch's
  const media = openFn.slice(openFn.indexOf("if (isImage || isPdf) {"), openFn.indexOf("folds.note();"));
  assert.match(media, /if \(objUrl === null\) return;[^\n]*\n\s*if \(pendingHeading !== null\) \{\n(?:\s*\/\/[^\n]*\n)*\s*const h = pendingHeading; pendingHeading = null;[\s\S]{0,300}noteBar\('No section named "' \+ shown \+ '" in this file\.'\);\n\s*\}\n\s*if \(svgSource && svgText !== null\) \{/, "after the loader's return, before the Source and picture branches");
