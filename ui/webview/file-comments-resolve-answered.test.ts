...
// the same comments, and its end at the person's next gesture (a press on the button itself excepted); nothing else in
// the panel resolving a comment. Synthetic fixtures only: the notes-api world, placeholder ids, the sessions "api" and "web".
...
test("the offer ends at the person's next gesture, a press on the offer itself excepted; the sent acknowledgment it displaced comes back", async (t: TestContext) => {
  const w = world(); t.after(() => w.close());
  const { aside } = await openPanel(w);
  // a send's acknowledgment stands first
  (aside.querySelector(".fc-send") as El).appendChild(new El("div"));   // nothing: the acknowledgment is the panel's own state
  headBtn(aside)!.click();
  act(aside, "fcresolveanswereddo")!.click(); await flush();
  answer(w, resolvedAll([answered.id]), lastOf(w, "fileComments", "resolve")); await flush(); await flush();
  answer(w, resolvedAll([answered.id, revised.id]), lastOf(w, "fileComments", "resolve")); await flush(); await flush();
  ...
  // a key ends it too, Tab and a modifier alone excepted; a wheel ends it
  const w2 = world(); t.after(() => w2.close());
