...
// row's keydown, element.click()) opens the card whatever selection stands. Driven here over the behavior suite's DOM
// stand-in with the selection faked per case (window.getSelection is what the panel reads), the pointer's click carrying
// detail 1 and the keyboard's 0 as browsers dispatch them; file-comments-markclick-browser.test.ts drags a real mouse in
// Chromium and Firefox. Synthetic fixtures only: the notes-api world, placeholder ids.
