...
function markClasses(): Record<string, string[]> {
  ...
  const rendered = /paintRendered\(root, src, c\.range, "([\w-]+)", undefined, \{ trim: false \}\)/.exec(SRC);   // the trim deferred to the caller's trimBlanks (paintAll and repaintPresel), since the Slice 4 review's round 14
