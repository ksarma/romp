...
test("chosenSource is the one reader of the browser's answer: currentSrc matched against the srcset carriers (the picture's sources, then the img) and named by the authored candidate rewriteFigureSrcs kept (FV_SRCSET); the img's own src, or no currentSrc, by pictureDest's rule; failedSource is that reader under the label's name, and the label's text still reads it there", () => {
  ...
  assert.match(VIEW, /\nfunction failedSource\(img: Element\): string \| null \{\n  const chosen = chosenSource\(img\);\n  if \(chosen \|\| img\.hasAttribute\("src"\)\) return chosen;\n  const parked = img\.getAttribute\(HEAL_RECORD\);\n  return parked && parked !== absUrl\(""\) \? parked : chosen;\n\}\n/, "failedSource delegates: the failed figure's label names the candidate the open would take, and only when that names nothing and the img has no src, the chat page's heal record of the address it parked (the file review's round 15, fresh-1), unless that record is the address an empty source resolves to (the file review's round 16, correctness-1; a sentence pin on the spelling, and file-view-figure-error.test.ts's parked cells hold the property through the listener)");
