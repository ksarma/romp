...
test("the one decision: figureWantsControl reads the figure's state by one rule (figureHasPicture: a control only for a state with a picture to name, loaded or a stand-in; fetching, failed and any later state get none), then the floor, the target and any link above; decideFigureControl adds or removes against the control standing, handing the keyboard to the viewer's body before a removal; the floor is 48px on either side, read from the loaded picture's laid-out box as it is while the figure is in the document (or its own size before it is); linkAbove reads the click's own set, figureLinkOf over FIGURE_LINK_SET; the load, the error and the figures' own ResizeObserver all run the decision, a 0 by 0 report runs none, no arm precedes the first paint, and the width watch's repaint runs none; figureTarget refuses on the same rule, and no reader names a refused state", () => {
  ...
  ], "decideFigureControl: the verdict against the control standing, one place that adds or removes, the target read once and the dress applied from it, the picture's title and then its mark before the control's branches (sentence pins, in order; the executed read of the title and the mark on a remote picture under the floor is file-figure-open-browser.test.ts's two under-the-floor cases, the hover read and the at-rest read under touch emulation, the file review's round 12, fresh-1 with tests-2)");
  ...
  // 1.25 and 0.8, and at a forced device scale of 0.9, the browser's own zoom, and these assertions hold the spelling alone)
  assert.match(box, /const r = i\.isConnected && typeof i\.getBoundingClientRect === "function" \? i\.getBoundingClientRect\(\) : null;\n\s*const z = i\.currentCSSZoom > 0 \? i\.currentCSSZoom : 1;[^\n]*\n\s*return r \? \{ w: Math\.round\(r\.width \/ z \* 16\) \/ 16, h: Math\.round\(r\.height \/ z \* 16\) \/ 16 \} : \{ w: i\.naturalWidth, h: i\.naturalHeight \};/,
    "the laid-out box, whatever it is, for a figure in the document (an author's width attribute counts, and so does no box at all), divided by the figure's zoom so it is in CSS pixels and rounded to 1/16 of one (the layout's grid of 1/64 of a layout pixel, the device scale times the CSS zoom, falls between CSS pixels under a zoom, the browser's own included), else the picture's own size (a detached box) (a sentence pin: the executed read is the open leg's floor cells under a body zoom and under a forced device scale)");
