...
test("render.ts: a spacer re-size of the active view files a spacer row; the cap comes from localStorage", () => {
  ...
  assert.match(RENDER, /for \(const \[rsid, a, b, c, d\] of rows\) scrollDiagRow\("spacer", rsid === live \? spacerRow\(rsid, a, b, c, d, sh, ch\) : spacerRow\(rsid, a, b, c, d, null, null\)\);/);   // the active view's row carries the frame's read, a switched-away view's none and no `view` marker: the field is withdrawn pending the owner's approval (PR E, the maintainer's round 1 addendum for the nulls; the reviewer's ruling of 2026-09-21 for the withdrawal; spacer-measure.test.ts runs it)
