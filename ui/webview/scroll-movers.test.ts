...
test("render.ts: a spacer re-size of the active view files a spacer row; the cap comes from localStorage", () => {
  // the row is queued and filed on the next animation frame with the scroller read there (PR E: reading scrollHeight in the render task
  // forced a layout; spacer-measure.test.ts drives the queue)
  ...
  assert.match(RENDER, /for \(const \[rsid, a, b, c, d\] of rows\) scrollDiagRow\("spacer", rsid === live \? spacerRow\(rsid, a, b, c, d, sh, ch\) : spacerRow\(rsid, a, b, c, d, null, null, "inactive"\)\);/);   // the active view's row carries the frame's read, a switched-away view's none (PR E, the maintainer's round 1 addendum; spacer-measure.test.ts runs it)
