...
test("render.ts: a spacer re-size of the active view files a spacer row; the cap comes from localStorage", () => {
  ...
  assert.match(RENDER, /for \(const \[rsid, a, b, c, d\] of rows\) scrollDiagRow\("spacer", rsid === live \? spacerRow\(rsid, a, b, c, d, sh, ch\) : spacerRow\(rsid, a, b, c, d, null, null, "inactive"\)\);/);   // the active view's row carries the frame's read, a switched-away view's none (PR E review round 1b; spacer-measure.test.ts runs it)
