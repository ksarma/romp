...
// written to left out, eight rows at most, federated names by host or by name), the insertion, the
// keyboard model (Enter picks while the card is open and is not consumed otherwise, so it keeps
// sending) and the transcript segmenter. The DOM wiring in render.ts has no jsdom harness, so it is
// pinned at the source level like the other composer tests, and so is the CSS.
import { test } from "node:test";
import * as assert from "node:assert/strict";
import * as fs from "node:fs";
import * as path from "node:path";
import { MENTION_MAX_ROWS, insertMention, matchMentions, mentionBareName, mentionKeyAction, mentionQuery,
         mentionSegments, mentionToken } from "./composer-mention";
...
test("the list stops at eight rows, best matches first", () => {
  ...
  assert.equal(MENTION_MAX_ROWS, 8);
  assert.deepEqual(out.map((x) => x.name), many.slice(0, 8).map((x) => x.name), "alphabetical within the prefix rank");
});

// ── the token and the insertion ──

test("the token is @ plus the name an agent's mail tools take, then one space: bare for a local, host:name for a remote", () => {
  assert.equal(mentionToken(ROSTER[1]), "@api ");
  assert.equal(mentionToken(ROSTER[4]), "@TESTHOST:web ", "postal's own disambiguating form");
});
...
test("a typed @name that names a live session is a segment with the hit; other words, emails and paths stay text", () => {
  ...
  for (const s of ["ask @web and @nobody.", "@web, @web!", "x @web"]) {
