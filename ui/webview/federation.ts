...
// local kernel persists for this browser), and the PER-INSTALL gear rows
// (Thinking summaries, Suggest /compact, User todos: each kernel's answer is its own — their sub-copy
// says "this kernel keeps its own copy" — so their ops post to the local kernel only, are never queued
// for or broadcast to another, and are not named anywhere in this file; gear.test.ts's PER_INSTALL pin
// holds that absence).
