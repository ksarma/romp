...
import { adoptViews, capsAdopts, announcedSeq } from "./views-writes";
 ...
 *
 *  The pane's clock anchor is a PAIR that travels with the frame: `now` (the emitting kernel's clock) and
 ...
 *  the pane sets its live clock from the payload it is handed: anchoring `now` on the EMIT time would move
 *  every age and tint back by however long the local board had been quiet (the 2026-09-03 review: a tab
 *  drag in another pane took a card from "1m ago" back to "<1m ago"; an attached kernel's 60 s reposts
 *  pinned the clock there for as long as the local board stayed quiet). The pair comes from the LOCAL
 *  host's frame; with no local frame yet, from the newest remote arrival that carries a clock — the same
 *  host for both halves, so they always describe one frame. Absent `arrivedAt` (a caller with no wire),
 *  no `nowAt` is set and the pane anchors on its own arrival. */
...
export class FederationManager {
  ...
  private localViewsAnnounced: number | null = null;   // the seq the last LOCAL caps frame announced as the kernel's current store when the tabOrder store adopted no kept blob — a LATER blob at exactly that seq is adopted below the stored one (announcedSeq); cleared on the next adoption
  ...
  inbound(host: string, msg: any): void {
    ...
    // announcedSeq): one slot per store, overwritten by each local caps frame, cleared on the next adoption;
    // null (no store at all) and a missing field announce nothing. The panes hold the same slot from the same
    // frame, handed on below, so the re-emit of that later adoption meets an open door there too.
    if (m && m.type === "caps") {
    ...
    if (m && m.type === "tabOrder") {
      ...
      if (host === LOCAL && m.views && typeof m.views === "object") {
        if (adoptViews(this.localViews, m.views, this.localViewsAnnounced)) { this.localViews = m.views; this.localViewsRejected = null; this.localViewsAnnounced = null; }
    ...
    if (m && m.type === "data" && m.data && typeof m.data === "object") {
      ...
      // turned away, for the caps frame, and the same door for the blob at the announced seq (round 8)
      ...
      if (held && held.views && m.data.views && !adoptViews(held.views, m.data.views, this.tlViewsAnnounced)) {
        ...
        if (host === LOCAL && m.data.views) { this.tlViewsRejected = null; this.tlViewsAnnounced = null; }
      }
      this.ensureHost(host);
      this.emitMergedTimeline(false);
      return;
  ...
  private closeRemote(host: string): void {
    ...
    // Stamped hostDrop (2026-08-18): a detach is the exact event after which this host's past
    ...
    for (const sid of this.perHostSids[host] || []) {
      ...
      // keeps the session's unsent draft for its return instead of clearing it like a close (T236), and
      // prunes the id from its kernelListed set (see the comment above this loop).
