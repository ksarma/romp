...
test("the tab menu's Billing submenu lists every login plus the key, the current one by WHICH login", () => {
  ...
  // (this fork's settings-pick hold leads the check: while a billing pick is HELD for live work the mark sits on the pick,
  // pick-held.ts heldMenuMarks, executed in auth-selector.test.ts; not held, T346's authChoiceCurrent as upstream reads it)
  assert.match(RENDER, /for \(const c of choices\) \{\s*\n\s*const cur = heldAuth \? \(heldAuth\.current \|\| st\.auth\) === c\.value : authChoiceCurrent\(st, c\.value\);/);
...
test("the picker rebuild runs: three choices are buttons, four are a dropdown, one usable choice is written out", () => {
  ...
  class El {
    ...
    constructor(tag: string, cls = "") { this.tag = tag; this.cls = new Set(cls.split(" ").filter(Boolean)); hideEdges(this); }
