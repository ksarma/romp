...
test("the poll's HEAD targets: the file, the sidecar the kernel named, config.json beside the root — never a client-computed sidecar path", () => {
  ...
  assert.match(SRC, /if \(!this\.ctx\.editing\(\)\) \{\n\s*if \(fileNow !== null\) this\.askReload\(fileNow\);\n\s*else if \(figureMoved\) this\.askReload\(null\);\n\s*\}/,
    "the file moved: one fetch, keyed on the mtime the HEAD saw, so the status that follows asks no second — never while the editor holds the body (Slice 5)");
...
test("the send sequence: build from the current status, set-tracked when asked, then send with the post-toggle verdict; a refusal aborts before the send", () => {
  ...
  assert.match(send, /const base = reply\.queued \? "Queued for " \+ who : "Sent to " \+ who \+ " at " \+ clock\(Date\.now\(\)\);\n\s*this\.sentNote = base;/, "the acknowledgment is the base alone: an accept resolves no comment (decision 42), so nothing moves to Resolved on a send");
...
test("every mutating verb: consent first, a fence from the current status, one retry on editing-off (re-consent) or a moved fence (fresh status), then the refusal verbatim with Reload", () => {
  ...
  // the composer's two writes go through writeFor, which marks the saving composer (savingFor) around mutate: a pin on where
  // the code lives. Executed in file-comments-save-held-composer.test.ts: the reply's write sent through mutate alone reds 5
  // tests there, among them "a reply refused after a new comment was started under its save…"; a passage comment's reds 21,
  // among them "a reply started while a comment saves opens empty…" and "a comment refused after a reply was started under
  // its save: its words are never filed as that reply"
  for (const [verb, sites] of [['"reply", { commentId: c.commentId, note }', 1], ['"comment", args', 2]] as Array<[string, number]>) {
    assert.equal(SRC.split("this.writeFor(c, " + verb).length - 1, sites, verb + " goes through writeFor() at each of its call sites, a region's and a passage's for a comment (a pin on where the code lives; the tests named above execute it)");
  }
  assert.match(SRC, /this\.savingFor = c;\n\s*try \{ return await this\.mutate\(verb, args, "composer"\); \}\n\s*finally \{ if \(this\.savingFor === c\) \{ this\.savingFor = null; this\.syncHeldBack\(\); \} \}/, "…and writeFor through mutate(), the composer marked for the round trip, unmarked however the write ends, a waiting comment's Bring it back following the unmarking (a pin on where the code lives. The mark is executed by file-comments-save-held-composer.test.ts, where a write that never marks its composer reds 25 tests, among them \"a new comment started while a comment saves opens empty…\" and \"while a comment saves, startReply (Reply) opens an empty box…\"; the unmark by file-comments-follow.test.ts \"edits on both sides of the selected copy…\", file-comments-region-tied.test.ts \"a region on the SECOND twin…\", file-comments-regions.test.ts \"a figure whose src the viewer rewrote through /file still matches its embed…\" and file-comments-save-held-composer.test.ts \"a refused comment waiting in its note is asked about…\", \"the close names every comment a yes would drop…\" and \"while a comment saves, the viewer's close asks…\", each red when the mark outlives the write; the following by the same file's \"on a fine pointer, a reply whose own save is refused…\" and \"on a coarse pointer, a new comment whose own save is refused…\", each red when the unmarking re-renders no reason)");
...
test("the sheets: the panel block is byte-equal in styles.css and feed.css, tokens only, sizes from the ladder", () => {
  ...
  assert.match(CHAT_CSS, /\.fileview-body \{ flex: 1 1 auto; min-height: 0; overflow: auto; \}/);   // no scrollbar gutter and no container: the table's cap reads the body's width off its ResizeObserver (--fv-body-w; Slice 3 of plans/markdown-viewer.md, review round 2; file-view-scrollbar-browser.test.ts)
});
