...
test("the Sessions pane's group-by-tag switch rides the same blob: present only while on, carried through the strip's writes (T399)", () => {
  ...
  assert.deepEqual(parseTabGroups('{"on":false,"timeline":true,"collapsed":["qa"]}'), { on: false, collapsed: ["qa"], expanded: [], pinned: [], hidden: [], timeline: true },
    "the switch rides beside this fork's hidden list, which every parse carries (tab-hide.test)");
  ...
  try {
    writeTabGroups({ on: true, collapsed: ["qa"], expanded: [], pinned: [], hidden: [], timeline: true });
    assert.equal(JSON.parse(store.get(TABGROUPS_KEY)!).timeline, true, "written while on");
    assert.deepEqual(readTabGroups(), { on: true, collapsed: ["qa"], expanded: [], pinned: [], hidden: [], timeline: true });
    const st = readTabGroups();
    writeTabGroups({ ...st, on: false });                                  // the strip's own toggle carries the pane's switch through
    assert.deepEqual(readTabGroups(), { on: false, collapsed: ["qa"], expanded: [], pinned: [], hidden: [], timeline: true });
...
test("executed: the PHONE layout sections like the desktop — the same headings in the same order, every member under its header — and folds NOTHING, whatever the store says", () => {
  ...
  const listener = "window.addEventListener(TABGROUPS_EVENT, () => { renderTabs(); viewsChanged(); });";   // the render re-arms the prefetch inside renderTabs (PR 1671 round four); this fork's notifier beside it (round 5 of the tab menu review)
...
test("a folded section renders its header alone with the folded-away count and one MEMBER-derived pip after it — a label, not a session; cycling skips folded ids", () => {
  ...
  // member needs you, the reason the user-todo flag exists); over the hidden members, after the count, under the
  // same ring switches the tab wears (the rings are widgets since 2026-09-14, T379: a ring switched off never shows on a header)
  assert.match(head, /const stand = standInPip\(hidden\.map\(\(id\) => \(\{ session: sessions\.get\(id\), ledger: ledgers\.get\(id\) \}\)\), ringSwitch\(settings\.tabWidgets\)\);/,
    "one summary pip, classified by tab-state.ts's rule, the same the tab itself wears (tab-state.test), with the feed's verdict folded in (tab-snapshot.ts standInPip; tab-hide.test) and the ring switches handed through");
...
test("headers are click-safe: data-act on the node, the action on the stable #tabs delegate, one render path via the event", () => {
  ...
  assert.match(RENDER, /window\.addEventListener\("storage", \(e\) => \{ if \(e\.key === TABGROUPS_KEY\) \{ renderTabs\(\); viewsChanged\(\); \} \}\);/, "…and a sibling pane's (the notifier beside the render there too; the open's reveal re-arms the prefetch inside renderTabs: strip-reveal-rearm.test.ts)");
...
test("the header's structure and gestures read as a label: the tag's chip, then the chevron (flips with the fold) and the count at the right; a keyboard button; hover/focus say fold, never open; tokens only (the user 2026-09-06)", () => {
  ...
  assert.match(RENDER, /const focusedGroup = \(focusedEl\?\.closest\("\.tab-group-head"\) as HTMLElement \| null\)\?\.dataset\.group;\s*\n\s*const focusedFlag = !!focusedEl\?\.classList\.contains\("tab-group-flag"\);\s*\n\s*const focusedDoor = !!focusedEl\?\.classList\.contains\("tab-group-door"\);[^\n]*\n\s*const focusedGear = !!focusedEl\?\.closest\("\.tab-widgets-gear"\);[^\n]*\n\s*const refocusTab = bar\.contains\(document\.activeElement\);/,
    "captured before the tab rule (chat-focus-model.test pins that rule's two-line shape); the open header's count button beside the flag (tab-hide.test), and the widgets gear (T395, T405)");
...
test("executed: the folded header's user-todo flag counts HIDDEN members only — a pinned member's own tab shows its glyph", () => {
  ...
  // meta's count for a skeleton or placeholder member (tab-group-flags.test.ts executes the count cases, 2026-09-22)
...
test("the toggle is a row in the tab menu's Tags flyout beside the Move-to rows: the home tag's chip, ✓ when on, per-section copy, the fold's own write and render path; the views adoption carries the pins across a rename (source pins)", () => {
  ...
  assert.equal((RENDER.match(/writeTabGroups\(setSectionCollapsed\(tabGroups\(\)/g) || []).length, 3, "the three fold writes (toggle-group, open-group, unfoldSectionOf — a pick of a folded-away tab)");
...
test("executed: a folded section whose EVERY member is pinned stays folded — the chevron tells the truth — and its header says so: the total, not 0, and why nothing is hidden", () => {
  ...
  // the section holding the tab being read: a phrase, not a different action (the user 2026-09-06) — open or folded
...
test("assistive tech hears a label: decoration is aria-hidden, the header's name is words (name, count, the pip's and the flag's phrases), and the active section's header is the same button, marked current", () => {
  ...
  assert.match(MAKE_HEAD, /head\.setAttribute\("aria-label", spoken\);\s*\n\s*head\.draggable = !settings\.tabsLocked;/, "set once, after the pip and the flag; an aria-label outranks name-from-content, so the header says what was appended and nothing that leaked in; the tab lock (T395) holds the groups too");
...
test("the folded-away active tab's header answers a tab's keys (the 2026-09-06 review): the window's ←/→ step from it, its own ←/→ step with focus following, Enter drops into the box while the transcript shows", () => {
  // the branch documented the header as the stand-in for focus AND stepping, but the window's ←/→ returned at
  // `if (i < 0) return;` for a folded active id, the header's keydown knew only Enter/Space, and onTabKey is
  // bound to .tab nodes alone, so from the header (where the box's Escape lands) the arrows did nothing, and
  // Enter folded the section and swapped the pane instead of returning to the box
  const win = RENDER.slice(RENDER.indexOf("// Window-level arrow nav"), RENDER.indexOf("// SELECT → TYPE → ⌘⏎"));
  assert.ok(win.length > 0 && win.length < 8000, "the window handler's block was found");
  assert.doesNotMatch(win, /if \(i < 0\) return;/, "the dead return is gone");
  assert.match(win, /const dir = e\.key === "ArrowRight" \? 1 : -1;\s*\n\s*const ord = visibleOrder\(\);[^\n]*\n\s*const i = ord\.indexOf\(activeId\);\s*\n\s*if \(i < 0\) \{\s*\n(?:\s*\/\/[^\n]*\n)*\s*const nb = collapsedTabIds\.has\(activeId\) \? neighborOfFolded\(lastStripItems, activeId, dir\) : null;\s*\n\s*if \(nb\) \{ e\.preventDefault\(\); setActive\(nb\); \}\s*\n\s*return;\s*\n\s*\}/,
    "folded away → step from the header's place (neighborOfFolded, the rule onTabKey and cycleTab share); a view-hidden active id still steps nowhere; no focus move (the window's rule)");
  // the header's own keys: ←/→ with focus following (onTabKey's shape), Enter into the box when it can, else the press
  const head = RENDER.slice(RENDER.indexOf("function makeGroupHead("), RENDER.indexOf("function sectionHeadOf("));
  assert.match(head, /const aid = activeId;\s*\n\s*const standIn = holdsActive && !!aid && collapsedTabIds\.has\(aid\) && sec\.ids\.includes\(aid\);\s*\n\s*if \(aid && standIn && \(e\.key === "ArrowLeft" \|\| e\.key === "ArrowRight"\)\) \{\s*\n\s*e\.preventDefault\(\);\s*\n\s*const nb = neighborOfFolded\(lastStripItems, aid, e\.key === "ArrowRight" \? 1 : -1\);\s*\n\s*if \(nb\) \{ setActive\(nb\); focusActiveTab\(\); \}\s*\n\s*return;\s*\n\s*\}/,
    "the stand-in alone answers the arrows: the MARKED folded holder (holdsActive), so with several folded holders only one header answers as the tab; any other header lets them bubble to the window's ←/→, which step the ACTIVE tab (upstream's #1301 review refinement, re-pinned at the 2026-09-15 pull-in)");
  assert.match(head, /if \(e\.key === "Enter" && standIn && !snapView && focusComposerOrAsk\(\)\) \{ e\.preventDefault\(\); return; \}\s*\n\s*if \(e\.key === "Enter" \|\| e\.key === " "\) \{ e\.preventDefault\(\); head\.click\(\); \}/,
    "Enter: the box while the transcript shows and the box takes input (the mirror of the box's Escape, which lands here); otherwise the press, as Space always is");
  // the stand-in condition, on the real plan: the active id folded away under this header; not a pinned active tab (on screen), not another section's
  const unions = viewTagUnion(V);
  const folded = setSectionCollapsed(parseTabGroups(null), "infra", true);
  const plan = planStrip(["web", "api", "tests"], unions, folded, "web", false);
  const infra = plan.items.find((i) => "head" in i && i.head.name === "infra") as { head: TabSection };
  assert.ok(plan.folded.has("web") && infra.head.ids.includes("web"), "web is folded away under infra: infra's header is its stand-in");
  const pinned = planStrip(["web", "api", "tests"], unions, { ...folded, pinned: [{ sid: "web", name: "infra", id: "g2" }] }, "web", false);
  assert.ok(!pinned.folded.has("web"), "a pinned active tab is on screen: no stand-in, the header's Enter and arrows are a plain header's");
  assert.equal(neighborOfFolded(plan.items, "web", 1), "api", "→ from the header lands on the first tab after it (wrapping)");
});
...
test("the section snapshot holds the reader's place in the hidden transcript (the T249 fold, 2026-09-08): captured in the pass that hides the views, written back when the snapshot hides; the scroll listener stays upstream's", () => {
  ...
  assert.match(SHOW, /if \(snapView && renderSnapshot\(\)\) \{\s*\n\s*setSnapMode\(true\);[^\n]*\n\s*for \(const v of views\.values\(\)\) v\.el\.style\.display = "none";\s*\n(?:\s*\/\/[^\n]*\n)*\s*const av = activeId \? views\.get\(activeId\) : null;\s*\n\s*if \(av && !snapKeep\) snapKeep = \{ v: av, scrollTop: av\.scrollTop, stick: av\.stick \};/,
    "captured once per visit, from the view's fields, in the synchronous pass that hides the views (the clamp's scroll event is only queued by then; the overview's mode class is set first, T322)");
...
test("the tab menu speaks for the right-clicked copy's group: Move to drops THAT tag, Show when folded pins THAT section, Rename edits THAT copy (T264b review)", () => {
  ...
  assert.ok(menu.indexOf("const homeNow = ") < menu.indexOf("const home = homeNow();   // read per build") && menu.indexOf("const homeNow = ") < menu.indexOf('"ctx-item ctx-item-toggle ctx-item-hide ctx-sub-capped"'),
    "the computation is declared above both its readers: the Tags flyout (section 2) and the Hide tab row (section 3, after the toggles since upstream regrouped the menu by kind, 2026-09-11)");
...
test("executed: activating a session under several tags springs no fold (T264b review, on the fork's folding-active rule): a holder showing a copy is marked, a hidden copy does not show, no copy showing → the first holder is the one stand-in", () => {
  ...
  assert.deepEqual([...planStrip(["s1", "s2", "s3", "s4"], unions, both, "s2", false).folded].sort(), ["s1", "s2", "s3", "s4"], "every copy off the strip: s2 is folded away too");
