// The editor chunk's id rule over the vendored field (plans/file-review.md, Slice 5): an id the decisions hold is
...
// field for the decisions — the next split of the same parent minted `X~1` a second time (found 2026-09-06). A save
...
// an id, every decision stays on record, and undo/redo restore what history recorded.
import { test } from "node:test";
import * as assert from "node:assert/strict";
import { EditorState, Transaction, type TransactionSpec } from "@codemirror/state";
import { undo, redo, isolateHistory } from "@codemirror/commands";
import { extensionsFor, trackSetup, type TrackDecisions } from "./editor-chunk";
...
const idsOf = (l: TrackDecisions) => [...l.accepted, ...l.rejected].map((e) => e.id);
...
function editor(doc: string, records: TrackRecord[], ext = "md") {
  ...
  const h = {
    ...
    disjoint: () => { for (const id of idsOf(h.decisions())) assert.ok(!h.ids().includes(id), `${id} is decided and still pending`); },
...
test("a decided fragment's id is not minted again by the next split of its parent: the save's two lists stay disjoint", () => {
  ...
  assert.deepEqual(h.decisions(), { accepted: [{ id: "X~1", oldText: "", newText: "fluffy " }], rejected: [] });
  const before = h.ops();
  ...
  assert.deepEqual(h.decisions().accepted.map((e) => e.id), ["X~1"], "the earlier decision is untouched");
});

test("a second decision joins the first among the decisions; undoing it leaves the first in place", () => {
  const h = editor(DOC, [X]);
  h.type(8, "very ");
  h.resolve(13, false);
  h.type(6, "g");
  ...
  assert.deepEqual(h.ids(), ["X"]);
  assert.deepEqual(h.decisions().accepted, [{ id: "X~1", oldText: "", newText: "fluffy " }, { id: "X~2", oldText: "", newText: "g " }]);
  h.disjoint();
  assert.equal(h.undo(), true);                                  // the second accept, not the first
  assert.deepEqual(h.ids(), ["X", "X~2"]);
  assert.deepEqual(h.decisions().accepted.map((e) => e.id), ["X~1"], "one undo removes one decision");
  assert.equal(h.redo(), true);
  assert.deepEqual(h.decisions().accepted.map((e) => e.id), ["X~1", "X~2"]);
});
...
test("undo of the typing restores the records before it; redo restores the renamed list history recorded", () => {
  ...
  assert.deepEqual(h.ids(), ["X"]);
  assert.deepEqual(h.decisions().accepted.map((e) => e.id), ["X~1"], "typing is no decision: undoing it leaves the decisions as they were");
  assert.equal(h.redo(), true);
...
test("a rejected id is taken too: the decisions' other side counts", () => {
  const h = editor(DOC, [X]);
  h.type(8, "very ");
  h.resolve(13, true);                                           // reject the right fragment: its text leaves
  assert.equal(h.doc(), "The big very cat sat.");
  assert.deepEqual(h.decisions().rejected.map((e) => e.id), ["X~1"]);
  h.type(6, "g");
  assert.deepEqual(h.ids(), ["X", "X~2"]);
  h.disjoint();
});

test("each decided split counts up: the decisions keep every fragment and the field never reuses one", () => {
  const h = editor(DOC, [X]);
  h.type(8, "very ");
  ...
  assert.deepEqual(idsOf(h.decisions()), ["X~1", "X~2"]);
  h.disjoint();
});
...
test("the parent of a fragment is its id less the last suffix: a decided X~1~1 makes the next split of X~1 mint X~1~2", () => {
  ...
  assert.deepEqual(idsOf(h.decisions()), ["X~1~1"]);
...
test("without a collision an ordinary keystroke is untouched: the records are exactly the engine's and the transaction carries no list effect", () => {
  ...
  // with decisions taken but no collision, still untouched
  h.resolve(13, false);
  const mid = h.ops();
...
test("a re-split keystroke that also reindents its line (indentOnInput adds a change to the same transaction) leaves records that fit the text", () => {
  ...
  assert.deepEqual(h.decisions().accepted.map((e) => e.id), ["X~1"]);
});

test("two splits of one parent in one transaction (a two-cursor keystroke): every fragment gets an id of its own, none the decisions hold", () => {
  const h = editor(DOC, [X]);
  h.type(8, "very ");
