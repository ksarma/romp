...
/** The Files page with the note open: as a FILE document (viewFile, the pane's own route) or as a URL document (openUrlView). */
async function inBrowser(t: any, kind: "file" | "url", body: (page: any, w: Watch) => Promise<void>): Promise<void> {
  ...
  try {
    ...
    await page.context().route("**/*", (route: any) => {
      ...
      if (u.href === URL_PATH || u.href === URL_SIBLING) {          // the URL kind's fetch, and a tab opened at the sibling
        w.docRequests.push(u.href);
        return route.fulfill({ status: 200, contentType: "text/markdown; charset=utf-8", body: u.href === URL_PATH ? NOTE : SIBLING_NOTE });
      }
    ...
    else await page.evaluate((u: string) => { (window as any).__rompProbe.openUrlView(u); }, URL_PATH);
...
test("a FILE document: every link shape keeps the Files document: a new tab for an absolute href, a scroll for a fragment, the viewer for a sibling; an image map is dropped and its picture is inert", { timeout: 90000 }, async (t) => {
  await inBrowser(t, "file", async (page, w) => {
    ...
    assert.deepEqual([svgXlink.tag, svgXlink.ns, svgXlink.href, svgXlink.xlink, svgXlink.target, svgXlink.rel],
      ["a", SVG, "https://example.invalid/xlink", "https://example.invalid/xlink", "_blank", "noopener"], "an SVG <a xlink:href> gets a plain href copy (mdBlock, before the module's walk), and the same stamps");
...
test("a URL document, with nothing in front of mdBlock's own stamps: an absolute href of every shape opens a new tab (the SVG anchors' target is an ATTRIBUTE), the relative sibling resolves against the document's URL, the fragment scrolls through the body's fv-anchor delegate", { timeout: 90000 }, async (t) => {
  await inBrowser(t, "url", async (page, w) => {
    ...
    // 2. the SVG anchor's relative `sibling.md`: the first LINK_SEL pass resolved it against the document's URL, the second
    //    stamped it, so it opens a tab at the sibling's URL (in the chat page the delegate would route that same-origin .md
    //    back into the viewer; there is no delegate here, and the tab is the stamps' own outcome)
    {
      const popups0 = w.popups.length, navs0 = w.navs.length;
      const tabUrl = await clickForTab(page, ".fx-svg-rel text", "the SVG anchor's relative href");
      assert.equal(tabUrl, URL_SIBLING, "the relative href resolved against the document's URL, not the page's");
      assert.equal(page.url(), START, "sibling: location.href is unchanged");
      assert.deepEqual(w.navs.slice(navs0), [], "sibling: no main-frame navigation");
      assert.equal(w.popups.length - popups0, 1, "sibling: one new tab");
    }
    ...
    assert.deepEqual([svgXlink.tag, svgXlink.ns, svgXlink.href, svgXlink.xlink, svgXlink.target, svgXlink.rel],
      ["a", SVG, "https://example.invalid/xlink", "https://example.invalid/xlink", "_blank", "noopener"], "an SVG <a xlink:href> gets a plain href copy, and the same stamps");
