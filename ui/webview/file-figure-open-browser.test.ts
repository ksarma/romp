...
// its round 18, extra5-2, the clicks after a drag in another pane and the cost cells after a drag among them),
...
// to 3 on it; its round 18, extra5-2, with the coordinator's decisions and the closing check after those fixes) ──
// file-figure-open-taps.ts holds the cells and says what
// each reads; file-figure-open-engines-browser.test.ts runs the same set in WebKit and Firefox, a leg of its own off the shared roster
// of browser legs. Chromium gives a tap's click the touch's own pointerId, a precondition each tap cell asserts, and the click finds
...
// opens with the control shown, where at 2f5d29017, the head before that drop, each overlay stood fixed over the control and no gesture
// opened. The drops narrow the cost the file review's round 16
