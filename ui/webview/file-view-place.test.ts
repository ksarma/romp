...
// column of boxes, the follow of a kept span through an edit, and the anchor map's two block-table reads and its Raw
// row span, run under node over the anchor-map suite's DOM stand-in (marked's output parsed into a minimal tree, no
...
import { topVisibleIndex, followPlace, readPlace, seatPlace, LOOKBACK, type Place } from "./reader-place";
import { renderedBlockSpan, renderedBlockAt, rawRowSpan, rawRowForOffset } from "./anchor-map";
...
// ── topVisibleIndex ────────────────────────────────────────────────────────────────────────────────
test("topVisibleIndex: the first box whose bottom lies below the edge; count when none; a taller box just above the hit wins; a hidden box (every edge 0) reads as above", () => {
  ...
  // a floated figure (index 1) reaching below the two paragraphs wrapping beside it: the search lands on 3, the lookback
  // finds 1 still showing, and the figure, first in the order, is the top block
  const floated = [40, 250, 120, 160, 200];
  assert.equal(topVisibleIndex(5, (i) => floated[i], 130), 1);
  // a figure (index 1, bottom 900) with more wrapped paragraphs beside it than the lookback reads is passed over for the
  // first paragraph ending below the edge; with one paragraph fewer between them the lookback reaches the figure
  const far = [40, 900, ...Array.from({ length: 12 }, (_, i) => 60 + i * 20), 950];
  const hit = 1 + LOOKBACK + 1;   // the first paragraph ending below the edge sits LOOKBACK + 1 boxes after the figure
  assert.equal(topVisibleIndex(far.length, (i) => far[i], far[hit] - 5), hit, "the lookback stops one box short of the figure");
  assert.equal(topVisibleIndex(far.length, (i) => far[i], far[hit - 1] - 5), 1, "one box nearer, and the figure is the top block");
...
// ── followPlace ────────────────────────────────────────────────────────────────────────────────────
test("followPlace: unchanged text keeps the offset; an insertion above shifts it by the inserted length; one below leaves it; a rewrite of the block itself is placed by the block before it (an insertion above at the same time included); with that block gone too, the edit's start", () => {
  ...
  const place = (source: string, i: number): Place => ({ source, ...span(source, i), top: -12, prev: i > 1 ? span(source, i - 1) : null });
  assert.deepEqual(followPlace(place(DOC, 5), DOC), { at: at(DOC, 5), next: false }, "the same text: the same offset");
  const above = DOC.replace(PARA(2), "Inserted A: new words.\n\nInserted B: more new words.\n\n" + PARA(2));
  assert.deepEqual(followPlace(place(DOC, 5), above), { at: at(above, 5), next: false }, "twenty (here two) paragraphs above: shifted by their length");
  ...
  assert.deepEqual(followPlace(place(DOC, 5), below), { at: at(DOC, 5), next: false }, "an edit below: the offset stands");
  const rewritten = DOC.replace(PARA(5), "Rewritten 5: the session replaced this paragraph.");
  assert.deepEqual(followPlace(place(DOC, 5), rewritten), { at: at(DOC, 4), next: true }, "the block itself rewritten: the block before it, and `next` says the one after that");
  const both = above.replace(PARA(5), "Rewritten 5: the session replaced this paragraph.");
  assert.deepEqual(followPlace(place(DOC, 5), both), { at: at(above, 4), next: true }, "inserted above AND rewritten: the block before, followed through the insertion (the edit's start would be twenty paragraphs up)");
  const rewrittenFrom4 = DOC.replace(PARA(4) + "\n\n" + PARA(5), "Rewritten 4 and 5: two paragraphs became one.");
  assert.deepEqual(followPlace(place(DOC, 5), rewrittenFrom4), { at: at(DOC, 4), next: false }, "the block before rewritten too: the edit's start, before the kept block's old offset");
  // the block's text moved whole to another place inside the edited region: followed there
  const swapped = DOC.replace(PARA(5) + "\n\n" + PARA(6), PARA(6) + "\n\n" + PARA(5));
  assert.deepEqual(followPlace(place(DOC, 5), swapped), { at: swapped.indexOf(PARA(5)), next: false }, "two blocks swapped: the block is found where it went");
  // a Raw row's span (one line) follows the same way
  assert.deepEqual(followPlace({ source: DOC, start: at(DOC, 7), end: at(DOC, 7) + 12, top: 3, prev: null }, above), { at: at(above, 7), next: false });
  // the whole text replaced: the offset is clamped into the new text
  assert.deepEqual(followPlace(place(DOC, 9), "short"), { at: 0, next: false });
});

// ── the anchor map's block table ──────────────────────────────────────────────────────────────────
test("renderedBlockSpan / renderedBlockAt: a top-level element answers its block's source span; an offset inside a block answers the block's element, one past the last block the last, one before the first null; a refused block (a table, a code block) answers both; whitespace between blocks answers no span", () => {
  const src = "# Title\n\nFirst paragraph here.\n\n| a | b |\n| - | - |\n| 1 | 2 |\n\n```\ncode line\n```\n\nLast paragraph.\n";
  const { md, blocks } = rendered(src);
  assert.deepEqual(blocks.map((b) => b.tagName), ["H1", "P", "TABLE", "PRE", "P"]);
  const spans = blocks.map((b) => renderedBlockSpan(El(md), src, El(b)));
  ...
  assert.equal(renderedBlockSpan(El(md), src, ws as unknown as Node), null);
  // offset → element
  assert.equal(renderedBlockAt(El(md), src, 0), El(blocks[0]));
  assert.equal(renderedBlockAt(El(md), src, src.indexOf("paragraph here")), El(blocks[1]), "inside the paragraph");
  assert.equal(renderedBlockAt(El(md), src, src.indexOf("| 1 |")), El(blocks[2]), "inside the table: its element, though the block is refused");
  assert.equal(renderedBlockAt(El(md), src, src.indexOf("code line")), El(blocks[3]));
  assert.equal(renderedBlockAt(El(md), src, src.indexOf("First") - 1), El(blocks[0]), "the blank line after a block belongs to the block before it");
  assert.equal(renderedBlockAt(El(md), src, src.length + 50), El(blocks[4]), "past the end: the last block");
  assert.equal(renderedBlockAt(El(md), src, -1), null, "before the first block: nothing stands there");
  // a block with no element of its own (an HTML comment) is passed over for the one before it
  const withComment = "First.\n\n<!-- a note to self -->\n\nSecond.\n";
  const r2 = rendered(withComment);
  const commentAt = withComment.indexOf("<!--");
  assert.equal(r2.blocks.length, 2, "the comment renders nothing");
  assert.equal(renderedBlockAt(El(r2.md), withComment, commentAt + 3), El(r2.blocks[0]), "an offset in the comment: the paragraph before it");
});
...
test("readPlace: the top-visible block's span and height in the Rendered view and the Raw view; null over a tree with no layout, over the loader, and when the edge is past every block", () => {
  ...
  // the Raw view: rows 20px apart from y=0; row 5 (y 100..120) is the first ending below the edge
  const w = raw(DOC, 0, 20);
  const q = readPlace(w.body as unknown as HTMLElement, DOC)!;
  const lines = DOC.split("\n");
  assert.equal(DOC.slice(q.start, q.end), lines[5], "row 5's line: " + lines[5]);
  assert.equal(lines[5], "", "which is a blank line between two paragraphs");
  assert.equal(q.top, 0, "its top at the body's top edge");
  assert.equal(DOC.slice(q.prev!.start, q.prev!.end), PARA(2), "the row before it with text");
...
test("seatPlace: scrolls the body by the difference between where the kept block's element stands and where it stood, following the span through the edit; a Rendered place seats in a Raw view at the block's first row; false when nothing stands at the offset", () => {
  // the reader had paragraph 5 with its top 12px above the body's top edge
  const kept: Place = { source: DOC, start: DOC.indexOf(PARA(5)), end: DOC.indexOf(PARA(5)) + PARA(5).length, top: -12, prev: { start: DOC.indexOf(PARA(4)), end: DOC.indexOf(PARA(4)) + PARA(4).length } };
  ...
  assert.deepEqual(followPlace(kept, above), { at: above.indexOf(PARA(5)), next: false });
  ...
  // the same in the Raw view: the row after paragraph 4's row that holds text (a blank row lies between)
  ...
  assert.equal(w4.body.scrollTop, (rewrittenLine * 20 - 100) - (-12), "row " + rewrittenLine + ", the rewritten line");
  // a Rendered place seats in the Raw view: the row holding the block's first character
  const w = raw(DOC, 0, 20);
  const line = DOC.slice(0, DOC.indexOf(PARA(5))).split("\n").length - 1;
  w.body.scrollTop = 0;
  assert.equal(seatPlace(w.body as unknown as HTMLElement, DOC, kept), true);
  assert.equal(w.body.scrollTop, (line * 20 - 100) - (-12), "row " + line + " brought to where the block stood");
  ...
  assert.equal(seatPlace(r.body as unknown as HTMLElement, DOC, { source: DOC, start: -5, end: -5, top: 0, prev: null }), false);
});
...
test("file-view.ts: the place is read before the text swap and seated after the hooks (the selection keeper's order); the width reflow seats the place read at scroll time; a text-size step reads and seats around its reflow; the URL viewer's switch seats too", () => {
  ...
  assert.match(local, /const notePlace = \(\) => \{ if \(shownText !== null && textShowing\(\)\) \{ place = readPlace\(body, shownText\); placeWidth = body\.clientWidth; \} \};/);
  assert.match(local, /body\.addEventListener\("scroll", \(\) => \{\n\s*if \(placeFrame\) return;\n\s*const read = \(\) => \{ placeFrame = 0; if \(body\.clientWidth === placeWidth\) notePlace\(\); \};/,
    "the scroll-time read, once per frame, and not under a width the last read did not see (the reflow's own clamp)");
