...
test("file-view.ts: the place is read before the text swap and seated after the hooks (the selection keeper's order); the width reflow seats the place read at scroll time; a text-size step reads and seats around its reflow; the URL viewer's switch seats too", () => {
  ...
  assert.match(url, /folds\.note\(\);[^\n]*\n\s*const kept = keptPlace\(\);[^\n]*\n(?:\s*\/\/[^\n]*\n)*\s*try \{\n\s*body\.replaceChildren\(fmt\.md === "rendered"\n[^\n]*\n[^\n]*\n\s*renderFell = null;\n\s*\} catch \(err\) \{\n[^\n]*\n[^\n]*\n\s*\}\n\s*folds\.restore\(\);[^\n]*\n\s*if \(fmt\.md === "rendered"\) stampBodyWidth\(\);[^\n]*\n\s*shownText = text;\n\s*seat\(kept\);/, "the URL viewer reads the folds and the place before its swap (inside the try Slice 7's item 1 put around the build and the swap), restores the folds, stamps the fresh tables' width and seats after it (the stamp before the seat, as the local viewer orders it)");
