...
test("file-view.ts: the place is read before the text swap and seated after the hooks (the selection keeper's order); the width reflow seats the place read at scroll time; a text-size step reads and seats around its reflow; the URL viewer's switch seats too", () => {
  ...
  assert.match(local, /const kept = keptPlace\(\);[^\n]*\n(?:\s*\/\/[^\n]*\n)*\s*try \{\n\s*body\.replaceChildren\(rendered \? mdBlock\(text, \{ kind: "file", path, sid: sid \|\| null \}\) : codeBlock\(text, path, true\)\);[^\n]*\n\s*renderFell = null;\n\s*\} catch \(err\) \{\n\s*const fell = fellMessage\(err\);\n\s*body\.replaceChildren\(renderFellLine\(fell\), codeBlock\(text, path, true\)\);\n\s*renderFell = fell;[^\n]*\n\s*\}\n(?:\s*if \(text === ""\) body\.prepend\([^\n]*\);[^\n]*\n)?\s*viewError = null;[^\n]*\n\s*folds\.restore\(\); restoreHeldFolds\(\);[^\n]*\n\s*stampBodyWidth\(\);[^\n]*\n\s*syncOutline\(\);[^\n]*\n\s*fireRendered\(\);[^\n]*\n\s*shownText = text;\n\s*seat\(kept\);/,
    "read, swap (inside the try Slice 7's item 1 put around the build and the swap: a render that throws paints the failure line and Raw rows and then records the message, the review's round 2, and the pass goes on), folds (the keeper's, then a remembered record's held past a Raw first paint; Slice 6), the tables' width stamp, the Outline button's visibility (the bar's layout, Slice 6), hooks, then seat over the new text");
  ...
  assert.match(local, /const seat = \(kept: Place \| null\) => \{\n\s*const clamped = kept && shownText !== null \? seatPlaceOutcome\(body, shownText, kept\)\.clamped : false;\n\s*if \(clamped && kept\) \{ place = kept; placeWidth = body\.clientWidth; placeScrollTop = body\.scrollTop; placeHeld = true; asideScrollTop = -1; scrollUnread = false; \}\n\s*else notePlace\(\);\n\s*\};/,
    "a seat reads the place anew after it, or holds the one it was given when the write was clamped; either way the aside hook's number is consumed, and the unread-scroll flag falls with the place made current (Slice 6 of plans/markdown-viewer.md, the review's closing pass)");
  assert.match(local, /const held = \(\): boolean => placeHeld && place !== null && body\.scrollTop === placeScrollTop;\n\s*const keptPlace = \(\): Place \| null => \(shownText === null \? null : held\(\) \? place : readPlace\(body, shownText\)\);/, "the held place is what the next swap keeps, while the body stands (held, the one test the leave shares since the review's closing pass)");
  assert.match(local, /const notePlace = \(\) => \{ if \(shownText !== null && textShowing\(\) && !unmeasurable\(\)\) \{ place = readPlace\(body, shownText\); placeWidth = body\.clientWidth; placeScrollTop = body\.scrollTop; placeHeld = false; asideScrollTop = -1; scrollUnread = false; \} \};/, "a measured read makes the place current and clears the unread-scroll flag (the review's closing pass)");
  ...
  assert.match(local, /body\.addEventListener\("scroll", \(\) => \{\n\s*if \(placeFrame\) return;\n\s*const read = \(\) => \{\n\s*placeFrame = 0;\n\s*if \(unmeasurable\(\)\) \{ scrollUnread = true; return; \}[^\n]*\n\s*if \(placeHeld && body\.scrollTop === placeScrollTop\) return;[^\n]*\n\s*if \(body\.clientWidth === placeWidth \|\| \(pastAside\(\) && !clamped\(\)\)\) notePlace\(\);\n\s*\};/,
    "the scroll-time read, once per frame: every scroll under the width last read; under a new width only a scroll past the aside hook's number, the clamp excepted; with no hook, none; a held place stands through the scroll event of the seat that was clamped; a frame that finds the body with no box (the pane hidden in the scroll's own task) marks the scroll unread for the show's repaint (Slice 6 of plans/markdown-viewer.md, the review's round 6)");
  assert.match(local, /paintedWidth = seenWidth;\n\s*if \(!textShowing\(\)\) return;\n\s*fireRenderedKeepingSelection\(\);[^\n]*\n(?:\s*\/\/[^\n]*\n)*\s*if \(unmeasurable\(\)\) return;\n(?:\s*\/\/[^\n]*\n)*\s*const moved = placeFrame !== 0;\n\s*const restored = scrollUnread && body\.scrollTop > 0 && place !== null && place\.source === shownText && body\.clientWidth === placeWidth && !\(moved && body\.scrollTop < placeScrollTop\);\n\s*if \(restored\) notePlace\(\); else seat\(place\);[^\n]*\n\s*landRemembered\(\); landTarget\(\);/, "the width reflow fires the hooks for every report of a text view, then seats the tracked place (or, at the show over a scroll the hide kept from being read, reads the offset the browser restored; round 6; unless the restore moved the body, its scroll event's read pending, to below the place last measured: then the place is seated; the closing pass), then a remembered record a first paint under a boxless body left pending (Slice 6, the review's round 4), then the target and the keyboard such a paint left pending; the report of the box going seats and lands nothing (round 5)");
  assert.match(local, /const kept = textShowing\(\) \? keptPlace\(\) : null;[^\n]*\n\s*apply\(\);\n\s*if \(textShowing\(\)\) \{ fireRenderedKeepingSelection\(\); seat\(kept\); if \(!keyboard\) takeKeyboard\(\); \}/, "a text-size step reads before the size changes and seats after the hooks (the bracket around textSizeControl's apply), then the keyboard rule (Slice 6 of plans/markdown-viewer.md, item 1)");
  const url = VIEW.split("export function openUrlView(")[1].split("\nexport function ")[0];
  assert.match(url, /folds\.note\(\);[^\n]*\n\s*const kept = keptPlace\(\);[^\n]*\n(?:\s*\/\/[^\n]*\n)*\s*try \{\n\s*body\.replaceChildren\(fmt\.md === "rendered"\n[^\n]*\n[^\n]*\n\s*renderFell = null;\n\s*\} catch \(err\) \{\n[^\n]*\n[^\n]*\n[^\n]*\n\s*\}\n(?:\s*if \(text === ""\) body\.prepend\([^\n]*\);[^\n]*\n)?\s*folds\.restore\(\);[^\n]*\n\s*if \(fmt\.md === "rendered"\) stampBodyWidth\(\);[^\n]*\n\s*shownText = text;\n\s*seat\(kept\);/, "the URL viewer reads the folds and the place before its swap (inside the try Slice 7's item 1 put around the build and the swap; item 6's empty-document line may follow the try), restores the folds, stamps the fresh tables' width and seats after it (the stamp before the seat, as the local viewer orders it)");
  ...
  assert.doesNotMatch(install.split("\n").filter((l) => /addEventListener\("scroll"/.test(l)).join("\n"), /hideFloatOnScroll/, "not among installLayout's scroll listeners, which feed the margin lock's mirrorScroll and act in the margin layout alone");
