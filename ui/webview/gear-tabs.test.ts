...
test("the Tab widgets section's rows come from the strip's own module: built once, painted in place, a sliding switch, house pickers for the options", () => {
  ...
  assert.match(GEAR_CSS, /#rsettings \.rs-row:hover \.rs-sub, #rsettings \.rs-row:has\(:focus-visible\) \.rs-sub, #rsettings \.rs-widget:hover \.rs-sub, #rsettings \.rs-widget:has\(:focus-visible\) \.rs-sub \{ display: block; position: absolute;/, "the widget rows share the panel's popover rule, on hover and on a keyboard focus (2026-09-20: the :has(:focus-visible) twins, gear-sub-focus-browser.test.ts)");
