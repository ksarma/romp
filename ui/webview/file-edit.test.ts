...
test("a conflict keeps the buffer, says why, and Reload asks before discarding", () => {
  assert.match(VIEW, /box\.insertBefore\(bar2, main\);/, "the error bar sits ABOVE the body row that holds the textarea (a child of the card since Slice 2 of plans/markdown-viewer.md) — the buffer survives");
