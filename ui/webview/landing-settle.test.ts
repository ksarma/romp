...
test("the census names every writer render.ts gives the write family, and nothing else; every writer is a plain string literal or a registered wrapper's own parameter (round five; round six; round seven, by the compiler's parser)", () => {
  ...
  assert.equal(classified.length, 27, "the census: 27 writers (round five; gap-fill, land-guess and land-cancel joined with the history regions, T386 stage 2; PR E's frame-end take writes nothing of its own: it asks for the paint, whose append-stick follows)");
  assert.ok(!("spacer-follow" in LS.WRITER_CLASS) && !RENDER.includes('"spacer-follow"'), "no writer inside the unit observer's callback (a write there re-sizes the view under its own observer: the ResizeObserver loop error)");
