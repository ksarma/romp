...
function renderFleetNode(ctx: SessCtx, n: LedgerNode, depth: number, container: HTMLElement, now: number, flat: boolean) {
  ...
  if (flat && depth === 0) {
    ...
    const sd = statusDot(s); if (sd) tag.appendChild(sd);   // the full status-pip language, not just working (2026-08-09)
...
function render() {
  ...
  if (grouped) {
    ...
    for (const { ctx, visibleRoots } of survivors) {
      ...
      const hd = statusDot(s); if (hd) head.appendChild(hd);   // the full status-pip language, not just working (2026-08-09)
