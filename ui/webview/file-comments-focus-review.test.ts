...
// click: nothing would center the card, and the cards above it went past the track's start); the keyboard stays on Show
...
test("a mark clicked in the list layout does not anchor the margin layout when the columns come back: a resize is not a click, so the cards lie by the push-down rule, the loose card at the track's start", async (t) => {
  ...
  assert.equal(w.top(whole.id), 8, "the loose card at the track's start, not above it");
  assert.equal(w.top(findings.id), desired(2), "the heading's card level with its mark");
