...
test("PER-INSTALL kernel settings are stamped like the queued class but stay OUT of KERNEL_SETTING", () => {
  // Thinking summaries and the Requests switch: each kernel's answer is its own, so the post goes to the LOCAL
  // kernel only (federation must never queue or broadcast it) while the kernel still orders applies by `gt`
  // (two dashboards on one kernel race), so the emitter stamps through the gesture clock under its own store
  // name exactly like the completeness pin below demands of the queued class. Extend PER_INSTALL when adding one.
