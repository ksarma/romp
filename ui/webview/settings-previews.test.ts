...
test("(5) the renderer is one module: render.ts imports it and keeps only the chat's live hooks", () => {
  ...
  // the pending hook also carries this fork's two reload flags (the settings-pick hold, review round 7: st.modePending and st.fastPending)
  assert.match(RENDER, /^const META_HOOKS: MetaHooks = \{\n\s*onPress: \(kind, btn, forSid\) => toggleMetaMenu\(kind, btn, forSid\),\n\s*pending: \(kind, st\) => \{ const s = st as Status; return \(kind === "mode" && !!s\.modePending\) \|\| \(kind === "fast" && !!s\.fastPending\) \|\| isMetaPending\(kind, s\); \},\n\};/m, "the chat's hooks: the picker and the pending heuristic");
  ...
  // the chat's sync wrapper settles the hold for every kind first (the hold-end frame retires a loader armed during it,
  // review round 6), runs the module's pass, then this fork's held post-pass (applyHeldMarks: the settings-pick hold's mark and tip)
  assert.match(RENDER, /^function syncMetaControls\(meta: HTMLElement, st: Status, forSid\?: string \| null\): void \{\n(\s*\/\/[^\n]*\n)*\s*for \(const kind of \["mode", "model", "effort", "fast"\] as MetaKind\[\]\) settleMetaHold\(forSid \|\| activeId \|\| "", kind, !!st\.pickHeld && st\.pickHeld\.surfaces\.includes\(kind\)\);\n\s*syncMetaControlsWith\(meta, st, forSid, META_HOOKS\);\n\s*applyHeldMarks\(meta, st, forSid \?\? null\);/m);
