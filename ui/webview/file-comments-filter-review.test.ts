...
test("pins: replyAway takes the filter into account before the Resolved fold and has no change fold case (the about follow-on: no comment is behind the changes fold); the saved line's state, its action, and its place; the head's confirm rows go above the filter row in the DOM while the source order the filter suite pins stands", () => {
  ...
  assert.match(SRC, /const hid = r !== null && c\.kind !== "reply" && this\.noteHiddenSave\(before, note\);\n\s+const lined = r !== null && this\.landSaved\(c, had, r, note\);[^\n]*\n\s+if \(r && this\.composer === c\) this\.closeComposer\(\);[^\n]*\n\s+else if \(this\.composer !== c\) this\.settleAway\(c, note, r\);[^\n]*\n\s+if \(hid \|\| \(lined && c\.kind !== "reply"\)\) this\.render\(\);/, "noted after the save, before the landing (which finds no card the filter hides, so raises no line of its own) and the composer's close; the cards rendered once more with the line (a pin on where the lines stand; the close's condition, the saving composer only, is executed by file-comments-save-held-composer.test.ts \"a reply started while a comment saves opens empty…\", \"a new comment started while a comment saves opens empty…\", \"while a comment saves, a new comment's Save waits…\" and \"with no save out the box still carries its words…\", each red when a landing closes whatever composer is open; the settle away by the same file, where settleAway never running reds 30 tests, among them \"a comment refused after a reply was started under its save, the reply's box still empty…\", \"a comment refused after the person typed in a reply started under its save…\" and \"Cancel during a comment's save, then a refusal…\")");
