...
test("the column commands exist under their new meanings, the move to a new column keeps the editor convention, the rest are unbound, and a new column gets the chords", () => {
  ...
  assert.match(MAIN, /\["f-chat", "f-fleet", "f-feed", "f-waiting", "f-files", "f-timeline", "f-settings"\]\.forEach\(\(id\) => wireKeys\(pane\(id\)\)\);/);   // + this fork's Waiting pane, in every pane list   // + the Files pane and the settings iframe (main, 2026-09-10: the gear's document holds the keyboard while open)
