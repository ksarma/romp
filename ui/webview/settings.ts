...
export interface RompSettings {
  ...
  perfMute: boolean;    // the kill switch beside it: on, this browser posts no timing row at all, and on a kernel page no connection or breadcrumb row either (the collector's minute and slowframe rows stop everywhere; the pane shim's return, close and stale rows, the panes' own breadcrumbs and the shell's rows stop on a kernel page, where every row passes the shim's send; in VS Code the extension forwards a pane's breadcrumbs to the kernel and this switch does not reach it). OFF by default; only the literal true turns it on; read raw by the collector, the shim and the shell script.
