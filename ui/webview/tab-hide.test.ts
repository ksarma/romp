...
const CTX = ui("webview", "ctx-menu.ts");   // the shared menu builder (upstream's v0.16.0 tidy): the tab menu's card, rows, placement and dismissal listeners live there
...
test("executed + pinned: nothing lost. An open header wears the pip and the todo flag over its hidden members, and its words say how many are hidden; the strip's signature reads the hides", () => {
  ...
  assert.match(GROUPS, /export type StripHead = \{ head: TabSection; folded: boolean; active: boolean; hidden: string\[\]; hides: string\[\]; packed: boolean \};/, "the item the signature serializes carries the hides (and the packed bit, upstream's fold-pack)");
...
test("executed: the pane's model. Hidden rows are flagged and keep needs-you; the heading counts them; the fold's words; the Hide and Show words; a flip alone is a new model", () => {
  ...
  assert.equal(m.rows[1].state, "", "a quiet session the feed files under needs-you has no state phrase: the word rides the chip, not the state (T322b)");
  assert.deepEqual(m.rows[1].chip, { state: "needsInput", text: "Blocked", peer: null }, "the feed's column word, on the row's chip");
  ...
  // the words
  assert.deepEqual(snapshotHeading("infra", 3, 1), { count: "3 sessions, 1 hidden", label: "Overview of infra: 3 sessions, 1 hidden; click one to open it" });
  assert.deepEqual(snapshotHeading("infra", 3), { count: "3 sessions", label: "Overview of infra: 3 sessions; click one to open it" }, "nothing hidden: as before (T322: the words the heading shows)");
  ...
  assert.equal(rowWords(m.rows[1]).label, "api; hidden from the strip; Blocked; Designing the notes schema", "a reader hears why the row sits under the fold, then the chip's word (T322b: the label says what is shown)");
...
test("executed: the phone layout sections like the desktop with every section open and applies no hide (upstream's phone picker order, 2026-09-16; the hides follow the folds' phone rule, since the picker is the phone's only switcher), and sectioning off renders the flat strip, hidden members included", () => {
  const phone = planStrip(ALL, unions, apiHidden, "web", true);
  assert.deepEqual(phone.items, planStrip(ALL, unions, { ...d, collapsed: [], expanded: ["archived"] }, "web", false).items, "the phone: the desktop's plan with every section open and nothing hidden");
  assert.ok(phone.items.some((i) => "id" in i && i.id === "api"), "the hidden member shows on the phone: a hide has no door there and would take a session out of its only switcher");
  assert.ok(!heads(phone.items).some((h) => h.folded || h.hidden.length || h.hides.length), "no header folded, none standing in for a hidden member");
  ...
  assert.deepEqual([...phone.folded], [], "nothing skipped on the phone");
});
...
test("docs and the sheet: the guide's paragraph, the reference's section, the sheet's rules (tokens only, the one sub-line size), and no em dash in the new text", () => {
  ...
  assert.match(GUIDE.replace(/\s+/g, " "), /To move a tab into another group, right-click it and pick \*\*Move to <tag>\*\* under \*\*Tags\*\*: one click adds that tag and drops the tag of the group you right-clicked it in, leaving its other tags alone\. A change from another pane or another dashboard can take the tab out of that group just before you click, or remove the group's tag and make it again under the same name: the click then moves nothing, the row flashes, and its tooltip says to click again\. If the change moved the tab into the group you picked, that row is gone instead \(the tag is one of the tab's own now, with a \*\*✕\*\* beside it\); if it only added that group, the row is still there and the click drops the group you right-clicked it in, the move you asked for\. \*\*Show when folded\*\* refuses the same way while the tab is still in that group or has left it for exactly one other group whose tag already exists\. When the tab has left for no group, for two or more \(the menu cannot tell which copy you mean\), or for a tag that is still being created, there is nothing to pin: its row leaves the flyout and the menu stays open\. \*\*Hide tab\*\* refuses the same way on a re-created tag, while the \*\*✕\*\* beside a tag and the row's \*\*\+\*\* act on the re-created tag at once\. The row's \*\*\+\*\* adds the tag without moving the tab\./, "round 11: the destination that now holds the tab is two cases in one sentence (the tab moved into the group you picked: that row gone, no cue; the group only added under the press, the tab still in the group you right-clicked: the row still there and the click the move you asked for, dropping the source), and the pin row's one-other-group clause takes the pending qualifier (a tag still being created counts as no group, so the row leaves the flyout); round 10: the Move to sentence names the cause once and both refusing cases (F executes the tab that left, E4 the re-created tag), the pin row's condition (G2: the cue on the one-holder re-home and under the re-created tag, the row gone under none or two or more, the menu standing), Hide tab's own refusal (K8) and the x and + acting at once (E4, H); round 9 said the pin row refuses the same way in every case and hung the cause on the re-created tag alone; round 8 named the first case alone and no test pinned the sentence");
  ...
  assert.match(flat, /the fold's head says so in red before you open it, and its row wears the \*\*Blocked\*\* chip, or the ⚑ alone for a request it flagged while still working\./, "ruling 15 b: the row's word is the shared status chip's (T322b); a request flagged while the session still works shows the flag alone");
  ...
  assert.match(ref, /The tab's right-click menu writes the same hide entry: \*\*Hide tab\*\* on a shown copy, \*\*Show tab\*\* on a hidden one \(a hidden copy has no tab on the strip; the way back is the view's \*\*Show\*\*, or the same menu from a right-click on its row there, and the group's count opens the view while the group is open\)\./, "the menu door (the user 2026-09-09); round 1: the way back names the click that reaches the view; the row's right-click (the user 2026-09-18)");
...
class FakeEl {
  ...
  text = ""; innerHTML = ""; title = ""; type = ""; placeholder = ""; maxLength = 0; value = ""; id = "";   // `text`: the node's own text; textContent (below) composes it with the children's
  ...
  /** a string is a text node (T321: the flyout's rows compose their label as `lb.append("Move to ", chip)`, a word and the tag chip) */
  append(...cs: Array<FakeEl | string>) { for (const c of cs) this.appendChild(typeof c === "string" ? FakeEl.text(c) : c); }
  static text(s: string): FakeEl { const t = new FakeEl("#text"); t.text = s; return t; }
  /** as the DOM reads it: the node's own text, then its descendants' in order; a write replaces the children with the text */
  ...
  hasAttribute(k: string) { return k in this.attrs; }   // the builder's rows() reads it (showMenuCard marks a hand-built row menuitem and focusable)
...
type MenuApi = { open: (id: string, copy?: string, at?: { x: number; y: number }) => FakeEl; at: () => { x: number; y: number } | null; push: (views: unknown) => void; changed: () => void; pending: () => unknown; doc: () => { fire: (k: string, ev?: unknown) => void; count: (k: string) => number } };
...
function liftShowTabMenuRaw(): (hooks: MenuHooks) => MenuApi {
  ...
  // THE MENU'S DISMISSAL LISTENERS ARE REAL TOO (round 9; the shared builder since upstream's v0.16.0 tidy): showTabMenu builds its card
  // with menuCard(), its standard rows with addMenuItem() and shows it through showMenuCard() (ctx-menu.ts), which installs the dismissal
  // listeners per open (a press outside on the document's pointerdown capture, Escape, a scroll outside the card, the window's blur) and
  // runs onTabMenuClosed at the close. The builder is lifted verbatim (its module state, the placement, the card, the row, the divider
  // and the show) and evaluated against the harness document and window, so a test that fires a scroll on the document drives the
  // builder's own exemption for the menu's own scroll, with this fork's instanceof guard on it (round 8 executed the exemption in the
  // browser leg alone, which skips where no browser is installed, CI included)
  const ca = CTX.indexOf("const MARGIN = 4;");
  const cb = CTX.indexOf("\n}\n", CTX.indexOf("export function showMenuCard(menu: HTMLElement, x: number, y: number, opts: CtxMenuOpts = {}): HTMLElement {", ca)) + 3;
  assert.ok(ca > 0 && cb > ca, "the builder's anchors moved (ctx-menu.ts, MARGIN to showMenuCard's end); re-anchor this lift");
  const BUILDER = CTX.slice(ca, cb).replace(/^export /gm, "");   // the module's exports as plain declarations in the harness's one scope
  // THE FLYOUT GESTURE IS REAL (T380): wireFlyout, the one hover-and-click gesture the Tags row is wired with (the fork's inline
  // armHoverClose pair moved into it), lifted with its constant, so a test's click on the Tags row opens the flyout through render.ts's
  // own handler; the hover timers ride the harness window's setTimeout, which never fires (the tests click)
  const fa = RENDER.indexOf("const HOVER_INTENT_MS = 120;");
  const fb = RENDER.indexOf("\n}\n", RENDER.indexOf("function wireFlyout(", fa)) + 3;
  assert.ok(fa > 0 && fb > fa, "wireFlyout's anchors moved; re-anchor this lift");
  const js = requireCjs("esbuild").transformSync(RENDER.slice(a, b) + RENDER.slice(la, lb) + notifier![0] + BUILDER + RENDER.slice(fa, fb), { loader: "ts" }).code;
  ...
  const prelude = `
    ...
    const KEYS_EVENT = ${JSON.stringify(KEYS_EVENT)};   // the hot-key store's event: the lifted listener slice carries its two tab-badge listeners (T379)
    const settings = { tabsLocked: false };   // the tab lock (T395): the Move to rows read it; off here
    const inRompShell = () => false;   // no shell around the harness: the Hot key row (T379) is not built
    const keyHint = () => "";   // no chord bound: the Notify me sub-line carries no keycap
    const tagChip = (label, _color, _opts) => { const c = new FakeEl("span"); c.textContent = label; return c; };   // the shared tag chip (T321) as its words: a row's label composes "Move to " + the chip
    ...
    FakeEl.focused = null;   // a fresh page: nothing has the focus (the builder focuses the card at the open and hands the focus back at the close)
    ...
    const Node = FakeEl;   // the builder's scroll listener's instanceof check (this fork's guard, ctx-menu.ts): a fake element is an element (round 9)
    ...
    // the page's document as the builder reads it: the body the card mounts on, createElement for the card and its rows, the focus
    // (activeElement, hasFocus) and the capture listeners showMenuCard installs per open (pointerdown, keydown, scroll), recorded and
    // fired like the window's, so a test drives the dismissal through the builder's own listener
    const document = { body: new FakeEl("body"), getElementById: () => null, get activeElement() { return FakeEl.focused; }, hasFocus: () => true,
      ...
      fire(k, ev = {}) { for (const fn of (this.listeners[k] || []).slice()) fn(ev); }, count(k) { return (this.listeners[k] || []).length; } };
    let ctxMenuEl = null, ctxMenuAt = null;
    let tabMenuSeat = "tab";   // where the open menu was opened from (the user 2026-09-18): showTabMenu says "tab" at every open; the section view's listener says "row" (tab-snapshot-menu.test.ts executes the Rename that reads it)
  `;
  const epilogue = `
    return { open: (id, copy, at) => { showTabMenu({ clientX: at ? at.x : 10, clientY: at ? at.y : 10 }, id, copy); return ctxMenuEl; }, at: () => ctxMenuAt,
             ...
             pending: () => pendingSessionViews, doc: () => document };   // the optimistic blob a remote-only edit mirrors into and renders from (round 8: the name pass is executed on it)
  `;
...
/** the store the menu reads and writes, as the earlier tests stub it. An async body keeps the store until its promise settles (round 8):
 *  a synchronous finally restored the page's localStorage at the body's first await, so every later read fell to readTabGroups's fresh
 *  state and every later write was swallowed by writeTabGroups's quota guard, and the round-7 cases after their first tick() read and
 *  wrote a store that was no longer there (a pin row's own write read back as unpinned) */
function withStore<T>(fn: (store: Map<string, string>) => T): T {
  ...
  try {
    const out = fn(store);
    if (out instanceof Promise) { settledLater = true; return out.finally(() => { g.localStorage = savedLS; }) as unknown as T; }
    return out;
  } finally { if (!settledLater) g.localStorage = savedLS; }
}
...
test("executed: THE MENU DOOR. Hide tab sits with the toggles after Notify me, names the copy's group, hides in THAT section on a click and dismisses; the label reads Show tab once hidden; the row is absent off the grouped strip", () => {
  ...
  withStore(() => {
    ...
    // its place: directly after Notify me, before Billing, inside the "what reaches you" section; Tags stands in the section above
    // ("where it belongs"), a divider between (upstream a3e2a0ad0: the menu grouped by what each item changes about the session)
    ...
    assert.ok(tagsAt > 0 && feedAt > tagsAt && bellAt > feedAt, "Tags, then the toggles: the four-section menu (where it belongs, then what reaches you)");
    assert.equal(hideAt, bellAt + 1, "directly after Notify me");
    assert.ok(items.slice(tagsAt, feedAt).some((it) => it.has("ctx-sep")), "a divider closes the membership section before the toggles");
    assert.ok(!items.slice(feedAt, hideAt + 1).some((it) => it.has("ctx-sep")), "no divider between the first toggle and Hide tab: one behaviour section");
...
test("pinned: the menu door in render.ts. The toggles' dress is one helper the Hide tab row re-uses on its one node; the write is the pin row's, explicit; the copy's home section is computed once in showTabMenu, tracked through the flyout's move and shared with it; the comment records the reversal", () => {
  ...
  const MENU = RENDER.slice(at, RENDER.indexOf("ctxMenuEl = showMenuCard(menu,", at));   // to the builder's show: the tail (the hook, the seat) is pinned on RENDER below
  ...
  assert.equal(MENU.split("sameSection(").length - 1, 4, "four comparisons of two sections (the declaration reads `sameSection = (`): the Hide tab row's click guard (round 6; heldCopy resolves in its own two passes, id then name; round 5 shared the comparison, so a same-named union under a new id was no match and the copy fell through), Move to's source guard (round 8: the copy's home at the click must be the section the row was built for), the carried cue's match (round 8: the refused row's section against each rebuilt row's) and the pin row's home guard (round 9: the copy's home at the click must be the row's section, Move to's rule; round 7 guarded on the row's union alone, so a copy re-homed under the press wrote a pin the prune dropped)");
  assert.equal(MENU.split("const sameSection = ").length - 1, 1, "declared once");
  assert.equal(MENU.split("homeNow()").length - 1, 5, "the row's gate (rowHome, which its refresh and its click share, round 6), the flyout's per-build read, the flyout's signature (round 6: the home resolution is an input its rows read; grouping is read through it, never bare), Move to's source at the click (round 8) and the pin row's home at the click (round 9)");
  ...
  assert.match(RENDER, /window\.addEventListener\("storage", \(e\) => \{ if \(e\.key === TABGROUPS_KEY\) \{ renderTabs\(\); viewsChanged\(\); \} \}\);\s*\n(?:\/\/[^\n]*\n)*window\.addEventListener\("storage", \(e\) => \{ if \(e\.key === "romp-chat-cols"\) renderTabs\(\); \}\);\s*\nwindow\.addEventListener\(TABGROUPS_EVENT, \(\) => \{ renderTabs\(\); viewsChanged\(\); \}\);/, "the two store listeners: the strip's render, then the open menu (between them the chat split's column-store listener, the strip alone: a column move re-dresses no menu)");
  ...
  assert.match(RENDER, /ctxMenuEl = showMenuCard\(menu, e\.clientX, e\.clientY, \{ onClose: onTabMenuClosed \}\);\s*\n\s*tabMenuViewsHook = \(\) => \{ void hold\.defer\(\(\) => \{ if \(gone\(\)\) return; refreshHideRow\(\); refreshTags\(\); \}\); \};[^\n]*\n\s*seatMenu\(e\.clientX, e\.clientY\);[^\n]*\n\}/, "set once the menu is on the page (the builder mounts and places it), before the seat: one run through the menu's hold (round 5), the row's refresh and the Tags block's, dropped once the menu is dismissed");
  assert.equal(RENDER.split("tabMenuViewsHook = ").length - 1, 2, "assigned by showTabMenu and cleared by onTabMenuClosed, the builder's close callback; nowhere else (the declaration reads `let tabMenuViewsHook:`)");
  ...
  const flyBlock = MENU.slice(MENU.indexOf('const sub = el("div", "ctx-menu ctx-sub ctx-sub-tags");'), MENU.indexOf('wireFlyout(menu, tagsItem, ".ctx-sub-tags", openTagsFly);'));   // the flyout's hover gesture lives in wireFlyout (T380), so the block runs to the wiring call
  ...
  assert.equal((flyBlock.match(/(?<![.\w])add\(/g) || []).length, 7, "four rows and three dividers go in front of the input (the local add helper alone: the tag chips' and the locked row's classList.add are not it)");
  ...
  assert.match(CTX, /\n  const onScroll = \(e: Event\) => \{ if \(e\.target instanceof Node && menu\.contains\(e\.target\)\) return; closeContextMenu\(\); \};\n/, "the builder's scroll listener (installed per open on the document's capture phase, ctx-menu.ts showMenuCard): a scroll whose target the card contains is the menu's own and is left alone; every other scroll dismisses; this fork's guard checks the target is a node first");
  assert.doesNotMatch(RENDER, /window\.addEventListener\("scroll"/, "render.ts installs no menu scroll listener of its own: the builder's is the one (the emoji picker's two listeners are mousedown and keydown)");
  ...
  assert.match(RENDER, /ctxMenuEl = showMenuCard\(menu, e\.clientX, e\.clientY, \{ onClose: onTabMenuClosed \}\);\s*\n\s*tabMenuViewsHook = [^\n]*\n\s*seatMenu\(e\.clientX, e\.clientY\);[^\n]*\n\}/, "and once at the cursor when the menu is on the page (round 4: the views hook is set just before it; the builder's placement is re-placed by the seat in the same task, so the clamp and the corner stand)");
  ...
  assert.equal(MENU.split("sub.style.top = ").length - 1, 1, "the flyouts' tops: the Tags flyout's in place(); Billing's and its default submenu's go through placeFlyBeside (T387), below");
  assert.equal(MENU.split("placeFlyBeside(").length - 1, 2, "the Billing flyout and the Set default billing submenu are placed by the shared helper (T387)");
  const hideAt = MENU.indexOf('"ctx-item ctx-item-toggle ctx-item-hide ctx-sub-capped"');
  const homeNowAt = MENU.indexOf("const homeNow = ");
  assert.ok(homeNowAt > 0 && homeNowAt < MENU.indexOf("const home = homeNow();   // read per build") && homeNowAt < hideAt, "declared before both readers: the flyout's per-build read (the Tags block, in the section above) and the row");
  ...
  assert.match(MENU, /const toggle = \(kind: "feed" \| "mail" \| "bell", off: boolean, lab: string, sub: string, fn: \(\) => void\) => \{\s*\n\s*return addMenuItem\(menu, \{ icon: ctxIcon\(kind, off\), label: lab, sub, pick: fn \}\);\s*\n\s*\};/, "the standard toggles go through the builder's row (the same classes, the same click contract: close, then the pick; the row returned so the Hide tab row seats after Notify me); the tab kind is dressToggle's alone");
  assert.match(MENU, /const bellItem = toggle\("bell", !onBell,/);
  assert.equal(MENU.split("dressToggle(").length - 1, 1, "one caller, the Hide tab row's refresh, which re-dresses its one node (the toggle helper builds a fresh row through addMenuItem since the builder tidy; the declaration reads `dressToggle = (`)");
  ...
  assert.ok(tagsAt > 0 && bellAt > tagsAt && hideAt > bellAt && billingAt > hideAt, "after the bell, before Billing; Tags stands in the section above (the four-section menu, upstream a3e2a0ad0)");
  // the block's own end (its build-time refresh and the closing brace): between it and Billing stands upstream's three-line note on the
  // retired hide-session mechanism (an em dash of upstream's own, ruling 21), which the fork's comment above re-words; the pins below read
  // the fork's block alone
  const rowStart = MENU.indexOf("// HIDE TAB (the user 2026-09-09)"), rowEnd = MENU.indexOf("\n    refreshHideRow();\n  }\n", rowStart);
  assert.ok(rowStart > 0 && rowEnd > rowStart && rowEnd < billingAt, "the Hide tab block's anchors: its comment's head and its closing refresh");
  ...
  assert.match(RENDER, /function ctxIcon\(kind: "feed" \| "mail" \| "bell" \| "bill" \| "folder" \| "tag" \| "pencil" \| "smile" \| "tab" \| "key", off: boolean\): HTMLElement \{/, "the tab kind, then upstream's key (the Hot key row, T379)");
...
test("pinned: a tag name never widens the MAIN menu (menu review rounds 2 to 5). .ctx-item-sub keeps the menus' one sub-line size, byte-equal in feed.css; a modifier the two menu rows whose sub-line carries a tag name wear (Hide tab, Tags), and no fixed row and no flyout row does, makes the body take the row's spare width and the text contribute nothing to the menu's intrinsic width, so the menu is as wide as its fixed rows alone in every face and the name elides; the Tags flyout's rows keep their natural width up to a per-row cap", () => {
  ...
  const MENU = RENDER.slice(RENDER.indexOf("function showTabMenu("), RENDER.indexOf("ctxMenuEl = showMenuCard(menu,", RENDER.indexOf("function showTabMenu(")));
  ...
  for (const fixed of [/return addMenuItem\(menu, \{ icon: ctxIcon\(kind, off\), label: lab, sub, pick: fn \}\);/, /const item = el\("div", "ctx-item ctx-item-toggle ctx-item-billing"\);/]) assert.match(MENU, fixed, "a fixed row wears no modifier: " + fixed.source);
...
test("executed: THE STORE'S OWN EVENTS AND THE CAPS FRAME REACH THE MENU (menu review rounds 5 and 6). Another pane's Hide, delivered by render.ts's own tab-groups store listeners through the one notifier, flips the open menu's row to Show tab and its click shows; a Group tabs by tag switch off takes the row away and turns the open flyout's Move to rows into + rows, on again brings both back; another pane's pin re-dresses the pin row; the pin row's own write, whose event runs the hook inside the write, rebuilds the flyout for the pin before the row's own build and keeps the typed text; onKernelCaps runs the notifier once the writes in flight are dropped or the kept blob adopted, and not when nothing shown changed", () => {
  ...
  withStore(() => {
    ...
    assert.deepEqual([win.count("storage"), win.count(TABGROUPS_EVENT), win.count("media:change")], [3, 1, 1], "render.ts's listeners are on the harness window: three storage listeners (the tab-groups store, the chat split's column store, the hot-key store), the store's own event and the media rule");
...
test("executed: THE FLYOUT'S ROWS ACT ON THE LIVE UNION (menu review rounds 7 and 8). A remote same-named tag joining the copy's union with the session, or an already-joined one taking the session, rebuilds the open flyout's rows, and the x then clears every tag of the name; under a pressed pointer the rebuild is parked and the x and Move to resolve the union again at the click, so the remote copy is cleared too; the click resolves the union as heldCopy does, by its id over every union and by its name when none carries the id (round 8: a rename under the press by the id; a remote-only union, a union whose local half went and a tag made again under the same name by the name); Move to's source and Show when folded's section are the copy's home at the click (round 9 for the pin row); a refused click's cue rides the rebuild to the row of the same kind and section, once, and a union gone at the click has its row's removal for the answer with nothing recorded; the + beside Move to, the + <name> row and Show when folded resolve at the click", async () => {
  ...
  // lands under the press, where the rebuild is parked and the rows are the old ones by design (C, C2); a union gone by the click is
  // refused, and its row's removal is the answer (D). Round 8: the resolution is heldCopy's, the id over every union first and the name
  ...
  const plusOf = (fly: FakeEl, name: string) => moveRow(fly, name).all().find((n) => n.has("ctx-tag-plus"))!;
  const joinRow = (fly: FakeEl, name: string) => fly.children.find((it) => it.label() === "+ " + name);
  const joinRows = (fly: FakeEl) => fly.children.filter((it) => it.label()?.startsWith("+ ") || it.label()?.startsWith("Move to ")).map((it) => it.label());
  const pinRow = (fly: FakeEl) => fly.children.find((it) => it.has("ctx-item-pin"));
  ...
  await withStore(async () => {
    const api = liftShowTabMenu()(hooks);
    const win = hooks.window!;
    const press = (menu: FakeEl) => menu.fire("pointerdown", { button: 0, stopPropagation() {} });   // the builder's card stops a press inside it from reaching the page (ctx-menu.ts showMenuCard)
    ...
    api.push(V_D);
    win.fire("pointerup");
    assert.ok(infraRow.isConnected && !infraRow.has("romp-acted"), "before the click the row stands (the rebuild is parked), uncued");
    xOf(fly, "infra").click();
    assert.equal(hooks.views, V_D, "nothing posted: the union is gone");
    assert.equal(heldRow(fly, "infra"), undefined, "the refusal rebuilt the rows at once: the infra row is gone before the parked run lands (round 7 left it until the release's tick)");
    assert.equal(infraRow.isConnected, false, "the row the click landed on is off the page");
    assert.deepEqual(cued(fly), [], "no node carries a cue: no row of the refused union's kind and section exists after the rebuild, and the row's removal is the answer");
    ...
    // the same rename: its source is the copy's home at the click, g1 under its new name, the same section the row was built for
    hooks.views = V_QA; hooks.writes = [];
    menu = api.open("api", "infra");
    fly = flyOf(menu);
    press(menu);
    api.push(renamed(V_QA, 0, "platform", 9));
    win.fire("pointerup");
    xOf(fly, "infra").click();
    after = hooks.views as Views;
    assert.deepEqual(holders(after), { local: ["web", "tests"], remote: [] }, "the x acted on the renamed tag by its id (a name-only lookup: nothing named infra, a refusal, api still under platform)");
    assert.deepEqual([heldRow(fly, "platform"), heldRow(fly, "infra"), cued(fly)], [undefined, undefined, []], "no platform row, no cue");
    assert.equal(rowOf(menu)?.sub(), HIDE_SUB("archived"), "the Hide tab row names archived, the one holder left after the x");
    await tick();
    hooks.views = V_QA; hooks.writes = [];
    menu = api.open("api", "infra");
    fly = flyOf(menu);
    const mvE = moveRow(fly, "qa");
    press(menu);
    api.push(renamed(V_QA, 0, "platform", 10));
    win.fire("pointerup");
    mvE.click();
    after = hooks.views as Views;
    assert.deepEqual(viewTagUnion(after).filter((g) => g.members.includes("api")).map((g) => g.name), ["archived", "qa"], "Move to qa moved the copy out of the renamed tag, its home at the click");
    assert.deepEqual(cued(fly), []);
    await tick();
    // E2 (round 8, the name pass): a REMOTE-ONLY union has no local id, so its ref is its name alone and the click finds it by the name.
    // web under infra and three remote-only tags, the menu on the ops copy, two remote-only unions ahead of ops in the drag order (an
    // unguarded id clause, null against null, would take the first of them): the x on ops mirrors the remove on ops alone (a remote-only
    // edit has no local op; the mirror is rendered and shows until the host's next push), ops2 and ops3 keep web, and no cue
    const R = (n: string) => ({ id: "TESTHOST:" + n, host: "TESTHOST", name: n, color: "#123456", members: ["web"] });
    const V_R3: Views = { ...V, remoteTags: [R("ops2"), R("ops3"), R("ops")], tagOrder: ["ops2", "ops3", "ops", "infra", "archived"], seq: 4 };
    hooks.views = V_R3; hooks.writes = [];
    menu = api.open("web", "ops");
    fly = flyOf(menu);
    assert.equal(rowOf(menu)?.sub(), HIDE_SUB("ops"));
    const r0 = hooks.renders;
    xOf(fly, "ops").click();
    assert.equal(hooks.renders, r0 + 1, "the remote-only edit rendered its mirror (a refusal renders nothing)");
    assert.deepEqual((api.pending() as Views).remoteTags!.map((t) => [t.name, t.members]), [["ops2", ["web"]], ["ops3", ["web"]], ["ops", []]], "the mirror took web out of ops alone, by its name (an unguarded id clause matched ops2, the first remote-only union in the drag order)");
    assert.equal(hooks.views, V_R3, "no local op: the store's blob stands until the host's push");
    assert.deepEqual(cued(fly), []);
    // E3 (round 8): the LOCAL HALF DELETED under the press while a remote same-named tag still holds the session: the union stands under
    // its name with no local id, the ref's id matches nothing, and the name pass finds it (round 7 refused, where the Hide tab row's guard,
    // which compares names when either side lacks an id, wrote). The pin row writes the section by its name alone, as a pin addresses a
    // remote-only section; the x mirrors the remove on the remote tag
    const V_E3: Views = { ...V_API_BOTH, tags: [V_API_BOTH.tags[1]], remoteTags: [R1], seq: 11 };   // g1 gone, the remote infra holds api
    hooks.views = V_API_BOTH; hooks.writes = [];
    menu = api.open("api", "infra");
    fly = flyOf(menu);
    const pinE3 = pinRow(fly)!;
    press(menu);
    api.push(V_E3);
    win.fire("pointerup");
    pinE3.click();
    assert.deepEqual(hooks.writes.map((w) => w.pinned), [[{ sid: "api", name: "infra" }]], "Show when folded pinned the infra section by its name, no id (round 7 refused: the ref's id matched no union)");
    assert.deepEqual(cued(fly), []);
    await tick();
    writeTabGroups(d);
    hooks.views = V_API_BOTH; hooks.writes = [];
    ...
    const r1 = hooks.renders;
    xOf(fly, "infra").click();
    assert.equal(hooks.renders, r1 + 1, "the x mirrored the remove on the remote infra (round 7 refused)");
    assert.deepEqual((api.pending() as Views).remoteTags!.map((t) => t.members), [[]], "api out of the remote infra");
    assert.deepEqual(cued(fly), []);
    await tick();
    // E4 (round 8): a tag DELETED AND MADE AGAIN under the same name under the press (g1 replaced by g9, both infra, g9 holding api): the
    // guide's rule makes it the same group (a tag removed and made again under the same name keeps the Hide tab row), and the x acts on it
    // by its name (round 7 refused by the id). Move to compares sections, as the Hide tab row's guard does (K8b), so it refuses once, the
    // cue on the rebuilt Move to qa row, and the second click, on a row built for g9, moves
    const V_QA_G9: Views = { ...V_QA, tags: [{ ...V_QA.tags[0], id: "g9" }, V_QA.tags[1], V_QA.tags[2]], seq: 12 };
    hooks.views = V_QA; hooks.writes = [];
    menu = api.open("api", "infra");
    fly = flyOf(menu);
    press(menu);
    ...
    xOf(fly, "infra").click();
    after = hooks.views as Views;
    assert.deepEqual(after.tags.map((t) => [t.id, t.members]), [["g9", ["web", "tests"]], ["g2", ["old1", "old2", "api"]], ["g5", []]], "the x took api out of the same-named tag under its new id (round 7 refused)");
    assert.deepEqual([heldRow(fly, "infra"), cued(fly)], [undefined, []]);
    await tick();
    hooks.views = V_QA; hooks.writes = [];
    menu = api.open("api", "infra");
    fly = flyOf(menu);
    const mvG9 = moveRow(fly, "qa");
    press(menu);
    api.push(V_QA_G9);
    win.fire("pointerup");
    mvG9.click();
    assert.equal(hooks.views, V_QA_G9, "Move to refused: the copy's home is g9 where the row was built for g1 (sameSection, the Hide tab row's rule)");
    let mvNew = moveRow(fly, "qa");
    assert.ok(mvNew !== mvG9 && mvNew.isConnected && mvNew.has("romp-acted") && mvNew.title === NOTE("Move to qa"), "the cue rides the rebuild to the new Move to qa row");
    mvNew.click();
    after = hooks.views as Views;
    assert.deepEqual(viewTagUnion(after).filter((g) => g.members.includes("api")).map((g) => g.name), ["archived", "qa"], "the second click, on a row built for g9, moves");
    await tick();
    ...
    // the source is gone and the home is archived, the one holder: refused, the cue on the rebuilt row
    const V_OUT: Views = { ...V_QA, tags: [{ ...V_QA.tags[0], members: ["web", "tests"] }, V_QA.tags[1], V_QA.tags[2]], seq: 13 };
    hooks.views = V_QA; hooks.writes = [];
    menu = api.open("api", "infra");
    fly = flyOf(menu);
    const mvF = moveRow(fly, "qa");
    press(menu);
    api.push(V_OUT);
    win.fire("pointerup");
    assert.ok(mvF.isConnected && !mvF.has("romp-acted"));
    mvF.click();
    assert.equal(hooks.views, V_OUT, "nothing posted: the copy left infra just before the click (round 7 posted the add: api under archived and qa)");
    assert.deepEqual(viewTagUnion(hooks.views as Views).filter((g) => g.members.includes("api")).map((g) => g.name), ["archived"]);
    mvNew = moveRow(fly, "qa");
    assert.ok(mvNew !== mvF && mvNew.isConnected, "the rows were rebuilt at the refusal: a new Move to qa row (the home is archived now, the one holder)");
    ...
    menu = api.open("api", "infra");
    fly = flyOf(menu);
    const mvF3 = moveRow(fly, "qa");
    press(menu);
    api.push(V_DEL);
    win.fire("pointerup");
    mvF3.click();
    assert.equal(hooks.views, V_DEL, "nothing posted: the source is gone");
    mvNew = moveRow(fly, "qa");
    assert.ok(mvNew !== mvF3 && mvNew.isConnected && mvNew.has("romp-acted") && mvNew.title === NOTE("Move to qa"), "the cue on the rebuilt row (round 7: on the detached one)");
    await tick();
    // G (round 8): SHOW WHEN FOLDED resolves at the click. A rename under the press pins the renamed tag by its id (a build-time closure
    // pins infra); the home's tag deleted under the press with the session left under one other tag: the pin row refuses (no union for
    // its section), writes nothing, and the cue rides the rebuild to the pin row, now for the one holder, archived
    hooks.views = V_QA; hooks.writes = [];
    menu = api.open("api", "infra");
    fly = flyOf(menu);
    const pinG = pinRow(fly)!;
    press(menu);
    api.push(renamed(V_QA, 0, "platform", 16));
    win.fire("pointerup");
    pinG.click();
    assert.deepEqual(hooks.writes.map((w) => w.pinned), [[{ sid: "api", name: "platform", id: "g1" }]], "pinned under the new name with the id: the section as it stands at the click (a build-time closure pins infra)");
    assert.equal(pinRow(fly)?.sub(), "stays on the strip while platform is folded");
    await tick();
    writeTabGroups(d);
    const V_WEB_BOTH: Views = { ...V, tags: [V.tags[0], { ...V.tags[1], members: ["old1", "old2", "web"] }], seq: 4 };
    hooks.views = V_WEB_BOTH; hooks.writes = [];
    menu = api.open("web", "infra");
    fly = flyOf(menu);
    const pinG2 = pinRow(fly)!;
    assert.equal(pinG2.sub(), "keep this tab on the strip while infra is folded");
    press(menu);
    api.push({ ...V_WEB_BOTH, tags: [V_WEB_BOTH.tags[1]], seq: 5 });   // infra deleted: web under archived alone
    win.fire("pointerup");
    pinG2.click();
    assert.equal(hooks.writes.length, 0, "nothing written: the row's section is gone");
    const pinNew = pinRow(fly)!;
    ...
    // press; a push moves web out of infra into a new ops tag while infra stands; the release; Show when folded: round 8 guarded on the
    // row's union alone (liveUnion), so infra standing let the click write a pin for web under infra, which the prune dropped at once (a
    // no-op write, its event, a strip re-render, no cue), while the rebuilt row spoke for ops. Now the home at the click (ops) is not the
    // row's section (infra): nothing written, the cue on the rebuilt pin row, which names ops, and the second click pins ops
    hooks.views = V; hooks.writes = [];
    menu = api.open("web", "infra");
    fly = flyOf(menu);
    const pinG3 = pinRow(fly)!;
    assert.equal(pinG3.sub(), "keep this tab on the strip while infra is folded");
    press(menu);
    const V_WEB_OPS: Views = { ...V, tags: [{ ...V.tags[0], members: ["api", "tests"] }, V.tags[1], { id: "g6", name: "ops", color: "#f0f", members: ["web"] }], seq: 5 };
    api.push(V_WEB_OPS);
    win.fire("pointerup");
    pinG3.click();
    assert.equal(hooks.writes.length, 0, "nothing written: the copy's home is ops where the row was built for infra (round 8 wrote a pin for infra, which the prune dropped)");
    const pinOps = pinRow(fly)!;
    ...
    pinOps.click();
    assert.deepEqual(hooks.writes.map((w) => w.pinned), [[{ sid: "web", name: "ops", id: "g6" }]], "the second click, on a row built for ops, pins ops");
    await tick();
    writeTabGroups(d);
    // the two-holder re-home: web out of infra into ops and archived at once; the copy has no home, so no pin row is rebuilt: nothing
    // written, no row, no cue (round 8 wrote the dropped pin here too)
    hooks.views = V; hooks.writes = [];
    menu = api.open("web", "infra");
    fly = flyOf(menu);
    const pinG4 = pinRow(fly)!;
    press(menu);
    api.push({ ...V_WEB_OPS, tags: [V_WEB_OPS.tags[0], { ...V_WEB_OPS.tags[1], members: ["old1", "old2", "web"] }, V_WEB_OPS.tags[2]], seq: 6 });
    win.fire("pointerup");
    pinG4.click();
    assert.equal(hooks.writes.length, 0, "two holders, neither the copy's own: nothing written");
    ...
    // would show nothing), and the second click pins g9 (round 8's liveUnion found g1 by its id and wrote a pin for api under platform,
    // which the prune dropped, so the byte-identical rebuild showed nothing at all)
    hooks.views = V_API_BOTH; hooks.writes = [];
    menu = api.open("api", "infra");
    fly = flyOf(menu);
    const pinG5 = pinRow(fly)!;
    press(menu);
    const V_G1_AWAY: Views = { ...V_API_BOTH, tags: [{ ...V_API_BOTH.tags[0], name: "platform", members: ["web", "tests"] }, V_API_BOTH.tags[1], { id: "g9", name: "infra", color: "#4EC9B0", members: ["api"] }], seq: 7 };
    api.push(V_G1_AWAY);
    win.fire("pointerup");
    pinG5.click();
    assert.equal(hooks.writes.length, 0, "nothing written: the home is g9 where the row was built for g1");
    const pinG9 = pinRow(fly)!;
    assert.ok(pinG9 !== pinG5 && pinG9.isConnected && pinG9.has("romp-acted") && pinG9.title === NOTE("keep this tab on the strip while infra is folded"), "the same words on the rebuilt row, so the cue is the whole acknowledgement");
    assert.equal(pinG9.sub(), "keep this tab on the strip while infra is folded");
    await tick();
    pinG9.click();
    ...
    // press: the + on Move to qa adds api to g55, the union the name finds (a build-time closure adds to g5, which the blob no longer has,
    // so nothing posts; round 7 refused). The trail's copy with the rows + infra and + archived, infra replaced by g9 under the press: + infra
    // adds loose to g9 and claims the copy there, so the Hide tab row appears for infra (a build-time closure adds nothing and claims nothing)
    const V_QA55: Views = { ...V_QA, tags: [V_QA.tags[0], V_QA.tags[1], { ...V_QA.tags[2], id: "g55" }], seq: 17 };
    hooks.views = V_QA; hooks.writes = [];
    menu = api.open("api", "infra");
    fly = flyOf(menu);
    const plusH = plusOf(fly, "qa");
    press(menu);
    api.push(V_QA55);
    win.fire("pointerup");
    plusH.click();
    after = hooks.views as Views;
    assert.deepEqual(after.tags.map((t) => [t.id, t.members]), [["g1", ["web", "api", "tests"]], ["g2", ["old1", "old2", "api"]], ["g55", ["api"]]], "the + added api to the same-named tag under its new id, its other tags kept (a build-time closure posts nothing; round 7 refused)");
    assert.equal(rowOf(menu)?.sub(), HIDE_SUB("infra"), "an add: the copy stays under infra");
    assert.deepEqual(cued(fly), []);
    await tick();
    const V_G9: Views = { ...V, tags: [{ ...V.tags[0], id: "g9" }, V.tags[1]], seq: 14 };
    hooks.views = V; hooks.writes = [];
    menu = api.open("loose", "");
    fly = flyOf(menu);
    const joinH = joinRow(fly, "infra")!;
    assert.deepEqual(joinRows(fly), ["+ infra", "+ archived"]);
    press(menu);
    api.push(V_G9);
    win.fire("pointerup");
    joinH.click();
    after = hooks.views as Views;
    assert.deepEqual(after.tags.map((t) => [t.id, t.members]), [["g9", ["web", "api", "tests", "loose"]], ["g2", ["old1", "old2"]]], "+ infra added loose to infra under its new id");
    assert.equal(rowOf(menu)?.sub(), HIDE_SUB("infra"), "and the copy is claimed there: the Hide tab row for infra");
...
test("executed: THE MENU'S OWN SCROLL LEAVES IT STANDING (menu review rounds 8 and 9). The builder's capture scroll listener (ctx-menu.ts showMenuCard, lifted onto the harness document), dismisses the menu when the page moves under it and not when the Tags flyout scrolls within itself: a scroll whose target is the flyout, a row in it or the menu is left alone; a scroll in another box dismisses, and so do the page's own (its target the document, an ancestor the menu does not contain), one whose target is no node at all and one with no target", () => {
  ...
  // the builder's own listener over the harness document, so a listener reverted to a bare dismissal fails here on the first scroll
  const hooks = menuHooks();
  withStore(() => {
    const api = liftShowTabMenu()(hooks);
    const doc = api.doc();
    assert.equal(doc.count("scroll"), 0, "no menu open, no listener: the builder installs its dismissal per open");
    const menu = api.open("api", "infra");
    assert.equal(doc.count("scroll"), 1, "the builder's one scroll listener, on the document's capture phase while the menu is open");
    const fly = flyOf(menu);
    const d0 = hooks.dismissed;
    doc.fire("scroll", { target: fly });
    assert.deepEqual([hooks.dismissed, menu.isConnected], [d0, true], "the flyout's own scroll (a wheel over it): the menu stands");
    doc.fire("scroll", { target: fly.children.find((r) => r.has("ctx-item-pin"))! });
    ...
    doc.fire("scroll", { target: new FakeEl("div", "elsewhere") });
...
test("executed: THE FLYOUT FOLLOWS THE ROW (menu review rounds 3 and 6). With the Tags flyout open, an x on the copy's tag takes the Hide tab row out below the Tags row and the flyout stays at the Tags row; an add brings the row back the same way; a menu clamped at the pane's bottom moves up by the row that came back, and the emoji picker's anchor follows; a flyout clamped at the pane's bottom that grows while the row's words stand (a push adding a tag, the user's New tag) or change (a rename beside new tags) is placed again at its new size", () => {
  ...
  // again, keyed on itself (the write's build), no timer. Since upstream's four-section menu (a3e2a0ad0) the Hide tab row stands BELOW
  // the Tags row, so F1 shows the row's leaving and return moving nothing above it; F2 and F4 carry the follow (the bottom clamp, the
  // flyout's growth). Executed over the harness's rect model: a menu's rows stack ROW apart under its top; a flyout's top is its style's
  const hooks = menuHooks();
  const rowOf = (menu: FakeEl) => menu.children.find((it) => it.has("ctx-item-hide"));
  const xOf = (fly: FakeEl, name: string) => fly.children.find((it) => it.label() === name)!.all().find((n) => n.has("ctx-tag-x") && !n.has("ctx-tag-plus"))!;
  ...
  withStore(() => {
    const api = liftShowTabMenu()(hooks);
    // F1: the row leaves and returns below the Tags row; the Tags row and its flyout stand where they were, the refresh reseating them all the same
    let menu = api.open("web", "infra");
    ...
    assert.ok(menu.children.indexOf(rowOf(menu)!) > menu.children.indexOf(tags), "the Hide tab row stands below the Tags row (the four-section menu)");
    xOf(fly, "infra").click();
    assert.equal(rowOf(menu), undefined);
    assert.equal(tags.getBoundingClientRect().top, tagsTop0, "the Tags row stands: the row that left was below it");
    assert.equal(top(fly), tagsTop0, "and the flyout with it: the reseat after the refresh reads the same row");
    fly.children.find((it) => it.label() === "+ infra")!.click();
    assert.ok(rowOf(menu));
    assert.equal(tags.getBoundingClientRect().top, tagsTop0);
    assert.equal(top(fly), tagsTop0, "the row back below, the flyout still at the Tags row");
...
test("executed: NO REBUILD UNDER A PRESSED POINTER (menu review rounds 5 and 6). A push that lands between a pointerdown on the menu and its release re-dresses the Hide tab row and rebuilds the flyout only on the release, a tick after the click, so the click lands on the node the user pressed; a push that changes nothing the row shows leaves its child nodes alone; a run parked when the click dismissed the menu paints nothing; the release listeners leave with the release; a same-named tag under a new id pushed mid-press: the refused click's re-dress changes no word, so the row pulses and its tooltip says to click again, and the second click writes the new id", async () => {
  ...
  await withStore(async () => {
    ...
    const blur0 = win.count("blur");   // the builder's per-open blur listener (showMenuCard installs it on the window at the open) is the baseline: a bare count held before any press
    press(menu);
    api.push(renamed(V_API_BOTH_QA, 0, "platform", 7));
    ...
    assert.equal(win.count("blur"), blur0, "the blur one too; the builder's own stays while the menu is open");
    ...
    menu = api.open("api", "infra");
    row = rowOf(menu)!;
    menu.fire("pointerdown", { button: 2, stopPropagation() {} });
    ...
    writeTabGroups(d);   // round 8: the store outlives the awaits now (withStore), so this hide would stand into K8b's write
...
test("executed: ABSENT ON THE PHONE LAYOUT (menu review round 1). Under the phone media rule the plan sections but applies no hide (upstream's phone picker order; the hides follow the folds' phone rule), so a hide would show nothing there and the menu builds no Hide tab row and writes nothing; the rest of the menu stands; off the phone the row is back", () => {
  ...
  // plan never applies (tab-groups.ts hideOf is gated off the phone, as foldOf is). The gate is the page's one media rule (phoneLayout).
  const hooks: MenuHooks = {
  ...
  assert.match(GROUPS, /const hideOf = \(sec: TabSection, id: string\): boolean => !phone && sec\.name !== null && isHidden\(st, sec, id\);/, "the plan's own phone rule: no hide applies there (the picker is the phone's only switcher), so a row absent on the phone hides nothing the phone would show");
  assert.match(RENDER, /const groupToggle = \{ label: "Group tabs by tag", on: \(\) => readTabGroups\(\)\.on,/, "the switch is one object on both mounts (upstream's phone picker order): it carries no phone gate of its own");
...
test("pinned: the Hidden fold's open state is THE SECTION'S OWN (round 1): opening it in one group's snapshot leaves every other group's fold closed", () => {
  ...
  assert.equal(SNAP.split("snapHiddenOpen").length - 1, 7, "the set; the head's toggle (has, delete, add); the focus rule and syncHiddenFold read it; rowSeatFor reads it (a Rename's row seat under the fold only while the fold is open, the user 2026-09-18; tab-snapshot-menu.test.ts executes it); nothing else");
...
test("executed: the store CARRIES A KEY THIS BUILD DOES NOT KNOW through its writes (round 1): an older pane's fold no longer drops a newer build's list", () => {
  ...
  assert.match(GROUPS, /RETIRING A KEY: taking it out of this set is not enough\.[\s\S]{0,400}const KNOWN_KEYS: ReadonlySet<string> = new Set\(\["on", "collapsed", "expanded", "pinned", "hidden", "followed", "followedSeq", "timeline"\]\);/);
