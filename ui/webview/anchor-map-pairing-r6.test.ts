...
test("an element the sanitizer unwraps inside an open wrapper (`<button>`, `<form>`, `<label>`, `<fieldset>`, `<marquee>`) is read as its own children, which the unwrap leaves where it stood (topTags flattens its kids into the wrapper's): with a badge picture in a `<button>` or a `<b>`, a `<p>` or a lead text and a `<b>` in a `<form>`, a `<label>` or a `<fieldset>` before the depth-1 wrapper, the nested paragraph and the paragraph after the wrapper map to their own offsets, the opener's block holds its two wrappers and the nested paragraph's `<p>` is its own block's, with a `<details><summary>` as the inner wrapper too (the review's round 6, a regression against 701728eae, and against main for the `<button><img>` badge shape: the unwrapped kid was listed by name and read as a text node, so the IMG or B the unwrap left stayed at k, the depth-1 rule found no inner element there, the holder was dropped, the nested paragraph took the leftover element as a mismatch and the paragraph after was no block's); a kept element kid and a text-only unwrapped kid still read as before (controls); and a marquee holding a badge picture or a `<b>` before the depth-1 wrapper, which the sanitizer removes with its content kept where it stood, reads the same (the file review's round 18, tests-1 with regression-1: the anchor map's mirror unwraps a marquee as the sanitizer does, and this suite's stand-in had kept it, so no test read the mirror; red with MARQUEE dropped from the anchor map's UNWRAPPED, where a selection in the nested paragraph was refused as not matching the file)", () => {
