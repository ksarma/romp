...
test("render.ts: the #content scroll listener keeps the active view's saved spot current (passive, no timer)", () => {
  ...
  // place held when an armed land missed with no fetch armed and took (PR E, the maintainer's round 2 ruling), the raw write when that restore has no row to
  ...
  // two roads after one (the maintainer's round 3 ruling B); a miss with a fetch armed writes raw, the take kept only under this attempt's
  // pre-jump (a source pin on where the rule sits; land-active-keep.test.ts executes every road)
  assert.match(RENDER, /if \(!v\.shown \|\| v\.stick\) writeScroll\(content, content\.scrollHeight, "land-bottom", true\);\n(?:\s*\/\/[^\n]*\n)*\s*else if \(!\(held && !fetchArmed && restoreScrollAnchor\(content, v, held\)\)\) \{ if \(!preJumped\) untakeMeasure\(v, figures\); writeScroll\(content, v\.scrollTop, "land-saved"\); \}/);   // (T262: every #content write rides writeScroll)
});
