...
test("with a unit predicate the tail is the last child that carries a unit: a hover's rail band, the thread's last child with none, is not the tail (PR E, the maintainer's round 2 ruling; the spacer rule stands for callers that pass none)", () => {
  ...
  assert.equal(tailLabel([{ className: "tx-loading" }], isUnit), "tx-loading", "the deferred build's loading hint, the only child of a non-empty session's view for one frame (render.ts appends it to an EMPTY view, never under a spacer); a row can name it after the compact toggle's rerender, which empties every view with `shown` kept, while a first visit's view is not yet shown and files no tailchange row");
