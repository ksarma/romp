...
// One vocabulary on the wire (T252c; the press-minted id of upstream #1224): the module names a copy's identity the way
// send-pending.ts and the kernel do (a queued copy's `qid`, a landed record's `qid` or one of its `qids`), and render.ts
// hands it the session's records as they are, with no adapter at the call site. The fork's 2026-09-09 renaming of these
// fields to sendId/sendIds is gone (4e Q1); a comment may still name the old words as history, so the guard reads code only.
test("the module and its call site speak the wire's names: qid/qids in code, no sendId/sendIds, no adapter", () => {
  const code = (f: string) => fs.readFileSync(path.resolve(process.cwd(), "..", "ui", "webview", f), "utf8").replace(/\/\/.*$/gm, "");
  for (const f of ["queued-held.ts", "render.ts"]) assert.doesNotMatch(code(f), /\bsendIds?\b/, `${f}: a copy's identity is qid/qids on this wire`);
