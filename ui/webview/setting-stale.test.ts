...
test("/version reports every gt-gated store's last-applied stamp, and the gear stamps above them", () => {
  ...
  assert.match(KERNEL, /"settingsGt": \{\*\*_settings_gt\(\), \*\*_mgt\}[,}]/, "/version carries the stamps (ints only: the route is auth-exempt); the three mesh-adopted stores' stamps ride the same snapshot as their values (T248b), and the fork's payload ends on this field, with no defaultDir or nativeDialogs after it");
