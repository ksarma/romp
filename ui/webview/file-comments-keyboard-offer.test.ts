...
const RECT_MOVED: Rect = { left: 100, top: 400, right: 300, bottom: 420, width: 200, height: 20 };   // the passage moved from under the button by a scroll: a top other than RECT_A's is all the scroll's hide needs (subjectHeld holds the float only while the subject's top AND right edges sit within a pixel of the offer's; this rect shares RECT_A's right), and the rect stays INSIDE the body's box, so the same selection after the hide is refused by onSelectionChange's record compare and not by the offer's box test (the review's round 2, fresh-2: at top 40, above the box, the box test refused it too, and the file's kill of the compare was gone: tests 1 and 6 green with the compare disabled)
