...
// code included. Measured here under `page.emulateMedia({ media: "print" })` as computed styles, then as a real A4 PDF
// whose page count for a 120-paragraph note is more than one (it was one), then back under screen media, where the
// screen values return. The pane and the chat modal (the feed shares feed.css's block, pinned byte-equal to styles.css's
// by the node test below). Skips loudly without a browser. Synthetic values only.
...
test("under print media the note is a static, unclipped, black-on-white document with its chrome hidden; the A4 PDF of a 120-paragraph note runs to several pages; screen media restores the viewer", { timeout: 120000 }, async (t) => {
  await inBrowser(t, async (browser) => {
    for (const mode of ["pane", "chat"] as Mode[]) {
      ...
      assert.ok(pages > 1, mode + ": the A4 PDF runs to several pages (" + pages + "; the same note gave " + before + " under screen media, the defect)");
