...
// through it) but boots initFileView WITHOUT registering a sink — Submit there had nowhere to
// deliver and was a dead button. The fix is honest gating, not a fake relay: no sink, no comment
// affordances at all (the GitHub-anchor treatment — no real target, no control). ──

test("no sink registered → the viewer offers NO comment affordances (the feed-hosted viewer)", () => {
  // the feed boots the viewer for saves/git-links but never registers a sink; render.ts does
...
// ── Submit during an edit (2026-08-19): the delivery tail used to closeFileView() unconditionally,
// so a dirty editor's close guard popped "Discard unsaved changes?" over a SUBMIT — and Cancel then
// stranded the button at "Submitting…" with a stale count (renderBody early-returns while editing,
// so syncReview never reran). ──
test("Submit while editing delivers and resets IN PLACE — never the courtesy close or its confirm", () => {
  const fn = VIEW.slice(VIEW.indexOf('submitBtn.addEventListener("click"'));
  const body = fn.slice(0, fn.indexOf("\n  });"));
  // the store clears on BOTH arms — the comments were delivered either way
