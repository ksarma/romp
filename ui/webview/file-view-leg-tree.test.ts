...
// regressed tree or red over a good one, with nothing in the output naming either. Two checks: the module's one
// environment read is the browser-legs switch, ROMP_BROWSER_LEGS_REQUIRE, which inBrowser reads to turn a skip into a
// failure under CI's browser-legs step and which redirects nothing (every other read is refused here by name, so a
// variable that picks the tree cannot return), and with a stale variable of the old name in the environment it still
// resolves the cwd's tree. Running a leg over another tree is done from that tree's vscode-extension, as for every leg.
...
test("real-viewer-leg.ts: the legs' tree is the cwd's ../ui/webview, and no environment variable redirects it", async () => {
  ...
  // every environment read in the module, by the name it reads (a member, a bracketed string, or the whole environment when
  // neither follows): the browser-legs switch is the one allowed, so a second read fails here naming it
  const envReads = Array.from(src.matchAll(/process\.env\b(?:\.([A-Za-z_$][\w$]*)|\[\s*["']([^"']*)["']\s*\])?/g),
    (m) => m[1] || m[2] || "(the whole environment)");
  assert.deepEqual(envReads, ["ROMP_BROWSER_LEGS_REQUIRE"], "the shared leg page reads one environment variable, the browser-legs switch, and none that redirects the tree: the tree under test is the cwd's, like every other leg's");
