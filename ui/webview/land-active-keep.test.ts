...
test("render.ts: showActive decides whether the scroller holds the view's reader BEFORE the display flip (a switch's entering view is still display:none then) and hands the decision to the synchronous land alone; the deferred build's land and the hidden pane's retry pass nothing. A source pin, keyed on where the decision is read and what each land is handed. Executed: the landing lab's road 16 (tests/test_landing_notice_browser.py) runs the hand-off, a deep link with no time on the displayed tab reaching landActive through showActive with every conjunct true, and goes red with the hand-off dropped (its B carries no time because a pre-jump on a fetch-armed miss stands whatever the saved place reads); the roads above run what the flag changes inside landActive, and the no-time road and the raw road go red without it. No executed road reaches six parts, which only source pins guard (this one for all six; keep-across-window.test.ts and tab-switch-defer.test.ts also match the two land calls): the synchronous land being handed the decision at all, the display conjunct (a switch; the decision read after the flip drops it), the pane-height conjunct, the pending-build conjunct, the deferred land passing nothing and the hidden pane's retry passing nothing", () => {
