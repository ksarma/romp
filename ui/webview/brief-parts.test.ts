...
test("the trailing still-open paragraph renders unstamped, and only the item paragraphs get ages", () => {
  ...
  assert.ok(/paras\.forEach\(\(p, i\) => \{[\s\S]*?if \(stampOk && i < bp!\.length\) \{[\s\S]*?stampAge\(age, bp!\[i\]\.since/.test(block),
...
test("each paragraph wears its own live age chip", () => {
  ...
  assert.ok(FEED.includes('stampAge(age, bp![i].since || nowS, "plain", false, nowS, relAge, ageTint);'),
