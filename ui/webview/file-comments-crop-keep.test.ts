// A PDF region's crop is kept as its page DRAWS, whether or not its card is open (plans/file-review.md, Slice 4: "the card
// shows the page crop"). The chunk draws a page as it nears the reader and gives a far page's bitmap back (a 0×0 canvas),
// and the panel cannot ask for a page (the chunk's only API is render()). Before this, a crop was cut and kept only for an
// OPEN card (cropFor, at render), so a card expanded after its page had scrolled away showed the not-drawn line although
// the page had been on screen a moment before, with the card closed. Now every draw's repaint (the seam's onRendered, from
// the chunk's onPage, into paintRegions) cuts and keeps the crop of every region on the drawn page (cutCrop), the card open
// or closed, resolved or not; the line is left for a page not drawn since the document was shown, or since the file changed
// (cropKey). Driven end to end through the DOM stand-in of file-comments-crop-wait.test.ts (copied: each test file is its
// own bundle). Synthetic fixtures only: the notes-api world, placeholder ids.
//
...
// ── a crop kept from a draw the card was closed for ────────────────────────────────────────────────

test("a page drawn while its card is closed: the crop is kept as the page draws, so the card expanded after the bitmap is gone shows the crop, not the line; a page never drawn still has the line", async () => {
  drawn.length = 0;
  const h = await harness(["drawn", "drawn", "undrawn"]);
  const c2 = pageComment(2), c3 = pageComment(3);
  await h.ok(withStore([c2, c3]));
  await h.open(withStore([c2, c3]));
  assert.equal(h.card(c2.id), null, "the cards start closed");
  const page2 = h.pages[1].querySelector("canvas")!, page3 = h.pages[2].querySelector("canvas")!;
  assert.ok(drawn.some((a) => a[0] === page2), "page 2's region was cut as the panel painted the drawn page, with the card closed");
  assert.ok(!drawn.some((a) => a[0] === page3), "nothing cut from page 3's 0×0 canvas");
  // page 2 scrolls away: the chunk gives its bitmap back
  h.evict(2); h.repaint();
  const before = drawn.length;
  h.expand(c2.id);
  const crop = h.crop(c2.id)!;
  assert.ok(crop, "the crop kept from the draw");
  assert.equal(h.wait(c2.id), null, "no line: the card has its picture");
  assert.equal(drawn.length, before, "shown from what was kept: nothing was cut from the 0×0 canvas");
  assert.equal(h.card(c2.id)!.childNodes[1], crop, "in the crop's place: after the head");
  assert.ok((h.card(c2.id)!.childNodes[2] as E).classList.contains("fc-body"), "…before the body");
  assert.match(crop.getAttribute("aria-label") || "", / of the page$/, "a page's crop");
  // page 3 was never drawn: nothing to keep, so the line
  h.expand(c3.id);
  assert.equal(h.crop(c3.id), null);
  assert.equal(h.wait(c3.id)!.textContent, "Page 3 is not drawn yet. Go to the page to see this region.");
  h.dispose();
});

test("a page drawn after the panel opened, its card closed, is kept too (the reader scrolled through it and back); a regenerated file drops the kept crop until the new page draws", async () => {
  drawn.length = 0;
  const h = await harness(["drawn", "undrawn"]);
  const c2 = pageComment(2);
  await h.ok(withStore([c2]));
  await h.open(withStore([c2]));
  assert.equal(drawn.length, 0, "page 2 is undrawn and nothing sits on page 1: nothing to cut yet");
  // the reader scrolls to page 2: the chunk draws it (onPage → fireRendered), the card still closed
  h.draw(2); h.repaint();
  const page2 = h.pages[1].querySelector("canvas")!;
  assert.equal(drawn[drawn.length - 1][0], page2, "the draw's repaint cut page 2's region, with the card closed");
  // …and back to page 1: page 2 evicted
  h.evict(2); h.repaint();
  h.expand(c2.id);
  const kept = h.crop(c2.id)!;
  assert.ok(kept, "the crop from the draw");
  assert.equal(h.wait(c2.id), null);
  // the PDF regenerated while page 2 is still evicted: the kept crop shows bytes the file no longer has, so the line returns
  await h.restatus({ ...withStore([c2]), fileHash: H2 });
  assert.equal(h.crop(c2.id), null, "dropped with the file's hash");
  assert.equal(h.wait(c2.id)!.textContent, "Page 2 is not drawn yet. Go to the page to see this region.");
  // the new page 2 draws: kept again, fresh
  h.draw(2); h.repaint();
  assert.ok(h.crop(c2.id), "cut fresh on the draw");
  assert.notEqual(h.crop(c2.id), kept);
  assert.equal(h.wait(c2.id), null);
  h.dispose();
});

test("a resolved region's crop is kept as well: expanded from the Resolved fold once its page's bitmap is gone, the card shows the crop", async () => {
  drawn.length = 0;
  const h = await harness(["drawn", "undrawn"]);
  const c1 = pageComment(1, { resolved: true });
  await h.ok(withStore([c1]));
  await h.open(withStore([c1]));
  const page1 = h.pages[0].querySelector("canvas")!;
  assert.ok(drawn.some((a) => a[0] === page1), "cut from page 1 with the card closed and resolved");
  h.evict(1); h.repaint();
  h.click('[data-act="fcresolved"]');
  h.expand(c1.id);
  assert.ok(h.crop(c1.id), "the kept crop");
  assert.equal(h.wait(c1.id), null);
  h.dispose();
});

test("source pins: the region pass keeps the crop of every region card on a drawn page before the resolved skip, and cropFor cuts through the same helper", () => {
  const SRC = web("file-comments.ts");
  assert.match(SRC, /if \(isCanvas\(img\) && this\.cropKey\(card\)\) this\.cutCrop\(img, card\);\n\s*if \(card\.resolved\) continue;/,
    "every region card on a page with a bitmap, open or closed, resolved or not, has its crop kept as the page is painted");
  assert.match(SRC, /const fresh = img \? this\.cutCrop\(img, c\) : null;/, "cropFor cuts and keeps through the same helper, so the two paths cannot drift");
});
