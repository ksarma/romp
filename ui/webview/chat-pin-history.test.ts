...
test("pathPins ride the chat events as a sibling map and thread into every linkify pass", () => {
  ...
  assert.match(RENDER, /pathLinks\?: Record<string, string>, pathPins\?: Record<string, string>, pathPreview\?: Record<string, string>,\s*\n\s*pathPreviewWhy\?: Record<string, string>, sid\?: string \| null, delegated = false, walkOpts\?: PathLinkOptions\): void/);   // pathPreview/pathPreviewWhy (T351) sit before the fork's three; walkOpts: the todo surfaces' targetSuffix (Slice 6 of plans/markdown-viewer.md)
...
test("the kernel pins at the resolve latch and serves pins shape-gated with live fallback", () => {
  ...
  assert.match(KERNEL, /_PIN_ID_RE = re\.compile\(r"\^\[0-9a-f\]\{64\}\\\.\[a-z0-9\]\{1,8\}\\Z"\)/);   // \Z: the id ends at its extension, no trailing newline
