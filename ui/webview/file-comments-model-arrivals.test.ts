...
// person's own writes (YOU, decision 6's label) — and the words of the line under the header. The Send confirm's accept
// option's words are the seen follow-on's (decision 41: the seen pending changes it accepts, the unseen ones it leaves
// pending), pinned in full in file-comments-model-seen.test.ts and checked in short here. The panel's use of them, over the
// stand-in, is file-comments-arrivals.test.ts. Synthetic fixtures only: the notes-api world, placeholder ids, the session
// names "api" and "web".
...
test("acceptOptionLabel: the seen pending changes the accept takes, the unseen ones it leaves pending after them; nothing seen, nothing to accept (the full pins are file-comments-model-seen.test.ts)", () => {
  ...
  assert.ok(!acceptOptionLabel(1, 1).includes("arrived"), "what arrived since the person last looked is the arrivals line's to say, not the option's");
});

test("acceptOptionLabel with the comments the accept resolves (the lost-update probe, 2026-09-09): one parenthesis, the resolve first, the unseen count after it, joined with a semicolon", () => {
  assert.equal(acceptOptionLabel(2, 0, 1), "accept the 2 pending changes you have seen (resolves 1 comment)");
  assert.equal(acceptOptionLabel(11, 0, 7), "accept the 11 pending changes you have seen (resolves 7 comments)");
