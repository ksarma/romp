...
// repaintPresel when a composer opens, closes or moves; a composer of a kind that paints no target (a reply, a change reply, a
// re-place, a comment on the file, a region, a refusal) changes no line box, and the Raw view's row mark is no Rendered mark. Round
...
// 1. Comment on this file opened and cancelled, Reply on the comment's card opened and cancelled, Reply on the change's card opened
...
test("in a browser, the real panel: a composer that paints no target (Comment on this file, Reply on a comment, Reply on a change) opens and cancels with zero Range measurements and zero trims, where the float's Comment on a selection and its Cancel each trim once with measurements", { timeout: 180000 }, async (t) => {
  await inBrowser(t, async (browser) => {
    ...
    const status = { ...STATUS, hunks: [HUNK], store: { ...STATUS.store, comments: [COMMENT] }, unsent: { ...STATUS.unsent, comments: [COMMENT.id] } };
    ...
    assert.ok(standing.cards >= 2, "a comment card and a change card: " + standing.cards);
    ...
    // a change card offers its Reply closed (renderChangeCard)
    const kinds: Array<[string, string, string | null]> = [
      ...
      ["Reply on the change", '.fileview-aside [data-act="fcchangereply"][data-id="' + HUNK.id + '"]', '.fc-sec-cards .fc-card[data-change="' + HUNK.id + '"]'],
    ];
