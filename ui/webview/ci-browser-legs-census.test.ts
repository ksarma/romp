...
// and 5 say in their holds field what else they hold, and the same test holds the table to that statement. Population figures are derived from the run and printed as
...
test("the plant table says which of its 51 round-3 rows (p38 to p88) discriminate against the census before round 3 and what the others hold: 44 red under that census, 7 hold one of four stated reasons instead (holds), and 3 of the 44 red on a property other than their section's and name the plant that carries it (carried); and which of its 67 round-4 rows (p89 to p155) discriminate against the census before round 4: 62 red under that census and 5 hold a stated reason instead; the discrimination itself was established by running each earlier census over the plants, recorded in the PR's notes, and is not re-run here, since neither census is in the tree at test time, so this test holds the TABLE's statement, not the fact", () => {
  ...
  assert.deepEqual(PLANT_TABLE.filter((p) => num(p) > R4_LAST).map(idOf), [], "every row past p88 is a round-4 row and takes a verdict below: a row numbered past R4_LAST (p" + R4_LAST + ") is outside the statement, so a builder who adds a row moves R4_LAST to it" + NOT_RERUN4);
  ...
  assert.deepEqual(PLANT_TABLE.filter((p) => (p.holds !== undefined || p.carried !== undefined) && !inRound3(p) && !inRound4(p)).map(idOf), [], "holds and carried are fields of the round-3 rows (p38 to p88) and the round-4 rows (p89 to p" + R4_LAST + "), the populations the statements are about; an earlier row carries neither");
});
