...
// SAFETY NET's refusal as its outcome until a fold reads the form), every one is red under the census before round 5 (the module at
...
const NET_MODREQ = "module.require, a loader callee the walker does not know: \"module.require\"";
...
const PLANT_TABLE: Plant[] = [
  ...
  // census before round 5. A fold that reads a form (round 5's later commits) re-aims its rows to the fold's sentence and accounts for
  // the mention by construction; a companion barrel's own net refusal reaches the importer through localRefusals' cannot-classify
  // arm, once. The relative node_modules form (n09) is planted under tests/fixtures/browser-legs-plants/relative-node-modules and run
  // over a synthetic root by its own test below, since its path resolves only beside a node_modules the fixtures cannot carry.
  { dir: W, file: "p156-n01a-reexport-named.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p156-n01a-reexport-named.test.ts:2: " + VIA_COMPANION("ui/webview/rcjs-barrel.ts", "ui/webview/rcjs-barrel.ts", NET_RCJS) }, // n01a (extra5-1): import { requireCjs } from a barrel that re-exports it from the launcher, then requireCjs("playwright").firefox.launch(): the barrel's export specifier is the unaccounted mention (before round 5: class none, no refusal, the Firefox launch unread)
  { dir: W, file: "p157-n01b-reexport-renamed.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p157-n01b-reexport-renamed.test.ts:2: " + VIA_COMPANION("ui/webview/rcjs-barrel-renamed.ts", "ui/webview/rcjs-barrel-renamed.ts", NET_RCJS) }, // n01b: export { requireCjs as load } from the launcher, then load("playwright").webkit.launch() (before: class none, no refusal)
  { dir: W, file: "p158-n01c-reexport-star-chain.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p158-n01c-reexport-star-chain.test.ts:2: " + VIA_COMPANION("ui/webview/rcjs-barrel-star.ts, which loads ui/webview/rcjs-barrel.ts", "ui/webview/rcjs-barrel.ts", NET_RCJS) }, // n01c: export * over the requireCjs barrel, the chain two modules long (before: class none, no refusal)
  { dir: W, file: "p159-n01d-reexport-star-as.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p159-n01d-reexport-star-as.test.ts:2: " + VIA_COMPANION("ui/webview/rcjs-barrel-star-as.ts, which loads ui/webview/rcjs-barrel.ts", "ui/webview/rcjs-barrel.ts", NET_RCJS) }, // n01d: export * as ns over the barrel, then ns.requireCjs("playwright") (before: class none, no refusal)
  { dir: W, file: "p160-n01e-reexport-both-calls.test.ts", leg: true, cls: "shared", gap: null, launcherImported: true, refused: "p160-n01e-reexport-both-calls.test.ts:3: " + VIA_COMPANION("ui/webview/rcjs-barrel.ts", "ui/webview/rcjs-barrel.ts", NET_RCJS) }, // n01e: inBrowser from the launcher AND requireCjs from the barrel, the Firefox launch inside inBrowser: the barrel's refusal reaches the importer while its own fold reaches (before: class shared, gap null, rosterable with the Firefox launch unread)
  { dir: W, file: "p161-n01f-launcher-star-both-calls.test.ts", leg: true, cls: "shared", gap: null, launcherImported: true, refused: "p161-n01f-launcher-star-both-calls.test.ts:4: " + NET_HEAD + NET_RCJS + MORE(1) + NET_REACH }, // n01f: inBrowser direct AND b.requireCjs("playwright") through a barrel that is export * from the launcher: the second clause, a reach (the shared call) that does not account for the requireCjs member off a local binding nor the package name it loads (before: class shared, gap null, the WebKit launch unread)
  { dir: W, file: "p162-n02a-require-alias.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p162-n02a-require-alias.test.ts:3: " + NET_HEAD + NET_PW + NET_NO_REACH }, // n02a (extra5-2, extra7-1): const r = require, then r("playwright").webkit.launch(): the package name as the argument of a call the walker knows no loader for (before round 5: class none, no refusal)
  { dir: W, file: "p163-n02b-require-call.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p163-n02b-require-call.test.ts:2: " + NET_HEAD + NET_PW + NET_NO_REACH }, // n02b: (require as any).call(null, "playwright").firefox.launch(): any call's argument is a specifier-capable position, .call's included (before: class none, no refusal)
  { dir: W, file: "p164-n02c-module-require.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p164-n02c-module-require.test.ts:2: " + NET_HEAD + NET_MODREQ + MORE(1) + NET_NO_REACH }, // n02c: module.require("playwright"): the member the walker does not know as a loader is the first mention, the package name the second (before: class none, no refusal)
  { dir: W, file: "p165-n02d-require-alias-launcher.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p165-n02d-require-alias-launcher.test.ts:3: " + NET_HEAD + NET_LAUNCHER + NET_NO_REACH }, // n02d: const r = require, const leg = r(the launcher), leg.inBrowser(t, ...): the launcher's name as the argument of a call the walker knows no loader for (before: class none, launcherImported false, no refusal)
  { dir: W, file: "p166-n02e-require-bind.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p166-n02e-require-bind.test.ts:3: " + NET_HEAD + NET_PW + NET_NO_REACH }, // n02e: const r = require.bind(null), then r("playwright") (before: class none, no refusal)
  { dir: W, file: "p167-n03a-createrequire-argument.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p167-n03a-createrequire-argument.test.ts:3: " + NET_HEAD + NET_PW + MORE(1) + NET_NO_REACH }, // n03a (extra7-1): loadPw(createRequire(__filename)) where loadPw returns r("playwright"): the package name inside loadPw is the first mention in source order, the createRequire value passed as an argument the second (before: class none, no refusal)
  { dir: W, file: "p168-n03b-createrequire-assigned.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p168-n03b-createrequire-assigned.test.ts:4: " + NET_HEAD + NET_CR("createRequire") + MORE(1) + NET_NO_REACH }, // n03b (extra5-2): let req, then req = createRequire(__filename): an assignment's right side is not the declaration initializer the walker binds a loader from (before: class none, no refusal)
  { dir: W, file: "p169-n03c-createrequire-in-object.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p169-n03c-createrequire-in-object.test.ts:3: " + NET_HEAD + NET_CR("createRequire") + MORE(1) + NET_NO_REACH }, // n03c: { req: createRequire(__filename) }.req("playwright") (before: class none, no refusal)
  { dir: W, file: "p170-n03d-createrequire-import-alias.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p170-n03d-createrequire-import-alias.test.ts:3: " + NET_HEAD + NET_CR("cr") + MORE(1) + NET_NO_REACH }, // n03d: import { createRequire as cr }, const req = cr(__filename): the alias resolved through its import binding, the declaration it initializes bound as no loader by the walker (which keys on the spelled name) (before: class none, no refusal)
  { dir: W, file: "p171-n03e-createrequire-alias-launcher.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p171-n03e-createrequire-alias-launcher.test.ts:3: " + NET_HEAD + NET_CR("cr") + MORE(1) + NET_NO_REACH }, // n03e: cr(__filename)(the launcher).inBrowser: the alias applied directly is read by no loader arm, the launcher's name the second mention (before: class none, launcherImported false, no refusal)
  { dir: W, file: "p172-n04-loader-aliased.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p172-n04-loader-aliased.test.ts:5: " + NET_HEAD + NET_PW + NET_NO_REACH }, // n04 (extra7-1): const req = createRequire(__filename), const load = req, load("playwright"): the createRequire call is accounted (req is a loader the walker bound), the load through the alias is not (before: class none, no refusal)
  { dir: W, file: "p173-n05-loader-parameter.test.ts", leg: false, cls: "none", gap: null, launcherImported: false, refused: "p173-n05-loader-parameter.test.ts:4: " + NET_HEAD + NET_PW + NET_NO_REACH }, // n05: loadPw(req) where loadPw returns r("playwright"): the loader passed as an argument (before: class none, no refusal)
  { dir: W, file: "p174-n06a-pw-argument.test.ts", leg: true, cls: "own", gap: "never imports the shared launcher", engines: [], launches: [], playwright: ["playwright"], refused: "p174-n06a-pw-argument.test.ts:2: " + NET_READ_THROUGH }, // n06a (correctness-2): const pw = require("playwright"), then go(pw) where go launches p.firefox: the load is accounted and the module reaches, and the read-through clause refuses a record with no engine and no launch (before round 5: class own, engines [], launches [], no refusal, the exclusions engine form unavailable)
  { dir: W, file: "p175-n06b-pw-object-literal.test.ts", leg: true, cls: "own", gap: "never imports the shared launcher", engines: [], launches: [], playwright: ["playwright"], refused: "p175-n06b-pw-object-literal.test.ts:2: " + NET_READ_THROUGH }, // n06b: const bag = { pw }, then bag.pw.webkit.launch() (before: as p174)
  { dir: W, file: "p176-n06c-pw-returned.test.ts", leg: true, cls: "own", gap: "never imports the shared launcher", engines: [], launches: [], playwright: ["playwright"], refused: "p176-n06c-pw-returned.test.ts:2: " + NET_READ_THROUGH }, // n06c: function get() { return pw }, then get().firefox.launch() (before: as p174)
  { dir: W, file: "p177-n06d-derived-handed.test.ts", leg: true, cls: "own", gap: "never imports the shared launcher", engines: [], launches: [], playwright: ["playwright"], refused: "p177-n06d-derived-handed.test.ts:2: " + NET_READ_THROUGH }, // n06d (tests-2): const { firefox } = pw, then go(firefox): the destructured element from a bound binding records no engine at this head (bindLoaded's derived arm notes the base chain) and the hand-on is unread (before: as p174)
  { dir: W, file: "p178-n06e-pw-helper-argument.test.ts", leg: true, cls: "own", gap: "never imports the shared launcher", engines: [], launches: [], playwright: ["playwright"], refused: "p178-n06e-pw-helper-argument.test.ts:3: " + NET_READ_THROUGH }, // n06e: launchWith(pw) into a companion (pw-launch-helper.ts) that launches p.webkit and names no package (before: as p174)
  { dir: W, file: "p179-n07a-nullish-assign-require.test.ts", leg: true, cls: "own", gap: "never imports the shared launcher", engines: [], launches: [], playwright: ["playwright"], refused: "p179-n07a-nullish-assign-require.test.ts:3: " + NET_READ_THROUGH }, // n07a (extra8-1, extra5-4): let pw, then pw ??= require("playwright") at module level, pw.firefox.launch(): the walker binds under = alone, the package is recorded where the call sits, the name binds nothing (before round 5: class own, engines [], launches [], no refusal)
  { dir: W, file: "p180-n07b-or-assign-require.test.ts", leg: true, cls: "own", gap: "never imports the shared launcher", engines: [], launches: [], playwright: ["playwright"], refused: "p180-n07b-or-assign-require.test.ts:3: " + NET_READ_THROUGH }, // n07b: pw ||= require("playwright") inside the test body, WebKit (before: as p179)
  { dir: W, file: "p181-n07c-and-assign-await-import.test.ts", leg: true, cls: "own", gap: "never imports the shared launcher", engines: [], launches: [], playwright: ["playwright"], refused: "p181-n07c-and-assign-await-import.test.ts:3: " + NET_READ_THROUGH }, // n07c: pw &&= await import("playwright") inside the test, Firefox (before: as p179)
  { dir: W, file: "p182-n08c-launcher-rebound-to-playwright.test.ts", leg: true, cls: "own", gap: "never calls its inBrowser through that import", launcherImported: true, engines: [], launches: [], playwright: ["playwright"], refused: "p182-n08c-launcher-rebound-to-playwright.test.ts:3: " + NET_READ_THROUGH }, // n08c (tests-1): let leg = require(the launcher), then leg = require("playwright"), leg.firefox.launch(): the name stays bound to the launcher, so the Firefox launch through it is unread and the playwright load has no engine and no launch (before round 5: class own, engines [], launches [], launcherImported true, no refusal)
...
test("the plant table says which of its 51 round-3 rows (p38 to p88) discriminate against the census before round 3 and what the others hold: 45 red under that census (44 at round 3, and p74 since round 5's safety net refused a text that census passed), 6 hold one of four stated reasons instead (holds), and 3 of the 45 red on a property other than their section's and name the plant that carries it (carried); which of its 67 round-4 rows (p89 to p155) discriminate against the census before round 4: 62 red under that census and 5 hold a stated reason instead; and which of its round-5 rows (p156 to R5_LAST) discriminate against the census before round 5, the module at the round-4 head: every one red under it unless R5_HELD names it with its reason; the discrimination itself was established by running each earlier census over the plants, recorded in the PR's notes, and is not re-run here, since none of those censuses is in the tree at test time, so this test holds the TABLE's statement, not the fact", () => {
  ...
  const R5_FIRST = R4_LAST + 1, R5_LAST = 186;
  const R5_HELD: string[] = [];
