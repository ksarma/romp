...
type Read = { paints: number; reflows: number; marks: number; stood: boolean; blankMarks: number; paddingOnly: string[]; bare: string[] };
/** The probe's counts, the marks (how many, whether the recorded ones still stand), and the trim's two oracles over the body: the
 *  blank marks of zero content width with a box of their own (padding-only), and the blank text nodes of the two link blocks that
 *  render with a width and carry no mark (bare). */
const read = (page: any): Promise<Read> => page.evaluate(() => {
  ...
  return { paints: w.__paints, reflows: w.__reflows, marks: marks.length, stood: (w.__kept as Element[] || []).every((m) => m.isConnected), blankMarks: blanks.length, paddingOnly, bare };
});
