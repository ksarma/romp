...
test("renderTodo shows the surfaced error instead of the task list", () => {
  ...
  // the task loop sits in the else-branch: per-task rendering is skipped when erroring. No early return, though:
  // the waiting-on-you section below renders from a DIFFERENT store (the requests from the session) and must
  // survive an unreadable task store (user-todos-card.test.ts pins that).
...
test("romp's own request store has its own key on the card when the kernel cannot read it", () => {
  // The card reads TWO stores, Claude's task store (`error`) and romp's request store (user-todos.json,
  // `userTodosError`), and renderTodo shows each text verbatim, so the wording is the kernel's; pinned here
  // beside the renderer that displays it. On a shared key the error branch would supplant the agent's
  // checklist under a heading blaming the store that WAS read; its own key keeps the checklist and heads its
  // own section.
