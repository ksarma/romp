...
test("one-click MOVE between groups (tab groups, 2026-09-04): 'Move to <name>' adds the target and drops the HOME tag on ONE blob; '+' adds without moving", () => {
  ...
  assert.match(RENDER, /let copyNow: string \| undefined = copy \|\| undefined;\s*\n\s*const homeNow = \(\): TagUnion \| undefined => \{\s*\n\s*if \(!readTabGroups\(\)\.on\) return undefined;\s*\n\s*const held = holding\(\);\s*\n\s*const home0 = \(copyNow !== undefined \? held\.find\(\(g\) => g\.name === copyNow\) : undefined\) \?\? \(held\.length === 1 \? held\[0\] : undefined\);\s*\n\s*return home0 && !home0\.pending \? home0 : undefined;\s*\n\s*\};/,
    "the computation itself, once, in showTabMenu's scope: the copy's group, tracked through a move (copyNow, which moveUnion writes) and an add from no group (claimIfLoose); else the one remaining holder; else nothing (round 3 of the tab menu review); only while sectioned, never a pending tag");
