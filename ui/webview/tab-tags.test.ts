...
test("one-click MOVE between groups (tab groups, 2026-09-04): 'Move to <name>' adds the target and drops the HOME tag on ONE blob; '+' adds without moving", () => {
  ...
  assert.match(fly, /const to = liveUnion\(ref\), fromNow = liveUnion\(from\); if \(!to \|\| !fromNow\) \{ refuse\(row, lb\.textContent \?\? ""\); return; \} moveUnion\(fromNow, to\); build\(\); sb\.textContent = subText\(\);/, "the row IS the move, of the copy's group to the destination as both stand at the click (round 7 of the tab menu review: resolved from the live unions by the refs this build read, the cue and no write when one is gone)");
