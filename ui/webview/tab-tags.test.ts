...
test("one-click MOVE between groups (tab groups, 2026-09-04): 'Move to <name>' adds the target and drops the HOME tag on ONE blob; '+' adds without moving", () => {
  ...
  assert.match(RENDER, /let copyNow: SectionRef \| undefined = copy \? refOf\(copy\) : undefined;\s*\n\s*const heldCopy = \(held: TagUnion\[\]\): TagUnion \| undefined => \{\s*\n\s*const c = copyNow;\s*\n\s*if \(!c\) return undefined;\s*\n\s*return \(c\.localId !== null \? held\.find\(\(g\) => g\.localId === c\.localId\) : undefined\) \?\? held\.find\(\(g\) => g\.name === c\.name\);\s*\n\s*\};\s*\n\s*const homeNow = \(\): TagUnion \| undefined => \{\s*\n\s*if \(!readTabGroups\(\)\.on\) return undefined;\s*\n\s*const held = holding\(\);\s*\n\s*let home0 = heldCopy\(held\);\s*\n\s*if \(!home0 && held\.length === 1 && !held\[0\]\.pending\) home0 = held\[0\];[^\n]*\n\s*if \(home0\) copyNow = sectionRef\(home0\);[^\n]*\n\s*return home0 && !home0\.pending \? home0 : undefined;\s*\n\s*\};/,
    "the computation itself, once, in showTabMenu's scope: the copy's group, tracked through a move (copyNow, a section ref since round 4 of the tab menu review, which moveUnion writes) and an add from no group (claimIfLoose), matched by its tag's id first and its name second; else the one remaining holder; else nothing (round 3); only while sectioned, never a pending tag");
