...
test("kernel flatten rolls a blocked descendant up to every non-done ancestor and marks it qderived", () => {
  ...
  // (the feed's flatten lives in _feed_session_entry since upstream's T368 memo came in with the 2026-09-15 pull-in,
  // which retired this fork's anchor-deps memo and the deps/writes arguments that fed it; the sid stays: it names
  // the session whose ledger memo a changed warm anchor invalidates, round-4 P3 a)
  assert.match(KERNEL, /_node_anchor_uuids\(nd, seg_trig, seg_uuid, sid=fsid\)/);
});
