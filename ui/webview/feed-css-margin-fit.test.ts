// The margin layout's sections in feed.css add up to the panel (plans/file-review.md, "The margin-layout follow-on
// (2026-09-07)"). The feed page loads only feed.css (ui/CLAUDE.md, click-safe buttons: `.romp-acted` is in both sheets
// for that reason), and the viewer mounts there too (feed.ts imports initFileView), so the panel's aside is dressed by
// this sheet alone on that page — styles.css's rules never reach it. Under `.fc-panel.fc-margin { overflow: hidden }`
// the aside never scrolls as a whole (the track is the scroller, locked to the body; a panel that scrolled would carry
// every card off its mark), so whatever the head, the composer, the Send section and the Log add up to beyond the
// track's floor has to come out of THEM. The second review round let every section but the track shrink and scroll
// inside itself, the grown ones first — in styles.css only: feed.css kept the first cut (head, composer, Send and Log at
// flex 0 0 auto, yielding only for a Send confirm or Log rows), the two sheets' byte-equal pins went red, and on the feed
// page the composer, the Track file/folder choice, the Reject all confirm and a refusal row still pushed Send and the Log
// toggle past the aside's bottom, a Tab onto one scrolling the clipped aside (the 2026-09-07 review, round 3). The
// byte-equal pins say the sheets match; nothing measured the fit under feed.css. This module does, as
// styles-fc-margin-fit.test.ts does for the chat page's sheet: the static leg holds feed.css to the two-tier shape and
// to styles.css's margin rules (CI installs no browser); the browser legs mount the worktree's panel under feed.css's
// own rules in Chromium and Firefox at the review's panel heights and measure: nothing past the aside's edge, the Log
// toggle under the pointer, Send under the pointer once its own section is scrolled, the collapsed sections uncut, and a
// focus or a real Tab onto Send or the Log leaving the aside unscrolled and every card where it was. Skips LOUDLY
// without a playwright browser. Synthetic values only: invented prose, placeholder ids.
...
/** The margin layout's rules in `css`, comments stripped: from the root rule to the block's end marker. */
function marginText(css: string, where: string): string {
  const a = css.indexOf(BLOCK_A), b = css.indexOf(BLOCK_B);
  assert.ok(a >= 0 && b > a, "the file-comments block's markers in " + where);
  const start = css.indexOf("\n.fc-panel.fc-margin {", a);
  assert.ok(start > a && start < b, "the margin rules follow the root rule inside the block in " + where);
  return css.slice(start, b).replace(/\/\*[\s\S]*?\*\//g, "");
}
...
/** feed.css's margin rules: each as its selector list and its declarations. */
function marginRules(): Array<{ selectors: string[]; decls: Map<string, string> }> {
  ...
  for (const m of marginText(FEED, "feed.css").matchAll(/([^{}]+?)\s*\{([^}]*)\}/g)) {
...
test("feed.css: the head, the composer, the Send section and the Log all shrink and scroll inside themselves under .fc-margin", () => {
  for (const sel of SECTIONS) {
    ...
    assert.equal(d.get("flex"), "0 1 auto", sel + " is its content's height with a shrink, never flex 0 0 auto (the section that could not give ran past the aside on the feed page)");
  }
});
...
test("feed.css: the grown tier — the composer, and each other section while it holds more than its controls — gives first, down to a floor", () => {
  ...
  // the first cut's one yield rule is gone: with it, the sections above it kept flex 0 0 auto and a Send confirm or Log
  // rows were the only growth that gave
  assert.doesNotMatch(marginText(FEED, "feed.css"), /\.fc-sec-send:has\(\.fc-confirm\), \.fc-margin > \.fc-sec-log:has\(\.fc-log-row\)/, "the first cut's confirm-or-rows yield rule");
});

test("feed.css: the track keeps its floor and the aside itself still clips — the fit comes from the sections, never from a panel that scrolls", () => {
...
test("styles.css: the chat page's sheet holds the same margin rules — a fix that reaches one sheet alone fails here by name", () => {
  // the whole-block pins (file-comments.test.ts, tests/test_guide_files_margin_layout.py) say the sheets differ; this one
  // says the margin layout is where, so the round that fixed the fit in styles.css alone is named at the sheet it missed
  assert.equal(marginText(FEED, "feed.css"), marginText(CHAT, "styles.css"), "feed.css's margin rules are styles.css's: the feed page's viewer lays its aside out the same way");
});
...
function bundle(): string {
  ...
  const r = esbuild.buildSync({
    stdin: {
      ...
      resolveDir: UI, loader: "ts", sourcefile: "feed-margin-fit-probe.ts",
    },
...
/** feed.css's rules the layout lives under: the viewer's card, body and prose, the buttons, and the whole file-comments block. */
function sheet(): string {
  const a = FEED.indexOf(BLOCK_A), b = FEED.indexOf(BLOCK_B);
  assert.ok(a >= 0 && b > a, "the file-comments block's markers in feed.css");
  return [rule(".fileview"), rule(".fileview-body"), rule(".fileview-md"), rule(".fileview-md p"), rule(".fileview-md h1, .fileview-md h2, .fileview-md h3, .fileview-md h4, .fileview-md h5, .fileview-md h6"), rule(".fileview-btn"), rule(".fileview-err"), FEED.slice(a, b)].join("\n");
}
...
${sheet()}
#wrap { width: 1000px; height: ${height}px; }</style></head><body><div class="fileview" id="wrap"><div class="fileview-main" id="main"><div class="fileview-body" id="body"><div class="fileview-md" id="md"></div></div></div></div><script src="/dist/feed-margin-fit.js"></script></body></html>`;
...
async function inBrowser(t: any, name: string, height: number, body: (page: any) => Promise<void>): Promise<void> {
  ...
  try {
    ...
    await pg.route("http://romp.test/**", (route: any) => {
      ...
      if (u.pathname === "/dist/feed-margin-fit.js") return route.fulfill({ status: 200, contentType: "application/javascript", body: js });
...
for (const name of ["chromium", "firefox"]) {
  test(`in ${name}, under feed.css: a 330px panel — the composer, then the Reject all confirm, come out of their own sections; Send and the Log toggle stay in reach`, async (t) => {
    await inBrowser(t, name, 330, async (pg) => {
      ...
      // Comment on this file: the composer opens at the top. Under the first cut's rules the Log toggle sat 11px past the
      // aside here, on the feed page too (the round-3 review measured it under this sheet: 351px of sections in 330).
      ...
      // Reject all: its confirm row joins the foot (it is a choice row, not the Send confirm the first cut keyed on).
  ...
  test(`in ${name}, under feed.css: a 500px panel with every row up — a focus and a Tab onto Send and the Log leave the aside unscrolled and every card on its mark`, async (t) => {
    await inBrowser(t, name, 500, async (pg) => {
      ...
      // the composer open, a refused Accept all's row under the foot, and the Reject all confirm (611px of sections in 500
      // under the first cut's rules)
  ...
  test(`in ${name}, under feed.css: a 200px panel — the collapsed controls alone outgrow the room, and still nothing leaves the aside; each control is a scroll of its own section away`, async (t) => {
