...
for (const [engine, devices] of [["webkit", ["phone", "hybrid"]], ["firefox", ["hybrid"]]] as Array<[TapEngine, TapDevice[]]>) for (const device of devices) for (const surface of ["chat", "pane"] as TapSurface[]) {
  ...
  const red = engine === "webkit"
    ? "red at 0ab74924c, whose gate matched a click to its press by pointerId alone: every tap opened nothing, the double tap too, and " + (device === "hybrid" ? "the right press's and the drag's cells and the clicks after a drag red there too; the covered clicks after a drag red under A18-R, the gate with the file review's round 18 rule reverted and no clear put in its place, the other pane's covered clicks red at X, the gate with a dragstart clear and records ended at no pointerup, and the cost cells red at ef686b029 by design, recording the cost; their reads a private witness kept out of the tree; " : "") + "the key cells green there by design"
