 ...
 *  and a Mouse method invoked indirectly, in six forms: by `call` (`page.mouse.click.call(page.mouse, ...)`), by `apply`, by
 ...
 *  `page.mouse` (`mouse.click(...)` inside the function): for call, apply and bind the member before the method is `click`, not
 *  `mouse`, the comma form and Reflect.apply call no member of `mouse` at all, and the parameter is a bare name, so none is read.
