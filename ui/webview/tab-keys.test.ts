...
test("render.ts: the tab menu's one row asks the shell to record a hot key; a bound one reads Update and the recorder removes too", () => {
  ...
  // the two bounds are read by name and must be found first: an absent anchor reads -1 and the lower bound goes vacuous (the case
  // stayed green with no Rename anchor at all once the row moved onto the builder's addMenuItem; the anchor is the row's label
  // now, as vscode-extension/src/tab-emoji.test.ts re-anchored it)
  const rename = RENDER.indexOf('label: "Rename"'), colors = RENDER.indexOf('const row = el("div", "ctx-colors")');
  assert.ok(rename > 0 && colors > 0, "both anchors are in render.ts (an absent one reads -1 and the bound below goes vacuous)");
  assert.ok(i > rename && i < colors, "with Rename, before the colour swatches: how this tab shows and holds on the strip (the partition has no split row, 2026-09-11)");
