...
// extension of the key (no such unit at the swept head); and outside the created files and the three records only the
// units carrying the vocabulary are read, and a round there with no review named before it in its unit is COUNTED, never
// faulted, so a phrase assembled at run time (a join, a concat, a `+=`, a hole's value) whose "round N" lands in a unit of
// its own is silent there, the unit not read when it carries no vocabulary and counted when it does, and red only inside
// the created files, where every unit is read and the full rule faults a round of no review; the diagnostic prints the
// file and line of every counted round.
 ...
 *  whose comment names that pass); eighteen since the fixes for the file review's round 11 (ui-1 with extra8-1: the web
 *  picture's control class, its dress and the import of its outbound glyph each carry a comment naming that round);
 ...
 *  round); twenty-three since the fixes for the same round's ui-1 with extra9-1 (the comment in the body's mousedown listener
 *  on the press cancelled on the web control, and the comment over the key gate that opens it only in view, each names that
 *  round); twenty-four since the fixes for the same round's fresh-1 (the docstring of figureFoldOf, the summary that toggles a
 *  fold, the sibling of the figure link set the title and the mark read, names that round); twenty-five since the fixes for the
 *  file review's round 15 (correctness-1 with extra5-1, extra6-2, tests-1 and extra9-3: the docstring of figureSourceCredentialed,
 *  the one sign-in rule every visible word of a picture's address reads, names that round); twenty-six since the fixes for the
 *  same round's fresh-1 (the docstring of failedSource, the failed label's reader, which reads the chat page's heal record of an
 *  img it parked, names that round); thirty since the fixes for the file review's round 16, extra5-1 (the one gate between a
 *  gesture and a web picture's tab: the comment over its press recorder and verdict, controlInView's docstring naming the gate
 *  among its readers, and the docstrings of figureSign, the sign a gesture must find shown, and revealSign, a refusal's reveal,
 *  each names that round); thirty-one since the fixes for the same round's fresh-1 (the docstring of FIGOPEN_MIN_PX, the floor,
 *  which it reads in CSS pixels at every zoom, names that round); thirty-two since the fixes for the same round's regression-2 (the
 *  docstring of DECODE_PASSES, the bound on the sign-in rule's decode, names that round); thirty-three since the fixes for the same
 ...
 *  a figure, names that round). A property pin: the count is compared to the derived list, whose lines the message prints. */
const VIEWER_UNITS = 33;
