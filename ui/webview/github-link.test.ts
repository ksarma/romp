...
// is a dashed anchor with the note as its caption. The states run FOR REAL against a small DOM stand-in
// (the Outline pane's live-clock test is the precedent): the module mounts through document.createElement
...
test("the GitHub link is the action REGISTRY's first entry, not another hand-wired button", () => {
  ...
  // one per-open seam ctx (plans/file-review.md Slice 1: path, sid, todoId plus the viewer closures), and
  // seating what it returns in the file group (T367: the GitHub link and the Comments mount ride beside Edit)
