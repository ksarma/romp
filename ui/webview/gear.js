...
function initGear(post) {
  ...
  // time.
  // Thinking summaries, User todos and Suggest /compact are kernel-side but PER-INSTALL: each post
  // goes to the LOCAL kernel only — deliberately not in federation's KERNEL_SETTING set (gear.test.ts
  // pins the membership), so none queues for or reaches another machine. Stamped all the same: two
