...
import { hideEdges } from "../test-dom-shim";   // the shared fake-DOM rule: a node's edges are non-enumerable (ui/test-dom-shim.ts)
...
class FakeEl {
  ...
  constructor(tag: string, cls = "") {
    ...
    hideEdges(this);   // the edges (children, parent) and the records are non-enumerable: a failing assertion never walks the tree
  }
...
test("executed: a FakeEl inspects as its own projection: the edges (children, parent) and the records are non-enumerable and still reachable", () => {
  // the shim's rule (ui/test-dom-shim.ts): node's failing-assertion diff inspects both operands with getters, and an
  // enumerable parent or children list walks the whole tree; a projection of the primitives alone keeps a dump short
  const root = new FakeEl("div", "root"); const kid = root.appendChild(new FakeEl("span", "kid"));
  root.dataset.k = "v"; root.setAttribute("title", "t"); root.addEventListener("click", () => {});
  for (const n of [root, kid]) {
    const keys = Object.keys(n);
    for (const k of ["children", "parent", "dataset", "attrs", "listeners", "style", "classList"]) assert.ok(!keys.includes(k), n.className + ": " + k + " is hidden");
    assert.deepEqual(keys.sort(), ["_nid", "className", "scrollTop", "tag", "textContent", "title"], n.className + " enumerates its primitives and the serial hideEdges stamps");
  }
  assert.equal(kid.parent, root); assert.equal(root.children.length, 1);   // hidden, not gone
  assert.equal(root.dataset.k, "v"); assert.equal(root.attrs.title, "t"); assert.equal(root.listeners.click.length, 1);
  assert.equal(root.querySelector(".kid"), kid);
});
