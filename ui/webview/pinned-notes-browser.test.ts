...
// next pin, and a stray tap minutes later unpinned in one step). Skips LOUDLY without a playwright browser
// (CI installs none), as file-comments-regions-browser.test.ts does. Synthetic values only.
...
function bundle(): string {
  ...
  const r = esbuild.buildSync({
    stdin: {
      contents: 'import { buildPinnedNotes, armUnpin, PINNED_ACT, PINNED_LINE_CHARS } from "./pinned-notes";\nimport { delegate } from "./actions";\n(window as any).__romp = { buildPinnedNotes, armUnpin, PINNED_ACT, PINNED_LINE_CHARS, delegate };\n',
...
type Scene = {
  strip: Box; content: Box; composer: Box; viewport: number; fontPx: number;
  scrollHeight: number; clientHeight: number; overflowY: string; maxHeight: string;
  rows: Array<{ cut: boolean; whiteSpace: string; textOverflow: string; hintVisible: boolean; hintBeforeUnpin: boolean; foldStartsWithText: boolean }>;
  mark: string;
};

/** Mount `n` notes at the bounds with every fold open, wire the delegate once, and report the layout. */
function mount(page: any, n: number, opts: { openAll: boolean; long: boolean }): Promise<Scene> {
  return page.evaluate(([n, opts, SID, LONG, DETAIL]: [number, { openAll: boolean; long: boolean }, string, string, string]) => {
    const w = window as any;
    const host = document.getElementById("pinned-notes")!;
    const notes = Array.from({ length: n }, (_, i) => ({ id: "pn-0000000" + i, text: opts.long ? LONG : "note " + i, detail: opts.long ? DETAIL : undefined, createdT: 1781200000 + i }));
    ...
    const rows = Array.from(host.querySelectorAll(".pn-item")).map((item) => {
      ...
      return {
        cut: t.scrollWidth > t.clientWidth, whiteSpace: ts.whiteSpace, textOverflow: ts.textOverflow,
    ...
    return {
      ...
      viewport: window.innerHeight, fontPx: parseFloat(getComputedStyle(document.body).fontSize),
...
function checkCap(s: Scene, where: string): void {
...
test("in a browser: eight notes at the bounds with every fold open stay within the cap, scroll inside it, and leave the transcript and the composer on screen (desktop and phone)", async (t) => {
  await inBrowser(t, { viewport: { width: 1200, height: 800 } }, async (page) => {
    checkCap(await mount(page, 8, { openAll: true, long: true }), "desktop");
  ...
  await inBrowser(t, { viewport: { width: 380, height: 700 }, hasTouch: true, isMobile: true }, async (page) => {
    checkCap(await mount(page, 8, { openAll: true, long: true }), "phone");
  });
});
