...
test("source: codeBlock and mdBlock run the one pass on the DOM they built; in mdBlock the fenced-block highlight runs over the sanitizer's body, then over the adopted box the markdown anchors are sorted, then the text; marked's parse carries the link-target hook per call; mdBlock has no fallback (the Raw rows a failed render falls back to are codeBlock's, linkified there)", () => {
  ...
  assert.match(mdFn, /const base = marked\.defaults\.walkTokens;\n(?:\s*\/\/[^\n]*\n)*\s*const dirty = viewerHtml\(text, \(t\) => \{\n(?:\s*if \(t\.type === "code"\)[^\n]*\n)?\s*if \(doc && doc\.kind === "file"\) viewerWalkTokens\(t\);\n\s*if \(base\) void base\.call\(marked, t\);\n\s*\}\);/,
    "the hook rides on this parse alone, and on the file kind's alone: the singleton is the chat's too, and a URL document has no directory for `notes.md:7` (the walk itself runs for every kind since the Slice 3 review, collecting the code tokens for Copy; since decision 52 the parse is marked's lexer, the literal-tags rule, the walk and marked's parser, called apart on this parse's tokens, and since that decision's review the exported viewerHtml runs them with the walk mdBlock hands it)");
  assert.match(VIEW, /export function viewerHtml\(text: string, walk\?: \(token: Token\) => void\): string \{\n\s*const opts = \{ \.\.\.marked\.defaults \};\n\s*const tokens = marked\.lexer\(text, opts\);\n\s*literalizeUnclosedTags\(tokens\);\n\s*if \(walk\) marked\.walkTokens\(tokens, walk\);\n\s*return marked\.parser\(tokens, opts\);/, "the recipe: the lexer, the rule, the caller's walk, the parser");
...
test("source: the body's delegate and its gesture: a plain click on a panel mark is the card's alone (an anchor's own open cancelled), a drag-select opens nothing, a plain path click opens through the host's opener and goes on to the document (a modified one stops before the row), the chat's body delegate serves the todo card alone, a section link scrolls the rendered document and never moves the page", () => {
  ...
  assert.match(FILES, /if \(sid && identity\) identities\.set\(sid, identity\);\n\s*const key = placeKey\(path, sid\);[^\n]*\n\s*const place = latestPlace\(recent, [^\n]*\n\s*if \(!openFileView\(path, sid, \{ todoId, at/);   // B1 adds the Recent rows' `place` beside it (the rows for the FILE by the viewer's placeKey since the review's round 5)
...
test("source: a close or a replace-open asks about an unsaved comment the way it asks about unsaved edits: one guard (the editor's ask, then every ask registered through the seam), one asker for both hosts (a confirm on the web, the notice bar in the VS Code webview), reset on both exits; the panel names what is at stake and asks nothing itself", () => {
  ...
  assert.match(FC, /draftAsk\(\): CloseAsk \| null \{\n(?:\s*\/\/[^\n]*\n)*\s*const c = this\.composer;\n(?:\s*\/\/[^\n]*\n)*\s*const typed = !!c && c\.kind !== "replace" && c !== this\.savingFor && !!this\.input\.value\.trim\(\);\n(?:\s*\/\/[^\n]*\n)*\s*const held = this\.heldRefusals\.length;\n(?:\s*\/\/[^\n]*\n)*\s*if \(!typed && !this\.savingFor && !held\) return null;\n(?:\s*\/\/[^\n]*\n)*\s*const p = this\.ctx\.path, name = p\.slice\(p\.lastIndexOf\("\/"\) \+ 1\);\n(?:\s*\/\/[^\n]*\n)*\s*const also = this\.savingFor \? " Another comment on " \+ name \+ " is still saving; if it is not saved, its words are lost too\." : "";\n(?:\s*\/\/[^\n]*\n)*\s*const refused = held === 1 \? "the comment that was not saved" : "the " \+ held \+ " comments that were not saved";\n(?:\s*\/\/[^\n]*\n)*\s*if \(typed && held\) return \{ question: "Discard the comment typed on " \+ name \+ " and " \+ refused \+ "\?" \+ also, kept: "This file stays open: the comment typed on " \+ name \+ " and " \+ refused \+ " are still here\. Save or clear the box, and bring back or dismiss " \+ \(held === 1 \? "the one that was not saved" : "the ones that were not saved"\) \+ ", then try again\." \};\n(?:\s*\/\/[^\n]*\n)*\s*if \(typed\) return \{ question: "Discard the unsaved comment on " \+ name \+ "\?" \+ also, kept: "This file stays open: the comment typed on " \+ name \+ " is not saved\. Save it, or clear the box, then try again\." \};\n(?:\s*\/\/[^\n]*\n)*\s*if \(held === 1\) return \{ question: "Discard the unsaved comment on " \+ name \+ "\?" \+ also, kept: "This file stays open: a comment on " \+ name \+ " was not saved\. Bring it back and save it, or dismiss it, then try again\." \};\n(?:\s*\/\/[^\n]*\n)*\s*if \(held\) return \{ question: "Discard the " \+ held \+ " unsaved comments on " \+ name \+ "\?" \+ also, kept: "This file stays open: " \+ held \+ " comments on " \+ name \+ " were not saved\. Bring each back and save it, or dismiss it, then try again\." \};\n(?:\s*\/\/[^\n]*\n)*\s*return \{ question: "A comment on " \+ name \+ " is still saving\. Close anyway\? If it is not saved, its words are lost\.", kept: "This file stays open: a comment on " \+ name \+ " is still saving\. Try again when it has finished\." \};/,
    "the composer's words, a comment whose save is out, and each refused comment waiting in its note under the box, make the ask, and the ask names every comment a yes would drop (a pin on where the code lives. Executed by file-comments-save-held-composer.test.ts: the save out by \"while a comment saves, the viewer's close asks…\", red when the early return or the asks ignore the save; the refused comments by \"a refused comment waiting in its note is asked about…\" and \"the close names every comment a yes would drop…\", red when the ask ignores the notes, asks about the typed words alone, or names one refused comment however many wait; the typed words by \"while a comment saves, the viewer's close asks…\", file-comments-behavior.test.ts \"the panel's draft ask (guardClose) names the unsaved comment…\" and file-comments-send-note.test.ts \"the viewer's close guard asks about the note…\", red when the ask ignores the typed words)");
