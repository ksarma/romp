...
test("source: codeBlock and mdBlock run the one pass on the DOM they built; in mdBlock the fenced-block highlight runs over the sanitizer's body, then over the adopted box the markdown anchors are sorted, then the text; marked's parse carries the link-target hook per call; mdBlock has no fallback (the Raw rows a failed render falls back to are codeBlock's, linkified there)", () => {
  ...
  assert.match(mdFn, /const base = marked\.defaults\.walkTokens;\n(?:\s*\/\/[^\n]*\n)*\s*const dirty = viewerHtml\(text, \(t\) => \{\n(?:\s*if \(t\.type === "code"\)[^\n]*\n)?\s*if \(doc && doc\.kind === "file"\) viewerWalkTokens\(t\);\n\s*if \(base\) void base\.call\(marked, t\);\n\s*\}\);/,
    "the hook rides on this parse alone, and on the file kind's alone: the singleton is the hover preview's and the anchor map's too (the chat's own instances render a reply and the user's bubble since 2026-09-19, chat-md.ts), and a URL document has no directory for `notes.md:7` (the walk itself runs for every kind since the Slice 3 review, collecting the code tokens for Copy; since decision 52 the parse is marked's lexer, the literal-tags rule, the walk and marked's parser, called apart on this parse's tokens, and since that decision's review the exported viewerHtml runs them with the walk mdBlock hands it)");
