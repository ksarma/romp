...
// This fork keeps the default list's six-row cap (`.bg-list:not(:has(.bg-task.open)) { max-height: 180px; }`, 4e D2:
// upstream's sheet has no default cap), so the dense cap must carry the default cap's exact guard to outrank it.
