...
import { ANYWAY_SENTENCE } from "./file-print-fixtures";   // the sentence's fixed test-side copy: case (15b) holds the rendered line to it at both reads (the round-7 review's cluster A)
...
test("(15b) a picture that lands while the line stands prints, and the line's last sentence stays true of it: the PRINT is read under print media while each line stands (the round-6 review's cluster D, 2026-09-20: the released picture complete with a non-zero box, the unreleased one still loading in the box case (15a) measured for it, 0 by 0 with no declared size and an empty box of the declared size otherwise; the landed sized picture measures 120 by 120, its declared width and the square picture's own height, where its parked box was 120 by 80), and the press then prints once with exactly the unreleased pictures incomplete; under the deadline's ask over two parked pictures, both released, the ask stands with its line unchanged (nothing listens under the ask; the count is the aim's, extra7-2), both pictures complete in non-zero boxes, and Print anyway prints once with every <img> complete; under Keep waiting's open-ended wait the picture with no declared size released alone leaves the line reading two, that picture complete with a non-zero box and the sized one still loading in its empty declared box while the line stands (read there, not inferred from the print's complete list; the branch's verification pass finding engines-1), and Print anyway prints once with the sized one the only <img> still loading; the sized one released alone leaves the picture with no declared size a 0 by 0 box, still loading, while the line stands, and Print anyway prints once with it the only one still loading; under Keep waiting both released print once on the settle, no press. The sentence beside the button is read off the rendered line and held to two texts at each of its two reads: the product's own, anywayWords(), the wiring (the line shows what the function says), and a fixed test-side copy the product never writes, ANYWAY_SENTENCE in file-print-fixtures.ts, so a wording change reds this case at that compare while the print reads beside it hold (the round-7 review's cluster A, 2026-09-21: from the round-7 fixes to the round-8 fixes the case held the line to anywayWords() alone, both sides moving with the product, and reddened at no wording; the round-6 review's ui-1, correctness-4 and tests-1: before the round-7 fixes the case's only red at the round-5 wording, \"Print anyway prints without them.\", was a compare against a copy spelled in this leg). The compare proves the wording reaches the rendered pre-press DOM, the line the person reads before the press; nothing here proves the sentence is printed, because it is not: the sheets hide the line under print media (case (15a) measures a 0 by 0 box for it), so the print reads hold at either wording (the round-5 review's ui-1, 2026-09-20)", { timeout: 240000 }, async (t) => {
  await inBrowser(t, async (browser) => {
    ...
    assert.ok(askLine.endsWith(" " + anywayWords()), "the sentence beside the button, read off the line before the press, is the product's own (anywayWords(), the wiring): " + askLine);
    assert.ok(askLine.endsWith(" " + ANYWAY_SENTENCE), "the sentence beside the button, read off the rendered line before the press, is the fixed test-side copy the product never writes (file-print-fixtures.ts ANYWAY_SENTENCE): this compare, not the wiring compare above it, is what reds at a wording change; the line read: " + askLine);
    await frames(page, 1);
    assert.equal((await bar(page)).phase, null, "the bar rested");
    await page.evaluate(() => { (window as any).FV.setPrintSettleMs(null); });
    assert.deepEqual(s.errors, [], "no script error");
    ...
    assert.ok(waitLine.endsWith(" " + anywayWords()), "the sentence beside the button, read off the line before the press, is the product's own (anywayWords(), the wiring): " + waitLine);
    assert.ok(waitLine.endsWith(" " + ANYWAY_SENTENCE), "the sentence beside the button, read off the rendered line before the press, is the fixed test-side copy the product never writes (file-print-fixtures.ts ANYWAY_SENTENCE): the compare that reds at a wording change; the line read: " + waitLine);
    await frames(page, 1);
    assert.equal((await bar(page)).phase, null, "the bar rested");
