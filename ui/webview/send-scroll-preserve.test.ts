...
test("every composer-shaped send rides the same gate — staged flush and provisional adoption included", () => {
  // the staged flush releases the run through routeUserMessage: a goal item on its own, the rest folded
  // into one body with the typed message (2026-09-08, one message not a series)…
  ...
  assert.match(RENDER, /for \(const s of batch\) if \(citesGoal\(s\)\) routeUserMessage\(sid, s\.text, s\.cites as Citation\[\]\);/);
