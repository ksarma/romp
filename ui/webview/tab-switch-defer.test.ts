...
test("the landing (scroll/anchor/diagnostics) is factored so sync + deferred paths land identically", () => {
  ...
  // nothing, since while a build is pending the scroller can hold the reveal's clamp (land-active-keep.test.ts executes what the flag
  // changes inside landActive; that the deferred land passes nothing is pinned by text alone, here and in land-active-keep's composition pin)
