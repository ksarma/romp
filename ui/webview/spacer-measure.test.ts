...
test("render.ts: the render task's spacer code holds no layout read; the unit observer records border-box heights and measures in both of its branches", () => {
  ...
  // re-show (land-saved, the raw write; an ARMED land that misses with no fetch armed takes and puts the saved place's row back, anchor-restore, and writes
  ...
  // land-active-keep.test.ts; a miss with a fetch armed writes raw, keeping the take only under this attempt's pre-jump, which was measured over it), and keepPlaceAcrossWindow over its restore. A switch's, a landing's or a hidden prebuild's sync passes no flag and applies
  ...
  assert.match(land, /else if \(!\(held && !fetchArmed && restoreScrollAnchor\(content, v, held\)\)\) \{ if \(!preJumped\) untakeMeasure\(v, figures\); writeScroll\(content, v\.scrollTop, "land-saved"\); \}/, "the saved-place fallback restores the captured row on a miss with no fetch armed; where that restore finds no row to put back the take is given back and the raw write follows, exact in the layout it was saved in: nothing armed (nothing taken, nothing given back), no row at the saved place, or the captured row gone with the attempt's window build; a miss with a fetch armed restores nothing, and its raw write keeps the take only when this attempt's pre-jump placed the reader over it (a source pin on where the rule sits; land-active-keep.test.ts executes every road)");
...
/** The ScriptKind a module is parsed under, by its suffix: an admitted .tsx or .jsx under its own kind (JSX under the TS kind misparses in
 *  silence, the maintainer's round 7 ruling, extra7-1: an element with an attribute hides the literal behind it, a fragment does not), a .js,
 *  .mjs or .cjs under the JS kind, the rest under TS. Module-level so the kind cell after the witness cell asserts the mapping and the element
 *  shape through the same function the census parses every loaded module with (the maintainer's round 8 ruling, tests-2). */
const kindOf = (m: string): ts.ScriptKind => /\.tsx$/.test(m) ? ts.ScriptKind.TSX : /\.jsx$/.test(m) ? ts.ScriptKind.JSX : /\.[mc]?js$/.test(m) ? ts.ScriptKind.JS : ts.ScriptKind.TS;
...
test("every module the page bundles load, read with the compiler: the only write of a `view` property onto an object that reaches a clientDiag post is spacerRow's shorthand under a spread of a literal conditioned on equality with the one word, and every other write of a `view` property in the modules is enumerated with the object it writes and why no post reads it (the maintainer's round 5 ruling, tests-1; the population derived from what the bundles load and tied to the directory by equality both ways, the maintainer's round 6 ruling, correctness-1 and extra8-2; the owner 2026-09-21, who approved the field)", async () => {
  ...
  // has a test of a JavaScript suffix or a module suffix under the directory, and the directory's one tracked stylesheet moves between two
  // classes this cell never reads (it reads the modules class alone), so that case is the tree's own witness of the order and of the widened regex.
  ...
  for (const m of modules) censusViewWrites(m, m === "ui/webview/render.ts" ? SF : ts.createSourceFile(m, fs.readFileSync(path.resolve(ROOT, m), "utf8"), ts.ScriptTarget.Latest, true, kindOf(m)), c);   // each under the kind its suffix names (kindOf, module-level; the kind cell below)
...
test("the script kind is chosen by suffix and is load-bearing, through the census's own walker: a synthetic .tsx source in the ELEMENT shape (a JSX element with an attribute before a clientDiag post's `view` literal) parsed under the kind kindOf hands a .tsx module, TSX, names the literal as a site, and the same source under the TS kind names nothing, so a kindOf that reduced .tsx to the TS kind would leave a minter behind an element in a loaded .tsx module unnamed; the fragment shape, named under both kinds, is the qualification the landing pass after the maintainer's round 7 ruling found; and the suffix-to-kind mapping is pinned whole (the maintainer's round 8 ruling, tests-2)", () => {
  // synthetic sources (no real data): the word is one the kernel refuses; the module name is what kindOf reads, the suffix .tsx
  const run = (module: string, src: string, kind: ts.ScriptKind): ViewCensus => { const c = newViewCensus(); censusViewWrites(module, ts.createSourceFile(module, src, ts.ScriptTarget.Latest, true, kind), c); return c; };
  const M = "ui/webview/zz.tsx";
  const element = 'export function plantPost(): unknown { const el = <div className="plant">tsx</div>; return { type: "clientDiag", data: { view: "away" }, el }; }';
  assert.deepEqual(run(M, element, kindOf(M)).sites.map(describeSite), ["ui/webview/zz.tsx plantPost: a literal property"], "under the kind kindOf picks for a .tsx module the `view` literal behind the JSX element is a site, named by module, owner and form: this half reds when kindOf hands .tsx a kind that is not TSX");
  assert.deepEqual(run(M, element, ts.ScriptKind.TS).sites.map(describeSite), [], "under the TS kind the same source names nothing: the compiler parses no JSX there, misreads the element and never reaches the literal, in silence (no diagnostic is read), which is why the kind is load-bearing; this half pins the compiler's parse of this shape, not kindOf, and a compiler that parsed further would red here with the reason to restate");
  // the qualification (the landing pass after the maintainer's round 7 ruling, its first verifier): the miss is specific to the element shape; a
  // JSX FRAGMENT before the same literal parses far enough under the TS kind for the literal to be named, so a .tsx module that opens with a
  // fragment is named under either kind and the kind's choice shows on the element shape alone. Executed so the sentence cannot outlive it.
  const fragment = 'export function plantedPost(): unknown { const frag = <><span>planted</span></>; return { type: "clientDiag", data: { view: "elsewhere" }, frag }; }';
  assert.deepEqual(run(M, fragment, ts.ScriptKind.TS).sites.map(describeSite), ["ui/webview/zz.tsx plantedPost: a literal property"], "the fragment shape under the TS kind is still named");
  assert.deepEqual(run(M, fragment, kindOf(M)).sites.map(describeSite), ["ui/webview/zz.tsx plantedPost: a literal property"], "...and under the kind kindOf picks");
  // the mapping itself, every suffix the module class admits, read as the enum's names so a red spells the kinds
  assert.deepEqual(["a.tsx", "a.jsx", "a.js", "a.mjs", "a.cjs", "a.ts", "a.mts", "a.cts"].map((f) => f + " " + ts.ScriptKind[kindOf("ui/webview/" + f)]), ["a.tsx TSX", "a.jsx JSX", "a.js JS", "a.mjs JS", "a.cjs JS", "a.ts TS", "a.mts TS", "a.cts TS"], "the suffix-to-kind mapping the census parses every loaded module under: .tsx TSX, .jsx JSX, .js/.mjs/.cjs JS, .ts/.mts/.cts TS");
});
