...
// absent, and in CI's job that runs npm test no browser install precedes that step (.github/workflows/ci.yml; the plan
...
// video, audio, track, image and feImage, with the attributes the gate reads per tag (the scene's FETCHING table names the same
// seven tags, pinned equal to FIGURE_SEL below; its attributes per tag are the scene's own copy of the gate's FETCH_ATTRS, which is
// not exported and is pinned nowhere until the follow-up PR's fresh-4), so this guard answers "did the gate's model see a leak" and
// not "did anything fetch"; an element outside those seven tags, an iframe, an object, an embed, a url() in an inline style, is
// invisible to it, which is the gate's blind spot and
...
/** The attributes an element fetches through, by tag (the scene's own table; its keys are pinned equal to figure-gate's FIGURE_SEL
 *  below, so the two walks name the same tags; the attributes per tag are a copy of the gate's FETCH_ATTRS, which is not exported
 *  and is pinned nowhere until the follow-up PR's fresh-4). `xlink:href` is the stand-in's key for the XLink attribute. */
 ...
 *  over FIGURE_SEL's seven tags and the attributes the gate reads per tag (the scene's FETCHING names the same tags, pinned equal
 *  to FIGURE_SEL; its attributes per tag are unpinned until the follow-up PR's fresh-4), so it answers
