...
test("source: the box is a textarea of COMPOSER_ROWS rows; keydown goes through composerKeyAction; input autosizes; Save is the delegate's fcsave; the hint rides the platform", () => {
  ...
  assert.match(SRC, /this\.input\.addEventListener\("input", \(\) => \{ this\.autosize\(\); this\.syncHeldBack\(\); \}\);/, "input autosizes, and a refused comment's Bring it back follows the box (a pin on where the code lives. The autosize is executed by this file's \"the box grows with its content and shrinks back…\" and \"the draft survives the poll's re-render and a refusal…\", file-comments-composer-review.test.ts \"a height dragged before the first keystroke stands…\", file-comments-reply-keep.test.ts \"the poll's re-render keeps the card node hosting the box…\", file-comments-reply-move.test.ts \"the session's track-edit answers the comment under the reply…\" and \"the change accepted elsewhere lands by the poll…\", and file-comments-reply-place.test.ts \"a comment answered by a change: Reply opens the box in the comment's own card…\", \"the poll's re-render keeps the box in the same comment's card…\" and \"the comment vanished: the box returns to the slot…\"; Bring it back's following by file-comments-save-held-composer.test.ts \"a comment refused after the person typed in a reply started under its save…\" and \"on a coarse pointer, where a title never reaches, a waiting Bring it back's reason…\"; each red with its call dropped from the listener)");
  ...
  assert.match(SRC, /const hint = el\("span", "fc-note fc-hint", composerHint\(IS_MAC\)\);\n(?:\s*\/\/[^\n]*\n)*\s*acts\.replaceChildren\(\.\.\.\(noSave \? \[\] : waits \? \[save\] : \[hint, save\]\), btn\("Cancel", "fccancel"\), \.\.\.\(!noSave && waits \? \[this\.waitLine\(waitWords\)\] : \[\]\)\);/,
    "the hint, Save and Cancel, and under a Save waiting on another comment's save the loader's line in place of the hint, after Cancel (a pin on where the code lives. The hint is executed by this file's \"every composer is one textarea of three rows…\", file-comments-composer-review.test.ts \"on a phone the hint under the box names Save…\" and file-comments-reply-place.test.ts \"Reply moves the box into the comment's card…\", each red with the hint dropped from the row; the waiting line by file-comments-save-held-composer.test.ts \"on a fine pointer, a new comment under a comment's save, whose Save waits…\", \"on a coarse pointer, a reply under a comment's save, whose Save waits…\" and \"on a fine pointer, a new comment under a reply's save, whose Save waits…\", red with the line dropped, the hint kept beside it, or the line put before Save or between Save and Cancel)");
