...
test("the gesture chip's ink is the outgoing-bubble blue on the chat's own ground", () => {
  ...
  assert.match(CSS, /\.user-bubble\.cmd-row:not\(\.echo-bubble\) \.slash-cmd-chip \{ background: var\(--bg\); color: var\(--you\);/);
  assert.match(CSS, /\.user-bubble\.cmd-row:not\(\.echo-bubble\) \.slash-cmd-chip \{[^}]*border-color: var\(--you\)/);
});
