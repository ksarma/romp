...
// BOTH sheets: the chat page loads styles.css alone and the feed page feed.css alone, and the feed page mounts the
// same viewer (feed.ts calls initFileView; file-view.ts points the feed bundle at the same pdf-chunk.js), so the
// token and the rule live in both and each sheet's own cascade is resolved here. fileview-parity.test.ts pins the
// viewer's shared heads byte-equal, but that is a second check, not a substitute: a head list says the bytes match,
// not that the rule wins on the chain or what ratio it yields, and a rule absent from the list is unchecked in both
// sheets at once — feed.css shipped without this token and rule while every test stayed green (2026-09-06).
...
test("one on-white amber across the two sheets: feed.css declares the same --warn-on-white as styles.css", () => {
  // the per-sheet tests above hold each sheet to the floor on its own; this holds the two to ONE colour, so a
  // retune in one sheet cannot leave the feed page's sheet a different amber from the chat page's
  assert.equal(SHEETS.length, 2, "both sheets under test");
  const [[, chat], [, feed]] = SHEETS;
  for (const opener of [":root {", "body.theme-light {"]) {
    const a = tokensOf(chat, opener).get("--warn-on-white"), b = tokensOf(feed, opener).get("--warn-on-white");
    assert.ok(a && b, opener + ": both sheets declare --warn-on-white");
    assert.equal(a, b, opener + ": the sheets disagree on the on-white amber");
  }
});
