...
// review of 2026-09-09), driven over the stand-in file-comments-arrivals.test.ts drives (copied here, as the sibling modules
...
// Resolved fold hides; the note box keeps a dragged height through typing, grows to SEND_NOTE_ROWS then scrolls, comes back
// at three rows after Cancel or a send that took the words and keeps its words and height through a refusal, keeps the
// keyboard through a status landing, and stops Escape at the box; the nothing-unsent caption is not stacked under the
// confirm's own line. The numbers a real engine measures are file-comments-arrivals-fixes-browser.test.ts. Synthetic
...
function textWorld(): World {
  ...
  w.ctx = {
    ...
    body: () => w.body as unknown as HTMLElement, mode: () => "raw", text: () => DOC, mtimeNs: () => w.viewMtime, error: () => null, media: () => null, mediaElement: () => null, renderedImages: () => [], pdfPages: () => [],
...
// ── the line through the press hold ───────────────────────────────────────────────────────────────
...
test("a primary press on an arrival's card marks it seen but the line and the accept option keep their words until the release: the dot comes off in place, the words change after the pointerup; the last arrivals' press keeps the line standing until the release too", async (t) => {
  ...
  head.dispatchEvent(new Ev("pointerdown", { button: 0 }));   // the person presses the card's head to open it
  assert.ok(!isNew(w.card(passage.id)), "seen: the dot came off in place (an attribute moves nothing)");
  assert.ok(!isNew(markOf(w, passage.id)), "and the highlight's");
  assert.equal(lineOf(w)!.textContent, "api made 1 change, 1 comment and 1 reply since you last looked", "the line keeps its words through the press: a shorter line would move the card under the pointer");
  assert.ok(w.aside().querySelector(".fc-head")!.contains(lineOf(w)!), "and its place");
  await release();
  assert.equal(lineOf(w)!.textContent, "api made 1 change and 1 comment since you last looked", "released: the words follow");
  // the last arrivals: the change at 360 and the session's comment at 480 in the box [340, 500)
  scrollBody(w, 340);
  w.body.dispatchEvent(new Ev("pointerdown", { button: 0 }));
...
test("a press with another button is no click's press (pressHold's rule): the words change at once", async (t) => {
  const { w, ok } = await open(t, textWorld());
  await land(w, ok, arrived());
  w.body.dispatchEvent(new Ev("pointerdown", { button: 2 }));
  assert.equal(lineOf(w)!.textContent, "api made 1 change, 1 comment and 1 reply since you last looked", "the findings' reply seen, the line rewritten with no release to wait for");
  w.close();
});

test("at source: the hold is installed before the gesture listeners on the same row — same target and phase run in registration order, so it is held when the gesture's pointerdown reads it", () => {
...
// ── the line's click reaches an arrival the list hides ────────────────────────────────────────────

test("under the Changes filter the session's replies on plain comments show no card: a wheel marks none seen, and the line's click takes the filter to All and shows the first of them, opened", async (t) => {
  const { w, ok } = await open(t, textWorld());
  filterButton(w, "changes").click(); await tick();
  assert.equal(w.card(findings.id), null, "the fixture: the comments' cards are hidden");
  await land(w, ok, arrived({ hunks: [hunk], store: storeWith([whole, findingsR, passageR, closing]) }));
  assert.equal(lineOf(w)!.textContent, "api made 2 replies since you last looked");
  gesture(w.body, "wheel");
  assert.equal(lineOf(w)!.textContent, "api made 2 replies since you last looked", "no card on screen: nothing seen");
  lineOf(w)!.click(); await tick();
  assert.ok(w.card(findings.id), "the filter went to All: the first arrival's card is in the list");
  assert.ok(w.card(findings.id)!.classList.contains("open"), "opened");
  assert.ok(w.card(passage.id), "the other comments' cards with it");
  assert.equal(lineOf(w)!.textContent, "api made 2 replies since you last looked", "the line stands through its click");
  w.close();
});

test("a reply on a resolved comment sits under the closed Resolved fold: the line names it, and its click opens the fold and the card", async (t) => {
  const { w, ok } = await open(t, textWorld());
  const closingDone: StoreComment = { ...apiReply(closing, T0 + 40000, "Ended on the recommendation."), resolved: true };
  await land(w, ok, arrived({ hunks: [hunk], store: storeWith([whole, findings, passage, closingDone]) }));
  assert.equal(lineOf(w)!.textContent, "api made 1 reply since you last looked");
  assert.equal(actIn(w.aside(), "fcresolved")!.textContent, "▸ Resolved (1)", "the fixture: the fold is closed");
  assert.equal(w.card(closing.id), null, "its card is not rendered");
  gesture(w.body, "wheel");
  assert.ok(lineOf(w), "not on screen: not seen");
  lineOf(w)!.click(); await tick();
  assert.equal(actIn(w.aside(), "fcresolved")!.textContent, "▾ Resolved (1)", "the fold opened");
  assert.ok(w.card(closing.id) && w.card(closing.id)!.classList.contains("open"), "the card is rendered and open");
  w.close();
});

// ── the list layout ───────────────────────────────────────────────────────────────────────────────

test("the list layout (the narrow column): a gesture marks seen the arrivals whose cards stand in the aside's box, by their rects, and leaves the ones below; the aside scrolled, the next gesture marks what is in the box then", async (t) => {
  narrow = true;
  const w = textWorld();
  const { ok } = await open(t, w);
  try {
    assert.equal(w.top(findings.id), null, "the fixture: no placement, the cards are in flow");
    await land(w, ok, arrived());
    // the list's own order: the change cards first, then the comments by position — chg:h1, chg:h2, the whole-file comment,
    // the findings', the passage's, the session's, the person's other, the closing; three closed cards fill the box
    const inBoxNow = (): string[] => w.aside().querySelectorAll(".fc-card").filter((c) => { const r = c.getBoundingClientRect(); return r.top >= LIST_TOP && r.top < LIST_TOP + LIST_VIEW; }).map((c) => c.dataset.id);
    assert.deepEqual(inBoxNow(), [CHG, CHG2, whole.id], "the fixture: the session's change is in the aside's box, the replied-to comments and its own comment below it");
    gesture(w.body, "wheel");
    assert.ok(!isNew(w.card(CHG2)) && !isNew(markOf(w, CHG2)), "in the box: seen, by the card's rect against the aside's");
    assert.ok(isNew(w.card(findings.id)) && isNew(w.card(passage.id)) && isNew(w.card(line9.id)), "below it: not");
    assert.equal(lineOf(w)!.textContent, "api made 1 comment and 2 replies since you last looked");
    listScroll = 600;                                   // the person scrolled the aside past every card: nothing on screen
    assert.deepEqual(inBoxNow(), []);
    gesture(w.body, "wheel");
    assert.equal(lineOf(w)!.textContent, "api made 1 comment and 2 replies since you last looked", "a card scrolled out is not on screen");
    listScroll = 120;                                   // three closed cards down: the two replied-to comments and the closing one (the comments in the order of their timestamps) in the box
    assert.deepEqual(inBoxNow(), [findings.id, passage.id, closing.id], "the fixture");
    gesture(w.aside(), "pointerdown"); await release();
    assert.ok(!isNew(w.card(findings.id)) && !isNew(w.card(passage.id)), "seen by the gesture after the scroll");
    assert.ok(isNew(w.card(line9.id)), "the session's own comment, below the box still, is not");
    assert.equal(lineOf(w)!.textContent, "api made 1 comment since you last looked");
    listScroll = 240;                                   // six cards down: the session's comment and the person's other one
    assert.deepEqual(inBoxNow(), [line9.id, mine2.id], "the fixture");
    gesture(w.body, "wheel");
    assert.ok(!isNew(w.card(line9.id)));
    assert.equal(lineOf(w), null, "every arrival seen");
  } finally { narrow = false; }
  w.close();
});

async function saveReplyInList(t: Ctx, scrolled: number): Promise<World> {
  narrow = true;
  const w = textWorld();
...
// ── the note box ──────────────────────────────────────────────────────────────────────────────────
// the row height rowCap reads, lent to the stand-in's window for these tests (a document with no computed style has no cap)
const NOTE_CS = { lineHeight: "20px", paddingTop: "5px", paddingBottom: "5px" };
const lendRows = (box: El): void => { win.getComputedStyle = (el: El) => (el === box ? NOTE_CS : { lineHeight: "normal", paddingTop: "0px", paddingBottom: "0px" }); };
const ROW_CAP = 8 * 20 + 10;                            // SEND_NOTE_ROWS rows of the line-height plus the padding: 170

test("the note box grows with its content to SEND_NOTE_ROWS rows, then scrolls; the box comes back at three rows after Cancel — the inline height it had grown to goes with the words", async (t) => {
  const { w, fc } = await open(t, textWorld());
  assert.equal(fc.SEND_NOTE_ROWS, 8);
  actIn(w.aside(), "fcsend")!.click();
  const box = noteBox(w);
  lendRows(box);
  try {
    assert.equal(box.style.height, undefined, "fresh: no inline height (the sheet's three rows)");
    typeNote(w, "one\ntwo\nthree\nfour", 4 * 20 + 10);
    assert.equal(box.style.height, "90px", "four lines: the content's height");
    typeNote(w, Array.from({ length: 15 }, (_, i) => "line " + i).join("\n"), 15 * 20 + 10);
    assert.equal(box.style.height, ROW_CAP + "px", "fifteen lines: eight rows, then the box scrolls (before: the composer's twelve)");
    actIn(w.aside(), "fcsendcancel")!.click();
    assert.equal(box.value, "", "closed on purpose: the words go");
    assert.equal(box.style.height, "", "and the height with them (before: an empty box eight rows tall on the next confirm)");
    actIn(w.aside(), "fcsend")!.click();
    assert.equal(noteBox(w), box, "one persistent box");
    assert.equal(box.style.height, "", "three rows again");
    typeNote(w, "one\ntwo\nthree\nfour", 4 * 20 + 10);
    assert.equal(box.style.height, "90px", "and it autosizes again");
...
test("a height the person dragged the box to stands through typing (the composer's rule), before the first keystroke and after; Cancel clears it; a successful send clears the words and the height; a refused send keeps both", async (t) => {
  ...
  lendRows(box);
  try {
    box.style.height = "300px";                         // the sheet's resize handle: an inline height autosize did not write, before a word was typed
    typeNote(w, "Keep the tone.", 20 + 10);
    assert.equal(box.style.height, "300px", "dragged before the first keystroke: the drag stands (before: snapped to the content, three rows)");
    actIn(w.aside(), "fcsendcancel")!.click();
    assert.equal(box.style.height, "", "Cancel: cleared");
    actIn(w.aside(), "fcsend")!.click();
    typeNote(w, "Keep the tone.\nAnd the numbers.", 2 * 20 + 10);
    assert.equal(box.style.height, "50px", "autosized");
    box.style.height = "250px";                         // dragged since
    typeNote(w, "Keep the tone.\nAnd the numbers.\nCite the run.", 3 * 20 + 10);
    assert.equal(box.style.height, "250px", "the drag stands until the confirm closes");
    // a refused send keeps the words and the height
    ...
    assert.equal(noteBox(w).value, "Keep the tone.\nAnd the numbers.\nCite the run.", "refused: the words stay");
    assert.equal(box.style.height, "250px", "and the height with them");
    // a send that goes takes both
    actIn(w.aside(), "fcsendgo")!.click(); await tick();
    const m2 = lastOf(w, "fileCommentsSend")!;
    assert.notEqual(m2.reqId, m.reqId);
    win.dispatchEvent(new MessageEvent("message", { data: { type: "fileCommentsSent", reqId: m2.reqId, queued: false } })); await tick(); await tick();
    await ok(status({ hunks: [], storeMtimeNs: "1757145600000000005" }));   // the send's refresh
    assert.equal(box.value, "", "sent: the words went with the message");
    assert.equal(box.style.height, "", "and the height (before: kept, an empty box at the last note's height on the next confirm)");
    assert.equal(w.aside().querySelector(".fc-confirm"), null, "the confirm is down");
  } finally { delete win.getComputedStyle; }
  w.close();
});

test("the keyboard stays in the note box through a status landing: the render moves the box into the fresh confirm, which drops the focus, and puts it back", async (t) => {
  const { w, ok } = await open(t, textWorld());
  actIn(w.aside(), "fcsend")!.click();
  const box = noteBox(w);
  box.focus();
  assert.equal(doc.activeElement, box, "the fixture: a textarea takes focus in this stand-in");
  typeNote(w, "Keep the tone.");
  await land(w, ok, status({ storeMtimeNs: "1757145600000000005" }));   // a poll's status while the person types
  assert.equal(noteBox(w), box, "the same box, in the fresh confirm");
  assert.equal(doc.activeElement, box, "the keyboard is back in it (the move dropped it to the body)");
  assert.equal(box.value, "Keep the tone.", "the words stand");
  w.close();
});

test("Escape in the note box stops at the box: the document's listener (the viewer's own close) never sees it, and the words and the confirm stay", async (t) => {
  const { w } = await open(t, textWorld());
  actIn(w.aside(), "fcsend")!.click();
  const box = noteBox(w);
  typeNote(w, "Half a note");
  let seen = 0;
  const onKey = (e: Ev) => { if (e.key === "Escape") seen++; };
  doc.addEventListener("keydown", onKey);              // the viewer's document-level Escape (file-view.ts onKey), a bubble listener
  try {
    box.dispatchEvent(new Ev("keydown", { key: "Escape" }));
    assert.equal(seen, 0, "stopped at the box");
    assert.equal(noteBox(w).value, "Half a note", "the words stay");
    assert.ok(w.aside().querySelector(".fc-confirm"), "the confirm stays");
    w.body.dispatchEvent(new Ev("keydown", { key: "Escape" }));
    assert.equal(seen, 1, "an Escape elsewhere in the row reaches the document");
  } finally { doc.removeEventListener("keydown", onKey); }
  w.close();
});

// ── the nothing-unsent caption ────────────────────────────────────────────────────────────────────
...
test("vocabulary: this module's own prose says a change's old and new text, file comment and run of turns; the words CONTEXT.md sets aside appear nowhere in it, nor the banned sessions-pane word, nor a home path", () => {
  const SELF = web("file-comments-arrivals-fixes.test.ts").split("\n").filter((l) => !l.includes("assert.doesNotMatch(SELF")).join("\n");
