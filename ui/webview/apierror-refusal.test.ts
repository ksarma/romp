...
test("the chat error card drops Retry on a refusal and names the real fix in plain terms", () => {
  ...
  // …and the countdown line says what happened + the fix, never "retrying soon…"
  assert.match(apiErr, /if \(refusal\) countdown\.textContent = "the model's safeguards refused this prompt — rewrite it or drop this thread";/);
