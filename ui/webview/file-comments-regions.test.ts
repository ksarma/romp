...
test("source pins: the overlay's wiring (data-act names, the coarse gate, the seam member, retarget on Re-place, the composer's target path)", () => {
  ...
  assert.match(SRC, /this\.paintPresel\(root, src, rendered, true\);\n\s*this\.trimBlanks\(\);\n(?:\s*\/\/[^\n]*\n)*\s*for \(const c of byCard\)[^\n]*\n\s*this\.paintRegions\(\);\n\s*if \(held\) this\.refocusMark\(held\);\n\s*this\.render\(\);/,
    "after the text pass and its one trim of collapsed blanks (trimBlanks, then the cards whose every mark was trimmed re-filed as not painted); the mark that held the keyboard is re-found once every pass has painted (heldMark / refocusMark), then the render");
