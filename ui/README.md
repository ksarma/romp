# ui/: the front-end source (all six panes)

The web UI the kernel serves: **chat**, **feed**, the outline (**fleet**), **timeline**,
**waiting** (the "Waiting on you" pane), and **files** (the file viewer's own pane).
...
- **`webview/`** — the pane sources: `render.ts` (chat), `feed.ts` + `feed.css`
  ...
  `waiting-pane.css` (waiting on you), `files.ts` + `files-pane.css` (files: the
  file viewer as its own column; the viewer itself is the shared `file-view.ts`), `timeline-main.ts` +
