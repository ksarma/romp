"""envsource: the credential set a configured COMMAND prints, handed to each child at launch and never
written to the kernel's own environment or to any file.

keysource.py selects the API key SOURCE: a static key line, a 1Password reference, or a command
(``ROMP_CREDENTIAL_COMMAND``, a non-secret shell command printing ``NAME=VALUE`` lines). The reference
is the built-in default command; an explicit command generalises it. This module is the command kind's
runtime: it runs the KeySource keysource selected, parses what the command prints, holds the resulting
SET in one private dict, and lets it out only for merging into a CHILD's environment (a session CLI's
launch options, a judge call's env, the catalog fetch's header). Nothing here decides the mode. Every
reader takes the KeySource (``source``); None, or a source of any kind but ``command``, is an empty set
and an unconfigured record, with no run. keysource reaches back through ``keysource.COMMAND_RESOLVER``
(the kernel wires it to ``resolve_key`` at import), so this module loads keysource and keysource never
loads it.

Configuration of the three tuning values (process environment first, then the same line in the
manager's env file, so a terminal outside the manager tree resolves the same values). Every reader
takes an optional ``environ``, and an explicit one stands in for the process ENVIRONMENT only: the env
file consulted after it is still the process's (``ROMP_SERVICE_ENV_FILE``, else the default under
``XDG_CONFIG_HOME`` or ``HOME``, keysource's rule).
...
The command runs as ``/bin/sh -c <command> sh <selector>``, so the selector is ``$1``, with the
kernel's own environment as its environment: nothing is claimed or scrubbed for a command, so an
operator command that itself runs ``op`` with a credential in the service environment works.
...
are dropped (romp owns them), and so are the names the CLI reads as its own authentication or endpoint
...
the one door this module keeps for the key; each drop is one problem line naming the names. The
parsing rule is keysource's, generalised to every name: blank and ``#`` lines skipped, ``NAME=value``
(an ``export`` prefix is accepted here, romp's own contract for the command's output, not the env
file's), last assignment wins, one layer of matching quotes stripped; lines are split on newlines only,
and a value with a NUL is a bad line, never injected.
...
* **Values never land in ``os.environ``.** They live in one private dict here and leave it through
  the value-bearing accessors alone (``injection``, ``take``, ``resolve_key``), whose callers merge
  them into a CHILD's environment or hand the one key to the one operation that asked.
* **Values never reach a log, a screen or a wire.** Every other function returns names, counts,
  ...
  The command's text is rendered by its fingerprint (keysource's ``command:`` rule), never quoted.
* **A failed run keeps the previous set, and is not served like a good one.** A successful run is
  ...
  authentication failure in a judge call or a session), until the selector file's own stat identity
  ...
  per session), and a caller arriving mid-run takes that run's result, good or bad, rather than
  running again. Every record carries ``attempt``, the ordinal of the run or refusal that produced it
  (the callers that coalesced on one run share it), so a reader that holds two records knows which
  is the newer.
...
# The key source's module, loaded the way sdk_backend loads it (one module object under one name), for
# the env-file path both share, the KeySource shape and the fingerprint rule. Its own parser is
# key-specific; the generalised rule below is pinned equal to it by tests/test_envsource.py.
_HERE = Path(__file__).resolve().parent
_keysrc = sys.modules.get("romp_keysource") or _SFL("romp_keysource", str(_HERE / "keysource.py")).load_module()

COMMAND_VAR = _keysrc.CMD_VAR          # keysource's to select from; named here for the callers that report it
...
def parse_lines(text, allow_export: bool = False) -> tuple[dict, int, int]:
    ...
    Lines are split on newlines only (CRLF normalised first), never with str.splitlines(), which also
...
# ---------------------------------------------------------------------------
# The source: keysource's KeySource, read here and decided nowhere here.
# ---------------------------------------------------------------------------

def _is_command(source) -> bool:
    """Whether `source` is a command kind: the one kind this module runs. None and every other kind
    (a static key, the reference, an error, nothing) are "no command source"."""
    return source is not None and getattr(source, "kind", "") == "command"


def _source_fp(source) -> str:
    """The source's configuration identity (keysource's `command:` rule), "" for no command source.
    Part of the cache identity, and the record's only rendering of the command."""
    return source.fingerprint() if _is_command(source) else ""
...
def run_command(cmd: str, selector=None, timeout=None, env=None, grace=None) -> RunResult:
    ...
    `env` None is the kernel's own environment, the command source's contract (nothing is claimed
    or scrubbed for a command).
...
def _empty_snapshot(configured: bool, source_fp: str) -> dict:
    return {"configured": configured, "sourceFp": source_fp, "ok": None,
            "reason": "" if configured else "no %s" % COMMAND_VAR,
...
def _run_locked(source, environ) -> None:
    ...
    snap = _empty_snapshot(configured, source_fp)
    if _snap is not None and _snap.get("sourceFp") != source_fp:
        _set_values({})                           # keep-last-good is per source
    if not configured:
        _set_values({})
        snap["generation"] = gen
    ...
    reason = ""
    try:
        source.validate()                         # keysource's shape rule for the command text: one line, bounded
    except _keysrc.KeySourceError as e:
        reason = str(e)                           # names the variable, never the text
    if not reason:
        reason = sel_err
...
def _ensure_locked(source, environ, retry_failed: bool = True) -> None:
    """Under _lock: make the record current for `source`. A run happens when there is no record, when
...
def helper_fingerprint(config_dir=None, environ=None, timeout=None, values=None, generation=None,
                       set_seq=None, source=None) -> tuple:
    ...
    is a run); None reads the current set for `source` (the KeySource; None is no command source, an
    empty overlay). `generation` is the generation those values were served under (take()'s record
    carries it): the run is FOR that generation and is stored under it, so a set taken before an
    invalidate() yields an entry that is already stale, and the next caller runs the helper again
    on the current set. Before this the generation was read here, after the overlay was built, so
    an invalidate() between a connect's take() and this call stored the pre-invalidation overlay's
    fingerprint under the post-invalidation generation, and every caller after was served it as
    current. With `values` given and no generation, the current one (the caller did not say); with
    `values` None, the generation of the set read here.
...
def _reset() -> None:
    """Tests only: forget everything, including the counters, except the record ordinal (_attempts),
    which moves forward like the generation does: a reader that orders records by `attempt` (the
    backend's noter) may outlive a reset, and a rewound ordinal would have it take every record after
    the reset for an older one."""
    global _gen, _values, _snap, _snap_selector_ident, _runs, _failures, _last_ok_at, _FILE_CFG
