...
def _asm_stat(key, n=1):
    ...
    here, so a judge parse and the pusher's parse never lose each other's increment (T398 follow-up, low 3). Returns
    the new value, so a caller acting on a count milestone (_assemble's fallback warning) reads the locked increment
    and never a second, unlocked read of the dict (the fork's 2026-09-06 free-threading review)."""
    with _ASM_CKPT_LOCK:
...
#                                       in a cycle with its index, may go unqueued and has nothing to remove); its callback takes no lock,
#                                       since it can fire at any decref, under this lock included, and the lock is not reentrant; the next
#                                       registration or release drains the queue under the lock (_mat_drain). The cap is the backstop,
#                                       counted in `evictions` for a live entry (a dead entry it drops counts `expired`): on that machine
#                                       the live entries are at most 1,005,102 (27 live indexes at the peak, times the largest document's
#                                       37,226 rows), so the cap sits about 7.7 times above them; a cap under the live entries rebuilds on
#                                       every pass (above).
...
#                                       _ListRef) whose reference was still reachable when the list died, which includes every list holding
#                                       an LRU entry (its entries hold the reference in _MAT_LRU); a list with no entry, freed in a cycle
#                                       with its index, may go unqueued and has nothing to remove. Queued by _mat_collected and drained
#                                       under _MAT_LOCK before each registration and at each release (_mat_drain)
...
class LazyIndex:
    ...
    the collection event 2026-09-24, see _MAT_LRU)."""
    ...
    def release(self):
        ...
        with _MAT_LOCK:
            ...
            for ref in self._minted:
                la = ref()
                if la is None:
                    continue                              # a collected list: the drain above took its entries (or the next one
                    #                                       will, if its callback has yet to queue it)
                live.append(ref)
...
class LazyAtoms(list):
    ...
    the list's collection takes whatever entries it still has (see _ListRef). The list's length is fixed at mint: every
    in-place list method is refused, since the collection event visits rows 0 to n - 1 with n taken here."""
    ...
    def _at(self, i):
        ...
        if a is not _UNMAT:
            with _MAT_LOCK:
                ...
                if ent is not None and ent[0]() is self:  # this list's own entry: the LRU touch
                    ...
                    _mat_drain()                                #  registered now, after the freed lists' entries leave; an eviction
                    _mat_register(self, i)                      #  or release between the read above and this lock left the slot
                    _mat_trim()                                 #  unbuilt, and then `a` is the caller's value
            return a
        ...
        with _MAT_LOCK:
            ...
            _mat_drain()                                  # the freed lists' entries leave before a list can register under a freed id
...
def _entry_stat(e, **kw):
    """The kernel's _entry_stat twin (judge.py and sdk_backend.py carry the same body): a scandir entry's stat, counted
    ...
    no wrapper). The kernel's source pin derives every scandir entry name in kernel/ and holds its .stat() to the
    kernel's helper and the three twins."""
