...
# instead of skipping. Before node --test this script checks the roster file alone, and every red names the line (or the
# file) and what to do:
...
# THE ROSTER RULE IS CHECKED BY THE PR'S REVIEWER, NOT BY THIS SCRIPT. A rostered leg launches its browser through inBrowser
# alone, with no playwright load, launch, skip or todo of its own, in Chromium, the one engine the job installs: only
# inBrowser reads the switch, so a launch or a skip of the leg's own stands outside it. Nothing here reads a leg's source,
# so the reviewer of the PR that adds a line checks the rule, and the step reads these four cases green: a rostered leg that
# launches its own browser and swallows a failed launch without skipping; a rostered module that launches nothing; a leg
# that drives a browser from a child process and tolerates the child's failure; a todo test that passes beside a real pass.
# tools/ci-browser-legs.test.mjs runs a synthetic leg of each case and reads it green.
# Nothing checks that every browser leg in the tree is rostered, and main has no such check: a leg with no line runs only
# under the Test step, before the job installs a browser. tools/ci-browser-legs.test.mjs (CI's Shell job, no node_modules)
# runs this script over synthetic trees, with a stub node on PATH that records the node --test call and writes the record
# a case hands it, and with the real node and the real reporter. `--check` runs the pre-run checks alone and starts no
# node --test (it does not check that the bundles are built, which the step's run does).
