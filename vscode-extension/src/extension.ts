...
// 2026-07-14, who wanted the button the web view has — the browser's drift banner just reloads, but
// VS Code loads bundled code from the on-disk VSIX, so a reload changes nothing; the fix is the SAME
// vscode-extension/install.sh a user would run by hand). We run it from the extension HOST, not the
// kernel: the host carries VS Code's resolved shell environment (the reliable PATH with
// node/npm/npx/code), whereas a launchd/manager-spawned kernel often does not.
//
