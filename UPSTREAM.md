...
| Fast mode for SDK sessions (the `fastMode` flag-settings opt-in, a per-session chat chip, the timeline lane's star, and the cost-view caveat) | PRs #12 (`900a37ce`) + #14 (`26921e7a`, `08d80126`) + #18 (`890be196`), on fork main | candidate | Upstream's SDK sessions hit the same refusal: the CLI declines fast mode to any non-interactive client that has not opted in, so `/fast` is unavailable there too. The fix is the CLI's own documented settings key, not a workaround. Offer all three TOGETHER: #14's star reads the `_fast_word` #12 adds, and #18 documents a gap that only EXISTS once fast mode does (upstream's cost view prices session tokens from the same per-model table, which fast mode is invisible to). |
