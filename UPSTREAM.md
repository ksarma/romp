...
| Fast mode for SDK sessions (the `fastMode` flag-settings opt-in, a per-session chat chip, and the timeline lane's star) | PRs #12 (`900a37ce`) + #14 (`26921e7a`, `08d80126`), on fork main | candidate | Upstream's SDK sessions hit the same refusal: the CLI declines fast mode to any non-interactive client that has not opted in, so `/fast` is unavailable there too. The fix is the CLI's own documented settings key, not a workaround. Offer the two PRs TOGETHER — #14's star reads the same `_fast_word` #12 adds, so it does not stand alone. |
