#!/usr/bin/env python3
"""File comments (plans/file-review.md, Slice 1) — end to end, with nothing stubbed below the wire.
...
spawns a PreToolUse hook (its JSON on stdin, the session's environment). Only what needs a live
session is scripted: `_send_or_park` records the message instead of injecting it, and the names
registry is a lambda that knows one session, `web`.
...
class World:
    ...
    dispatcher answers a fake client whose send() flips an Event."""
    ...
    def ws(self, msg, wait=True):
        """One frame through the real dispatcher; the file-comments ops answer from their own thread."""
        self.rid += 1
    ...
    def fence_of(self, status):
        return {"storeMtimeNs": status["storeMtimeNs"] or "", "configMtimeNs": status["configMtimeNs"] or ""}
