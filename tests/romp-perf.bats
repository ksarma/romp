...
# flag is refused. Nothing here touches a real kernel: curl is a stub that serves two synthetic
# snapshots in turn.
...
setup() {
    ...
    # Two snapshots ten seconds apart: 20 cycles, 60 wakes, 6 s of cycle time (4 s of it in push, 3 s of
    # that in the chat block), 2 chat rebuilds against 18 cache hits, 1 MB sent as chat full frames, 100
    # goal loads, 2 judge passes, 5 /tick requests, 0.5 s of CPU.
    cat > "$SNAP_A" <<'JSON'
{"now": 1000.0, "since": 900.0, "uptime_s": 100.0, "log": false,
 ...
 "pusher": {"cycles": 100, "wakes": 300, "wakes_event": 250, "wakes_backstop": 50, "cycle_ms_sum": 30000.0,
            "cycle_ms_max": 900.0, "cycle_ms_last": 200.0, "cycle_ms_p50": 180.0, "cycle_ms_p90": 400.0, "ring_n": 100},
 ...
 "judge": {"passes": 30, "ms_sum": 30000.0, "ms_last": 1000.0, "ms_mean": 1000.0},
 "http": {"/tick": {"count": 50, "ms": 25.0}, "/sessions": {"count": 5, "ms": 10.0}}}
JSON
    cat > "$SNAP_B" <<'JSON'
{"now": 1010.0, "since": 900.0, "uptime_s": 110.0, "log": false,
 ...
 "pusher": {"cycles": 120, "wakes": 360, "wakes_event": 300, "wakes_backstop": 60, "cycle_ms_sum": 36000.0,
            "cycle_ms_max": 900.0, "cycle_ms_last": 250.0, "cycle_ms_p50": 190.0, "cycle_ms_p90": 420.0, "ring_n": 120},
 ...
 "judge": {"passes": 32, "ms_sum": 32400.0, "ms_last": 1200.0, "ms_mean": 1012.5},
 "http": {"/tick": {"count": 55, "ms": 27.5}, "/sessions": {"count": 5, "ms": 10.0}}}
JSON
    # Stub curl: records argv AND stdin (the auth header rides stdin as a curl config). A POST answers
    # the toggle's ack; a GET serves snapshot A first, then B, so two reads see counters move.
    cat > "$MOCK/curl" <<'MOCK'
#!/usr/bin/env bash
echo "$*" >> "$CURL_LOG"
...
[ -n "${CURL_FAIL:-}" ] && exit 22
if [[ "$*" == *"-X POST"* ]]; then
    printf '{"ok": true, "log": %s}\n' "$([[ "$*" == *'"log": true'* ]] && echo true || echo false)"
    exit 0
fi
n=0; [ -f "$CURL_CALLS" ] && n="$(cat "$CURL_CALLS")"
echo $((n + 1)) > "$CURL_CALLS"
if [ "$n" -eq 0 ]; then cat "$SNAP_A"; else cat "$SNAP_B"; fi
MOCK
    chmod +x "$MOCK/curl"
...
@test "romp perf: prints rates computed from two snapshots" {
    ...
    [[ "$output" == *"cpu 5.0% of one core"* ]]          # 0.5 s of CPU in 10 s
    [[ "$output" == *"p50 190"* ]]
    [[ "$output" == *"rss 410 MB"* ]]
    [[ "$output" == *"pid 4242"* ]]
}
...
@test "romp perf: builds, sends, goals, judge and http lines carry the deltas" {
    run "$ROMP_SCRIPT" perf --interval 0
    [ "$status" -eq 0 ]
    [[ "$output" == *"chat 2 built / 18 cached (40 ms avg)"* ]]
    [[ "$output" == *"full 102 KB/s (chat 2)"* ]]        # 1048576 bytes over 10 s
    [[ "$output" == *"deduped 176 KB/s (feed 18)"* ]]
    [[ "$output" == *"10.0 loads/s   2.0 saves/s   0.5 writes/s"* ]]
    [[ "$output" == *"2 passes (0.20/s)   last 1200 ms"* ]]
    [[ "$output" == *"/tick 5 (0.5 ms avg)"* ]]
...
@test "romp perf log on|off: POSTs the toggle to /perf" {
