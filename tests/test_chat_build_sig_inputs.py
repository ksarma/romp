...
CENSUS = {
    ...
    "_reconnecting_event": ("pure", "over the live row's effortPending, fastPending, modePending, modeSwitching, effort and pickHeld (row)"),
...
class Census(unittest.TestCase):
    ...
    def test_every_kernel_call_hands_the_liveness_map_as_live_map(self):
        ...
        for name in ("kernel.py", "sdk_backend.py"):
            src = Path(os.path.join(os.path.dirname(HERE), "kernel", name)).read_text()
            for m in re.finditer(r"(?<![\w.])_chat_build_sig\(", src):
...
class Differential(_World):
    ...
    def test_a_user_todo_write_misses_under_todos_and_a_peers_write_does_not(self):
        # a user todo registered, answered or withdrawn changes the split card and the payload's userTodos field
        # with no transcript write; the fold is per sid (_user_todo_fp), so a peer's write moves nothing here
        a = self.sig()
        km._add_user_todo(SID, "Need the auth-scheme decision to wire login")
        b = self.sig()
        self.assertEqual(self.moved(a, b), ("todos",), "a todo of this session is a component of its own")
        self.assertEqual(self.sig(), b, "byte-stable while the rows stand")
        km._add_user_todo(PEER, "Need a test credential for the api session")
    ...
    def test_the_live_task_rows_miss_under_bg_as_the_builds_own_filter_returns_them(self):
        ...
        km._bg_live_norm = lambda sid, path, **kw: list(rows)   # the kernel hands the row as live= (C4)
        try:
            a = self.sig()
