...
class TheCountersOneSite(unittest.TestCase):
    ...
    def test_the_walk_has_one_shared_load_site_and_the_counter_is_bumped_beside_it(self):
        ...
        text as importlib.util.decode_source gives it differs from that read (a file whose first line opens with a lone CR above a
        utf-7 declaration passes that check: the module docstring's limit, under the precondition of an honest source), and each whose text
        ...
        # the refusal, over every file the glob matches and before the population filter below: the file's text as
        # importlib.util.decode_source gives it must equal the UTF-8 read the census parses, the module docstring stating the lone-CR
        # limit (review round 10, extra5-1: in a utf-7-declared file, a counter write the interpreter runs was a comment to the UTF-8
        # read)
        undecoded = []
        for p in files:
            try:
        ...
        self.assertEqual(undecoded, [], "every file the glob kernel/*.py matches reads the same by importlib.util.decode_source over "
                                        "its bytes (which honours a PEP 263 declaration it finds on the first two lines split at LF; "
                                        "the module docstring states the lone-CR limit) as by the UTF-8 read this census parses, "
                                        "checked before the population filter: a file whose decoded text differs, or whose decode "
                                        "raises, is refused, since the census would parse text other than decode_source's (a utf-7 "
                                        "declaration can turn a line the UTF-8 read parses as a comment into a write of the counter): "
                                        "%s" % ", ".join(undecoded))
