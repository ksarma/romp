...
# No test may read the REAL service unit, launchd plist or Claude Code settings: constructing the SDK
# backend takes the key-source verdict (sdk_backend.key_source_verdict), which reads the systemd unit and
# its drop-ins, the launchd plist and $CLAUDE_CONFIG_DIR/settings.json, so every backend construction in
# the suite would otherwise read this machine's unit and the developer's own apiKeyHelper setting (and
# envsource.helper_fingerprint could RUN that helper). The three are floored to empty temp dirs under the
...
# ROMP_TESTS_REAL_CLAUDE_CONFIG_DIR, set here once. Import time plus a per-test re-assert, on the same
# reasoning as the manager-port floor; a test that needs its own dirs points the variables at temp paths in
# setUp, which runs after the fixture.
_EMPTY_DIRS = {
