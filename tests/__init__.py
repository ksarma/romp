...
# conftest.py's each minted a state root per process and never removed it (nine per `pytest -n 8`
# run), and about three hundred test modules call tempfile.mkdtemp with no cleanup of their own. On a
# machine where many sessions run the suite all day that left about 1.4 million directories under
# /tmp, enough that listing /tmp took minutes and tens of gigabytes of memory, and the out-of-memory
# kills that followed took a live kernel down. The fix is at the source: every tempfile.mkdtemp call
# made in this process is recorded, and everything recorded is removed at interpreter exit (and, under
# pytest, at session end; see conftest.py). Only directories this process created, and only under the
# system temp directory, are ever removed. tempfile.TemporaryDirectory goes through the same mkdtemp
# and cleans itself first, so the exit sweep finds nothing of it. This package imports before
# conftest.py (pytest imports `tests.conftest` through the package), so conftest's own root is
# recorded too.
_MADE_DIRS = []
