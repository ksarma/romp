...
const { writeGate, serveToken } = require(MGR);
...
async function manager({ token } = {}) {
  ...
  const h = { dir, state, port, mgr, log: '', pids: new Set() };   // pids: every kernel the manager reported, reaped at cleanup
  ...
  h.kernel = async () => {
    ...
    const k = (st.kernels || [])[0] || {};
    if (k.pid) h.pids.add(k.pid);
    return k;
  };
  for (let i = 0; i < 100; i++) {                       // up: /status answers and names a kernel
    try { if ((await h.kernel()).pid) break; } catch (e) { /* not yet */ }
    await h.sleep(50);
  }
  h.cleanup = async () => {
...
test('writeGate: no token to compare against is 503, a missing or wrong header is 401, a match is ok, and no answer carries a token', () => {
  const have = { token: TOKEN };
  ...
  const missing = writeGate(TOKEN, { token: '', why: '/x/state/serve-token: ENOENT' });
  ...
  assert.equal(writeGate(TOKEN, { token: '' }).code, 503);
...
test('no token file: writes are refused with 503 (never open) while reads work; the file appearing opens the doors with no manager restart', async () => {
...
// The control client is how `romp refresh` (restart-all), `romp down` (down) and the dashboard's single
// restart reach the doors: it reads the same file and sends the header; the read verb sends none. The
