...
class KeySourceBlock(unittest.TestCase):
    """The `keySource` block beside `cliScope`: the boot verdict's facts plus what is live now. Additive
    (API_HEALTH_SCHEMA stays 1) and value-free: fingerprints, names, reasons with counts. The default (the
    file kind, no command, no key) is what every other test here sees."""
    ...
    def test_the_block_is_present_with_the_documented_fields(self):
        ...
        self.assertIsNone(ks["lastRun"], "the file kind: no command ran")
