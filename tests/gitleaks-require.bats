...
# The suite's case count, from its own text, so the expectations below move with it (fourteen at
# this writing). A count of zero would make every assertion below vacuous, so it is refused.
suite_cases() {
    local n
