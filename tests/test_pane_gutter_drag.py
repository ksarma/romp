...
The dashboard's chat | Outline | feed | Waiting | Files panes are sized by flex-grow weights the gutter script
(_LANDING_JS in kernel/kernel.py) writes as --g-* variables on .row. Each write re-lays out the row, and with
it every same-origin pane document in that frame. Writing the pair's two grows on every mousemove made a drag
cost one full relayout of every pane per pointer step, which is a freeze once any pane holds a large document.
...
on the Files gutter (gv-d), whose left pane is the rightmost shown of the Waiting pane, feed, the Outline
and chat (the Waiting pane stays hidden here, so gv-c, the Waiting gutter, is never driven):
  ...
  Files gutter pairs the Files pane with the Waiting pane, else the feed, else the Outline, else the chat, by
  what is shown (the Waiting pane is hidden throughout, so the three drives exercise the last three).
...
// after the 7 px gutter: the divider sits at x = 600; the Files pane hidden until the last drags; the Waiting
// pane (between feed and Files) hidden throughout (BODY carries no po-waiting), so gv-c, its gutter, is never
// driven here
EL['chat-pane'] = mkEl('chat-pane', 600, 0, 'flex');
EL['fleet-pane'] = mkEl('fleet-pane', 300, 0, 'none');
EL['feed-pane'] = mkEl('feed-pane', 400, 607, 'flex');
EL['waiting-pane'] = mkEl('waiting-pane', 300, 0, 'none');
...
function layout(widths) {
  let x = 0;
  [['chat', 'chat-pane'], ['fleet', 'fleet-pane'], ['feed', 'feed-pane'], ['waiting', 'waiting-pane'],
   ['files', 'files-pane']].forEach(([k, id]) => {
...
// 5) the Files pane shown: gv-d's left pane is the rightmost SHOWN of the Waiting pane, feed, the Outline and
//    chat; the Waiting pane stays hidden, so the chain is exercised from feed down. All four shown first
//    (chat 300 | Outline 100 | feed 300 | files 200, the divider at 714), then the feed hidden
...
EL['gv-d'].fire('mousedown', { preventDefault() {}, clientX: 714 });
out.grab5 = snap();
...
EL['gv-d'].fire('mousedown', { preventDefault() {}, clientX: 557 });
out.grab6 = snap();
...
EL['gv-d'].fire('mousedown', { preventDefault() {}, clientX: 500 });
out.grab7 = snap();
...
class PaneGutterDragExecutes(unittest.TestCase):
    ...
    def test_the_grab_normalises_the_shown_panes_and_shows_the_ghost_at_the_divider(self):
        g = self.out["grab"]
        # the two shown panes are written as their px widths (the hidden Outline, Waiting and Files panes keep
        # their grows)
        self.assertEqual(g["writes"], self.out["boot"]["writes"] + 2)
        self.assertEqual(g["grows"], {"--g-chat": 600, "--g-fleet": 34, "--g-feed": 400, "--g-waiting": 34, "--g-files": 40})
    ...
    def test_the_release_writes_the_pair_once_hides_the_ghost_and_persists(self):
        ...
        self.assertEqual(r["grows"], {"--g-chat": 500, "--g-fleet": 34, "--g-feed": 500, "--g-waiting": 34, "--g-files": 40})
        ...
        self.assertEqual(r["store"], {"chat": 500, "fleet": 34, "feed": 500, "waiting": 34, "files": 40}, "the grows persist at release")
    ...
    def test_a_far_drag_clamps_the_ghost_and_the_release_at_the_pair_minimum(self):
        ...
        self.assertEqual(g2["grows"], {"--g-chat": 500, "--g-fleet": 34, "--g-feed": 500, "--g-waiting": 34, "--g-files": 40})
        ...
        self.assertEqual(r2["grows"], {"--g-chat": 880, "--g-fleet": 34, "--g-feed": 120, "--g-waiting": 34, "--g-files": 40})
        self.assertEqual(r2["store"], {"chat": 880, "fleet": 34, "feed": 120, "waiting": 34, "files": 40})
    ...
    def test_with_the_outline_pane_shown_the_line_is_placed_from_its_client_left(self):
        ...
        self.assertEqual(g3["grows"], {"--g-chat": 300, "--g-fleet": 200, "--g-feed": 200, "--g-waiting": 34, "--g-files": 40})
    ...
    def test_a_narrow_pair_clamps_at_a_quarter_of_the_pair(self):
        ...
        self.assertEqual(r3["grows"], {"--g-chat": 300, "--g-fleet": 100, "--g-feed": 300, "--g-waiting": 34, "--g-files": 40}, "the chat grow is untouched")
        self.assertEqual(r3["store"], {"chat": 300, "fleet": 100, "feed": 300, "waiting": 34, "files": 40})
    ...
    def test_the_files_gutter_pairs_the_files_pane_with_the_rightmost_shown_column_to_its_left(self):
        ...
        # what is shown at grab time: feed | files, then Outline | files, then chat | files (the Waiting pane heads
        # gv-d's chain but is hidden throughout, so it is never the left pane and never normalised)
        ...
        self.assertEqual(g5["grows"], {"--g-chat": 300, "--g-fleet": 100, "--g-feed": 300, "--g-waiting": 34, "--g-files": 200})
        ...
        self.assertEqual(g6["grows"], {"--g-chat": 300, "--g-fleet": 250, "--g-feed": 250, "--g-waiting": 34, "--g-files": 250}, "the hidden feed keeps its grow")
        ...
        self.assertEqual(r7["store"], {"chat": 450, "fleet": 200, "feed": 250, "waiting": 34, "files": 350}, "the grows persist at release")
