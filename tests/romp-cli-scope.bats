...
# The line is the `refused:` form, not `fallback:`: no CLI ran, so the kernel logs and counts no
# fallback for it and leaves it to the launch-error card (tests/test_cli_scope.py FallbackNotice).
