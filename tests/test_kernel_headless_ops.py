...
class _RouteServer(unittest.TestCase):
    ...
    def setUp(self):
        # (Until the tmux backend's removal, 2026-09-11 upstream and the 2026-09-15 pull-in here, the routes' 404
        # path asked a tmux probe whether the live scan answered, and this fixture switched the probe off per test.
        # The live map is the SDK registry and the Codex backend now; a test that needs a live generation of a name
        # patches km._codex with _codex_owning or km._sdk with _sdk_reporting.)
...
class HeadlessRoutes(_RouteServer):
    ...
    def test_interrupt_route_paints_nothing_when_the_backend_refuses(self):
        ...
        km._interrupt_clicked.pop("runaway", None)        # the sibling test above stamped it: this one reads a fresh chip
        with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda sid: fake)):
            code, resp = self._post("/interrupt", {"name": "runaway"})   # KNOWN_SIDS' one uniquely named session (this fork's by-name gate refuses "web", which two sids carry)
    ...
    def test_interrupt_route_says_when_the_stop_did_not_land(self):
        ...
        km._interrupt_clicked.pop("runaway", None)
        with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda sid: fake)):
            code, resp = self._post("/interrupt", {"name": "runaway"})   # KNOWN_SIDS' one uniquely named session (see above)
        self.assertEqual(code, 200)
        self.assertIs(resp.get("ok"), False, "a dropped stop is never answered ok")
        self.assertEqual(resp.get("error"), "the stop was not delivered: runaway is still working")
        sid = fake.interrupt.call_args[0][0]
...
def _reg_flipping_kill(tsid):
    ...
    (the record the end-on-idle sweep reads for a thread) before the CLI is shut down."""
...
class UnknownSessionRefused(_RouteServer):
    ...
    "killed" it, the death corroboration of the time found nothing listed and certified the death, and
    `romp end <typo>` printed a bare ok while the real session kept running; /send handed the phantom to
    the fall-through backend of the time, whose refusal the route folded into ok:true. A caller that trusted those oks had to
    ...
    and so is a comment threads' store that will not read. The gate is asked with the live map _resolve_sid
    already read, so the refusal path reads it once."""
    ...
    def test_a_comment_thread_passes_the_gate_by_name_and_by_id(self):
        ...
        try:
            with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda sid: fake)), \
                ...
                # (the kill path every ended session takes): the death record, _comment_kill_all and the closed frame
    ...
    def test_a_threads_deferred_end_is_served_by_the_sweep_at_its_settle(self):
        ...
        # design), so the sweep took its absent branch, where the death corroboration of the time certified
        # the thread dead by the bare existence of its reg, and the wish was spent with no kill: the caller
        # was told the thread was closing while it kept running (review round 2, 2026-09-09). The sweep now
        # reads the reg's alive+threadOf as the owner's answer, waits on the thread's own transcript through
        # its reg (discovery lists no threads, so _path_of is None for one), and at the settle runs the one
        # kill path the immediate arm runs (upstream's, since the 2026-09-15 pull-in): the kill, the death
        # record, _comment_kill_all and the closed frame, so the deferred and the immediate `romp end self` leave
        ...
        try:
            with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda sid: fake)), \
                ...
                self.assertFalse(json.loads(reg_path.read_text()).get("alive"), "the flipped reg is the owner's answer")
    ...
    def test_both_end_doors_leave_a_thread_in_byte_identical_state(self):
        ...
        # (the immediate /end) from inside a comment thread run ONE kill path (upstream's: the kill, the death
        # record, the threads' teardown, the closed frame), so the
        ...
        # _record_death and _comment_kill_all run; only the clock is pinned, so both doors stamp the same second.
        fake = mock.Mock()
        fake.busy.return_value = None
        fake.kill.side_effect = _reg_flipping_kill(THREAD_TSID)
        frames = []
    ...
    def test_a_send_the_backend_refuses_is_ok_false_not_an_ok(self):
        ...
        # received (review round 3, 2026-09-09). The refusal is upstream's shape since the 2026-09-15 pull-in
        # (this fork's 409 retired for it): a 200 whose body says ok:false and names the caller's spelling and
        # the undelivered message, so `romp send` prints it and exits non-zero; nothing is parked. The gate
        # still admits the sid: it is known, it is just not running.
        fake = mock.Mock()
        fake.busy.return_value = None
        ...
        try:
            with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda sid: fake)), \
                ...
                for who in ("sid-q", THREAD_NAME, THREAD_TSID):
                    ...
                    self.assertNotIn("queued", resp, who)
    ...
    def test_a_far_kernels_answer_is_relayed_in_its_own_words(self):
        ...
        # a far kernel of an older build answered /send's backend refusal with a 409 that already ends in
        # "the message was not delivered" (this build answers it 200 ok:false with the same sentence, the
        # 2026-09-15 pull-in); the arm's own coda doubled it in the relayed body (review round 4, 2026-09-09).
        # The far words come through verbatim, whatever the status, the sentence once
    ...
    def test_the_ws_drive_gate_and_the_http_gate_give_one_verdict_per_state(self):
        ...
        # state dir, a stubbed Codex backend; a stubbed tmux server until that backend's removal), never a
        # hand-built map. A hand-built row stood for "running",
        ...
        # through to the fall-through backend. Running, for a torn record, means the SDK backend runs the session
        # now (running_sids): an SDK row in the map is a reg that says alive, never liveness, and another backend's
        # row carrying the sid is no writer for the reg, so it is the record's verdict too (round 9; rounds 6 to 8 admitted it, an answer that
        # depended on the list_regs cache, since a warm cache serves the torn reg's last good row as an SDK row
        # that overwrote the pane's in the merge). A reg that is a JSON list is on disk before the SDK backend is
        # first built, so the build reads it. Since the 2026-09-15 pull-in the second backend is the Codex one
        # (_codex_owning): its rows stand where the stubbed panes stood.
        ...
        bad_live_cx, bad_live_sdk, bad_running = ("abab1111-2222-3333-4444-555555555555",
                                                    "baba1111-2222-3333-4444-555555555555",
                                                    "cdcd1111-2222-3333-4444-555555555555")
        sdir = km.jd.STATE / "sdk"
        sdir.mkdir(parents=True, exist_ok=True)
        regs = [sdir / (s + ".json") for s in (dead, bad_thread, bad_named, bad_list, bad_live_cx, bad_live_sdk,
                                                bad_running)]
        (sdir / (dead + ".json")).write_text(json.dumps({"sid": dead, "alive": False, "cwd": "/tmp"}))
        for sid in (bad_thread, bad_named, bad_live_cx, bad_running):
        ...
        _register(bad_live_cx, "live-bad-cx")
        ...
        states = (("no record", self.GHOST, "unknown", None),
                  ...
                  ("a Codex session carries the sid, unregistered", live, "admitted", None),
                  ...
                  ("names-registered, a reg that will not read, a Codex session carries the sid", bad_live_cx,
                   "unreadable", ("live-bad-cx", "unreadable")),
        ...
        try:
            fake.kill.side_effect = lambda sid: reached.append(("end", sid)) or True   # /end's kill path: be.kill, then the writers below
            with mock.patch.object(km, "_codex", lambda cx=_codex_owning([live, bad_live_cx]): cx), \
                 mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda sid: fake)), \
                ...
                km._thread_reg_failed.clear()
                scan = km.Sessions.live()
                self.assertEqual({s: r.get("backend") for s, r in scan.items() if s in (live, bad_live_cx, bad_live_sdk)},
                                 {live: "codex", bad_live_cx: "codex", bad_live_sdk: "sdk"},
                                 "the real scan: the stubbed Codex rows on codex, the cached last good row on sdk")
            ...
            _drop_regs(regs)
            _rm_thread(THREAD_PARENT, THREAD_TSID)
            for sid in (THREAD_PARENT, named, bad_named, bad_live_cx, bad_live_sdk, bad_running):
                _unregister(sid)
            km._thread_reg_memo.clear()
            km._thread_reg_failed.clear()
    ...
    def test_a_dormant_sessions_record_that_breaks_while_the_kernel_runs_is_unreadable_at_both_doors(self):
        ...
        # fell to the fall-through backend of the time, whose send went to a target that did not exist with no
        # modal, no undelivered.jsonl row and no log line, and the end corroboration of the time promised "try
        # again". The 503 the tests modelled appeared only with a cold cache (after a kernel restart). Round 6's
        # rule was that running means the backend runs the session now, never an SDK row in the map; since round
        # 9 it is running_sids alone, the SDK backend's own set (review rounds 6 and 9, 2026-09-09). Through the
        # REAL Sessions.live() (the SDK half; no Codex session here), by id and by name, at both doors.
    ...
    def test_a_reg_that_is_a_json_list_hides_no_other_sdk_session(self):
        ...
        try:
            ...
            with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda s: fake)), \
                 mock.patch.object(km, "_send_or_park", lambda be, sid, text, **k: False), \
                ...
                self.assertEqual((code, resp), (200, {"ok": True, "queued": False}))   # False: handed over now (upstream's contract)
    ...
    def test_a_record_that_will_not_read_is_a_503_naming_the_read_not_a_404(self):
        ...
        # absent OR unreadable, while the death corroboration of the time on the same reg answered None,
        # "exists but would not read". A failed read is never reported as a session that does not exist (the fail-loudly rule):
    ...
    def test_a_torn_record_of_the_name_is_the_records_verdict_and_the_repaired_name_reaches_its_live_generation(self):
        # round 7 answered a failed scan before a torn dormant generation of the name and reached a live
        # generation of the name once the scan answered, so the by-name verdict for a torn record rode the probe
        # and, at a warm list_regs cache, the cache. The doors' resolution reads the on-disk names registry first
        # (round 10, 2026-09-09): a name registered to a torn record is the record's verdict, the text saying that
        # if a live session of the name runs it is reachable by it once no torn record bears the name (conditional
        # since round 11: with no namesake the repaired name is the dormant rule's 404, so round 10's "reachable by
        # that name again once the file is repaired or removed" promised what never held); with the file removed
        # the same name reaches its live generation, here a Codex session (the 2026-09-15 pull-in: the tmux pane
        # that stood for the live generation, and the probe states, left with that backend).
        ...
        try:
            with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda sid: fake)), \
                 mock.patch.object(km, "_push_soon", lambda *a, **k: None), \
                 mock.patch.object(km, "_codex", lambda cx=_codex_owning([]): cx):
                for path, body in (("/end", {"name": "web"}), ("/interrupt", {"name": "web"}),
                                   ("/send", {"name": "web", "text": "hello"})):
                    code, resp = self._post(path, body)
                    self.assertEqual(code, 503, (path, resp))
                    self.assertIn("could not read the record for 'web'", resp.get("error", ""), path)
                    self.assertIn(km._tilde(str(torn_path)), resp.get("error", ""), path)
                    self.assertIn(again, resp.get("error", ""), path)
                    self.assertNotIn("reachable by that name again", resp.get("error", ""),
                                     (path, "the unconditional promise is gone: no namesake runs here"))
                    self.assertNotIn("try again", resp.get("error", "").lower(), path)
                fake.interrupt.assert_not_called()
                fake.kill.assert_not_called()
                fake.send.assert_not_called()
            # the file removed: the name reaches the Codex generation registered under it (sid-x, KNOWN_SIDS)
            _drop_regs([torn_path])
            km._thread_reg_memo.clear()
            km._thread_reg_failed.clear()
            with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda sid: fake)), \
                 mock.patch.object(km, "_push_soon", lambda *a, **k: None), \
                 mock.patch.object(km, "_codex", lambda cx=_codex_owning(["sid-x"]): cx):
                code, resp = self._post("/interrupt", {"name": "web"})
            self.assertEqual((code, resp), (200, {"ok": True}), "the file removed: the live generation is the name's")
            fake.interrupt.assert_called_once_with("sid-x")
        finally:
            _drop_regs([torn_path])
            km._thread_reg_memo.clear()
    ...
    def test_the_ws_by_name_door_reads_a_torn_dormant_record_as_the_routes_do(self):
        # the WS compact and sendCommand door resolved its NAME through _sid_of, which never consulted the torn
        # dormant record, so it said "no session with id web" where the HTTP routes said 503 "could not read" for
        # the same name: a torn dormant names-registered reg the live map does not list (cold list_regs cache;
        # round 6 regressed the door, 478ccdc0 agreed with HTTP). One miss path then (_named_miss, round 7), so the
        # door refused naming the read, with the typed spelling in the modal, the undelivered.jsonl row and the
        # stderr cause; since round 10 the torn record is the resolution's (_resolve_sid's door read,
        # _unreadable_dormant_named, gated by _session_gate's unreadable arm), the order the body below drives; a
        # name nobody holds stays the unknown refusal (review rounds 7 and 10, 2026-09-09). The failed-scan half of
        # this test (a tmux probe that did not answer) left with that backend at the 2026-09-15 pull-in; a live
        # generation of the name is a Codex session here.
        torn_path = km.jd.STATE / "sdk" / ("sid-q" + ".json")
        torn_path.parent.mkdir(parents=True, exist_ok=True)
        undelivered = km.jd.STATE / "undelivered.jsonl"
        frames = []
        ...
        try:
            with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda sid: fake)), \
                 ...
                 mock.patch.object(km, "_send_or_park", lambda be, sid, text, **k: fake.send(sid, text)), \
                ...
                # the record repaired away: the store arm and the live generations below read a clean registry
                _drop_regs([torn_path])
                km._thread_reg_memo.clear()
                km._thread_reg_failed.clear()
                ...
                with mock.patch.object(km, "_thread_names", lambda: None):
                    ...
                    self.assertEqual(fake.method_calls, [], "nothing reached a backend")
                # a live Codex generation of the name: the op reaches the backend for its sid
                with mock.patch.object(km, "_codex", lambda cx=_codex_owning(["sid-x"]): cx):
                    errs, rows, log = drive("compact", "web")
                    self.assertEqual(errs, [], frames)
                    fake.compact.assert_called_once_with("sid-x")
                # no backend runs a session of the name: a registered name no session runs is unknown
                with mock.patch.object(km, "_codex", lambda cx=_codex_owning([]): cx):
                    errs, rows, log = drive("compact", "web")
                    self.assertEqual(len(errs), 1, frames)
    ...
    def test_a_store_resolved_name_with_no_record_takes_the_miss_path_at_both_doors(self):
        ...
        # sid as unknown, and the WS compact and sendCommand door, which took the miss path for a name handed
        # back unchanged only, refused it as a session that does not exist, by the thread's sid, where the routes
        # went on by the typed name. Both doors take the miss path on every unknown verdict for a typed name now:
        # with no backend running a session of the name, the unknown refusal at both (the routes' 404 by the typed
        # name, the WS door's by the resolved sid). The corollary: a torn names-registered generation of the same
        # name answers its record's 503 at the WS door as the routes do (review round 8, 2026-09-09). The failed
        # tmux probe's half of this test (the scan's 503 at both doors) left with that backend at the 2026-09-15
        # pull-in.
        _register(THREAD_PARENT, "web-parent")
        _mk_thread(THREAD_PARENT, THREAD_TSID, THREAD_NAME)
        ...
        try:
            ...
            with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda sid: fake)), \
                ...
                # no backend runs a session of the name: the unknown refusal at both doors
    ...
    def test_a_torn_record_at_a_cold_cache_is_the_records_verdict_by_id_and_by_name(self):
        # for a names-registered sid whose SDK reg is torn, at a cold list_regs cache (the map lists no row for
        # the sid), _session_gate answered the record's 503 (round 5); rounds 7 and 8 then made it the scan's
        # verdict there (try again) and admitted the sid the moment a pane carried it. Both read the live map,
        # whose answer for a torn reg depends on the cache, so the verdict flipped with the warmth of the cache and
        # not with the world. The gate reads no map for a torn record now: by id the record's verdict, the file
        # named, no "try again", at every door (round 9). By NAME the same since round 10: the doors' resolution
        # reads the torn record from the names registry ahead of any scan, and the text says that if a live session
        # of the name runs it is reachable by it once no torn record bears the name (review rounds 9 and 10,
        # 2026-09-09; rounds 7 and 8 pinned the flips this test denies). The tmux states the fork drove this in
        # (the probe down, the no-server exit, a pane carrying the sid) left with that backend at the 2026-09-15
        # pull-in; the cold cache is the one state left.
        sid, name = "abab7777-8888-9999-0000-111111111111", "torn-cold-web"
        reg_path = km.jd.STATE / "sdk" / (sid + ".json")
        reg_path.parent.mkdir(parents=True, exist_ok=True)
        _register(sid, name)
        reg_path.write_bytes(b"{not json")
    ...
    def test_a_torn_record_the_map_lists_is_the_records_verdict(self):
        ...
        # good row), but no probe answer could change that verdict: the SDK row wins the merge in Sessions.live()
        ...
        # map out of the gate's reading altogether: a torn record is the record's verdict at a warm cache (this
        # test) as at a cold one (the test above and the pair tests below). The warm state is built the real
        # way: a good reg, one Sessions.live() read, the file torn, the thread-reg memos cleared; _drop_regs
        # evicts the cached row at the end. (The tmux states the fork drove this in left with that backend at
        # the 2026-09-15 pull-in.)
    ...
    def test_a_torn_record_is_one_verdict_at_a_cold_and_a_warm_cache_at_the_routes(self):
        # one reg, torn, gave two verdicts by the warmth of the list_regs cache: cold (after a kernel restart, the
        # reg never read while readable) the map listed another backend's row for the sid and the gate admitted
        # it; warm (the reg read once, then torn) the cached last good row, an SDK row, won Sessions.live()'s merge
        # and the same request was the record's 503. A verdict must not depend on a cache: a torn reg is the
        # record's verdict at both warmths, at every route by id and by name (round 9 by id; round 10 by name, the
        # doors' resolution reading the names registry ahead of the map). A second SDK
        # generation of the name that the SDK backend RUNS is reached by name at both warmths,
        # the call carrying its sid, and so is the torn generation itself when it is the running one
        ...
        # Fails before on the cold side (review rounds 9 and 10, 2026-09-09), for the running generation by name
        # at round 10's head, and for the Codex and conserved generations by name at round 11's head. The tmux
        # pane blocks (a namesake pane, a pane carrying the torn sid) and the three probe states every block
        # ran in left with that backend at the 2026-09-15 pull-in.
        ...
        # the one backend state every by-name block runs in (the three tmux probe states left with that backend)
        backend_states = (("one backend set", contextlib.nullcontext),)
        ...
        def reached(label, running, live_by=None):
            ...
            # reaches it with the call carrying that sid, at a cold and a warm cache; by id the torn record keeps
            # the record's verdict unless it is the running one. /end's kill path is upstream's: be.kill (the
            # fake records it), then the death record and the threads' teardown (no-ops here)
            fake.kill.side_effect = lambda s: calls.append(("end", s)) or True
            for backend_label, backend_state in backend_states:
                with backend_state(), \
                     mock.patch.object(km, "_record_death", lambda s, t, kind: None), \
                     mock.patch.object(km, "_comment_kill_all", lambda s, be: None), \
                    ...
                    for warmth in ("cold", "warm"):
                        where = "%s, %s, %s" % (warmth, backend_label, label)
        ...
        try:
            with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda s: fake)), \
                ...
                # sid, at both warmths (round 11: every by-name
                ...
                # warmths (verdicts asserts the torn sid's path, the lower of the two; round 13)
                ...
                for backend_label, backend_state in backend_states:
                    with backend_state():
                        for cold_cache in (True, False):
                            where = "two torn generations, %s, %s" % ("cold" if cold_cache else "warm", backend_label)
                ...
                # and 404 by name, holds beside a torn record too). The walk reads no map, and the warm premise's
                # scan is the SDK half alone
    ...
    def test_a_torn_record_is_one_verdict_at_a_cold_and_a_warm_cache_at_the_ws_door(self):
        # the WS door's half of the pair above: interrupt and sendMessage by id and compact and sendCommand by
        # name through _drive, the err frame naming the record at both warmths; the frames' text, sid and copy
        ...
        # name. A second SDK generation of the name that the SDK backend RUNS is reached by name at both warmths,
        # the call carrying its sid, and so is the torn generation itself when it is the
        ...
        # sid answers (round 12; the routes pair test's header says why and how). Fails before on the cold side
        # (no err frame, fake.interrupt called) (review rounds 9 and 10, 2026-09-09), for the running
        # generation by name at round 10's head, and for the Codex and conserved generations at round 11's. The
        # tmux pane blocks and the probe states left with that backend at the 2026-09-15 pull-in.
        ...
        def reached(label, running, live_by=None):
            ...
            # carrying that sid, at a cold and a warm cache; the by-id ops on the torn record keep the record's
            # frame unless it is the running one
            for backend_label, backend_state in backend_states:
                with backend_state(), \
            ...
            with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda s: fake)), \
                ...
                for order in ([mate, sid], [sid, mate]):
                    with mock.patch.object(km, "_sdk", lambda be=_sdk_reporting(order): be):
                        ...
                        reached("two running generations, the backend listing %s first" % order[0][-4:], sid)
                self.assertEqual(fake.method_calls, [], "nothing but the lower running generation reached a backend")
                # two torn dormant generations: the frame names the lower sid and its file at both warmths (round 13)
                mate_path.write_bytes(b"{not json")
                _forget_regs([mate_path])
                km._thread_reg_memo.clear()
                km._thread_reg_failed.clear()
    ...
    def test_a_spelling_outside_the_name_alphabet_is_the_unknown_refusal_before_any_file_is_read(self):
        ...
        try:
            with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda s: fake)), \
                 ...
                 mock.patch.object(km, "_comment_kill_all", lambda sid, be: None), \
                 mock.patch.object(km, "_push_soon", lambda *a, **k: None):
                fake.kill.side_effect = lambda sid: ended.append(sid) or True
    ...
    @unittest.expectedFailure   # pull-in 2026-09-15: upstream's _REG_MISSING_ERRNOS lacks ENAMETOOLONG, so _thread_reg_read reads a
    #                             too-long spelling as "unreadable" and the gate answers 503 naming a file that cannot exist; the
    #                             fix is the kernel's tuple (and postal_service.py's twin, the parity test): code request PT-B ->
    #                             romp-upfold-1 in pullin2/agents/code-requests.md. Remove this mark when it lands.
    def test_a_name_too_long_for_the_filesystem_is_the_unknown_refusal_not_a_torn_record(self):
        ...
        try:
            with mock.patch.object(km.Sessions, "backend_for", staticmethod(lambda s: fake)), \
                 ...
                 mock.patch.object(km, "_comment_kill_all", lambda s, be: None), \
                 mock.patch.object(km, "_push_soon", lambda *a, **k: None):
                fake.kill.side_effect = lambda s: ended.append(s) or True
                for path, body in (("/end", {"id": who}), ("/end", {"name": who}), ("/end", {"name": who, "when": "idle"}),
                                   ("/interrupt", {"id": who}), ("/interrupt", {"name": who}),
                                   ("/send", {"id": who, "text": "hello"}), ("/send", {"name": who, "text": "hello"})):
                    err = io.StringIO()
    ...
    def test_named_miss_reads_a_spelling_outside_the_alphabet_as_no_local_name_as_a_backstop(self):
        ...
        # deleted it alone kept host:name off the scan-failed 503 of the time) and is pinned directly: four
        # spellings outside the alphabet answer None under an unreadable store, and the inside-alphabet control
        # is the store's verdict (review round 11, 2026-09-10; the scan-failed arm left with the tmux backend's
        # live-list probe at the 2026-09-15 pull-in, so the one arm is the store's).
        self.assertIsNone(km._named_miss("web/2", None, False))
        self.assertIsNone(km._named_miss("TESTHOST:web", None, False))
        self.assertIsNone(km._named_miss("../x", None, True))
        self.assertIsNone(km._named_miss("web 2", None, True))
        self.assertIsNone(km._named_miss("web", None, False), "a bare name with a readable store: nothing more to say here")
...
class SdkSingleFlight(unittest.TestCase):
    ...
    def test_concurrent_calls_build_one_backend(self):
        ...
        # and its siblings, jd._USAGE_REFRESH_FN included; the key wires went with romp's own key sources,
        # 2026-09-08, and the unpicked-auth wire with the declaration rule at the 2026-09-15 pull-in); with the
        # module a Mock those wires would outlive this test and
