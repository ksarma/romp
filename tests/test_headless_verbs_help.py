#!/usr/bin/env python3
"""The three surfaces that describe `romp send`, `romp interrupt` and `romp end` agree: the verb's own
`--help`, the `romp help` row, and the docs/reference.md command table spell one invocation, name the
same flags (`self`, `--now`, `--when-idle`, `--tag`) and each say that an unknown session is refused with
the kernel's reason and exit 1. The per-verb help was added with the 404 refusal (2026-09-09) and was the
only surface that named `self` and the two `end` flags; the row and the reference were the bare forms
(review find, rules-2). The precedent is tests/test_keyswap_refusal.py's HelpAndDocsAgree.

`romp <verb> --help` is rendered by running the script hermetically (no kernel: ROMP_KERNEL_PORT=1, a temp
HOME); the row and the reference are read from the source. Synthetic env only, nothing is posted."""
import os
import re
import subprocess
import tempfile
...
def _verb_help(verb):
    ...
    return subprocess.run(["bash", ROMP, verb, "--help"], env=env, capture_output=True, text=True, timeout=30)
