...
PICTURE = ("A picture in a rendered file that comes from a file or a web address has an **Open the picture** but"
                ...
                " a fold's title line holds it (a small picture from the web outside such a line has the dashed borde"
                "r itself, on a mouse or trackpad while the pointer is over it, and at all times on a phone or tablet"
                " and on a laptop with a touchscreen, since a finger gets no tooltip); and a picture inside a link th"
                "at holds more than the picture (a caption beside it), where a click follows the link (a link with no"
                " address left, or an anchor that only marks a place, is not a link a click can follow, so a picture "
                "inside it keeps its button and its tooltip, and a plain click opens it), while a picture that is all"
                " its link holds keeps its button beside the link.")
