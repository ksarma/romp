...
PICTURE = ("A picture in a rendered file that comes from a file or a web address has an **Open the picture** but"
                ...
                "gs the **Outline** button opens, or the menu of the text size buttons, say) opens nothing and scroll"
                "s it into view, and the next one opens once it shows (a button partly on the screen counts as shown)"
                ", and the button and the picture both show that before the click: the button's tooltip says it opens"
                " a new tab at the address's host, its border is dashed and its glyph is an arrow leaving a box, and "
                "the picture's own tooltip shows the address's origin (its scheme, host and port, never its path, que"
                "ry or fragment), on a line after the author's title when there is one; when the address has an @ any"
                "where after its scheme, so that it may carry a sign-in, both tooltips say the address is withheld an"
                "d show none of it, even for a harmless name such as a@2x.png; a click on a picture in a fold's title"
                " line (a `<details>` block's summary) opens or closes the fold and opens nothing, with or without Cm"
                "d, and a picture from the web there shows no address in its tooltip, while its button, where it has "
                "one, still opens it; a figure waiting behind its host's box gets its button once it has loaded, as d"
                "oes one still on its way (a click on it before then opens nothing), and once the browser has answere"
                "d for a picture, four kinds have none: a picture that failed to load, which opens nothing either; a "
                "`data:` picture, whose bytes are written into the file itself and which does not open; a picture sma"
                "ller than 48 pixels on either side (a badge, an inline icon), which the button would cover, and whic"
                "h a plain click still opens when neither a link nor a fold's title line holds it, though no key open"
                "s it, since the button is the keyboard's only way to a picture, and a picture that shrinks below 48 "
                "pixels as the pane narrows loses its button and that way with it (a small picture from the web outsi"
                "de such a line has the dashed border itself, on a mouse or trackpad while the pointer is over it, an"
                "d at all times on a phone or tablet and on a laptop with a touchscreen, since a finger gets no toolt"
                "ip); and a picture inside a link that holds more than the picture (a caption beside it), where a cli"
