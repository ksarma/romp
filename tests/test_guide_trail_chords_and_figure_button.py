...
PICTURE = ("A picture in a rendered file that comes from a file or a web address has an **Open the picture** but"
                ...
                "s over the picture or the button holds the keyboard focus, and at all times on a phone or tablet and"
                " on a laptop with a touchscreen, that opens the picture on its own in the viewer, with Back returnin"
                "g you to the file at that place; a plain click on the picture does the same while the Comments panel"
                " is closed (with the panel open, a click offers a comment as before, and so does a Cmd-click on the "
                "picture on a mouse or trackpad, and a drag draws a rectangle unless it starts on the button, which t"
                "akes the press), a Cmd-click (Ctrl on Windows and Linux) on the picture while the panel is closed, o"
                "r on the button at any time, opens the picture in a browser tab, and a picture from the web opens it"
                "s address in a new tab, as a link to that site does (from the keyboard, Enter or Space on its button"
                " opens it only while the button is on the screen), and the button and the picture both show that bef"
                "ore the click: the button's tooltip says it opens a new tab at the address's host, its border is das"
                "hed and its glyph is an arrow leaving a box, and the picture's own tooltip shows the address without"
                " its sign-in part, query and fragment, on a line after the author's title when there is one; a click"
                " on a picture in a fold's title line (a `<details>` block's summary, or the title of a callout writt"
                "en `[!note]-` or `[!note]+`) opens or closes the fold and opens nothing, with or without Cmd, and a "
                "picture from the web there shows no address in its tooltip, while its button, where it has one, stil"
                "l opens it; a figure waiting behind its host's box gets its button once it has loaded, as does one s"
                "till on its way (a click on it before then opens nothing), and once the browser has answered for a p"
                "icture, four kinds have none: a picture that failed to load, which opens nothing either; a `data:` p"
                "icture, whose bytes are written into the file itself and which does not open; a picture smaller than"
                " 48 pixels on either side (a badge, an inline icon), which the button would cover, and which a plain"
                " click still opens when neither a link nor a fold's title line holds it (a small picture from the we"
                "b outside such a line has the dashed border itself, on a mouse or trackpad while the pointer is over"
                " it, and at all times on a phone or tablet and on a laptop with a touchscreen, since a finger gets n"
                "o tooltip); and a picture inside a link that holds more than the picture (a caption beside it), wher"
                "e a click follows the link (a link with no address left, or an anchor that only marks a place, is no"
                "t a link a click can follow, so a picture inside it keeps its button and its tooltip, and a plain cl"
                "ick opens it), while a picture that is all its link holds keeps its button beside the link.")
