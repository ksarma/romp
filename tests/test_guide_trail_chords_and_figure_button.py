...
PICTURE = ("A picture in a rendered file that comes from a file or a web address has an **Open the picture** but"
                ...
                "ure from the web opens its address in a new tab, as a link to that site does (from the keyboard, Ent"
                "er or Space on its button opens it only while the button is on the screen), and the button and the p"
                "icture both show that before the click: the button's tooltip says it opens a new tab at the address'"
                "s host, its border is dashed and its glyph is an arrow leaving a box, and the picture's own tooltip "
                "shows the address's origin (its scheme, host and port, never its path, query or fragment), on a line"
                " after the author's title when there is one; when the address has an @ anywhere after its scheme, so"
                " that it may carry a sign-in, both tooltips say the address is withheld and show none of it, even fo"
                "r a harmless name such as a@2x.png; a click on a picture in a fold's title line (a `<details>` block"
                "'s summary) opens or closes the fold and opens nothing, with or without Cmd, and a picture from the "
                "web there shows no address in its tooltip, while its button, where it has one, still opens it; a fig"
                "ure waiting behind its host's box gets its button once it has loaded, as does one still on its way ("
                "a click on it before then opens nothing), and once the browser has answered for a picture, four kind"
                "s have none: a picture that failed to load, which opens nothing either; a `data:` picture, whose byt"
                "es are written into the file itself and which does not open; a picture smaller than 48 pixels on eit"
                "her side (a badge, an inline icon), which the button would cover, and which a plain click still open"
                "s when neither a link nor a fold's title line holds it (a small picture from the web outside such a "
                "line has the dashed border itself, on a mouse or trackpad while the pointer is over it, and at all t"
                "imes on a phone or tablet and on a laptop with a touchscreen, since a finger gets no tooltip); and a"
                " picture inside a link that holds more than the picture (a caption beside it), where a click follows"
                " the link (a link with no address left, or an anchor that only marks a place, is not a link a click "
                "can follow, so a picture inside it keeps its button and its tooltip, and a plain click opens it), wh"
                "ile a picture that is all its link holds keeps its button beside the link.")
