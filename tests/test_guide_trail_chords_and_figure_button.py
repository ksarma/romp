...
PICTURE = ("A picture in a rendered file that comes from a file or a web address has an **Open the picture** but"
                ...
                "s address in a new tab, as a link to that site does, and the button and the picture both show that b"
                "efore the click: the button's tooltip says it opens a new tab at the address's host, its border is d"
                "ashed and its glyph is an arrow leaving a box, and the picture's own tooltip shows the address, on a"
                " line after the author's title when there is one; a figure waiting behind its host's box gets its bu"
                "tton once it has loaded, as does one still on its way (a click on it before then opens nothing), and"
                " once the browser has answered for a picture, four kinds have none: a picture that failed to load, w"
                "hich opens nothing either; a `data:` picture, whose bytes are written into the file itself and which"
                " does not open; a picture smaller than 48 pixels on either side (a badge, an inline icon), which the"
                " button would cover, and which a plain click still opens when no link holds it (a small picture from"
                " the web has the dashed border itself, on a mouse or trackpad while the pointer is over it, and at a"
                "ll times on a phone or tablet and on a laptop with a touchscreen, since a finger gets no tooltip); a"
                "nd a picture inside a link that holds more than the picture (a caption beside it), where a click fol"
                "lows the link (a link with no address left, or an anchor that only marks a place, is not a link a cl"
                "ick can follow, so a picture inside it keeps its button and its tooltip, and a plain click opens it)"
