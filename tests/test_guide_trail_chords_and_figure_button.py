...
PICTURE = ("A picture in a rendered file that comes from a file or a web address has an **Open the picture** but"
                ...
                " on a picture in a fold's title line (a `<details>` block's summary) opens or closes the fold and op"
                "ens nothing, with or without Cmd, and a picture from the web there shows no address in its tooltip, "
                "while its button, where it has one, still opens it; a figure waiting behind its host's box gets its "
                "button once it has loaded, as does one still on its way (a click on it before then opens nothing), a"
                "nd once the browser has answered for a picture, four kinds have none: a picture that failed to load,"
                " which opens nothing either; a `data:` picture, whose bytes are written into the file itself and whi"
                "ch does not open; a picture smaller than 48 pixels on either side (a badge, an inline icon), which t"
                "he button would cover, and which a plain click still opens when neither a link nor a fold's title li"
                "ne holds it (a small picture from the web outside such a line has the dashed border itself, on a mou"
                "se or trackpad while the pointer is over it, and at all times on a phone or tablet and on a laptop w"
                "ith a touchscreen, since a finger gets no tooltip); and a picture inside a link that holds more than"
                " the picture (a caption beside it), where a click follows the link (a link with no address left, or "
                "an anchor that only marks a place, is not a link a click can follow, so a picture inside it keeps it"
                "s button and its tooltip, and a plain click opens it), while a picture that is all its link holds ke"
                "eps its button beside the link.")
