...
standing as its own word (_BANG) in a test's text, outside a comment, a quoted string and a here-document's body, with no grammar
of where a pipeline begins. Test bodies are bash_test_extents, bash's own parse of the file as bats-preprocess rewrites it (an
opener line's tail and a one-line test are in the population). Comment, string and heredoc are bash's call too: the test's text up
to the `!`, with ` || ||` appended, goes through `bash -n`, which refuses the `||` token only in command text (_in_command_text);
a backtick substitution's text is opaque to bash -n, so a `!` inside one is a candidate whatever surrounds it. Three exclusions are
lexical, by the word before the `!`: `[ !`, `[[ !` and `run !` (_OPERATOR_OF). A `!` that is another command's argument (`find .
! -name x`) is a candidate, and bats reports it as inert or undecided: a false report on the visible side, none in the tree today.
The one piece of grammar left is the negated pipeline's extent: from the `!` to the line's first top-level `;`, `&&`, `||`, lone
`&`, unmatched `)` or comment, `|` and `|&` inside, a trailing `\` or `|` running on to the next line (negated_pipeline_end),
decided on the safe side and pinned by the register below.

Over the 45 tests/*.bats at this head (BatsSuites lists a file's candidates and judges none): 208 `!` words file-wide, 191 of them
`[ !`; 12 candidates in test bodies (bootstrap-sh.bats 184; install-optional-deps.bats 505; install-sh.bats 329, 400, 411;
pr-orphans.bats 125; romp-serve.bats 117, 309, 334, 382; romp-service.bats 683; romp-sessions.bats 84), in 4.7 s with the extents.
Five more `!` words sit at file scope in helpers and a setup (bats-state-isolation.bats 125, 126 and 129 twice; romp-postal.bats
47): outside the subject, since a `!` there has no enclosing test to run alone, and the one class this instrument does not see.

The register (ground_truth_shapes, BatsGroundTruth) is the gate on the two things the instrument still asserts. Recall: every `!`
of a shape is a candidate unless NOT_A_NEGATION declares it text or an operator, and a test recorded `ok` with no candidate holds
only declared `!` words or none (NO_NEGATION); the expected set is derived from the shapes and the record, not from any
classification of this module's, so a spelling the predicate misses reds it. Agreement: with bats on PATH the shapes go through the
...
Measured at this head: 349 shapes, 359 tests; under `! true` 211 ok and 148 not ok, under `! false` 352 ok and 7 not ok (the four
condition heads whose branch fails the test, `command _h` and `env _h`, which find no shell function, and `run ! true`, which run
itself fails). The 250 shapes of the earlier register keep their 260 recorded `! true` verdicts and are 260 ok under `! false`;
every negation of the register is a candidate, the 139 recorded-inert line-start sites the earlier register counted and every one
...
# a `!` standing as its own word: after the line's start, a blank or a metacharacter that ends the word before it (`(`, `;`, `|`,
# `&`), and before a blank, the line's end or a `(` (bash parses `!(true)` as a negation). `!=`, `$!` and `!cmd` are none
...
# a `!` word the predicate found in command text of a test: the test's index in bash_test_extents, the line index and the column
Candidate = collections.namedtuple("Candidate", "test line col")


def _bash_n(lines):
    """(exit status, stderr) of `bash -n` over these lines as a script."""
...
def _test_bangs(lines, o, c):
    """(line index, column) of every `!` word in the text of the test bash opens at lines[o] and closes at lines[c]: the opener
    line's tail after its brace (a one-line test's whole body), then every line before the close. The close line is `}` and
    whatever follows it, which bash runs at file scope, so it is not the test's."""
    for m in _BANG.finditer(lines[o], _TEST_LINE.match(lines[o]).start(2)):
        yield o, m.start()
    for i in range(o + 1, c):
        for m in _BANG.finditer(lines[i]):
            yield i, m.start()
...
def _in_command_text(rewritten, o, i, j):
    """Whether column j of rewritten[i] is command text to bash, given the test's lines from its rewritten opener rewritten[o]: the
    text up to there with ` || ||` appended goes through `bash -n`, which refuses the `||` token exactly where a command may begin
    (_COMMAND_TEXT) and reads on past it inside a quoted string, a here-document's body, a comment, an arithmetic expansion or a
    `[[ ]]`; inside backticks it reports the open backtick, taken as command text (_IN_BACKTICKS)."""
    err = _bash_n(rewritten[o:i] + [rewritten[i][:j] + " || ||"])[1]
    return _COMMAND_TEXT in err or _IN_BACKTICKS in err
...
def candidates(lines, extents):
    ...
    `run` (the word before it, _OPERATOR_OF) and sits in command text by bash's own reading of the test up to it (_in_command_text):
    ...
    for t, (o, c) in enumerate(extents):
        ...
        for i, j in _test_bangs(lines, o, c):
            if _word_before(lines[i], j) in _OPERATOR_OF:
                continue
            if _in_command_text(rewritten, o, i, j + shift if i == o else j):
                out.append(Candidate(t, i, j))
    return out
...
def negated_pipeline_end(lines, cand):
    ...
    is open or the line ends in `\` or in `|`. None when it runs off the end of the text: undecided, the safe side."""
...
def rewritten_negation(lines, cand, end, repl):
    ...
    if ei != cand.line and tail and not tail[0].isspace():
        tail = " " + tail
    out = list(lines)
    out[cand.line] = lines[cand.line][:cand.col] + repl + tail
...
class Candidates(unittest.TestCase):
    ...
    def test_a_candidate_is_a_bang_word_in_command_text_of_a_test_and_nothing_else_is(self):
        ...
        # line-start negation, and the `!` inside backticks (opaque to bash -n, a candidate whatever quotes surround it). Not: the
        # comment, the strings, the heredoc text, `[`'s and `[[`'s and `run`'s operators, the arithmetic `!`, `!=` and `$!`
        self.assertEqual(candidates(lines, extents), [Candidate(0, 0, 20), Candidate(1, 1, 22), Candidate(2, 16, 7), Candidate(2, 17, 8),
                                                      Candidate(2, 18, 6), Candidate(2, 19, 4), Candidate(2, 20, 14)])
    ...
    def test_the_negated_pipeline_ends_at_the_first_top_level_operator_or_comment_and_follows_a_continuation(self):
        ...
        self.assertIsNone(negated_pipeline_end(['@test "x" {', '    ! true \\'], Candidate(0, 1, 4)), "a continuation past the text is undecided")

    def test_the_rewrite_keeps_the_line_count_and_what_follows_the_pipeline(self):
...
# the `!` words of the register that are not a command's negation, by shape and 1-based line, each with what it is: a heredoc's
# text, a comment, a string, or the operator of `[`, `[[` or `run`. The recall gate (BatsGroundTruth) reads every other `!` of a
# shape as a negation the predicate must find, so a shape's `!` that is text must be declared here to be excused, and a declared
# line must hold a `!` the predicate does not take (a stale declaration reds)
NOT_A_NEGATION = dict(
...
class BatsGroundTruth(unittest.TestCase):
    ...
    versions the record was verified against. The gate has two halves. Recall: every `!` of a shape is a candidate unless declared
    an operator or text (NOT_A_NEGATION), and a test recorded `ok` with no candidate holds only declared `!` words or none
    (NO_NEGATION), so a spelling the predicate misses reds here and a new shape costs one entry. Agreement: with bats on PATH the
    ...
    def test_every_negation_of_the_register_is_a_candidate_unless_declared_an_operator_or_text(self):
        ...
        # succeeding, are the ones a miss would hide; the read ones are counted too): the expected set is every `!` word of the test
        # by _test_bangs, minus the lines NOT_A_NEGATION declares, derived from the shapes and the record and from no classification
        # of this module's
    ...
    def test_an_ok_test_without_a_candidate_holds_only_declared_bangs_or_none_and_every_declaration_holds(self):
        ...
        for name in sorted(shapes):
            ...
            for t, ((o, c), v) in enumerate(zip(extents, verdicts)):
                bangs = list(_test_bangs(lines, o, c))
            ...
            for ln, what in declared.items():
                on_line = [(i, j) for o, c in extents for i, j in _test_bangs(lines, o, c) if i == ln - 1]
                if not on_line:
            ...
            for t in NO_NEGATION.get(name, ()):
                if not 1 <= t <= len(extents) or verdicts[t - 1] != "ok" or list(_test_bangs(lines, *extents[t - 1])):
