...
class RemoteEndRelaysTheRefusal(ClosedHonestyBase):
    ...
    def test_the_remote_refusal_reaches_the_caller_verbatim(self):
        ...
        # the arm reads the status beside the body (_remote_forward_status), so that is the seam stubbed
        self._saved["_remote_forward_status"] = km._remote_forward_status
        km._remote_forward_status = lambda r, path, body, method="POST": (self.forwarded.append((path, body)) or (200, refusal))
        code, resp = self._post_end()
    ...
    def test_a_remote_success_still_relays_ok_true(self):
        self._saved["_remote_forward_status"] = km._remote_forward_status
        km._remote_forward_status = lambda r, path, body, method="POST": (200, {"ok": True})
        code, resp = self._post_end()
        self.assertEqual((code, resp), (200, {"ok": True}))
