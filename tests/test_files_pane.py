...
- the /files page: the chat's styles.css for the viewer's dress, files-pane.css read live for the
  ...
  loading state), the shim with the ready hold and the stale opt-out (a page that receives no pushed view never arms
  the shared "may be stale" prompt), federation.js before files.js.
- the shell: a sixth column after Waiting, off by default, with its gutter, grow var, focus, Escape and
...
class Plumbing(unittest.TestCase):
    ...
    def test_files_is_outside_the_feed_audience(self):
        ...
        self.assertTrue(km._feed_audience([{"app": "waiting"}]), "this kernel's Waiting pane rides the feed")
    ...
    def test_the_page_carries_the_hold_and_the_stale_opt_out_the_shared_dress_and_no_loader(self):
        page = km._files_page()
        self.assertIn('_shim("files", v, caps=READY_GATE_CAP, no_stale=True)', SRC)
        self.assertNotIn('_shim("files", v, caps=FEED_DELTA_CAP', SRC)
        self.assertIn("app=files", page)
        self.assertIn('var CAPS="readyGate";', page, "the ready hold rides the caps")
        self.assertIn("var NOSTALE=true;", page, "the stale opt-out is the shim's keyword, not a cap")
        self.assertNotIn('var CAPS="feedDelta', page)
...
class Shell(unittest.TestCase):
    ...
    def test_mobile_tab_and_the_palette_command(self):
        ...
        _has(self, "#f-chat.m-on,#f-fleet.m-on,#f-feed.m-on,#f-waiting.m-on,#f-files.m-on{display:block}", self.html)
        pal = (UI / "palette-main.ts").read_text()
        _has(self, '["files", "files", "Files"]', pal)
...
class Relay(unittest.TestCase):
    ...
    def test_the_pane_arm_brings_the_files_pane_forward_and_forwards_the_identity(self):
        ...
        self.assertEqual(js.count("m.romp==='viewFile'"), 2, "the pane arm and the feed route, its else branch; no third")
        self.assertEqual(SRC.count("m.romp==='viewFile'"), 2, "and no viewFile arm anywhere else in the kernel")
        branch = self._code(js.split(self.HEAD)[1].split("else if(m.romp==='viewFile')")[0])
    ...
    def test_the_feeds_browse_relay_is_untouched(self):
        ...
        # the phone's switch to the Feed tab rides the feed's browseOpened ack, not the relay (review round 2)
        arm = self._code(js.split("else if(m.romp==='browseFiles'){var bf=document.getElementById('f-feed');")[1].split("if(m.romp==='browseOpened'){")[0])
        _lacks(self, "__rompMobileTab", arm, "the relay switches no tab")
        _has(self, "if(curf!=='feed'){window.__rompFeedTabFrom=curf;window.__rompMobileTab&&window.__rompMobileTab('feed');}", js)
        _has(self, "if(m.romp==='browseClosed'&&(window.__rompFeedWasOff||window.__rompFeedWasOffView)){", js)
...
class BrowseRelay(unittest.TestCase):
    ...
    def test_a_browse_naming_no_pane_or_another_pane_takes_the_feeds_route_exactly_as_before(self):
        for key in ("noPane", "otherPane"):
            ...
            self.assertEqual(n["tabs"], [], key + ": the relay switches no tab; the feed's browseOpened ack arms the phone's switch")
    ...
    def test_the_pane_branch_precedes_the_feed_branch_and_names_no_feed_token(self):
        ...
        _lacks(self, "__rompMobileTab", self._code(feed.split("if(m.romp==='browseOpened'){")[0]), "the relay switches no tab; the feed's ack does")
        _has(self, "if(curf!=='feed'){window.__rompFeedTabFrom=curf;window.__rompMobileTab&&window.__rompMobileTab('feed');}", feed)
