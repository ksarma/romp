...
The composition the ruling asked for, per pane: a viewport of 390 by 508 (inside the shell the pane iframe is sized to
the visible height, so the pane's own innerHeight is the keyboard's signal; 508 is a phone's visible height with the
keyboard up, above the fold's 480px threshold), a todo with a near-300-character ask, a file chip, a link chip and a
forty-line detail whose first line carries an address; Reply tapped, fourteen lines typed, then a real click at Send's
painted centre. Asserted: Send and Cancel inside the box's clip and the frame, elementFromPoint at Send's centre IS Send,
the click puts a userTodoAnswer frame for the todo on the page's socket and the sheet closes by that send (never by the
backdrop: on the round-1 tree the answer box, capped at a share of the WINDOW, laid the buttons out below an overflow
hidden box; a tap there fell on the backdrop, which closes the sheet with no save, and nothing was sent); the detail
keeps its floor of two lines in that state (it resolved to 0px before). At 300px (the fold on): the detail keeps two
lines, scrolls within itself, its first line's address is under a finger once the box is scrolled to it, the box
scrolls, Send is inside the clip and under a finger at the box's bottom. And the tree each builder emits, read from the
real pages: the pane's chips are flex children of the box, the chat's sit inside the quoted line, and the elements the
fix's four rules key on (the answer box, the detail, the box) match their selectors in both.
...
# address, and a detail whose first line carries an address
...
def _todo_store(now):
    ...
    return {SID: [
        {"id": "ut-0000a001", "text": "Which layout should the quarterly report use?", "createdT": now - 400, "detail": DETAIL},
...
class ReplySheetServed(unittest.TestCase):
    ...
    def _drive(self, engine, pane):
        ...
        cfg = {"engine": engine, "pane": pane, "url": url, "healthz": "http://127.0.0.1:%d/healthz" % self.port,
               "sid": SID, "tid": TID, "bootTimeoutMs": 30000}
    ...
    def _leg(self, engine, pane):
        ...
        o, t, s = r["opened"], r["typed"], r["short"]
        ...
        # opened at 508: no fold, three rows, the detail a scroll container over its cap, the box scrolling at this height too
        ...
        # the short window (300px, the fold on): the floor, the scroll, the address under a finger, Send at the box's bottom
        self.assertTrue(s["tight"], where + "300px folds: %r" % (s,))
        self.assertGreaterEqual(s["detailH"], 2 * s["detailLineH"] - 1, where + "300px: the detail keeps two lines (the floor); it resolved to 0px before: %r" % (s,))
        ...
        # THE COMPOSITION at 508 with fourteen lines typed: the buttons inside the box's clip and the frame, a finger at
        # Send reaches Send, the detail keeps its floor
        ...
        for b in ("send", "cancel"):
            self.assertTrue(t[b]["top"] >= t["box"]["top"] - 0.5 and t[b]["bottom"] <= t["box"]["bottom"] + 0.5,
                            where + "typed: %s is inside the box's clip (on the round-1 tree the box grown to 40%% of the window laid it out below): %s" % (b, rec))
        ...
        self.assertGreaterEqual(t["detailH"], 2 * t["detailLineH"] - 1, where + "typed: the detail keeps its two-line floor in the deficit state (0px on the round-1 tree): " + rec)
        # the tap: the page put ONE userTodoAnswer frame for the todo on its socket, and the sheet closed by that send
        ...
        self.assertEqual(len(r["after"]["sent"]), 1, where + "the tap SENT the answer (one userTodoAnswer frame for the todo on the socket); on the round-1 tree the tap fell on the backdrop and sent nothing: " + rec)
