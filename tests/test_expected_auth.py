...
class DeclarationInTheKeySourceVerdict(unittest.TestCase):
    """The declaration's two readers in the boot verdict (sdk_backend.key_source_verdict): =login while
    the credential command prints a key, and =key with nothing to inject and no apiKeyHelper. Pure calls
    on an explicit environ; the developer's shell never reaches them."""
    ...
    def test_key_declared_with_nothing_to_inject_needs_a_helper_under_the_command_kind(self):
        ...
        # the file kind says nothing new at boot: the init-time mismatch line (this file's other classes)
        ...
        # nor does the reference: _check_env_file_vs_declaration weighs the file's selection against =login
        v = sb.key_source_verdict({"ROMP_EXPECTED_AUTH": "key", "ROMP_API_KEY_REF": "op://vault/item/field"})
        self.assertEqual(self._problems(v), [])
