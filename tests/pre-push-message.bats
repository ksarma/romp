...
# .githooks/pre-push — the MESSAGE scan: the subject and body of each commit a
# push publishes, driven by hand against real commits.
#
...
# that failed or the box it failed on. A string a file carries can be redacted
# forward once it is out; a string a message carries cannot — only a rewrite of
# the commit takes it back — so the hook greps every new commit's message like
# lines of content, the whole denylist, and names the commit and the line.
#
# Two limits, both pinned here: only what THIS push publishes is read (a message
...
setup() {
    ...
    # the denylist: an invented login and host, nothing that exists on any real box
...
commit_msg() {   # <path> <paragraph>...
    ...
    printf '%s\n' "the web session's work on $path" > "$REPO/$path"
    git -C "$REPO" add "$path"
...
@test "a banned string in the SUBJECT is refused, naming the commit and line 1" {
    commit_msg web.txt "fix the path /home/zzsynthuser/code on the box"
    sha="$(git -C "$REPO" rev-parse HEAD)"
    run_hook
    [ "$status" -ne 0 ]
    ...
    # the tree and the added lines were clean: neither content report, nor the address remedy
...
@test "the author's and committer's NAMES are not read: they are on every commit they make" {
    ...
    GIT_AUTHOR_NAME=zzsynthuser GIT_COMMITTER_NAME=TESTHOST commit_msg web.txt "clean message"
...
@test "the message remedy is not offered for a content hit" {
...
@test "a merge commit's message is read like any other commit's" {
    # a merge's message is typed by whoever resolves it, and a conflict note is where a box's name lands
...
@test "a message hit and an address hit on one push report both remedies" {
    ...
    GIT_AUTHOR_EMAIL=dev@TESTHOST.example GIT_COMMITTER_EMAIL=dev@TESTHOST.example \
        commit_msg web.txt "broke under /home/zzsynthuser"
    run_hook
    [ "$status" -ne 0 ]
    [[ "$output" == *"the MESSAGE of commit"* ]]
