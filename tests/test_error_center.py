...
class ErrorCenterExecutes(unittest.TestCase):
    ...
    def test_the_filter_bar_has_one_toggle_per_kind(self):
        ...
        # 'refused' joined with the state-readers change (as 'not saved'): a setting, bell, tag or order the
        # kernel REFUSED because it could not read the file that holds it files an entry of its own kind, so
...
class ErrorCenterWiring(unittest.TestCase):
    def test_the_shell_mounts_bell_popover_and_script(self):
        ...
        # undo) write nothing (review round 6 said a refused card gesture writes nothing there, which was false
        # for every gesture refused at the drive gate; review rounds 7 and 8, 2026-09-09). The webview pin
        ...
        self.assertNotIn("interrupt or end files a row", html, "the compact joined the list")
...
post({ romp: 'wsState', app: 'chat', state: 'parked' });
out.afterRepark = { n: notes().length, red: EL['rail-errs']._cls.has('has') };
// and its open, at the tap, reads up as ever
post({ romp: 'wsState', app: 'chat', state: 'up' });
...
class ParkedPaneCue(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        script = HARNESS + km._LANDING_ERRS_JS + PARKED_DRIVER
        with tempfile.NamedTemporaryFile("w", suffix=".js", delete=False) as f:
            f.write(script)
            path = f.name
        try:
    ...
    def test_a_re_park_clears_the_live_cue_and_an_open_reads_up(self):
        self.assertEqual(self.out["afterRepark"], {"n": 1, "red": False}, "parked is not down: the live cue clears, the entry stays")
        self.assertEqual(self.out["afterUp"], {"n": 1, "red": False})
