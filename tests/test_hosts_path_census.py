...
RESIDUAL = {
    ...
    ('kernel/session_host.py', 'read_journal_dir', 'glob', 'd', 1):
        ('by-path', FOLLOW_UP, FOLLOW_UP_ITEM + ": the orphan journal's segment listing is a glob over the directory; the descriptor form is a scandir off the <sid> descriptor with a name match, a rewrite of the function and its two callers (the orphan road's tail check, the replay transport's _read_journal). Through the state root's guarded reader since fork PR 874, which judges the components by path and then lists by path: a guard, not a descriptor, so the item stands"),
    ('kernel/session_host.py', 'read_journal_dir', 'read_text', 'd / "gaps.json"', 1):
        ('by-path', FOLLOW_UP, FOLLOW_UP_ITEM + ": gaps.json read by the path of the directory the glob lists; converted with the glob (through the guarded reader since fork PR 874, by path still)"),
    ('kernel/session_host.py', 'read_journal_dir', 'open', 'p', 1):
        ('by-path', FOLLOW_UP, FOLLOW_UP_ITEM + ": each segment opened by the path the glob yields; converted with the glob (through the guarded reader since fork PR 874, by path still)"),
