...
class ForkedSessionChipMatchesFeed(unittest.TestCase):
    """Round 3 (2026-08-26), item 2: the chip and the card read the SAME fact. build_feed resolves
    a session's transcript via discover — the registry's lastSid file for a /cleared or
    resume-forked SDK session — while _session_stamp_read resolved _sdk_transcript_path, the DEAD
    anchor file, so the chip's ask-unit discriminator (_pure_delegation_top's anchor resolve)
    answered from a transcript the feed no longer reads: the feed suppressed a machine-anchored
    coordination top while the chip lit 'waiting on peers' for it. One fact, two answers. The
    stamp cache also keys on the resolved path + parse warmth now: a not-yet-latched node's
    verdict is the only cache-temperature-dependent input left (a LATCHED askAnchor rides the goal
    store, whose mtime is already in the key), and without the temperature key the chip served a
    cold-beat fail-open long after the feed's fresh read had warmed."""
