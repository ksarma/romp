...
class EndOnIdle(unittest.TestCase):
    def setUp(self):
        ...
        # (until the tmux backend's removal 2026-09-11 the sweep corroborated a kill against the liveness
        # owner, _confirmed_ended; upstream's kill path records the death on be.kill's return, the SDK and
        # Codex kills being authoritative, so nothing here stubs a corroboration)
    ...
    def test_a_sid_already_dead_retires_its_request_without_a_kill(self):
        km._end_on_idle_save({SID})
        km._end_on_idle_sweep(1000, {})              # absent from the liveness map: dead by some other path
