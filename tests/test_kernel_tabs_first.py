...
class TabsFirst(unittest.TestCase):
    def test_push_taborder_carries_name_and_color_per_tab(self):
        ...
        # the frame's `live` set (T258) is fed from the collapse guard's map the whole chat block trusts this
        # cycle (chat_tmux): on a carried cycle it lists the carried sids as live, consistent with `order`
        self.assertIn('_send_client(c, ("taborder",), _tab_order_frame(tab_order, tab_meta, chat_tmux))', src,
    ...
    def test_every_tab_order_frame_names_this_kernels_own_host(self):
        ...
        # ...each naming the liveness map its sender trusted: the pusher's guarded map for the cycle, the
        # off-cycle push's guarded read, the close confirmation's guarded read (never a raw _tmux_sessions())
        self.assertEqual(text.count('_send_client(c, ("taborder",), _tab_order_frame(tab_order, tab_meta, chat_tmux))'), 1)
        self.assertEqual(text.count('_send_client(c, ("taborder",), _tab_order_frame(tab_order, tab_meta, tmux))'), 1)
        self.assertEqual(text.count("frame = _tab_order_frame(tab_order, tab_meta, guarded)"), 1)
    ...
    def test_connect_ready_handler_sends_no_tab_order_of_its_own(self):
        ...
        # T258 spelled that frame through the builder (`_frame = _tab_order_frame(_o, _tabs, _tm)`); the
        # fork removed the block on 2026-09-03, so neither spelling has a home here
        text = open(KPATH).read()
