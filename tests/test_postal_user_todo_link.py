...
class WiredToTheKernel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        (Path(os.path.realpath(self.td.name)) / "state").mkdir()
        jd._rebind_state(Path(os.path.realpath(self.td.name)) / "state")   # the seam floors a umask-mode root at 0700 (round 4, 2026-09-21): the per-module chmod is gone
