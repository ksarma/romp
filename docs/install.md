...
- The agent-side tooling for [file comments and tracked changes](guide.md#files), from the copy of
  ...
  commands newly refuses. This guard is best-effort against known write forms: it refuses the shell
  writes it models and, by design, allows anything it does not recognise, so it never blocks
  ...
  source lies in a tracked project or is one it cannot read. A value it can read (a name the command
  set to a plain string earlier in the same command, HOME, PWD, OLDPWD, `~+` and `~-`, none of them
  once the command names or may fill in the name) is resolved first and the real path judged. These
  write forms are not modelled and still reach a tracked file: rsync; awk with a redirect inside its
  program; ed; ex; make; find with -delete or -exec; a git subcommand that writes the working tree
  (checkout, stash, apply, reset, rm, clean, mv); a computed or escaped path inside an interpreter
  (a name, sys.argv, os.environ or process.env, a concatenation or an escape sequence in the string,
