...
- The agent-side tooling for [file comments and tracked changes](guide.md#files), from the copy of
  ...
  refusals from the tracked cwd and their 22 allowances from the cwd in no project. This guard is
  ...
  it cannot read. A value it can read (a name the command set to a plain string earlier in the same
  command, HOME, PWD, OLDPWD, `~+` and `~-`, none of them once the command names or may fill in the
  name) is resolved first and the real path judged. These write forms are not modelled and still
  reach a tracked file: rsync; awk with a redirect inside its program; ed; ex; make; find with
  ...
  rsync) that a later modelled write follows; shuf -o; a cd through CDPATH; and an opaque expansion
  from a cwd outside every project, leading or after a literal head outside every project (a `..`
  inside the value could climb into a project; from a cwd in a tracked project the same word is
  refused as not literal). If you had installed track-changents yourself, the installer re-points
