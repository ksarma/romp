...
- Under that rule the sidecar now carries three additive fields on a comment, `target` (a region),
  ...
  whole by the other editors. romp never writes the format's own `suggestionId`, the other editors'
  thread key, and reads one it finds as the change that answered the comment.
