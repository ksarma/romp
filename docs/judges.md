...
With `ROMP_CREDENTIAL_COMMAND` set (see the reference, "Installing without keys
on disk"), a judge child env also carries the set that command printed, minus
`ANTHROPIC_API_KEY`, which is re-added only for a key-mode call, through the
same explicit decision. A credential-class refusal on a judge call marks the
cached set stale, so the next call re-runs the command; the model catalog fetch
reads the set's `ANTHROPIC_LP_API_KEY` the same way.
