...
| `romp new --in <tag> <name>` | Put the new SDK session in `<tag>`, so its tab lands in that group (repeatable; a name that does not exist yet creates the tag). Applies to `<name>` if it already runs. The kernel echoes `tags`; a missing echo, or a tag the kernel did not apply, prints a warning |
| `romp new --no-inherit <name>` | Run inside a romp session, `romp new` sends that session's stable id (`ROMP_SID`) as the new session's `parent`, and the kernel copies the parent's tags onto the child. This flag withholds the parent, so the new session starts outside them. Raw POST /new callers pass `parent` (a live name or a known sid; an unknown one is a 400) and `tags` (a list of names); opening a name that already runs never inherits |
