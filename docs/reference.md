...
| `romp billing <session> [<pick> [--now]] \| --all-following <pick>` | A Claude Code session's billing from the shell (see [Per-session billing](#per-session-billing-login-vs-api-key)). With a session alone it prints three lines: the side the running CLI launched on and what the CLI reports it bills, the session's own pick or that it follows the machine default, and the machine default. With a pick (`key`, `login`, `login:<id>` for a stored login, or `default` to follow the machine default again) it posts the same change the dashboard's Billing menu posts: the session reconnects to apply it at its next quiet moment, at once when it is quiet, and the output says which; `--now` cuts the turn in flight and reconnects at once (live subagents and background tasks still hold the pick until they finish, and the output says so; a compaction is not cut). `--all-following <pick>` writes the pick on every running session on this machine that follows the machine default, so they stop following it, and prints the names moved and skipped. A session an attached machine runs is changed by that machine's kernel. An unknown session is refused with the kernel's reason and exit 1; a kernel that took the request but answered late is exit 3 (it may already have acted: do not retry blindly) |
...
turn in flight so the reconnect follows at once; a compaction is not cut. The
...
it runs the other side. `romp billing --all-following <pick>` writes the pick
on every running session that follows the machine default, through the same
walk that reconnects the followers when the default changes; those sessions
then carry their own pick and stop following the default. A session an
attached machine runs is changed by that machine's kernel, and the walk
covers this machine's sessions only.
