...
| `romp new --model <id> <name>` | Model for the SDK session, as a full id such as `claude-fable-5`; re-asserted if `<name>` already runs |
| `romp new --effort <level> <name>` | Reasoning effort for the SDK session (`high`, `ultracode`, ...); re-asserted if `<name>` already runs |
| `romp new --env NAME=VALUE <name>` | A per-session env var for the SDK session, repeatable; re-asserted if `<name>` already runs |
...
every session there and outlives them all. Keep real secrets out of it: each
value is copied into per-session files and the session registry under
`~/.local/state/romp/`. Keys and credentials stay in `service.env` or the
`apiKeyHelper`.
