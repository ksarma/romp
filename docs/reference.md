...
it runs the other side; it never queues, so during a move it is refused. `romp
...
landing has stamped yet is left to that landing), the ones skipped for a
pick of their own, and any whose record would not read (nothing was written
for those). A session an attached machine runs is changed by that machine's
