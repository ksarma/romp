...
| `romp-service` | Bash | Installs/removes the launchd (macOS) / systemd-user (Linux) login agent, rewrites it in place without a restart (`rewrite`: the unit or the plist keeps its own ExecStart, ROMP_DIR, PATH, service.env path and instance lines and gets the release's, systemd reloaded on Linux and its NeedDaemonReload flag read back, the running manager left as it is; what `install.sh` runs while the manager is up; exit 3 when none is installed, 1 when a value in the environment differs from the file's line), and stops/starts it (`stop`/`start`: the supervisor halves of `romp down` / `romp up`; `stop` exits 3 when none is installed and 4 when one is installed but not running). Run by `install.sh`. |
