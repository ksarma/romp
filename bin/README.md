...
| `romp-manager` | Node | The kernel **supervisor**: spawns kernels via `romp-serve`, respawns on crash, `up/ensure/restart-all/status/down`. Reached via `romp up` / `romp refresh` / `romp status`. Before every SIGTERM it sends, it appends a `manager-sigterm` row (with a `trigger`: `restart`, `restart-all`, `refresh`, `cli-down`, `stop`) to the kernel's `restart-audit.jsonl`; that row says the manager was the messenger, and readers after who asked skip it for the requester's own row beneath. |
| `romp-serve` | Bash | The manager→kernel seam: maps the manager's spawn contract onto the kernel's env, picks the python (`ROMP_PYTHON`, refused with one line if it is not an executable interpreter; else the interpreter the SDK venv's `pyvenv.cfg` records, checked to run and to be the recorded X.Y; else another `pythonX.Y` of that same minor on PATH or in `~/.local/bin`, which the venv still matches; else the newest `pythonX.Y` found, which needs a `romp-sdk-setup` rebuild), then `exec`s the kernel (PID preserved for the supervisor; the kernel self-builds stale UI bundles). |
...
| `romp-sdk-setup` | Bash | Provisions the Agent SDK venv for the SDK backend, with the same python pick as `romp-serve` so the two agree: a re-run keeps the venv's interpreter and upgrades the SDK, moves to another python of the same minor if the recorded one is gone, and rebuilds for a different interpreter only when `ROMP_PYTHON` names one or no python of the venv's minor is left, saying so. A `ROMP_PYTHON` that is not an executable interpreter is refused as such, not called a too-old python. Run by `install.sh`. |
