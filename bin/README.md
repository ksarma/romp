...
| `romp-service` | Bash | Installs/removes the launchd (macOS) / systemd-user (Linux) login agent, rewrites it in place without a restart (`rewrite`: the unit or the plist from the current environment, systemd reloaded on Linux, the running manager left as it is; what `install.sh` runs while the manager is up; exit 3 when none is installed), and stops/starts it (`stop`/`start`: the supervisor halves of `romp down` / `romp up`; `stop` exits 3 when none is installed and 4 when one is installed but not running). Run by `install.sh`. |
