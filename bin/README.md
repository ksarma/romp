...
| `romp-perf-upload` | `cli/perf_upload.py` | Sends one export written by `romp perf export --public` to the configured receiver (`--receiver`, else `ROMP_PERF_RECEIVER`, else `~/.config/romp/perf-receiver`; refused, naming them, when none is set): the file is checked again as it stands, the path, size and the URL it will dial are printed, a yes is asked for on a terminal (`--yes` is the agent's form), then one POST; a `201` receipt is the only answer accepted (`romp perf upload <file>`). |
