...
| `romp-serve` | Bash | The manager→kernel seam: maps the manager's spawn contract onto the kernel's env, picks the python (`ROMP_PYTHON`, else the interpreter the SDK venv's `pyvenv.cfg` records, else the newest `pythonX.Y` found), then `exec`s the kernel (PID preserved for the supervisor; the kernel self-builds stale UI bundles). |
...
| `romp-sdk-setup` | Bash | Provisions the Agent SDK venv for the SDK backend, with the same python pick as `romp-serve` so the two agree: a re-run keeps the venv's interpreter and upgrades the SDK, and rebuilds for a different interpreter only when `ROMP_PYTHON` names one or the recorded one is gone, saying so. Run by `install.sh`. |
