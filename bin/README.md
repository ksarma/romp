...
| `romp-service` | Bash | Installs/removes the launchd (macOS) / systemd-user (Linux) login agent, and stops/starts it (`stop`/`start`: the supervisor halves of `romp down` / `romp up`; exit 3 when none is installed). Run by `install.sh`. |
...
| `romp-manager` | Node | The kernel **supervisor**: spawns kernels via `romp-serve`, respawns on crash, `up/ensure/restart-all/status/down`. Reached via `romp up` / `romp refresh` / `romp status`, and by `romp down` when no login service supervises it. `ensure` holds while a `romp down` marker exists; `up` clears it. |
