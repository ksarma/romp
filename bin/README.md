...
| `romp-service` | Bash | Installs/removes the launchd (macOS) / systemd-user (Linux) login agent, rewrites it in place without a restart (`rewrite`: the unit or the plist keeps its own ExecStart, ROMP_DIR, PATH, service.env path, instance lines and state root and gets the release's, systemd reloaded on Linux and its NeedDaemonReload flag and FragmentPath read back, the running manager left as it is; what `install.sh` runs while the manager is up; exit 3 when none is installed, 5 when a value in the environment or this clone differs from the file's; `rewrite --check` runs the checks alone, which the kernel's self-update does before it moves the checkout), and stops/starts it (`stop`/`start`: the supervisor halves of `romp down` / `romp up`; `stop` exits 3 when none is installed and 4 when one is installed but not running). Run by `install.sh`. |
