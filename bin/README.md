...
| `romp-perf-export` | `cli/perf_export.py` | Writes one paste-safe copy of the kernel's `GET /perf` counters, or of a saved snapshot, under the state directory (`romp perf export --public`): the shape rule and denylist of `cli/perf_public.py` (no bin entry; imported by this, by `romp-perf-upload`, by `romp-restart-metrics --public` and by the served-snapshot invariant test) applied to the whole document, so no path, session id, hostname, pid, username or absolute clock stamp is written; measurements stay (paste-safe, not unlinkable). |
| `romp-perf-upload` | `cli/perf_upload.py` | Sends one export written by `romp perf export --public` to the configured receiver (`--receiver`, else `ROMP_PERF_RECEIVER`, else `~/.config/romp/perf-receiver`; refused, naming them, when none is set): the file is checked again as it stands against the export's rule (paste-safe, not unlinkable), the path, the size of the body and the URL it will dial are printed, a yes is asked for on a terminal (`--yes` is the agent's form), then one POST whose body is the checked document written out again by the export's own writer, never the file's own bytes, so what is sent is what was checked; a `201` receipt is the only answer accepted (`romp perf upload <file>`). |
