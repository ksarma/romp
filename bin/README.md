...
| _(no bin entry)_ | `kernel/keysource.py` | The live source of the manager's API key where an installation keeps one in `service.env`: its `ANTHROPIC_API_KEY=` line, a `ROMP_API_KEY_CMD` key command the kernel runs at use time (any secret manager's CLI printing the key), or a `ROMP_API_KEY_REF` reference that `op` resolves at runtime (a provider names where the key lives; inspecting the source never fetches or runs it), re-read at every session launch (file mode). Shared by the kernel and `romp keyswap`'s report, so the two cannot disagree about the path or the parse. This fork writes no key line (the named swap is refused in file mode). |
...
| `romp-keyswap` | `cli/keyswap.py` | Reports which API key the sessions bill (fingerprints only; a source is inspected, never fetched or run) and whether the kernel reads what the shell reads; with `ROMP_CREDENTIAL_COMMAND` set, `romp keyswap <name>` selects a declared credential by writing the selector file and `--refresh` makes the kernel re-run the command; after a rotation, `--cycle-all` reconnects quiet running sessions so their new processes pick the new credential up, with no manager restart. Where the key source lives in a file, upstream's named swap (a rewrite of `service.env` from a sibling profile, whether it carries a key line, a `ROMP_API_KEY_CMD` key command or a `ROMP_API_KEY_REF` 1Password reference) is refused: this fork does not write API keys to files. |
