...
| `0009-skill-non-literal-target-refused.patch` | no (romp-only) | The skill says that in a project that tracks files a shell write whose target is not a literal path (a variable, a `$(...)`, a glob or brace list the guard cannot expand) is refused, whatever the word would expand to, since the guard cannot tell which file it would write and reads none of the agent's variables to find out (`HOME` alone, so a leading `$HOME/` is read as `~/` is), with one exception, a target whose only expansions are `$$` or `${$}`, the shell's process id, at an absolute path where no tracked file could land (nothing else: `$RANDOM`, `$SECONDS` and every other name can be unset or shadowed and then hold a path, so they are refused in every shell), names the relative path after a `cd` the guard cannot follow as refused too, and says what to do (spell the path out, name a temp file with `$$`, give a folder a literal name, or write outside the tracked project): the refusal romp's Bash-side guard (`hooks/romp-track-bash-guard.mjs`) emits since 2026-09-18, after a research session's report (2026-09-17, through the box admin) of a `cp` built from variables that landed raw on a tracked file while the same cp spelled out was refused, as bounded by that PR's round-1 review the same day and corrected by its rounds 2 and 3 (2026-09-19). Round 4 (2026-09-19) added a second paragraph: the guard reads the common wrappers, runs `env -C`/`sudo -D` in DIR, refuses an unknown cp/mv/install/ln option, treats `$HOME`/`~` as unreadable once the command reassigns HOME, refuses a variable whose literal head parents or sits under a tracked project, and states its best-effort contract with the list of unmodelled writers that pass; the walk-around lens second pass (2026-09-19) extended it with the six families it closed (a glued option short form and env -S; any HOME assignment form; an in-command rm/mv/link mutation; a stat error refusing from any cwd; a marker nested under the project; and a cd the guard cannot know ran); the third pass (2026-09-19) re-keyed six of those rules on what the guard can see and names each (any mention of HOME outside an expansion; a wrapper option the guard does not parse in full, with `env -S` refused outright; a link whose source is not literal; a shell option not known to be inert for paths; a tracked project at any depth under a variable's literal head; an unknown writer option refused wherever the writer is reached), and made the contract paragraph identical to the other three surfaces, its allow-by-default and refused-class sentences included; the fifth commit (2026-09-19) added what a fourth attack found and the reviewer ruled on (a variable name the shell fills in; zsh's clobber-override redirections; a link followed into a numeric name's folder; a plain-string interpreter path judged by its text and a template path refused; an alias putting the project its source lies in in play from any working directory; the inert shell options decided by a stated criterion; the remedy line's `--file` path single-quoted) and the contract paragraph's new refused forms and widened out-of-model list; its second commit (B2 as the reviewer ruled it) says the guard reads a name the command sets to a plain string, HOME, PWD, OLDPWD, `~+` and `~-` (none of the three once the command names the name or may fill it in), judges the resolved path, and leaves what stays opaque to the working directory's verdict, refused from a tracked cwd and allowed from a cwd in no project, the residual stated with its boundary; a pin addendum (2026-09-19) adds that `cp --parents`, python's glued `-c` and node's `--eval=` are read, that a triple-quoted python path is a plain string and a template literal holding a quote a template, and that a concatenation or an escape sequence in an interpreter's path is among what passes; round 5 (2026-09-20) names the two escalated false refusals and says a concatenation that does not open with a string literal stays out of model, and its addendum says that no declaration flag, and no `declare`, `typeset` or `local` (dash has none of the three; `export` and `readonly` with no option word are the two declarations every shell performs), keeps a name readable, that a `{ }` group opened after `&&`, `||` or `|` does not either (each of those leaves the name unreadable), and drops coproc from the unmodelled wrappers, since the guard reads the command behind it. Its title line carries a plain dash since the third pass. Other hosts have no such guard, so this stays romp's. |
