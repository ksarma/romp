...
| `0009-skill-non-literal-target-refused.patch` | no (romp-only) | The skill says that in a project that tracks files a shell write whose target is not a literal path (a variable, a `$(...)`, a glob or brace list the guard cannot expand) is refused, whatever the word would expand to, since the guard cannot tell which file it would write and reads no environment to find out, and asks for the path spelled out: the refusal romp's Bash-side guard (`hooks/romp-track-bash-guard.mjs`) emits since 2026-09-18, after a research session's report (2026-09-17, through the box admin) of a `cp` built from variables that landed raw on a tracked file while the same cp spelled out was refused. Other hosts have no such guard, so this stays romp's. |
