...
| `0009-skill-non-literal-target-refused.patch` | no (romp-only) | The skill says that in a project that tracks files a shell write whose target is not a literal path (a variable, a `$(...)`, a glob or brace list the guard cannot expand) is refused, whatever the word would expand to, since the guard cannot tell which file it would write and reads none of the agent's variables to find out (`HOME` alone, so a leading `$HOME/` is read as `~/` is), with one exception, a target whose only expansions are `$$`, `$RANDOM` or `$SECONDS` (the last two not inside a script handed to `sh`, where they are ordinary variables; `$BASHPID` never, since zsh lets a command assign it) at an absolute path outside the project, and asks for the path spelled out: the refusal romp's Bash-side guard (`hooks/romp-track-bash-guard.mjs`) emits since 2026-09-18, after a research session's report (2026-09-17, through the box admin) of a `cp` built from variables that landed raw on a tracked file while the same cp spelled out was refused, as bounded by that PR's round-1 review the same day and corrected by its round 2. Other hosts have no such guard, so this stays romp's. |
