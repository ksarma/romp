...
command names or may fill in the name in any other form. These write forms are
not modelled and still reach a tracked file: rsync; awk with a redirect inside
...
project (a `..` inside the value could climb into a project; from a cwd in a
tracked project the same word is refused as not literal). Whichever way you
