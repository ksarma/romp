...
OFFERED 2026-09-07: offered upstream as their PR #970 (2026-09-07, label feature, head d64ac698), together with the viewer-title chip (`viewer-title-session-name`). PARTIAL as approved: only the cross-document quote-to-composer seed (commit `3ac26e30`) rode; the pane itself (`_files_page`, `/files`, `files.ts`, `fileLinkPane`) stays home.

MERGED 2026-09-07: their PR #970 merged as-is (merge 26219f4c, head e8da8022; two review nits folded before the merge). PARTIAL as offered: only the cross-document quote-to-composer seed (commit `3ac26e30`) rode, together with the viewer-title chip (`viewer-title-session-name`); the pane itself (`_files_page`, `/files`, `files.ts`, `fileLinkPane`) stays home.
