...
status: candidate
where: kernel/kernel.py (_LANDING_APIH_JS: load, histHTML, descText, winRow, transRows, anchor, show, open, the cell's listeners; the tip stylesheet; _sdk_locked passes boot_at; the route's bootAt to the millisecond), kernel/sdk_backend.py (SdkBackend boot_at, handed to ApiHealth; ApiHealth._seed clamps the stamp past the restored tail), docs/reference.md "The bottom bar's indicator" and the payload's bootAt field, docs/guide.md; tests tests/test_api_health_hover.py, tests/test_api_health_hover_browser.py, ui/webview/api-health-hover.test.ts, tests/test_api_health_rail.py (three pins rewritten), tests/test_kernel_auth_hardening.py (the bootAt/started assertion)
added: 2026-09-08
pr:
tier: feature
offered:
