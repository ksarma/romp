...
test('decision 51 records the report, the yes, the three fields, the rules in order, the third field\'s reason and the tests', () => {
  ...
  assert.ok(decisions.includes("the rules run in order: the count of copies unchanged since the fields were written, the ordinal's copy, confirmed; else exactly one copy under the stored heading path, that copy, confirmed; else the copy nearest the position, a guess, as before."));
...
test('the contract names the three fields beside anchorAt with their rule, and the host writes them at creation, refreshes them with the position and carves them out of the decisions\' check', () => {
  ...
  assert.ok(contract.includes('the ordinal\'s copy while the count of copies is unchanged, else the one copy under the stored heading path, confirmed either way; else the nearest copy to the position, a guess. Every reply carries the verdicts (`placed`, per comment id), and the panel paints a confirmed copy plainly. The other editors and the CLIs write the whole object back, so the six fields survive them.'));
...
test('the painting paragraph and the note state the confirmed paint and the words, and the panel paints a confirmed copy plainly from the status and ends the guessed copy\'s words with the sentence', () => {
  assert.ok(ux.includes('Since the tie-break (2026-09-11, decision 51) the status carries the host\'s verdict for every such comment (`placed`): a copy the host confirmed from the fields stored with the comment (the ordinal while the count of copies is unchanged, else the one copy under the stored heading path) is the painter\'s hint in place of the stale position (`placedAt`), so it is painted as the chosen one, with no dashed ring and no tag; a guessed verdict, or none, paints as before, and the card\'s words end by saying how to confirm the copy.'));
