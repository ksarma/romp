...
test('the save\'s setter is recorded on scrollCard\'s path as the verification review had it; since decision 43 landSaved sets the focus itself, with no gesture before it, and scrollCard keeps its setter for the saved line\'s click', () => {
  ...
  assert.ok(/private landSaved\(c: Composer, had: Set<string>, r: Status, note: string\): boolean \{\n\s*const saved = [^\n]*\n\s*if \(saved === null\) return false;\n\s*const key = this\.cardKey\(saved\);\n(?:\s*\/\/[^\n]*\n)*\s*if \(this\.margin && \(!this\.composer \|\| this\.composer === c\)\) this\.focusOn\(key\);/.test(panel), 'landSaved makes the card the focus with no gesture of its own before it, while no composer is open or the saving one is (a pin on where the code lives; the focus\'s condition is executed by file-comments-margin-fixes.test.ts "a reply open on its card while another comment\'s reply lands…", red when any landing takes the focus, and "a reply\'s save out, its box cancelled, then the landing…", red when a landing takes it only for the saving composer still open) (the scroll it once made through scrollCard went with decision 43)');
