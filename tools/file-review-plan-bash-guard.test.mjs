...
test('decision 47 states what passes, and the hook agrees: reads, opaque commands, non-text files, no ROMP_SID', () => {
  ...
  assert.ok(hook.includes("case 'xargs': sawOpaqueCommand = true; break;") && hook.includes("case 'eval': case 'trap': {") && hook.includes("if (unread) { if (name === 'eval') sawOpaqueCommand = true; break; }"), 'xargs is opaque; since round 6\'s second commit eval and trap with text the resolver reads are scripts of this shell, and eval with text it does not read stays opaque');
...
test("decision 47, the hook header and the ledger record round 5's addendum: the conditional and timed groups, the compound body's opener, brace body and one-command body, the coproc frame, the freeze narrowed to an unwrapped plain-sequence readonly with no option word, the declarations dash lacks, and the census's stated reach", () => {
  ...
  assert.ok(hookFlat.includes(RULE) && hook.includes('function splitAtClosers(segments) {') && hook.includes('return { segments: splitAtClosers(segments), opaque };'), 'the lexer states the rule and cuts every segment through it');
...
test("round 6, fourth commit: decision 47 and the hook header record the descriptor feed, the exec feed, the alias body, the bound path, the compound producer, the option terminator, the head candidates, the IFS rule, the fed substitution, the sed file, the valued names and env's operand, the hook has the functions and tables that carry them, and the property's fourth class names the head candidates", () => {
  ...
  for (const fn of ['fdOfName', 'execFeeds', 'fedTexts', 'noteCandidate', 'candidateName', 'readValuedWords', 'SCRIPT_VALUED_NAMES', 'STARTUP_FILE_NAMES', 'sedFileBodies', 'mayReadStdin', 'ifsNamed']) assert.ok(hook.includes(fn), `the hook has ${fn}`);
  assert.ok(hook.includes("if (s.fd != null && s.fd !== '0' && s.fd !== fd) continue;"), 'textsOf reads a `<` on the descriptor the operand names (pinned by execution in the rows test)');
