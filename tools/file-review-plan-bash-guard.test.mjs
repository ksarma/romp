...
test('decision 47 records the fifth commit, each item tied to its hook function', () => {
  ...
  assert.ok(d47.includes('`unreadableExpandedNames`') && hook.includes('function unreadableExpandedNames(segments)') && hook.includes("unreadableNames.has('PWD')") && hook.includes("unreadableNames.has('OLDPWD')") && hook.includes("kind: 'namedExpansion'"), 'B2 as ruled: an expanded name the command names or may fill in is not read, and the refusal says why');
...
test('decision 47 and the hook header record the seventh pass: the readability rule as one predicate, and no claim that the attacker filed nothing', () => {
  ...
  for (const fn of ['plainSequence', 'plainValue', 'recordPlainWord', 'recordSegment', 'taintWord', 'identifierTokens', 'INERT_DECLARATION_FLAGS', 'refTargets', 'homeUnreadableNow']) {
