...
test('decision 47 states what passes, and the hook agrees: reads, opaque commands, non-text files, no ROMP_SID', () => {
  ...
  assert.ok(hook.includes("case 'xargs': sawOpaqueCommand = true; break;") && hook.includes("case 'eval': case 'trap': {") && hook.includes("if (unread) { if (name === 'eval') { sawOpaqueCommand = true; if (positionals !== null) rebindHere(seg, idx, cmd, '`eval`', UNKNOWN_POSITIONALS,"), 'xargs is opaque; since round 6\'s second commit eval and trap with text the resolver reads are scripts of this shell, and eval with text it does not read stays opaque (its bind of the list to values not read goes through THE BIND\'S FRAME since round 7\'s nineteenth commit)');
...
test('decision 47 records the fifth commit, each item tied to its hook function', () => {
  ...
  assert.ok(d47.includes('`aliasSourceInPlay`') && hook.includes('function aliasSourceInPlay(why, memo)') && hook.includes('return { targets, opaque: opaque || sawOpaqueCommand, unresolved, links, dir, unknownDir, unknownWhy, oldDir, moved: movedAny || dir !== ctx.dir || unknownDir !== !!ctx.unknownDir, positionals, positionalsWhy };'), 'M3 and B1: the links returned, the alias source asked');
...
test("decision 47, the hook and the prose surfaces record the fifth addendum's third fix-up: the six rules (the unquoted body, the resolved substitution, the default word, zsh's `=(cmd)`, the consumer's stdin, the split operand) at their homes and on decision 47 in the same words, the functions, the two matrix fixtures with their populations, the residuals named with the brace-body closer on the header, the decision and the README, and the three surfaces' sentence", () => {
  ...
  assert.ok(hook.includes("stdin: stdin != null ? stdin : (walkIdx >= 0 ? stdinBodies(walkIdx) : inheritedStdin),") && hook.includes("for (const body of stdinBodies(idx, sh.fd)) recurse(body, name, undefined, '', []);"), 'a -c script inherits its caller\'s stdin; a script fed on stdin passes nothing on (the descriptor the operand names is read too since round 6\'s fourth commit)');
...
test("round 6, twelfth commit: decision 47 and the hook header record the subscripted positional, the vanished head text, the stale positional candidate, the body's own list and the option flags; the hook has the constants and functions that carry them; the fifth class names the command substitution conduit on every surface", () => {
  ...
  for (const fn of ['const ZSH_POSITIONAL_SUBSCRIPT = ', 'const NEVER_EMPTY = ', 'const vanishedHeadTexts = (w) => {', "const candKnown = !/^[0-9@*]+$/.test(name)", "if (name === 'set' && positionalsApply())"]) assert.ok(hook.includes(fn), `the hook has ${fn.trim()}`);
...
test("round 6, thirteenth commit: decision 47 and the hook header record the empty positional, the stale positional candidate, the known set and the subscripted positional beyond one numeric index as rows under the positional-model class with the table's count; the class stands on every surface the property stands on and in RESIDUAL_CLASSES; the table holds its 152 rows and the three witnessed members", () => {
  for (const [name, text] of [['decision 47', d47], ['the hook header', hook]]) {
    const flat = text.replace(/\/\//g, ' ').replace(/\s+/g, ' ').toLowerCase();
  ...
  for (const [name, text] of Object.entries(surfaces)) assert.ok(text.replace(/\/\//g, ' ').replace(/\s+/g, ' ').includes(CLASS), `${name} states the ninth class`);
  const guardTest = read('tools', 'romp-track-bash-guard.test.mjs');
  assert.ok(guardTest.includes("  'a positional the resolver reads at the word by a model the shell does not keep': '"), 'RESIDUAL_CLASSES carries the class (the table test holds the header gloss equal to it)');
  const under = (guardTest.match(/^\s*\['RT-(ep|sp|sb|zs)-[^']+', 'a positional the resolver reads at the word by a model the shell does not keep', /gm) || []);
  // round 7's nineteenth commit (the reviewer's extra5-1): the fifteen sb rows whose `set` stood in a subshell, a pipeline, a background job, an
  // untaken body, a list or behind a wrapper (13 RT-sb-list-*, RT-sb-head-subshell, RT-sb-head-if-false) are refused now and pinned as refusals in
  // the guard's test ("round 7, nineteenth commit, the rows"), so the class holds 137 rows, 16 of them sb (the name rows the known set still reads)
