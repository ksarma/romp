...
test('decision 47 records the fifth commit, each item tied to its hook function', () => {
  ...
  assert.ok(d47.includes('B2 (the second commit') && hook.includes('const resolveWord = (w) =>') && hook.includes('const recordAssignments = (seg, idx, cmd) =>') && hook.includes('const valueOf = (name) =>'), 'B2 as ruled: the values the guard can read are resolved');
