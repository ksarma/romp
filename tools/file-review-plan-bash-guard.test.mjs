...
test('decision 47 states what passes, and the hook agrees: reads, opaque commands, non-text files, no ROMP_SID', () => {
  ...
  assert.ok(hook.includes("case 'xargs': sawOpaqueCommand = true; break;") && hook.includes("case 'eval': case 'trap': {") && hook.includes("if (unread) { if (name === 'eval') { sawOpaqueCommand = true; if (positionals !== null) bindPositionals(UNKNOWN_POSITIONALS,"), 'xargs is opaque; since round 6\'s second commit eval and trap with text the resolver reads are scripts of this shell, and eval with text it does not read stays opaque');
...
test("round 6, thirteenth commit: decision 47 and the hook header record the empty positional, the stale positional candidate, the known set and the subscripted positional beyond one numeric index as rows under the positional-model class with the table's count; the class stands on every surface the property stands on and in RESIDUAL_CLASSES; the table holds its 152 rows and the three witnessed members", () => {
  ...
  assert.equal(under.length, 152, `the class has its rows in the table (${under.length})`);
  for (const [prefix, n] of [['ep', 30], ['sp', 31], ['sb', 31], ['zs', 60]]) assert.equal(under.filter((l) => l.includes(`['RT-${prefix}-`)).length, n, `the ${prefix} population has ${n} rows`);
