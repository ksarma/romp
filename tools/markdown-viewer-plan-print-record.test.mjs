...
test('P7: bodyReady classes each child against three closed lists and answers not in for an unknown child, the PDF kind excepting the loader; the node census derives the seated roots from file-view.ts by the command the section names and holds the unknown side; the nine roots the section names are the lists\' union', () => {
  ...
  assert.ok(census.includes('type SeatRead = { in: string; on: string; via: string; is: string; decl?: string };') && census.includes('const bindingOf = (id: ts.Identifier): { decl: string; bindingAt: number } => {') && census.includes('where SEATS_READ_BY_HAND read `" + (entry.decl ?? "no binding") + "`: the receiver is not the one read by hand') && census.includes('covers seats on " + seen.size + " bindings of " + e.on + " (lines "'), 'an entry names the declaration its receiver is bound to, and a seat bound elsewhere or an entry covering two bindings is refused');
  assert.ok(census.includes('for (const e of SEATS_READ_BY_HAND) { const s = live.sites.find((x) => x.fn === e.in && x.on === e.on && x.via === e.via)!; assert.equal(e.decl, s.decl,'), 'every entry\'s binding is held to the live site\'s');
  assert.ok(census.includes('an expression the census cannot resolve to a root (" + (e as Error).message + ")') && !census.includes('assert.throws(unresolvable'), 'the resolver\'s failure is a refusal with the line, not a throw (the round-6 review\'s census-4)');
  assert.ok(census.includes('on: flat(r.getText(sf)), via, text: text(n)') && !census.includes('r.getText(sf).replace(/\\s+/g, "")'), 'the receiver is spelled with single spaces, not with its whitespace removed (the round-6 review\'s census-5)');
  // the census header's own roster sentence, pinned (the round-6 review's census-3: rewriting it to the old default kept every pin green)
  ...
  assert.ok(census.includes('const SEATS_READ_BY_HAND: SeatRead[] = [') && census.includes('type SeatRead = { in: string; on: string; via: string; is: string; decl?: string };'), 'the table: a site is its function, receiver and form, with what the receiver is and, for a bare name, its binding (round 7)');
  ...
  assert.ok(P7.includes('a method by any other name fails with its line') && P7.includes('so an entry reads ONE binding and a second declaration of a listed name inside the entry\'s function fails with its line') && P7.includes('a member read by a computed name and stored (`const f = md[m]`) passes only as a site listed (`INDEX_READS_BY_HAND`)') && P7.includes('the form axis now passes a listed name and refuses every other with its line, each entry names its binding, the resolver\'s failure is a refusal with the line'), 'the record states the form axis\'s rule and its history (round 7: the round-6 review\'s census-1, census-2, census-4)');
  ...
  for (const form of ['body?.append(x)', 'body[\\"append\\"](x)', 'body.append?.(x)', 'body.append.call(body, x), .bind and .apply', 'a bare body.append', 'body.classList.add(...) still passes', '(body).append(x)', '(body as HTMLElement).append(x)', 'an alias const b = body', '[body].forEach(...)', 'Element.prototype.append.call(body, x)', 'a helper handed the body', 'a column-0 // comment or after ;//', 'a listed root\'s seat moved into one kept the census green',
    ...
    // round 7: the round-6 review's census-1 (the form axis), census-2 (the binding), census-4 (the spread), census-5 (the spelling)
    'a seating method reached through call, bind or apply on md or md.parentElement', 'Reflect.apply(md.append, ...)', 'a bound seat', 'a Range\'s insertNode', 'setHTMLUnsafe', 'moveBefore', 'a method by a name the census does not list', 'a bare read of md.append or md[\\"append\\"]', 'a member read by a computed name and stored', 'a parenthesised callee', 'Object.assign(md, { innerHTML })', 'Reflect.set(md, \\"innerHTML\\", ...)', 'a block\'s const main = md.parentElement! and a callback\'s parameter named main seated under the entry for openFileView\'s main', 'a second seat on the listed binding passes', 'a spread into body.append threw with no line', 'a receiver with a cast inside is spelled with its spaces']) assert.ok(mutant.includes(form), 'the mutant case names the planted form ' + form);
...
test('Open points 6 and 7 record two pre-existing gate observations, each against the gate\'s source: the origin judgment with the hostname key, and a redirect the gate never reads', () => {
  ...
  // and in ONE place (the round-6 review's copies-2, 2026-09-20: the pointers called it the one full statement while loadGatedHost's doc carried a second): the full
