 ...
 *  string is mid-line and stays). Two comment forms stay with the code, by the same rule: a `//` that no space precedes
 *  (`;// ...`) and a block comment opening mid-line (one that follows a statement on its line); a pin that reads through this says it is met by a
 *  statement and not by a comment of the two stripped forms, no more (the author's closing pass over round 8, the verifiers'
 *  r8v-F-G-H-I-2: widening the pattern to those forms would mis-parse a `//` after a colon in a URL literal and a `/*` in a
 *  regex, the reason ui/test-code-only.ts gives for its own shape). */
const code = (src) => src.replace(/^[ \t]*\/\*[\s\S]*?\*\//gm, '').replace(/^\s*\/\/[^\n]*$/gm, '').replace(/[ \t]\/\/[^\n]*/g, '');
/** `src` with a backslash line continuation joined and its whitespace runs collapsed to one space, so a text wrapped across
 *  lines, inside a string literal or out of it, reads as one. Run AFTER `code`: collapsing first puts the whole file on one
 *  line, and the trailing-`//` rule then strips everything after the first comment (the round-7 review's regression-2: a pin
 *  that read the raw text was satisfied by a copy wrapped across a newline and a comment marker; the author's closing pass
 *  over round 8: a literal continued with a backslash escaped the collapse). A literal built from parts by concatenation is
 *  outside what any read of this text can find, and the pins that read it say so. */
const flat = (src) => src.replace(/\\\n/g, '').replace(/\s+/g, ' ');
/** `s` as a regex source matching itself. */
const escapeRe = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
/** The sentence beside Print anyway, derived from its one test-side home, ui/webview/file-print-fixtures.ts (a module the
 *  product never imports; the pins below hold that), never spelled here: a fixture that no longer exports it as one string
 *  literal refuses at load. */
const ANYWAY_SENTENCE = (() => {
  const m = /^export const ANYWAY_SENTENCE = "([^"\n]+)";$/m.exec(read('ui', 'webview', 'file-print-fixtures.ts'));
  assert.ok(m, 'ui/webview/file-print-fixtures.ts exports ANYWAY_SENTENCE as one double-quoted string literal');
  return m[1];
})();
...
test('P2: the wait, its count, the eager flip and the ask cover the pictures that reach the paper: each of the three collections is filtered by printable, the browser\'s answer included, the egress leg executes the shared-host fold with the folded route parked and with both routes answering, and the node test executes the three collections', () => {
  ...
  assert.ok(collect.includes('body.querySelectorAll("image").forEach((im) => { if (!svgReachesPaper(im, body)) return;'), 'the svg image element, through svgReachesPaper: the container walk (round 7) and, under a pattern, a mask or a marker, the reference to it (round 8)');
  assert.equal([...collect.matchAll(/querySelectorAll\(/g)].length, 3, 'three collections, no fourth outside the rule');
  assert.equal([...collect.matchAll(/if \(!(printable\(\w+\)|svgReachesPaper\(\w+, body\))\) return;/g)].length, 3, 'each filtered: the img and the video by printable, the svg image by svgReachesPaper, which is printable through rendering containers and the referrer\'s printable through a referenced one');
  const reach = between(flow, 'function svgReachesPaper(el: PrintableNode & SvgWalkNode, body: ParentNode, depth = 0): boolean {', '\n}');
  assert.ok(reach.includes('if (inRenderingSvg(el)) return printable(el);'), 'under rendering containers the rule is printable, the browser\'s answer read on the image itself');
  assert.ok(reach.includes('if (!container || !REFERENCED_CONTAINERS.includes(container.localName) || typeof container.getAttribute !== "function" || depth >= REFERENCE_DEPTH) return false;') && flow.includes('const REFERENCED_CONTAINERS: readonly string[] = ["pattern", "mask", "marker"];'), 'under any other container the rule is off the paper unless the container is a pattern, a mask or a marker with an id and the depth is within the cap');
  assert.ok(reach.includes('&& svgReachesPaper(r, body, depth + 1));') && flow.includes('const REFERENCE_ATTRS: readonly string[] = ["fill", "stroke", "mask", "marker-start", "marker-mid", "marker-end"];'), 'a referrer through fill, stroke, mask or one of the three marker longhands reaches the paper by the same rule, so the browser\'s answer is read on it (the shorthand `marker` is no presentation attribute and left the list in the author\'s closing pass over round 8)');
  assert.ok(reach.includes('const ids = paintingIds(container, id, body);') && reach.includes('return ref !== null && ids.has(ref);') && between(flow, 'function paintingIds(container: SvgWalkNode, id: string, body: ParentNode): Set<string> {', '\n}').includes('if (container.localName !== "pattern" || typeof body.querySelectorAll !== "function") return ids;'), 'a pattern inheriting the container\'s content through href is an alias of it, for a pattern alone (the author\'s closing pass over round 8)');
  ...
  assert.ok(P2.includes('An svg `<image>` is decided by `svgReachesPaper`: the container walk (`inRenderingSvg`, the test `shows` runs below a figure\'s root) and, where the walk meets a `<pattern>`, a `<mask>` or a `<marker>`, the reference to it. The wait collects a picture when it can paint on the paper, and the test of that is measured ink, never a rect the engine reports: the browser\'s answer for an image inside `<defs>`, a `<symbol>`, a `<clipPath>`, a `<mask>`, a `<pattern>` or a `<marker>` is the engine\'s whether the container paints or not (Chromium reports no rect and Firefox and WebKit one), so it is never read on such an image'), 'P2 states the property (measured ink, never an engine rect), the per-engine reading and the walk');
  assert.ok(P2.includes('one inside a `<pattern>`, a `<mask>` or a `<marker>` paints in every engine when a printable svg element that reaches the paper names the container by `url(#id)` in fill, stroke, mask or a marker property, and is collected when such a referrer exists, the browser\'s answer read on the referrer'), 'P2 states the reference rule');
  assert.ok(!P2.includes('and no engine paints it') && !flow.includes('is not a picture the print shows in any engine, whatever\n *  rect the engine reports for it (rendered') && !/which no engine paints[:;,]/.test(read('ui', 'webview', 'file-print.ts')), 'the universal over the six containers is gone from P2 and the module: it was false of a referenced pattern, mask or marker in every engine (the round-7 review\'s ui-3)');
  assert.ok(between(read('ui', 'webview', 'file-print.ts'), '/** Whether every SVG ancestor of the svg element `el`', '\nfunction inRenderingSvg').includes('An svg <image> inside <defs>, a <symbol>, a <clipPath>\n *  or <metadata> is not a picture the print shows in any engine') && between(read('ui', 'webview', 'file-print.ts'), '/** Whether every SVG ancestor of the svg element `el`', '\nfunction inRenderingSvg').includes('Chromium reports no rect for an image inside\n *  any of these containers, referenced or not, so the browser\'s own answer is read on the referencing element, never on the\n *  image.'), 'inRenderingSvg\'s docstring narrows the universal to the four containers and says where the browser\'s answer is read (a comment, read as one)');
  ...
  // round 8 (the round-7 review's cluster E): the reference case per engine, its expectation derived from the ink
  const fig = code(read('ui', 'webview', 'file-print-figure-browser.test.ts'));
  assert.ok(fig.includes('for (const engine of ENGINES) test("collectPictures in " + engine + " over sanitized bodies whose svg image stands inside a <pattern>, a <mask> or a <marker> that a rect or a line references by url(#id)'), 'the reference case runs once per engine');
  assert.ok(fig.includes('const paints = (c: RefCell): boolean => screen[c.i].pixels > 0 && key(screen[c.i]) !== key(screen[twinOf(c).i]);') && fig.includes('const expected = cells.filter((c) => paints(sibling(c))).map((c) => ORIGIN + c.url).sort();') && fig.includes('assert.deepEqual(got, expected, "collectPictures collects exactly the images of the containers whose picture reaches the paper in " + engine'), 'the case derives what is collected from the ink (a cell that inks other than its 404 twin) and holds the collection to it');
  assert.ok(fig.includes('served.filter((c) => c.spelling === "author" && paints(c))') && fig.includes('const clean = w.FVF.sanitizeMd(s.markup) as HTMLElement;'), 'the author-spelled reference through the sanitizer is the control, and every body goes through the sanitizer');
  assert.ok(fig.includes('reads[c.i].rendered !== RENDERED_READS[engine][c.container]') && fig.includes('reads[c.i].renderedReferrer !== true'), 'the case holds `rendered` on the image to the per-engine table and on the referrer to true, so the docstrings\' claim about where the answer is read is executed');
  assert.ok(TESTS.includes('and a reference case per engine over sanitized bodies whose svg image stands inside a pattern, a mask or a marker inside `<defs>` that a rect or a line references by `url(#id)`') && TESTS.includes('collectPictures collects exactly the images of the containers whose served cell paints, the twin of each among them, the author-spelled cells the control collected in no engine'), 'the Tests list names the reference case and what it holds');
...
test('P7: bodyReady classes each child against three closed lists and answers not in for an unknown child, the PDF kind excepting the loader; the node census derives the seated roots from file-view.ts by the command the section names and holds the unknown side; the nine roots the section names are the lists\' union', () => {
  ...
  assert.equal(seatSentence.length, 1, 'the seat-keyed table\'s size stands in P7 once, in the sentence\'s form (a sentence the pattern does not find is a loud failure here, never a default)');
  assert.deepEqual([seatSentence[0][1], seatSentence[0][2], seatSentence[0][3], seatSentence[0][5], seatSentence[0][6], seatSentence[0][7]], [String(entries), String(seats), WORDS[twice.length], WORDS[seats - entries], String(entries - tripleAtRound7), String(seats)], 'P7 states the seat-keyed table\'s size, its entries reading two seats, its rise over the round-7 triple count and the site population as derived from the census file (' + entries + ' entries over ' + seats + ' seats, ' + twice.length + ' reading two, a rise of ' + (entries - tripleAtRound7) + ')');
  ...
  assert.ok(census.includes('SEATS_READ_BY_HAND.length + " entries listed over " + SEATS_READ_BY_HAND.reduce((n, e) => n + (e.times ?? 1), 0) + " seats, one entry per seat spelling, `times` for a repeat), "'), 'the census case\'s diagnostic prints the entries and the seats the sentence names, its parenthesis stating the rule with its exemption (the round-7 review\'s extra6-3)');
  ...
  assert.ok(census.includes('test("P7\'s derived numbers are the tables this module runs: the seat-keyed table\'s entries, seats, entries reading two seats and the rise over the round-7 triple count') && census.includes('assert.deepEqual([sEntries, sSeats, sTwice, sExcess, sRise, sPopulation], [String(entries), String(seats), COUNT_WORDS[twice.length], COUNT_WORDS[seats - entries], String(entries - tripleAtRound7), String(seats)]'), 'PRESENCE: the census module carries the P7 derived-numbers case, by its title and its key assertion, executed under npm test; this is a text read through code() (met by a statement; a comment that opens its line or follows a space does not meet it, those being the two forms code() strips, and a `//` no space precedes or a block comment opening mid-line stays in the text, so a rename left in one of those would: the author\'s closing pass over round 8, the verifiers\' r8v-F-G-H-I-2)');
  ...
  assert.ok(P7.includes('a method by any other name fails with its line') && P7.includes('so an entry reads ONE binding and ONE seat: a second declaration of a listed name inside the entry\'s function fails with its line') && P7.includes('a second seat at a listed site is a second entry and fails until read, and a seat spelled byte for byte alike in two branches of one function declares `times` and is read at each, so one entry is one seat SPELLING') && P7.includes('a member read by a computed name and stored (`const f = md[m]`) passes only as a site listed (`INDEX_READS_BY_HAND`)') && P7.includes('the call axis now passes a listed name and refuses every other with its line, each entry names its binding, the resolver\'s failure is a refusal with the line'), 'the record states the call axis\'s rule and its history (the author\'s closing pass: findings census-1, census-2, census-4)');
...
test('Open points 6 and 7 record two pre-existing gate observations, each against the gate\'s source: the origin judgment with the hostname key, and a redirect the gate never reads', () => {
  ...
  const collect = between(flow, 'export function collectPictures(body: ParentNode, base: string, probe: (url: string) => Picture): Picture[] {', '\n}');
  assert.ok(!/filter|clip-path|paintRefs|cssUrls/.test(collect) && [...collect.matchAll(/probe\(/g)].length === 2, 'the collection awaits no paint reference\'s URL: two probe calls, the poster\'s and the svg image\'s, and no fourth collection; the same-document url(#id) svgReachesPaper reads (round 8) decides whether an svg image inside a pattern, a mask or a marker reaches the paper and mints no probe of its own, so open point 9 stands as recorded');
...
test('P2: the ask\'s line and the open-ended wait\'s line end with the button\'s own sentence, what Print anyway does, and the record says what a still-loading picture prints as, scoped to the shape measured (the round-4 review\'s HIGH 1)', () => {
  ...
  assert.ok(!flat(code(read('ui', 'webview', 'file-print-browser.test.ts'))).includes(ANYWAY_SENTENCE), 'the gated leg executes no literal of the sentence (comments stripped, a backslash continuation joined, then unwrapped: its header spells the sentence twice in comments wrapped across lines, which is documentation, not a read; a literal built by concatenation is outside this read): every read of it is anywayWords()');
  // the read's own shapes, executed (the author's closing pass over round 8, the verifiers' r8v-A-E-4): a flat literal, a template
  // literal wrapped across lines and a literal continued with a backslash are found; a comment copy is not; a concatenation is not
  const sentenceIn = (src) => flat(code(src)).includes(ANYWAY_SENTENCE);
  assert.ok(sentenceIn('x("' + ANYWAY_SENTENCE + '");'), 'a flat literal is found');
  assert.ok(sentenceIn('x(`' + ANYWAY_SENTENCE.replace(' any ', ' any\n    ') + '`);'), 'a template literal wrapped across lines is found');
  assert.ok(sentenceIn('x("' + ANYWAY_SENTENCE.replace(' any ', ' any \\\n') + '");'), 'a literal continued with a backslash is found (FAILS BEFORE the closing pass: the backslash stood between the halves)');
  assert.ok(!sentenceIn('// ' + ANYWAY_SENTENCE.replace(' any ', ' any\n// ') + '\nx();'), 'a comment copy wrapped across lines is not');
  assert.ok(!sentenceIn('x("' + ANYWAY_SENTENCE.replace(' any ', ' any " + "') + '");'), 'a literal built by concatenation is not: the limit the messages state');
  assert.ok(code(read('ui', 'webview', 'file-print.test.ts')).includes('the ask\'s line and the open-ended wait\'s line each end with the button\'s own sentence, what Print anyway does (FAILS BEFORE:'), 'the node test pins the words (a text read through code(): the author\'s closing pass over round 8 moved this read and the arity read below off the raw text, where a comment quoting the title met them)');
...
test('P2: the sentence beside Print anyway takes no count and names no picture, since a counted picture that lands while the line stands prints; the driver leg\'s case (15b) releases pictures under both lines and reads what prints, and the Tests list names it (the round-5 review\'s ui-1)', () => {
  ...
  inOrder(between(driver, 'test("(15b) a picture that lands while the line stands prints', '\n});'), ['await s.release([SLOW, SLOW2]);', '{ phase: "stalled", line: STALLED_TWO, buttons: [ANYWAY_WORDS, KEEP_WORDS], prints: 0 }', 'const askLine = b.line!;', 'const askPrint = await paper(page, "print");', '{ parkedComplete: true, sizedComplete: true, sized: { w: 120, h: 120 } }', 'await page.click(ANYWAY_BTN);', 'assert.deepEqual(named(p[0].incomplete), [], "every <img> complete at the print', 'assert.ok(askLine.endsWith(" " + anywayWords())', 'assert.ok(askLine.endsWith(" " + ANYWAY_SENTENCE)', 'await page.click(KEEP_BTN);', 'await s.release([SLOW]);', '{ phase: "preparing", line: waitingWords(2), buttons: [ANYWAY_WORDS], prints: 0 }', 'const waitPrint = await paper(page, "print");', '{ parkedComplete: true, sizedComplete: false, sized: { w: 120, h: 80 } }', 'assert.ok(waitPrint.parked && waitPrint.parked.w > 0 && waitPrint.parked.h > 0,', 'await page.click(ANYWAY_BTN);', 'assert.deepEqual(named(p[0].incomplete), ["sized"]', 'assert.ok(waitLine.endsWith(" " + anywayWords())', 'assert.ok(waitLine.endsWith(" " + ANYWAY_SENTENCE)', 'await page.click(KEEP_BTN);', 'await s.release([SLOW2]);', 'const sizedPrint = await paper(page, "print");', '{ parked: { w: 0, h: 0 }, parkedComplete: false, sized: { w: 120, h: 120 }, sizedComplete: true }', 'await page.click(ANYWAY_BTN);', 'assert.deepEqual(named(p[0].incomplete), ["parked"]', 'await s.release([SLOW, SLOW2]);', 'await printsReach(page, 1);'], 'case (15b): both released under the ask, both pictures read complete in their boxes under print media while the ask stands, then the print with every picture complete and the line\'s sentence held to anywayWords() and to the fixture ANYWAY_SENTENCE; the unsized one released under Keep waiting, read complete with a box while the sized one is still loading in its declared box (the author\'s closing pass: finding engines-1), then the print with the sized one alone still loading; the sized one released under Keep waiting, the unsized one read as a 0 by 0 box still loading while the line stands (the round-6 review\'s cluster D), then the print with it alone still loading; both released under Keep waiting, the settle\'s print');
  const driverCode = code(driver);
  assert.ok(!flat(driverCode).includes(ANYWAY_SENTENCE), 'the driver leg executes no literal of the sentence (comments stripped, a backslash continuation joined, then unwrapped; a literal built by concatenation is outside this read): its reads are anywayWords(), the wiring, and the imported fixture ANYWAY_SENTENCE, the fixed compare (the round-6 review\'s cluster D: a hand-spelled copy was the only red at the retired words, and it measured the copy and not the print; the round-7 review\'s cluster A: a compare against the product\'s own function alone reds at no wording, so the fixed text lives in a fixture the product never writes)');
  assert.ok(driverCode.includes('import { ANYWAY_SENTENCE } from "./file-print-fixtures";'), 'the driver leg imports the fixture');
  const fifteenB = between(driverCode, 'test("(15b) a picture that lands while the line stands prints', '\n});');
  assert.equal([...fifteenB.matchAll(/assert\.ok\((askLine|waitLine)\.endsWith\(" " \+ ANYWAY_SENTENCE\)/g)].map((m) => m[1]).join(','), 'askLine,waitLine', 'case (15b) holds the rendered line to the fixture at both of its reads, the ask\'s and the open-ended wait\'s, beside the wiring compares and the print reads');
  ...
  // the round-7 review's regression-3: the census module's spellings are counted as a substring over the comment-stripped,
  // unwrapped text, and a RULE holds them, not a typed count: every line carrying the sentence is an expectation on a words
  // function; the count is derived and stated in the message
  const censusCode = code(read('ui', 'webview', 'file-print.test.ts'));
  const carrying = censusCode.split('\n').filter((l) => l.includes(ANYWAY_SENTENCE));
  const spelled = [...flat(censusCode).matchAll(new RegExp(escapeRe(ANYWAY_SENTENCE), 'g'))].length;
  const expectation = new RegExp('^\\s*assert\\.equal\\((anywayWords|waitingWords|stalledWords)\\([^)]*\\), "[^"]*' + escapeRe(ANYWAY_SENTENCE) + '"\\);');
  assert.ok(spelled > 0 && spelled === carrying.length, 'the census module spells the sentence ' + spelled + ' times (comments stripped, then unwrapped), one per carrying line (a string literal does not wrap): ' + carrying.length + ' carrying lines');
  assert.deepEqual(carrying.filter((l) => !expectation.test(l)), [], 'every one of the ' + spelled + ' carrying lines in the census module is an expectation on a words function, assert.equal(<wordsFn>(...), "..."): the count-prefixed lines of waitingWords and stalledWords (anywayWords itself is held to the fixture, so none is its)');
  assert.ok(censusCode.includes('assert.equal(anywayWords(), ANYWAY_SENTENCE,'), 'the census module holds anywayWords() to the fixture, not to a literal of its own');
  assert.equal([...flat(flow).matchAll(new RegExp(escapeRe(ANYWAY_SENTENCE), 'g'))].length, 1, 'the product module spells the sentence once, in anywayWords (comments stripped)');
  assert.equal([...flat(code(read('ui', 'webview', 'file-print-fixtures.ts'))).matchAll(new RegExp(escapeRe(ANYWAY_SENTENCE), 'g'))].length, 1, 'and the fixture once: the product\'s home and the test side\'s, and every other read derives from one of them');
  assert.deepEqual(fs.readdirSync(path.join(REPO, 'ui', 'webview')).filter((f) => f.endsWith('.ts') && !f.endsWith('.test.ts') && code(read('ui', 'webview', f)).includes('file-print-fixtures')), [], 'no module but a test imports the fixture: the product never writes the sentence\'s test-side home');
  ...
  const entry15b = between(TESTS, 'Since the round-5 review (2026-09-20), case (15b)', 'Since the round-7 fixes');
  assert.ok(entry15b.includes('the line held to a fixed test-side copy of the sentence the product never writes (`ANYWAY_SENTENCE`, ui/webview/file-print-fixtures.ts, the constant the census module\'s pin imports too; FAILS at the round-5 wording, "Print anyway prints without them.", at that compare, inside the case whose other reads are the print\'s boxes under print media and the print stub'), 'the (15b) entry says which compare reds at the round-5 wording and what stands beside it');
  assert.equal(/FAILS/.test(entry15b), fifteenB.includes('.endsWith(" " + ANYWAY_SENTENCE)'), 'the (15b) entry claims a red at the round-5 wording exactly when the case holds the line to the fixture (the round-7 review\'s ui-2: the entry credited a red the case could no longer produce)');
  assert.ok(/FAILS/.test(entry15b) && !entry15b.includes('FAILS BEFORE: the line read'), 'the claim stands, as the compare\'s red, and the round-6 clause that credited the deleted copy is gone');
  assert.ok(censusCode.includes('assert.equal(anywayWords.length, 0, "the sentence takes no count'), 'the node test pins the arity (a text read through code())');
  // round 7 (the round-6 review's cluster D): the case asserts the print, and the plan says so in P2 and the Tests list
  assert.ok(P2.includes('Since the round-7 fixes (the round-6 review\'s cluster D, 2026-09-20) that case reads the PRINT under print media while each line stands, the released picture complete with a non-zero box and the unreleased one still loading in the box case (15a) measured for it, and holds the press\'s print to exactly the unreleased pictures incomplete; the sentence beside the button is read off the rendered line and held to `anywayWords()`, the wiring (the line shows what the function says), and to a fixed test-side copy the product never writes, `ANYWAY_SENTENCE` in ui/webview/file-print-fixtures.ts, the one constant the census module\'s text pin and the leg both import, so a wording change reds the case at that compare beside the print reads'), 'P2 says the case reads the print and holds the sentence to the product and to the fixture');
  assert.ok(P2.includes('what the compare proves is that the wording reaches the rendered pre-press DOM, never that it is printed'), 'P2 says what the compare proves and what it does not (the sheets hide the line under print media)');
...
test('the Tests list\'s figure-leg entry names the rows the leg holds per engine, per engine, from the leg\'s built table (the round-6 review\'s cluster E, regression-1 and extra7-1, 2026-09-20: the entry stated single answers for the svg link at display contents, the six never-rendering containers and opacity 1e-9 while the leg held them per engine, and a literal-sentence pin held the stale sentences; the entry now says what the leg measures and carries the derived list): the frame and the entries are read here by text, and the list\'s equality with the table `shapes()` builds is file-print.test.ts\'s, under npm test, where the compiler is', () => {
  ...
  assert.ok(bullet.includes('`1e-9` inks nothing in any engine, and what each engine\'s root read, twin oracle and flow make of it is recorded per engine, below') && bullet.includes('the image inside defs, symbol, clipPath, mask, pattern, marker and metadata, referenced by nothing, inks nothing in any engine') && bullet.includes('Firefox and WebKit report a box for the image inside the six that never render their content, recorded per row, below') && bullet.includes('on a link (Firefox inks the image inside, Chromium and WebKit do not, recorded per engine, below)'), 'the three clauses the round named point at the per-engine list');
});
