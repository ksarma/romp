...
/** `src` with its whitespace runs collapsed to one space, so a text wrapped across lines reads as one. Run AFTER `code`:
 *  collapsing first puts the whole file on one line, and the trailing-`//` rule then strips everything after the first
 *  comment (the round-7 review's regression-2: a pin that read the raw text was satisfied by a copy wrapped across a
 *  newline and a comment marker). */
...
test('P2: the wait, its count, the eager flip and the ask cover the pictures that reach the paper: each of the three collections is filtered by printable, the browser\'s answer included, the egress leg executes the shared-host fold with the folded route parked and with both routes answering, and the node test executes the three collections', () => {
  ...
  assert.ok(reach.includes('&& svgReachesPaper(r, body, depth + 1));') && flow.includes('const REFERENCE_ATTRS: readonly string[] = ["fill", "stroke", "mask", "marker-start", "marker-mid", "marker-end", "marker"];'), 'a referrer through fill, stroke, mask or a marker property reaches the paper by the same rule, so the browser\'s answer is read on it');
...
test('P2: the ask\'s line and the open-ended wait\'s line end with the button\'s own sentence, what Print anyway does, and the record says what a still-loading picture prints as, scoped to the shape measured (the round-4 review\'s HIGH 1)', () => {
  ...
  assert.ok(!flat(code(read('ui', 'webview', 'file-print-browser.test.ts'))).includes(ANYWAY_SENTENCE), 'the gated leg executes no literal of the sentence (comments stripped, then unwrapped: its header spells the sentence twice in comments wrapped across lines, which is documentation, not a read): every read of it is anywayWords()');
