...
// what a text read cannot check. Synthetic: only the repo's own text.
...
test('P7: bodyReady classes each child against three closed lists and answers not in for an unknown child, the PDF kind excepting the loader; the node census derives the seated roots from file-view.ts by the command the section names and holds the unknown side; the nine roots the section names are the lists\' union', () => {
  ...
  assert.ok(P7.includes('the receiver\'s hold is by binding, not name (the maintainer\'s round-8 question, 2026-09-21') && P7.includes('two things stay keyed on text and are stated in the census header, the entry\'s `in`, a bare function name two function nodes can share'), 'P7 states the round-8 answer on the receiver binding and points at the census header for the residues');
  assert.ok(between(census, '// bodyReady classes a child it has never seen as UNKNOWN', '/** file-view.ts with its comments blanked').replace(/\n\/\/ /g, ' ').includes("THE RECEIVER'S HOLD IS BY BINDING, NOT NAME (the maintainer's round-8 question, 2026-09-21"), 'the census header states the binding hold');
  ...
  assert.deepEqual([sizes[0][1], sizes[0][2], sizes[0][3], sizes[0][5]], [String(args), String(argSeats), WORDS[argTwice.length], String(urls)], 'P7 states today\'s table sizes as derived from the census file (' + args + ' over ' + argSeats + ', ' + argTwice.length + ' reading two, ' + urls + ')');
  assert.equal(sizes[0][4], argTwice.map((m) => m[1] + "'s two `" + m[2] + '`').join(', '), 'the sentence names each entry reading two hand-offs, in table order');
  assert.ok(P7.includes('they list ' + args + ' argument sites over ' + argSeats + ' hand-offs, ' + WORDS[argTwice.length] + ' entries standing for two byte-identical hand-offs each (' + sizes[0][4] + '), and ' + urls + ' URL writes today, four numbers the record test counts from the census file by text and holds this sentence to, and the census module holds to the tables it runs and prints in its `second read:` diagnostic'), 'P7 states today\'s table sizes with the times clause, as derived from the census file');
  assert.ok(urlSeats === urls, 'no URL write is spelled alike twice at this head (a `times` would be read here if one were)');
  assert.ok(census.includes('second.handedNodes.length + " nodes handed to callees and " + second.elementsHanded.length + " elements handed to callees the file does not declare (" + ARGS_READ_BY_HAND.length + " entries listed over " + ARGS_READ_BY_HAND.reduce((n, e) => n + (e.times ?? 1), 0) + " hand-offs), " + second.urlWrites.length + " URL writes (" + URL_WRITES_READ_BY_HAND.length + " entries listed over "'), 'the census case\'s diagnostic prints the two tables\' sizes the sentence names');
  ...
  const mutantBody = between(census, 'test("the census refuses its unknown and derives its population, executed over mutants of file-view.ts\'s source:', '\n});');
...
test('Derivations and their unknown cases: the paragraph stands between P7 and the Tests, each answer it gives is the source\'s, and each case it names exists', () => {
  ...
  assert.deepEqual([...self.matchAll(/^import .* from '([^']+)';$/gm)].map((m) => m[1]).filter((s) => !s.startsWith('node:')), [], 'this module imports node\'s own modules alone');
  const selfCode = self.replace(/'(?:[^'\\\n]|\\.)*'|"(?:[^"\\\n]|\\.)*"/g, '""');   // string literals blanked, so this pin's own probes are not read as a road
  for (const road of ['create' + 'Require', 're' + 'quire(']) assert.ok(!selfCode.includes(road), 'this module opens no ' + road + ' road to node_modules (a require of the compiler failed the shell job at load, 2026-09-21)');
...
test('P2, P6, open point 8 and the guide carry the figure-level grant sentence: the restore is the whole figure\'s, so a remote URL inside a non-painting element of a painting figure is counted, named and fetched (the round-4 review\'s HIGH 2, a consent-text correction), and the code road is recorded as needing an owner', () => {
  ...
  const promised = AXES.replace(/^the /, '').replace(/ axes$/, '').split(/, | and /).sort();
  assert.deepEqual(declared, promised, 'the census header declares exactly the axes the promise names (' + declared.join(', ') + ')');
  assert.deepEqual(promised, ['argument', 'assignment', 'call', 'receiver', 'write'], 'and those are the five');
  for (const axis of ['THE CALL AXIS', 'THE ASSIGNMENT AXIS', 'THE RECEIVER AXIS', 'THE ARGUMENT AXIS', 'THE WRITE AXIS']) assert.ok(headerText.includes(axis + ':') || headerText.includes(axis + ' ('), 'the census header declares ' + axis);
  assert.ok(!censusHeader.includes('THE VERB AXIS'), 'the verb vocabulary is gone from the census');
  assert.ok(headerText.includes('a URL written from a value that is not a literal, or from a javascript: literal, passes only as a site URL_WRITES_READ_BY_HAND lists') && headerText.includes('by setAttribute or setAttributeNS under any name but an `aria-*` or `data-*` one') && headerText.includes('by a method of `location` (assign, replace, reload, any) or by `window.open`'), 'and the URL write rule the promise\'s write axis names, over the assignment, the setAttribute, the location-method and the window.open spellings (the verifiers\' census-1 and census-4)');
...
test('the post-filter index (the round-4 review\'s correctness-4, tests-7 and regression-3; the round-5 review\'s correctness-3, tests-2, regression-2 and extra7-1, one defect filed seven times; the round-6 review\'s tests-4: the shape space widened from one form): the two fails-before records stand in the figure leg by their exact text, once each and each followed by its FAILS BEFORE assertion, a named helper stands at each site the records replaced, and the census module\'s record list is this one; the census by syntax over the print test modules, which refuses every other chain with its module and line and says what it does not read, is file-print.test.ts\'s, under npm test, where the compiler is', () => {
  ...
  assert.ok(census.includes('function postFilterIndexChains(name: string, src: string): string[] {') && census.includes('for (const file of printTestModules()) {'), 'the census module walks the print test modules by syntax');
