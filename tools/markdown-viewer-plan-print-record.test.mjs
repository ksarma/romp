...
test('P7: bodyReady classes each child against three closed lists and answers not in for an unknown child, the PDF kind excepting the loader; the node census derives the seated roots from file-view.ts by the command the section names and holds the unknown side; the nine roots the section names are the lists\' union', () => {
  ...
  assert.ok(census.includes('on: flat(r.getText(sf)), via, text: text(n)') && !census.includes('r.getText(sf).replace(/\\s+/g, "")'), 'the receiver is spelled with single spaces, not with its whitespace removed (the branch\'s verification pass finding census-5)');
  ...
  assert.ok(census.includes('const SEATS_READ_BY_HAND: SeatRead[] = [') && census.includes('type SeatRead = { in: string; on: string; via: string; is: string; decl?: string };'), 'the table: a site is its function, receiver and form, with what the receiver is and, for a bare name, its binding (the author\'s closing pass)');
