...
test('P2: the wait, its count, the eager flip and the ask cover the pictures that reach the paper: each of the three collections is filtered by printable, the browser\'s answer included, the egress leg executes the shared-host fold with the folded route parked and with both routes answering, and the node test executes the three collections', () => {
  ...
  assert.ok(read('ui', 'webview', 'file-print.test.ts').includes('test("collectPictures reads the container walk for an svg image as well as the browser\'s answer (the round-7 fixes, 2026-09-20:'), 'the node case over namespaced stand-ins');
...
test('P7: bodyReady classes each child against three closed lists and answers not in for an unknown child, the PDF kind excepting the loader; the node census derives the seated roots from file-view.ts by the command the section names and holds the unknown side; the nine roots the section names are the lists\' union', () => {
  ...
  // the author's closing pass (findings census-1, census-2, census-4, census-5): the form axis is an allowlist; an entry names its binding; the resolver refuses with the line; the receiver keeps its spaces
  ...
  const header = between(census, '// bodyReady classes a child it has never seen as UNKNOWN', '/** file-view.ts with its comments blanked').replace(/\n\/\/ /g, ' ');
  ...
  assert.ok(header.includes('EVERY OTHER SEAT and every site-read call passes only as a SITE the census lists, read by hand') && header.includes('a method by ANY OTHER NAME fails the census with its line') && header.includes('A seating or site-read method READ WITHOUT BEING CALLED fails with its line'), 'the header states the form axis\'s rule');
  assert.ok(!header.includes('DEFAULT PASSES'), 'and not the old one');
  assert.ok(census.includes('const SEATS_READ_BY_HAND: SeatRead[] = [') && census.includes('type SeatRead = { in: string; on: string; via: string; seats: string; is: string; decl?: string; holds?: string; times?: number };'), 'the table: an entry is one seat, its function, receiver, form and what it seats, with what the receiver is and, for a bare name, its binding (round 7)');
  ...
  const seats = entries + [...tableSrc.matchAll(/, times: (\d+)[,} ]/g)].reduce((n, m) => n + Number(m[1]) - 1, 0);
  assert.ok(entries > 60 && seats >= entries, 'the table has more entries than the 60 the triple key held (' + entries + ' over ' + seats + ' seats)');
  assert.ok(P7.includes('the table went from 60 entries keyed on the triple to ' + entries + ' entries over ' + seats + ' seats at the round-7 head, a rise of ' + (entries - 60) + ' entries, which is what the triple hid; the three numbers after the 60 are derived at every run and never carried: the record test counts the table\'s entries and their `times` in the census file and holds this sentence to them, and the census case prints the same two in its `second read:` diagnostic, `node esbuild.js --tests` in vscode-extension and then `node --test out-tests/ui/webview/file-print.test.js`)'), 'P7 states the seat-keyed table\'s size and its rise as derived from the census file (' + entries + ' entries over ' + seats + ' seats, a rise of ' + (entries - 60) + '), once, with the command that prints them');
  ...
  // item 7's first run: the count is that run's fact, and the tables list the same sites today, counted here
  const args = [...between(census, 'const ARGS_READ_BY_HAND: ArgRead[] = [', '\n];').matchAll(/^  \{ in: "/gm)].length;
  const urls = [...between(census, 'const URL_WRITES_READ_BY_HAND: UrlWriteRead[] = [', '\n];').matchAll(/^  \{ in: "/gm)].length;
  assert.ok(args > 0 && urls > 0, 'the argument axis\'s tables list sites (' + args + ' arguments, ' + urls + ' URL writes)');
  assert.ok(P7.includes('refused ' + (args + urls) + ' sites (the count that run printed as its refusals, recorded here as that run\'s fact; the tables list the same ' + args + ' and ' + urls + ' sites today, and the record test counts their entries in the census file and holds these numbers to them): ' + args + ' nodes handed to five callees (') && P7.includes(') and ' + urls + ' URL writes ('), 'P7\'s first-run count is the sum of the two tables\' entries as listed today (' + args + ' + ' + urls + '), stated once with its attribution');
  assert.equal([...P7.matchAll(/refused (\d+) sites/g)].length, 1, 'the first-run count stands in P7 once');
  assert.ok(census.includes('const NON_SEATING_WRITES: Array<{ name: string; why: string }> = [') && census.includes('if (w.through === undefined && !NON_SEATING_WRITES.some((e) => e.name === w.name)) refused.push(') && census.includes('writes a member of `" + w.on + "` by a computed name') && !census.includes('the two such properties an element has'), 'the write axis is an allowlist with its default refusing, a computed name refused, and the two-name sentence is gone (round 7: cluster B)');
  ...
  for (const form of ['body?.append(x)', 'body[\\"append\\"](x)', 'body.append?.(x)', 'body.append.call(body, x), .bind and .apply', 'a bare body.append', 'body.classList.add(...) still passes', '(body).append(x)', '(body as HTMLElement).append(x)', 'an alias const b = body', '[body].forEach(...)', 'Element.prototype.append.call(body, x)', 'a helper handed the body', 'a column-0 // comment or after ;//', 'a listed root\'s seat moved into one kept the census green',
    ...
    // the author's closing pass: findings census-1 (the form axis), census-2 (the binding), census-4 (the spread), census-5 (the spelling)
    ...
    // round 7 (the round-6 review's clusters A to C, item 7, extra6-4): the seat key, the write axis, the reflection globals by binding, the argument axis and the string roads, a select's add
...
test('Derivations and their unknown cases: the paragraph stands between P7 and the Tests, each answer it gives is the source\'s, and each case it names exists', () => {
  ...
  assert.ok(TESTS.includes('Since the round-7 fixes (2026-09-20, after the round-6 review) it also derives the seat-keyed table\'s size and the argument axis\'s listed sites from the census file, the figure leg\'s per-engine rows from the table the leg builds, scans the whole section by slice and the ledger entry for the shape count, pins each of the two count sentences as one composition with the count inside it, and reads the post-filter census\'s shape space, saying what it does not read.'), 'the Tests list names what this module derives since round 7');
...
test('P2, P6, open point 8 and the guide carry the figure-level grant sentence: the restore is the whole figure\'s, so a remote URL inside a non-painting element of a painting figure is counted, named and fetched (the round-4 review\'s HIGH 2, a consent-text correction), and the code road is recorded as needing an owner', () => {
  ...
  const P7 = part('P7. **', '**Derivations and their unknown cases.**');
  const AXES = 'the receiver, argument, verb, assignment and write axes';
  assert.ok(P7.includes('red test rather than a dead button: since the round-7 fixes it refuses a seat it has not read on ' + AXES + ', by name: the receiver a seat stands on, read by its binding (a reflection global among them); a node of the tree handed to any callee; a method call, by its name; a member assignment, by its name, a computed name refused wherever it stands; and a write of a URL member from anything but a literal, read for whether the value can reach the network and what gates it, or of an attribute under a name the census cannot read as a literal. A root the viewer seats by any of the five reds until read, and the promise holds on those five axes and on no other: what a listed helper does with a node, what a listed receiver is and what a listed method does by its name are read by hand when listed, never derived)'), 'P7 states the promise with its five axes, each named by what it reads, and its limit');
  assert.ok(!P7.includes('verb and write axes') && !P7.includes('those four axes'), 'the four-axis wording is gone from P7 (the assignment axis, the round-6 review\'s cluster B, is named as its own)');
  assert.ok(flowRaw0().replace(/\n \*  /g, ' ').includes('a red test rather than a dead button: since the round-7 fixes it refuses a seat it has not read on ' + AXES + ', by name: a seat\'s receiver by its binding, a node handed to any callee, a method call\'s name, a member assignment\'s name, and a URL member written from anything but a literal or an attribute set under a name it cannot read as a literal, so a root the viewer seats by any of the five reds until read, and on no other axis'), 'bodyReady\'s docstring states the same five (a comment, read as one)');
  assert.ok(read('ui', 'webview', 'file-print.test.ts').includes('a red test rather than a dead button, on ' + AXES + ' since the round-7 fixes (a seat\'s receiver by its binding, a node handed to a callee, a method call\'s name, a member assignment\'s name, a URL member written from a non-literal or an attribute set under a non-literal name)'), 'the census pin\'s message names the five axes');
  // the axes the promise names are the axes the census header declares, each in capitals as the heading of its rule
  const censusHeader = read('ui', 'webview', 'file-print.test.ts');
  for (const axis of ['THE VERB AXIS', 'THE WRITE AXIS', 'THE RECEIVER AXIS', 'THE ARGUMENT AXIS']) assert.ok(censusHeader.includes(axis + ':') || censusHeader.includes(axis + ' ('), 'the census header declares ' + axis);
  assert.ok(censusHeader.includes('a URL member (href, src, srcdoc, location) written') && censusHeader.includes('passes only as a site URL_WRITES_READ_BY_HAND lists'), 'and the URL write rule the promise\'s write axis names');
  assert.ok(censusHeader.includes('a setAttribute or setAttributeNS whose name is not a string literal') && censusHeader.includes('passes only as a site ATTR_NAMES_READ_BY_HAND lists'), 'and the attribute-name rule it names');
