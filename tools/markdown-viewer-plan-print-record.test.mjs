...
test('P7: bodyReady classes each child against three closed lists and answers not in for an unknown child, the PDF kind excepting the loader; the node census derives the seated roots from file-view.ts by the command the section names and holds the unknown side; the nine roots the section names are the lists\' union', () => {
  ...
  assert.ok(census.includes('for (const m of src.matchAll(/\\.(replaceWith|after|before|replaceChild|insertAdjacentElement|insertAdjacentHTML)\\(/g)) if (!inLiteral(m.index!)) refused.push('), 'a child-level seat anywhere outside a literal is refused on sight');
  ...
  assert.ok(!/replaceChildren.*prepend.*appendChild/.test(rootsDoc) && rootsDoc.replace(/\n\s*\*\s*/g, ' ').replace(/\s+/g, ' ').includes('every `body` token in the viewer\'s source that file-print.test.ts\'s census can class (the rule, not a roster of method names)'), 'the READY_ROOTS comment states the rule, not the round-2 roster (round 5: the round-4 review\'s regression-2; a comment, read as one, its hard wraps collapsed)');
  assert.ok(read('ui', 'webview', 'file-view.ts').replace(/\n\s*\/\/\s*/g, ' ').includes('refuses every token it cannot class (a computed name, a bare read of a seating method, a call, bind or apply on one, a member it has not seen, an alias, a parenthesised or cast receiver, a helper handed the body that its BODY_HANDED_TO list does not name)'), 'the viewer\'s install comment states the same rule (a comment, read as one, its hard wraps collapsed)');
