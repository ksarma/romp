...
test('L3 opens on the exceptions it names, and names the floor by the source\'s constant and number, the measure, the decision at the load and the error, and the link exclusion by its function, in the order the verdict runs them', () => {
  ...
  assert.ok(L3.includes('the residual the skip leaves, a figure hidden after its load by any road but the viewer\'s own keeping a standing control until its next report with a box, has one road in the product, an expanded callout (`> [!note]+`, a `<details open>`) folded by the reader, on which the control is harmless by a mechanism that is not the skip: the fold reports nothing, so nothing reaches the observer while the callout is folded, the control is folded with the figure and stands over no prose, and the first report with a box after the reopen decides the figure again'), 'L3 records the residual, the product\'s road to it and why the control is harmless there');
