...
test('L3 opens on the exceptions it names, and names the floor by the source\'s constant and number, the measure, the decision at the load and the error, and the link exclusion by its function, in the order the verdict runs them', () => {
  ...
  inOrder(watch, ['if (typeof ResizeObserver !== "function") return null;', 'if (e.contentRect.width === 0 || e.contentRect.height === 0) continue;', 'due.add(e.target);', 'if (due.size && !frame) frame = requestAnimationFrame(decideDue);', 'if (!img.isConnected || figureState(img) === "standin") continue;', 'decideFigureControl(img, filePath);', 'body.querySelectorAll(".fileview-md img").forEach((img) => { ro.observe(img); });', 'onRendered((why) => { if (why !== "reflow") rearm(); });'], 'watchFigureBoxes: the guard, the 0 by 0 report skipped, each reported figure marked due and decided once at the next animation frame (the file review\'s round 18, extra6-3), every figure observed, armed at each text paint through the seam\'s onRendered, the first paint\'s included');
  assert.ok(L3.includes('at the next animation frame and never inside the observers\' delivery'), 'L3 says the decision runs at the next animation frame (the file review\'s round 18, extra6-3)');
  // a 0 by 0 report runs no decision (found before the file review's round 3: decided over it, a hidden figure gained a control
  // while hidden and lost it at the show); L3 states the skip as a rule over the report, whatever produced it (the file review's
  // round 4, regression-3: the reason before it named its roads as a closed list, and a loaded figure the author gave no box was
  // outside it), the transient reports and the final one, and the residual with the product's one road to it and why the
  // control is harmless there (the file review's round 4, ui-1: the record before it said the product had no such road)
  assert.ok(L3.includes('a report of 0 by 0 runs no decision, a rule over the report whatever produced it: a 0 by 0 report decides nothing, and the figure is decided by its load or its error, by the gate\'s restore or by its next report with a box'), 'L3 states the skip as a rule over the report');
  assert.ok(L3.includes('the viewer\'s hide and a gated placeholder\'s img until its click are transient reports, on both of which the show or the restore reports the real box, which is decided'), 'L3 names the transient reports as examples, not as the roads');
  assert.ok(L3.includes('the residual the skip leaves, a figure hidden after its load by any road but the viewer\'s own keeping a standing control until its next report with a box, has one road in the product, a `<details>` folded by the reader (an expanded callout, `> [!note]+`, renders as a `<details open>`), on which the control is harmless by a mechanism that is not the skip: the fold reports nothing, so nothing reaches the observer while the callout is folded, the control is folded with the figure and stands over no prose, and the first report with a box after the reopen decides the figure again'), 'L3 records the residual, the product\'s road to it and why the control is harmless there');
  assert.ok(L3.includes('the final report is a LOADED figure whose real box is 0 by 0'), 'L3 names the final report (the file review\'s round 3, correctness-1: the floor\'s, not the skip\'s)');
...
test('L6 names tests among the directories the branch changes, from the merge-base, and the two files under it, which the Tests paragraph names too and which exist, and no longer calls the records commit the follow-on\'s last', () => {
  assert.ok(L6.includes('`git diff --name-only $(git merge-base origin/main HEAD) HEAD` lists files under ui, docs, plans, tools, upstream or tests, and SECURITY.md, alone'), 'the directory list carries tests beside the command, derived from the merge-base, and the one root file the delta holds');
...
test('L3 names the candidate the browser chose by its function, which figureTarget reads first and the failed label delegates to, and names the two chosen modules, as the Tests paragraph does; both exist and name the follow-on', () => {
  ...
  assert.ok(viewer.includes('function failedSource(img: Element): string | null {\n  const chosen = chosenSource(img);\n  if (chosen || img.hasAttribute("src")) return chosen;\n  const parked = img.getAttribute(HEAL_RECORD);\n  return parked && parked !== absUrl("") ? parked : chosen;\n}'), 'the failed label delegates, and reads the chat page\'s heal record only when the candidate names nothing and the img has no src (the file review\'s round 15, fresh-1), and not when the record is the address an empty source resolves to (the file review\'s round 16, correctness-1; a sentence pin on the spelling, and file-view-figure-error.test.ts\'s parked cells hold the property)');
