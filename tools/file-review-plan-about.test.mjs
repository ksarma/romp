...
// tie-break's 51, each branch given its number up front so the merge renumbers nothing); the merge that brought
// them emptied it, and the run is whole again.
const DECISIONS_END = 53;
const RESERVED = [];
...
test('the paragraph stands under Slice 2 after the arrivals note, and the two decisions follow 44, each paraphrasing the user', () => {
  ...
  assert.deepEqual(decisionNumbers(plan), consecutive(DECISIONS_END), 'the list stays consecutive and ends at 53');
