...
// and an "Open the picture" control on every figure of a rendered file that opens the picture through the same trail
// push. The record names each decision's key sentence, the words the bar and the control show, the sheets' rules, the
...
const FIRST = 'The viewer keeps a trail of the files you reach through the links inside a file: **Back** and **Forward**, the two arrow buttons at the left of its title bar (Alt+Left and Alt+Right, or Cmd+[ and Cmd+] on a Mac, do the same while no text box holds the keyboard), return you to the file you followed a link from, at the place and in the view you left it, and take you on again; a file opened from the chat, from a listing or from the **Recent** list starts the trail over, and closing the viewer ends it.';
const SECOND = 'Every picture in a rendered file has an **Open the picture** button at its top-right corner (top-left for a picture floated to the right), shown while the pointer is over the picture or the button holds the keyboard focus, that opens the picture on its own in the viewer, with Back returning you to the file at that place; a plain click on the picture does the same while the Comments panel is closed (with the panel open, a click offers a comment as before), a Cmd-click (Ctrl on Windows and Linux) opens the picture in a browser tab, and a picture from the web opens in a tab.';
