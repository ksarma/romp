...
test('L3: the control\'s words are the viewer\'s literal, quoted by the section and the guide; the one decision puts a marked glyph button after the anchor and never a wrapper; the target is the model\'s join or a tab; the click listener yields as the section says; the walks skip it; the chat\'s and the feed\'s sheets carry the rules under screen and the print block names it nowhere, and no other sheet a page of either host loads names the control', () => {
  ...
  assert.ok(build.includes('const words = target !== null && target.kind === "web" ? (figureSourceCredentialed(target.src) ? FIGURE_OPEN_WEB_WITHHELD : figureOpenWebTitle(targetHost(target.href))) : FIGURE_OPEN_TITLE;') && build.includes('if (b.title !== words) { b.title = words; b.setAttribute("aria-label", words); }'), 'the words in the title and the aria-label: the one word set for a file, the host and the new tab for a picture from the web, or the new tab and the withheld address when its source appears to carry a sign-in (a sentence pin)');
  ...
  assert.ok(target.includes('if (/^https?:/i.test(dest) || dest.startsWith("//")) return { kind: "web", href: absUrl(dest), src: dest };'), 'a remote picture is a tab');
  ...
  const REST = '.fileview-md .fv-figopen { position: relative; z-index: 1; vertical-align: top; margin: 0; margin-inline: -28px 6px; top: 6px; padding: 3px; background: var(--bg); opacity: 0; }';   // logical margins, so the control follows the text's direction (the file review's round 15, ui-2)
  ...
  const LEFT = '.fileview-md .fv-figopen-left { float: left; margin: 0 6px 0 -28px; }', RIGHT = '.fileview-md .fv-figopen-right { float: right; margin: 0 -28px 0 6px; }';
  ...
  const FOCUS_OFF = '@media screen { .fileview-md .fv-figopen-web:focus-visible { outline-offset: 2px; } }';   // the web control's focus ring off its border, so its dashed line shows under a keyboard focus (the same ask)
  const PRESS_OFF = '@media screen { .fileview-md .fv-figopen-web:active { transform: none; } }';   // the button family's press cue off on the web control alone, whose release opens the tab while it is held (the same ask, the maintainer's ruling)
  const TAP_OFF = '@media screen { .fileview-md .fv-figopen-web, .fileview-md img[data-fv-figweb] { -webkit-tap-highlight-color: transparent; } }';   // the browser's tap highlight off on the web control and the mark: a phone's painted over the control in the tap's pressed frames and took its line under 3:1 (the tap-highlight ruling of 2026-09-24)
  const DEAD_DRESS = '@media screen { .fileview-md a.fv-dead:has(.fv-figopen-web), .fileview-md a.fv-dead:has(img[data-fv-figweb]) { opacity: 1; color: color-mix(in srgb, currentColor 70%, transparent); } }';   // a dead link holding the dress dims by colour, not by opacity (the same ask); its selector names the class inside :has()
  ...
  for (const { name, css, loadedBy } of SHEETS) {
    ...
    assert.ok(css.includes('\n' + WEB_REST) && css.includes('\n' + FOCUS_OFF) && css.includes('\n' + PRESS_OFF) && css.includes('\n' + TAP_OFF) && css.includes('\n' + DEAD_DRESS), name + ': the web control at rest at full opacity, its focus ring off its border, its press cue off, the browser\'s tap highlight off on it and on the mark, and the dead link holding the dress dimming by colour, under screen (the spelling; the closed set below holds the property)');
    ...
    assert.deepEqual(rules[name].filter((r) => r.screen).map((r) => r.text), [REVEAL, NOHOVER, WEB_REST, FOCUS_OFF, PRESS_OFF, TAP_OFF, DEAD_DRESS], name + ': the rules under a screen-only at-rule are the reveal, the at-rest rule (no hover, or any coarse pointer), the web control\'s full opacity at rest, its focus ring off its border, its press cue off, the browser\'s tap highlight off on it and on the mark, and the dead link holding the dress');
...
const SECOND = 'A picture in a rendered file that comes from a file or a web address has an **Open the picture** button at its top-right corner (the top-left in right-to-left text, and on a picture floated to one side, the top corner away from that side), shown while the pointer is over the picture or the button, while a focus you reached from the keyboard is on the button, under any focus for a picture from the web, and at all times on a phone or tablet and on a laptop with a touchscreen, that opens the picture on its own in the viewer, with Back returning you to the file at that place (where the button is not shown at all times, a Cmd-click on a local picture\'s button, a Ctrl-click on Windows and Linux, or a press dragged off it leaves the button focused but hidden until you press a key, so Enter then opens the picture in the viewer with nothing shown first); a plain click on the picture does the same while the Comments panel is closed (with the panel open, a click offers a comment as before, and so does a Cmd-click on the picture on a mouse or trackpad, and a drag draws a rectangle unless it starts on the button, which takes the press), a Cmd-click (Ctrl on Windows and Linux) on the picture while the panel is closed, or on the button at any time, opens the picture in a browser tab, and a picture from the web opens its address in a new tab, as a link to that site does, but only while its button, or on a small picture its dashed border, is on the screen with nothing over it that would take a click: a click, a tap, Enter or Space while it is off the screen or covered (by the list of headings the **Outline** button opens, the menu of the text size buttons, or something the file itself lays over the picture, say) opens nothing and scrolls it into view, and the next one opens once it shows (a button partly on the screen counts as shown), and the button and the picture both show that before the click: the button\'s tooltip says it opens a new tab at the address\'s host, its border is dashed and its glyph is an arrow leaving a box, and the picture\'s own tooltip shows the address\'s origin (its scheme, host and port, never its path, query or fragment), on a line after the author\'s title when there is one; when the address has an @ anywhere after its scheme, so that it may carry a sign-in, both tooltips say the address is withheld and show none of it, even for a harmless name such as a@2x.png; a click on a picture in a fold\'s title line (a `<details>` block\'s summary) opens or closes the fold and opens nothing, with or without Cmd, and a picture from the web there shows no address in its tooltip, while its button, where it has one, still opens it; a figure waiting behind its host\'s box gets its button once it has loaded, as does one still on its way (a click on it before then opens nothing), and once the browser has answered for a picture, four kinds have none: a picture that failed to load, which opens nothing either; a `data:` picture, whose bytes are written into the file itself and which does not open; a picture smaller than 48 pixels on either side (a badge, an inline icon), which the button would cover, and which a plain click still opens when neither a link nor a fold\'s title line holds it, though no key opens it, since the button is the keyboard\'s only way to a picture, and a picture that shrinks below 48 pixels as the pane narrows loses its button and that way with it (a small picture from the web outside such a line has the dashed border itself, on a mouse or trackpad while the pointer is over it, and at all times on a phone or tablet and on a laptop with a touchscreen, since a finger gets no tooltip); and a picture inside a link that holds more than the picture (a caption beside it), where a click follows the link (a link with no address left, or an anchor that only marks a place, is not a link a click can follow, so a picture inside it keeps its button and its tooltip, and a plain click opens it), while a picture that is all its link holds keeps its button beside the link.';
...
test('the guide\'s Links in a file paragraph ends with the two sentences, whole, after the drag sentence; the old wording is gone; the in-place sentence still stands; the browser plan\'s pointer is one sentence in its navigation-stack section', () => {
  ...
  // the file review's round 16, ui-1: the plan's open point 7 said that on any hover-capable device the control was transparent until
  // the pointer or a focus, false for a local button a Cmd/Ctrl-click or a dragged press leaves focused and for a touchscreen laptop
  assert.ok(!section.includes('On a hover-capable device the control is transparent until the pointer is over the picture or the button holds the focus'), 'open point 7\'s over-claim is gone from the plan (a sentence pin on the retired wording alone; no pin reads the corrected point)');
