...
test('the about paragraph records the withholding with its condition\'s three parts, each as the panel\'s helper has it: the status\'s bytes as the body shows them (paintCurrent, over textCurrent), a text to cut from (indexedText), a text view', () => {
  assert.ok(note.includes('For a spanned change, an insertion or a substitution, the card offers Comment on this change only while the view carries the change\'s text (`spanCarried`: the bytes the view shows are the status\'s, whose offsets place the span (`paintCurrent`: `textCurrent` compares the status\'s mtime with the mtime of the view\'s last paint while an svg picture\'s landing waits for its paint, else with the view\'s mtime); there is a text to cut it from (`indexedText`: the view\'s, or while the editor is up the file as the editor loaded it, never the buffer); and the view shows text at all, not the picture of a media file)'), 'the plan\'s sentence states the gate as spanCarried, paintCurrent and indexedText have it below');
  assert.ok(panel.includes('return !!s && this.paintCurrent(s) && this.indexedText() !== null && this.ctx.mode() !== "media";'), 'spanCarried: the three parts and nothing else, the first read over the text the body shows (paintCurrent); a source pin only, whose executed witnesses are file-comments-about-fixes.test.ts\'s case on Comment on this change withheld while the view\'s bytes are not the status\'s, and file-comments-changes-review2.test.ts\'s case on the change cards between an svg picture\'s landing and its paint');
  assert.ok(panel.includes('private paintCurrent(s: Status): boolean {\n    return this.textCurrent(s, this.landedAhead ? this.paintedAt : this.ctx.mtimeNs());'), 'paintCurrent: textCurrent over the mtime of the view\'s last paint while a landing the seam reported waits for its paint (landedAhead, paintedAt), else over the view\'s mtime; a source pin only, whose executed witness is file-comments-changes-review2.test.ts\'s case on the change cards between an svg picture\'s landing and its paint');
  assert.ok(panel.includes('private textCurrent(s: Status, vm = this.ctx.mtimeNs()): boolean {\n    return !vm || !s.fileMtimeNs || vm === s.fileMtimeNs;'), 'textCurrent: the mtime it is given, the view\'s by default, is the status\'s; a source pin only, executed with the view\'s mtime through paintCurrent by file-comments-about-fixes.test.ts\'s case on Comment on this change withheld while the view\'s bytes are not the status\'s');
...
test('the paragraph names Reveal\'s and the not-shown tag\'s sibling gate, inFlux, which renderChangeCard derives from the same paintCurrent', () => {
  ...
  assert.ok(panel.includes('const inFlux = !!s && !this.paintCurrent(s);'), 'inFlux: the status\'s bytes are not the ones the body shows (paintCurrent, the read spanCarried makes); a source pin only, whose executed witness is file-comments-changes-review2.test.ts\'s case on the change cards between an svg picture\'s landing and its paint, where the not shown tag and the Reveal wait for the paint');
