...
export const CENSUS = {
  ...
  PREFIXES: { side: 'WRITE', consumer: 'if (!PREFIXES.has(name)) return { name, args: words.slice(k + 1), chdirs, writes, wrapped, wrappers, wrapperIdx };', why: 'an unlisted wrapper is read as its own command, an unmodelled writer by the contract, so the set is stated on the four surfaces and the unlisted wrappers the passes found are on the contract\'s list' },
  IN_PLACE_BINDERS: { side: 'WRITE', consumer: "else if (c0 && (IN_PLACE_BINDERS.has(c0.name) || (head0 && !head0.literal))) seedPartial = true;", why: 'a command that runs a text in this shell, or defines an alias a later name may be, that the list lacks makes the values the pre-walk read gives a positional parameter look complete while a `set` in that text rebinds the list, so a name composed from one reads a stale value (round 7\'s twenty-fifth commit: THE LIST BEFORE THE WALK; the walk notes each value again where it runs, THE VALUE AT THE WALK)' },
  TEXT_MOVERS: { side: 'WRITE', consumer: 'if (markedHead || (hw && !hw.literal) || TEXT_MOVERS.has(c.name)) moved = true;', why: 'a command that moves the shell inside a text that the list lacks leaves a later relative operand of the text read against the directory before it, so a tracked file the operand names after the move is missed (round 7\'s twenty-fifth commit: THE UNREAD OPERAND inside a text)' },
  SHELL_OWN: { side: 'WRITE', consumer: "const name = spelled.includes('/') && SHELL_OWN.has(path.basename(spelled)) ? spelled : path.basename(spelled);", why: 'a shell-own name not listed is read as the builtin when a head spells it with a slash, so `/usr/bin/set -- other.md` binds a list no shell binds and `/usr/bin/cd ../scratch` moves the walk where every shell stays, and the shell writes a tracked file the guard allowed (round 7\'s twenty-first commit: THE FILE AT THE HEAD)' },
  ...
  SHELL_GRAMMARS: { side: 'REFUSE', consumer: "for (const g of SHELL_GRAMMARS[shell] || ['bash', 'dash', 'zsh'])", why: 'a shell name not listed has its option words read under all three grammars and every reading judged (THE OPTION GRAMMAR, round 7\'s twenty-fourth commit), so a script one grammar finds is read' },
  ...
  SED_OPT: { side: 'REFUSE', consumer: 'if (!SED_OPT.knownLong.has(name)) return { unknown: a.text };', why: 'a sed `--` option not in the exact table refuses by name (the reviewer\'s extra4-1); a known long option missing here would be a false refusal' },
  SORT_OPT: { side: 'REFUSE', consumer: 'if (!SORT_OPT.knownLong.has(name)) return { unknown: t };', why: 'a sort `--` option not in the exact table refuses by name, and an unknown short letter too (the reviewer\'s extra4-1); a known option missing here would be a false refusal' },
  ...
  SCRIPT_VALUED_NAMES: { side: 'WRITE', consumer: 'if (SCRIPT_VALUED_NAMES.has(m[1])) {', why: 'a name the shell runs as a command that the list lacks (a prompt string whose `$(..)` runs when the prompt is printed or a traced command echoed, a command run before each prompt) has its value read as text, so the write in it passes (round 6\'s fourth commit: `PS4=\'$(cp a b)\'; set -x; :` and `PROMPT_COMMAND=\'cp a b\' bash -i` ran the copy while allowed)' },
  STARTUP_FILE_NAMES: { side: 'WRITE', consumer: 'if (STARTUP_FILE_NAMES.has(m[1]))', why: 'a name whose value the shell sources at startup that the list lacks has its `<(..)` value read as a path, not as the script it prints, so the write in it passes (round 6\'s fourth commit: `ENV=<(echo \'cp a b\') dash -i` ran the copy while allowed)' },
