...
test('the fork PR\'s round-2 push: the where: line and the body state the rule over mdBlock, name the fence scene and the Copy case of the img leg and the seam test\'s re-parse pin, and the leg holds those two tests', () => {
  ...
  assert.ok(where.includes('a fourth scene, the fence pass\'s re-parse of raw multi-line fences holding an svg image with src or srcset, red in all three engines with the pass after the adoption; a fifth case, the Copy button clicked for real after the move; and a sixth case, an svg anchor split across lines in a raw fence, with an author\'s id and without one, marked dead in the three engines'), 'where: names the fence scene, the Copy case and the split-anchor case');
  ...
  assert.ok(where.includes('ui/test-code-only.ts (new), the comment stripper five test modules read through (the seam test, md-url-view.test.ts, md-sanitize-viewer-links.test.ts, code-block.test.ts and file-view-links.test.ts), the TypeScript compiler\'s comment ranges'), 'where: names the stripper module and its five readers');
  const scene = read(NODE_SCENE);
