...
test('decision 41 records the guard against the clicked mark, the review that narrowed it, the marks whose press collapses nothing, the edge tolerance and the pulse', () => {
  ...
  assert.ok(decision.includes('A plain click or a tap on a change\'s text or on a comment highlight collapses a standing selection at the press, so it opens the card as before'), 'the collapse-at-press premise is claimed for text targets only');
  assert.ok(!decision.includes('A plain click or a tap collapses the selection at the press and opens the card as before'), 'the unqualified premise the review found untrue is gone');
