...
// math breakout class widened from one shape to the class the browser suite records (its closing check, 2026-09-19, dropped the
// class's closed bound and stated the svg bound of the map, and this module follows); the loss before the rule told apart for
...
test('decision 52 records the fragment target a converted tag loses as left, beside the heading slug and phrased for the user\'s call, with the untouched shapes; file-view-links.ts dresses a fragment link with no target dead with the title the record quotes', () => {
  ...
  assert.ok(d52.includes('lost the count unless the reopened `a` held exactly one block'), 'the loss before the rule is recorded by its mechanism: the reopened `a` and the pairing\'s count');
  assert.ok(d52.includes('with the `[jump](#spot)` link in the next paragraph the link\'s own `<a>` closed it holding nothing but a line feed, an extra top-level element, so a selection in that paragraph or in any after it was refused, in the last with `The selection could not be matched to the file text.`'));
  assert.ok(read('ui', 'webview', 'anchor-map.ts').includes('refuse("The selection could not be matched to the file text."'), 'the refusal the record quotes is the map\'s (anchor-map.ts)');
...
test('decision 52 records the math breakout class the rule opened, bounded to an inline math and an element on the parser\'s breakout list whether closed, self-closed or void, RECORDED and not modelled, its svg bound stated of the map and not the text; the browser suite carries the class sentence, the two text representatives and the closing check\'s entries by top-level tags and verdicts, the rules suite the reader\'s side of ma5, and the Slice 5 note the record cites says not modelled', () => {
  assert.ok(!d52.includes('one shape moved into the breakout class'), 'the one-shape wording is gone');
  assert.ok(!d52.includes('so a CLOSED HTML element after it'), 'the closed bound is gone (the closing check, 2026-09-19: the void and the self-closed members break out too)');
  assert.ok(d52.includes('an integration point of an INLINE `<math>` left open (`<mtext>`, `<mi>`, `<mo>`, `<mn>` or `<ms>`, or an `<annotation-xml>` with the html or the xhtml encoding) is literal text now, so an HTML element after it whose start tag is on the parser\'s foreign-content breakout list (the 44 names the HTML standard lists there, and `font` when it carries `color`, `face` or `size`) stands in the math\'s foreign content with no integration point around it: the parser breaks out of the math at it, whether the tag is closed, self-closed or void, and shows its text, and the reader drops the math whole.'));
  ...
  // the void members and the self-closed spelling, by the DOM's top-level tags against the base tree's
  assert.ok(d52.includes('the void members break out with no closing, `<br>` and `<img>` inside `<math><mtext>` landing in the paragraph as a line break and a picture (the top-level elements `P[BR] H2 P P` and `P[IMG] H2 P P` against the base tree\'s `P H2 P P`, the text the same on both sides), the tag\'s paragraph refused with the mismatch sentence where the base tree mapped it and every later block mapping; `<hr>` closes the paragraph as well (`P HR P H2 P P`), so the paragraph\'s tail stands in an extra top-level element and every later block is refused with `The selection could not be matched to the file text.`, where the base tree mapped every block'));
  assert.ok(d52.includes('the self-closed `<b/>` breaks out and opens, a wrapper around every later block (`P[B] B[H2,P,P]`, every passage refused with the mismatch sentence)'));
  // the svg bound: the text agrees, the map does not
  assert.ok(!d52.includes('the same shape inside an inline `<svg>`, which the sanitizer keeps, so both sides read'), 'the svg agreement is no longer stated as if it held for the map');
  assert.ok(d52.includes('A closed block-level member splits the paragraph the same way in either root: `<div>x</div>` or `<p>x</p>` after the `<mtext>` (`P DIV P H2 P P`, `P P P H2 P P`, the `ma5` shape\'s) and inside an inline `<svg>` after a `<title>`, `<desc>` or `<foreignObject>` left open (`P[svg] DIV P H2 P P`, `P[svg] P P H2 P P`; `<hr>` after the `<title>` the same), every later block refused with the could-not-be-matched sentence, where at the base tree the element sat inside the integration point, a scope boundary the parser closes no `<p>` across, and every later block mapped.'));
  assert.ok(d52.includes('Inside the `<svg>` the TEXT agrees on both sides for a member, since the sanitizer keeps svg text: `sv1 <svg><title><b>x</b></svg> y1` reads `sv1 <title>x y1` on both sides, the `b` broken out into the paragraph (`P[svg,B]`) and every block mapping; the map is what differs for a block-level member, and the browser suite compares text, so the splits are RECORDED there by the DOM\'s top-level tags and each passage\'s verdict, not by text alone.'));
  assert.ok(d52.includes('The `<title>` half of the svg split stands as prose, executed and not pinned: `tools/markdown-viewer-plan-decision52-pointers.test.mjs` refuses a `<textarea>`, `<plaintext>`, `<title>` or `<noscript>` written without the self-closing syntax in a RECORDED source, since such a tag left open is literal text under this decision and cannot itself be a recorded divergence, and the svg-title shape\'s divergence is the div\'s breakout, which the guard cannot tell from a title left open, so the split is pinned through `<foreignObject>` and `<desc>`.'), 'the title half is prose, and the record says why (the closing check\'s ruling on PR 804, 2026-09-19)');
  assert.ok(d52.includes('Not in the class: a closed element whose start tag is not on that list (`<kbd>`, `<a>`) stays in the foreign content and goes with the dropped math on both sides (`ma7 y7`); inside an inline `<svg>` it stays in the drawing, where the sanitizer keeps an svg name (`<a>`, its text shown and every block mapping) and removes any other with its text, which the reader keeps as the drawing\'s, so `Lead <svg><foreignObject><kbd>x</kbd></svg> tail t1.` reads `Lead <foreignObject> tail t1.` in the DOM against the reader\'s `Lead <foreignObject>x tail t1.` (the same after a `<title>`), a text divergence new with this decision, RECORDED in the same suite; and a `<math>` inside an html block, whose tags the rule does not read.'));
  assert.ok(d52.includes('records it as not modelled and pre-existing), so the class is RECORDED in that suite, the text representatives `ma5` and `ma7` and the closing check\'s entries for the void members, the self-closed `<b/>`, the `font` member, the block-level splits in both roots and the svg out-of-class drop, and not modelled; `ui/webview/anchor-map-html-rules.test.ts` pins the reader\'s side of `ma5`.'));
  const browser = read('ui', 'webview', 'anchor-map-html-text-browser.test.ts');
  const comments = flat(browser.replace(/\n\s*\/\/ ?/g, ' '));
  assert.ok(browser.includes('an integration point of an INLINE `<math>` left'), 'the class sentence in the browser suite');
  assert.ok(comments.includes('the parser breaks out of the math at it, whether the tag is closed, self-closed or void, and shows its text'), 'the browser suite\'s class sentence carries no closed bound');
  assert.ok(!comments.includes('so a CLOSED HTML element after it'), 'the closed bound is gone from the browser suite too');
  assert.ok(comments.includes('Inside an inline `<svg>` the TEXT agrees for a member, since the sanitizer keeps svg text') && comments.includes('and the MAP does not for a block-level member'), 'the browser suite states the svg bound of the map');
  assert.ok(comments.includes('the svg-title shape\'s divergence is the div\'s breakout, which the guard cannot tell from a title left open, so the split is pinned through `<foreignObject>` and `<desc>`'), 'the browser suite says why the title half has no RECORDED entry');
  assert.ok(!/src: (?:[A-Za-z_$][\w$]*\()?["'][^"']*<title>/.test(browser.slice(browser.indexOf('const RECORDED'))), 'no RECORDED src opens a <title>: the title half is prose');
  ...
  // the closing check's entries, each by the DOM's top-level tags and the map's verdicts over the note's passages
  for (const entry of [
    'src: noteWith("<math><mtext><br></math>"), dom: NOTE_TEXT, reader: NOTE_TEXT, shape: "P[BR] H2 P P", verdicts: "mismatch maps maps maps"',
    'src: noteWith(\'<math><mtext><img src="a.png"></math>\'), dom: NOTE_TEXT, reader: NOTE_TEXT, shape: "P[IMG] H2 P P", verdicts: "mismatch maps maps maps"',
    'src: noteWith("<math><mtext><hr></math>"), dom: NOTE_TEXT, reader: NOTE_TEXT, shape: "P HR P H2 P P", verdicts: "mismatch unmatched unmatched unmatched"',
    'src: noteWith("<math><mtext><b/></math>"), dom: NOTE_TEXT, reader: NOTE_TEXT, shape: "P[B] B[H2,P,P]", verdicts: "mismatch mismatch mismatch mismatch"',
    'src: noteWith("<math><mtext><div>x</div></math>"), dom: "Lead x tail t1. Second heading t2 Para after the heading t3. Closing words t4.", reader: NOTE_TEXT, shape: "P DIV P H2 P P", verdicts: "mismatch unmatched unmatched unmatched"',
    'src: noteWith("<svg><foreignObject><div>x</div></svg>"), dom: FO_TEXT, reader: FO_TEXT, shape: "P[svg] DIV P H2 P P", verdicts: "mismatch unmatched unmatched unmatched"',
    'src: noteWith("<svg><foreignObject><p>x</p></svg>"), dom: FO_TEXT, reader: FO_TEXT, shape: "P[svg] P P H2 P P", verdicts: "mismatch unmatched unmatched unmatched"',
    'src: noteWith("<svg><desc><div>x</div></svg>"), dom: DESC_TEXT, reader: DESC_TEXT, shape: "P[svg] DIV P H2 P P", verdicts: "mismatch unmatched unmatched unmatched"',
    'src: noteWith("<svg><foreignObject><kbd>x</kbd></svg>"), dom: "Lead <foreignObject> tail t1. Second heading t2 Para after the heading t3. Closing words t4.", reader: FO_TEXT, shape: "P[svg] H2 P P", verdicts: "mismatch maps maps maps"',
    'src: noteWith(\'<math><mtext><font color="red">x</font></math>\'), dom: "Lead x tail t1. Second heading t2 Para after the heading t3. Closing words t4.", reader: NOTE_TEXT, shape: "P[FONT] H2 P P", verdicts: "mismatch maps maps maps"',
  ]) assert.ok(browser.includes(entry), 'the browser suite records ' + entry);
  assert.ok(browser.includes('const LATER = ["tail t1", "Second heading t2", "Para after the heading t3", "Closing words t4"];') && browser.includes('if (d.verdicts !== undefined) assert.equal(r.verdicts.join(" "), d.verdicts'), 'the verdicts are asserted over the note\'s passages');
  assert.ok(browser.includes("return r.reason === MISMATCH ? 'mismatch' : r.reason === UNMATCHED ? 'unmatched' : 'refused: ' + r.reason;"), 'the verdict labels name the two sentences');
  assert.ok(browser.includes('if (d.shape !== undefined) assert.equal(r.shape, d.shape'), 'the top-level tags are asserted');
