...
test('on the Raw fixture, the host places the moved offset on the selected copy and refuses the selection-time one, as the criterion states', () => {
  ...
  assert.ok(panel.includes('ctx.onRendered((why) => { this.hideFloat(); this.retargetComposer(); if (why === "reflow") { this.trimBlanks(); this.scheduleLayout(); } else { this.reloadOut = false; this.paintedAt = ctx.mtimeNs(); this.landedAhead = false; this.paintAll(); } });'), 'the follow runs when the body is repainted, on a paint and on a reflow alike (a reflow also re-measures the marks\' collapsed blanks, anchor-map.ts trimCollapsedMarks; a paint also ends the record of a re-fetch the panel asked, reloadOut, and records the mtime the body now shows, ending a landing\'s wait for its paint, paintedAt and landedAhead); a source pin only, whose executed witnesses are file-comments-follow.test.ts\'s case on a note on the second of three identical paragraphs with a long paragraph inserted above (the follow at a paint) and file-comments-changes-review2.test.ts\'s case on the change cards between an svg picture\'s landing and its paint (the paint\'s record)');
  ...
  assert.ok(panel.includes('if (r && this.composer === c) this.closeComposer();'), 'a refusal keeps the note in the composer, and a landing closes the saving composer only (a pin on where the line lives. The refusal half is executed by ui/webview/file-comments-behavior.test.ts "the passage is gone after the reload…" and "an unrelated kernel `warn`…", file-comments-composer.test.ts "the draft survives the poll\'s re-render and a refusal…", file-comments-follow.test.ts "edits on both sides of the selected copy…", file-comments-panel.test.ts "a status refusal is not a wait and not a moved fence…", file-comments-region-tied.test.ts "a region on the SECOND twin, the file changed at both ends…", file-comments-regions.test.ts "a write about a figure is fenced on its bytes…", file-comments-reply-place.test.ts "the poll\'s re-render keeps the box in the same comment\'s card…" and file-comments-review-fixes-3.test.ts "a No on the consent popup…" and "an editing-off refusal from the kernel…", each red when a refusal closes the composer; the landing half by ui/webview/file-comments-save-held-composer.test.ts "a reply started while a comment saves opens empty…", "a new comment started while a comment saves opens empty…", "while a comment saves, a new comment\'s Save waits…" and "with no save out the box still carries its words…", each red when a landing closes whatever composer is open)');
});
